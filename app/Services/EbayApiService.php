...
class EbayApiService
{
    ...
    /**
     * Update bullet points/features for an eBay listing (150 char limit).
     *
     * @param string $itemId eBay ItemID
     * @param string $bulletPoints Bullet points text (truncated to 150 chars by caller)
     * @return array{success: bool, message: string}
     */
    public function updateBulletPoints(string $itemId, string $bulletPoints): array
    {
        ...
        $bulletPoints = mb_substr($bulletPoints, 0, 150);
        ...
        try {
            ...
            $newDesc = ($existingDesc && strlen((string) $existingDesc) > 50)
                ? $existingDesc . '<br/><br/>' . $bulletHtml
                : $bulletHtml;
            ...
            $response = Http::withHeaders([
                ...
                'Content-Type' => 'text/xml',
            ])->withBody($xml->asXML(), 'text/xml')->post($this->endpoint);
            ...
            Log::error('eBay updateBulletPoints exception', ['itemId' => $itemId, 'error' => $e->getMessage()]);
