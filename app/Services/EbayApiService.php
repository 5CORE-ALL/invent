    ...
    public function getValidTrackingRate()
{
    ...
    public function updateImages(string $identifier, array $images): array
    {
        ...
        $saved = $this->saveImageUrlsToMetrics('ebay_metrics', $identifier, $row, $urlsForMetrics);
        if (! $saved) {
    ...
    public function updateVideos(string $identifier, array $videos, string $mode = 'replace'): array
    {
        ...
        $saved = $this->saveVideoUrlsToMetricsRow('ebay_metrics', $sku, $urlsForMetrics);
        if (! $saved) {
            $res['message'] = ($res['message'] ?? 'eBay video updated.').' Metrics save failed.';
        }
    ...
    /**
     * Long-form listing description (HTML body).
     *
     * @return array{success: bool, message: string}
     */
    ...
    /**
     * Description Master: return the eBay listing's raw Description HTML (seller-facing description body)
     * for one SKU, without DM2 parsing. Read-only.
     *
    ...
    public function fetchRawDescriptionHtml(string $identifier): array
    {
        ...
        return ['success' => true, 'message' => 'eBay listing description loaded.', 'html' => $desc];
    }
    ...
    {
        $appId = config('services.ebay.app_id', env('EBAY_APP_ID'));
        $certId = config('services.ebay.cert_id', env('EBAY_CERT_ID'));
        $refresh = config('services.ebay.refresh_token', env('EBAY_REFRESH_TOKEN'));
    ...
    public function reviseInventoryStatus(string $itemId, int $quantity, ?string $sku = null, ?float $price = null): array
    {
        ...
        try {
            ...
            $xmlResp = simplexml_load_string($body);
            if ($xmlResp === false) {
                Log::warning('eBay1 ReviseInventoryStatus: invalid XML', [
                    'itemId' => $itemId,
                    'sku' => $sku,
                    'status' => $response->status(),
    ...
    public function reviseVariationQuantity(string $itemId, string $sku, int $quantity): array
    {
        ...
        try {
            ...
            $ack = $data['Ack'] ?? 'Failure';
            $msg = $this->flattenEbayErrors($data);
            $errors = isset($data['Errors'][0]) ? $data['Errors'] : (isset($data['Errors']) && is_array($data['Errors']) ? [$data['Errors']] : []);
            if ($ack === 'Success' || $ack === 'Warning') {
                if ($this->ebayErrorLooksLikeNonVariationListing($errors, $msg)) {
            ...
            if ($this->ebayErrorLooksLikeNonVariationListing($errors, $msg)
                || str_contains(strtolower($msg), '21916587')) {
                return $this->reviseItemQuantity($itemId, $quantity);
            }
    ...
    public function testConnection(): array
    {
        if (! $this->isConfigured()) {
            return [
                'success' => false,
                'message' => 'Configure EBAY_APP_ID, EBAY_CERT_ID, and EBAY_REFRESH_TOKEN in .env.',
            ];
        }
        ...
        try {
            $token = $this->generateBearerToken();
            if (empty($token)) {
                return ['success' => false, 'message' => 'Failed to generate eBay 1 bearer token.'];
            }
            ...
            return [
                'success' => true,
                'message' => 'Connected. Bearer token generated (no sample item_id in ebay_metrics yet).',
                'sample_item_id' => null,
            ];
        } catch (\Throwable $e) {
    ...
    private function parseEbayError(array $error): string
    {
        ...
        return implode(' ', array_filter($parts)) ?: 'Unknown eBay API error';
    }
    ...
    public function policyIds(): array
    {
        return EbaySellAccountPolicies::resolveValid(
            ...
            EbaySellAccountPolicies::defaultsForChannel('ebay')
        );
    }
