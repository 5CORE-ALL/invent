...
/**
 * File-backed Reverb Push Std Prc job state (survives page refresh).
 * Same queue pattern as ChannelPushPrmtJobStore.
 */
class ReverbPushStdJobStore
{
    ...
    public function create(array $tasks): array
    {
        ...
        $queueMsg = 'Push Std queued ('.count($normalized).' SKU(s)).';
    ...
    public function createOrAppend(array $tasks): array
    {
        ...
        if ($this->isActive($current)) {
            if ($this->isStale($current, 180)) {
                $this->forceStop('Cleared a stale Push Std job (no worker was processing it).');
    ...
    public function toApiResponse(array $state): array
    {
        ...
        return [
            ...
            'tasks' => $taskSummaries,
            'message' => $done
                ? "Push Std done: {$ok} ok, {$fail} failed."
                : ($state['last_message'] ?? 'Push Std in progress…'),
        ];
    }
    ...
    private function normalizeTasks(array $tasks): array
    {
        ...
        foreach ($tasks as $task) {
            ...
            $std = round((float) ($task['std'] ?? $task['price'] ?? $task['standard_price'] ?? 0), 2);
            if ($sku === '' || $std <= 0) {
                continue;
            }
            $normalized[] = [
    ...
    {
        return storage_path('app/reverb-push-std/job.json');
    }
