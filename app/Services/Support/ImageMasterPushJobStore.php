...
class ImageMasterPushJobStore
{
    ...
    public function create(string $sku, string $mode, array $tasks, ?array $mainByMarketplace = null): array
    {
        ...
        foreach ($tasks as $task) {
            ...
            $images = array_values(array_filter(array_map('trim', $task['images'] ?? []), fn ($s) => $s !== ''));
            $normalized[] = [
                'marketplace' => $mp,
                'images' => $images,
                'status' => 'pending',
        ...
        $state = array_merge($this->defaultState(), [
            ...
            'last_message' => 'Image push queued.',
            'messages' => [[
                'time' => now()->format('H:i:s'),
                'ok' => true,
                'message' => 'Image push queued.',
            ]],
        ]);
    ...
    {
        return storage_path('app/image-master-push/job.json');
    }
