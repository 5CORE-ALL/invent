...
/**
 * Background worker: push Std Prc to live Reverb listings via API.
 */
class ReverbPushStdRunner
{
    ...
    public function run(): int
    {
        ...
        if (! $lockHandle || ! flock($lockHandle, LOCK_EX | LOCK_NB)) {
            $logger->info('Reverb Push Std runner skipped — another process holds the lock');
            if ($lockHandle) {
                fclose($lockHandle);
            }
    ...
    private function runLocked(ReverbPushStdJobStore $store, \Psr\Log\LoggerInterface $logger): int
    {
        ...
        while (true) {
            ...
            foreach ($indexes as $index) {
                ...
                $error = null;
                try {
                    $res = $api->updatePrice($sku, $std);
                    if (! empty($res['success'])) {
                        $ok = true;
                        $this->recordSuccess($promo, $sku, $std);
                    } else {
                        $error = (string) ($res['message'] ?? 'Push Std failed');
                    }
                } catch (\Throwable $e) {
                    $error = $e->getMessage();
                    $logger->error('Reverb Push Std exception', [
                        'sku' => $sku,
                        'std' => $std,
                        'error' => $error,
                    ]);
                ...
                $store->update(function (array $state) use ($index, $sku, $ok, $error) {
                    ...
                    if ($ok) {
                        ...
                        $state['tasks'][$index]['error'] = $error ?: 'Push Std failed';
                        $state['tasks'][$index]['message'] = $error ?: 'Push Std failed';
    ...
    private function recordSuccess(ChannelPromoPricingService $promo, string $sku, float $std): void
    {
        try {
            $promo->upsert('reverb', $sku, [
                'push_std_prc_status' => 'pushed',
                'push_std_prc_value' => $std,
                'push_std_prc_pushed_at' => now(),
            ]);
        } catch (\Throwable $e) {
            Log::warning('Reverb Push Std recorded API ok but promo save failed', [
                'sku' => $sku,
                'error' => $e->getMessage(),
            ]);
        }
        ...
        try {
            ...
            Log::warning('Reverb Push Std local price update failed', [
                'sku' => $sku,
                'error' => $e->getMessage(),
            ]);
        }
