...
/**
 * File-backed channel Push CPN % job state (survives page refresh).
 * Same queue pattern as ChannelPushPrcJobStore — one job file per channel.
 */
class ChannelPushCpnJobStore
{
    ...
    public function create(array $tasks): array
    {
        ...
        $queueMsg = 'Push CPN% queued ('.count($normalized).' SKU(s)).';
    ...
    public function createOrAppend(array $tasks): array
    {
        ...
        if ($this->isActive($current)) {
            if ($this->isStale($current, 180)) {
                $this->forceStop('Cleared a stale Push CPN% job (no worker was processing it).');
    ...
    public function toApiResponse(array $state): array
    {
        ...
        foreach ($state['tasks'] ?? [] as $task) {
            ...
            $taskSummaries[] = [
                ...
                'status' => $st,
                'cpn' => $task['cpn'] ?? null,
                'coupon_code' => $task['coupon_code'] ?? null,
        ...
        $stepTotal = max(1, (int) ($state['current_step_total'] ?? 1));
        $stepFrac = ($pushing > 0 && $step > 0)
            ? min(0.95, ($step - 0.2) / $stepTotal)
            : 0.0;
        $pct = $total > 0
            ? min($done ? 100 : 99, (int) round((($finishedCount + $stepFrac) / $total) * 100))
            : 0;
        if ($done) {
            $pct = 100;
        }
        ...
        return [
            ...
            'tasks' => $taskSummaries,
            'message' => $done
                ? "Push CPN% done: {$ok} ok, {$fail} failed."
                : ($state['last_message'] ?? 'Push CPN% in progress…'),
        ];
    }
    ...
    private function normalizeTasks(array $tasks): array
    {
        ...
        foreach ($tasks as $task) {
            ...
            $cpn = isset($task['cpn']) ? (int) round((float) $task['cpn']) : 0;
            if ($cpn < 0) {
                $cpn = 0;
            }
            $normalized[] = [
    ...
    {
        return storage_path('app/'.$this->channel.'-push-cpn/job.json');
    }
