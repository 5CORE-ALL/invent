...
/**
 * File-backed Reverb Apply Prmt% job state (Drop the Price By PRMT% — listing price unchanged).
 */
class ReverbPushPrmtJobStore
{
    ...
    private function normalizeTasks(array $tasks): array
    {
        ...
        foreach ($tasks as $task) {
            ...
            $std = round((float) ($task['std'] ?? $task['standard_price'] ?? 0), 2);
            $prmt = max(0, (float) ($task['prmt'] ?? $task['prmt_pct'] ?? 0));
            if ($sku === '') {
                continue;
            }
            $normalized[] = [
    ...
    {
        return storage_path('app/reverb-push-prmt/job.json');
    }
