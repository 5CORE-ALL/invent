...
class ImageMasterPushRunner
{
    public function __construct(
        private readonly ImageMasterPushJobStore $store,
    ) {}
        ...
        while (true) {
            ...
            try {
                $result = app(ImageMasterController::class)->runQueuedMarketplacePush(
                    $sku,
                    $mp,
                    is_array($task['images'] ?? null) ? $task['images'] : [],
                    $mode,
                    $mainMap
                );
                ...
                $result = ['success' => false, 'message' => $message, 'metrics_saved' => false];
                Log::warning('ImageMasterPushRunner task failed', [
                    'sku' => $sku,
                    'marketplace' => $mp,
                    'error' => $e->getMessage(),
                ]);
