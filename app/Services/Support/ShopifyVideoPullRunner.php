...
class ShopifyVideoPullRunner
{
    public function __construct(
        private readonly ShopifyVideoPullJobStore $store,
    ) {}
    ...
    public function run(): int
    {
        while (true) {
            ...
            $ok = false;
            $message = "{$sku}: failed - Unable to pull Shopify videos.";
            try {
                $response = app(VideoMasterController::class)->pullShopifyVideosToMaster(new Request(['sku' => $sku]));
                $payload = method_exists($response, 'getData') ? $response->getData(true) : [];
                $ok = (bool) ($payload['success'] ?? false);
                $message = $ok
                    ? "{$sku}: {$payload['status']} - ".count($payload['shopify_videos'] ?? $payload['shopify_images'] ?? []).' video(s) from '.($payload['source'] ?? 'shopify')
                    : "{$sku}: failed - ".($payload['message'] ?? 'Unable to pull Shopify videos.');
            } catch (\Throwable $e) {
