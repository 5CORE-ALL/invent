...
/**
 * Same fetch logic as TemuAdsApiReportService:
 * temu.searchrec.ad.reports.goods.query + ad.detail.query,
 * store full raw + Overall summary fields, sync L30/L60 clicks to temu3_metrics.
 */
class Temu3AdsApiReportService
{
    public function __construct(protected Temu3ApiService $temuApiService)
    {
    }
    ...
    public function resolveGoodsIds(?string $specificGoodsId = null): array
    {
        ...
        return Temu3Metric::whereNotNull('goods_id')
            ->where('goods_id', '!=', '')
            ->pluck('goods_id')
            ->unique()
            ->map(fn ($id) => (string) $id)
    ...
    public function fetchAndStore(string $goodsId, string $period = 'L30'): array
    {
        ...
        try {
            ...
            if (! is_array($result)) {
                $existing = Temu3AdsApiReport::where('goods_id', $goodsId)->where('period', $period)->first();
                Temu3AdsApiReport::updateOrCreate(
            ...
            $this->syncTemu3MetricClicks($goodsId, $period, $row);

            return ['ok' => true, 'goods_id' => $goodsId, 'period' => $period];
        } catch (\Throwable $e) {
            Log::error('Temu3AdsApiReportService::fetchAndStore failed', [
                'goods_id' => $goodsId,
                'period' => $period,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        $query = Temu3AdsApiReport::query()->whereNotNull('raw_response');
        if ($period) {
        ...
        foreach ($query->cursor() as $report) {
            ...
            try {
                ...
                $report->save();
                $this->syncTemu3MetricClicks((string) $report->goods_id, (string) $report->period, $metrics);
                $ok++;
            } catch (\Throwable $e) {
                $fail++;
                Log::error('Temu3AdsApiReportService::reparseStored failed', [
    ...
    public function refreshAdStatuses(?string $specificGoodsId = null): array
    {
        ...
        foreach ($goodsIds as $goodsId) {
            ...
            $rows = Temu3AdsApiReport::where('goods_id', $goodsId)->get();
            if ($rows->isEmpty()) {
                $sku = Temu3Metric::where('goods_id', $goodsId)->value('sku');
                Temu3AdsApiReport::create([
                    'goods_id' => $goodsId,
                    'sku' => $sku,
                    'period' => 'L30',
    ...
    private function syncTemu3MetricClicks(string $goodsId, string $period, array $row): void
    {
        if ($period === 'L30') {
            Temu3Metric::where('goods_id', $goodsId)->update([
    ...
    private function resolveSku(string $goodsId, string $period): ?string
    {
        ...
        $existing = Temu3AdsApiReport::query()
