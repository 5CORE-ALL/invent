...
class EbayThreeApiService
{
    ...
    /**
     * Update bullet points for eBay3 listing (150 char limit).
     */
    public function updateBulletPoints(string $itemId, string $bulletPoints): array
    {
        ...
        try {
            ...
            $response = Http::withHeaders([
                'X-EBAY-API-COMPATIBILITY-LEVEL' => config('services.ebay3.compat_level', '1189'),
                'X-EBAY-API-DEV-NAME' => config('services.ebay3.dev_id'),
                'X-EBAY-API-APP-NAME' => config('services.ebay3.app_id'),
                'X-EBAY-API-CERT-NAME' => config('services.ebay3.cert_id'),
                'X-EBAY-API-CALL-NAME' => 'ReviseItem',
                'X-EBAY-API-SITEID' => (string) (config('services.ebay3.site_id', 0)),
                'Content-Type' => 'text/xml',
            ])->withBody($xml->asXML(), 'text/xml')->post(config('services.ebay3.trading_api_endpoint', 'https://api.ebay.com/ws/api.dll'));
            ...
            Log::error('eBay3 updateBulletPoints exception', ['itemId' => $itemId, 'error' => $e->getMessage()]);
            return ['success' => false, 'message' => $e->getMessage()];
        }
    }
