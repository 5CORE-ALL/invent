...
/**
 * Fetch eBay 1 orders (Sell Fulfillment API) into ebay1_order_metrics.
 */
class Ebay1OrderSyncService
{
    ...
    {
        if (! Schema::hasTable('ebay1_order_metrics')) {
            return ['success' => false, 'message' => 'ebay1_order_metrics table missing.', 'upserted' => 0, 'pages' => 0];
        }

        if (! MarketplaceSyncSettings::canFetchOrders('ebay1')) {
            return ['success' => true, 'message' => 'Order fetch disabled in settings.', 'upserted' => 0, 'pages' => 0];
        }

        $token = $this->getAccessToken();
        if (! $token) {
            return ['success' => false, 'message' => 'eBay 1 API credentials missing or token failed.', 'upserted' => 0, 'pages' => 0];
        }
        ...
        try {
            do {
                ...
                if (! $response->successful()) {
                    return [
                        'success' => false,
                        'message' => 'eBay 1 order fetch failed: '.$response->body(),
                        'upserted' => $upserted,
                        'pages' => $pages,
                    ];
                }
            ...
            return [
                'success' => false,
                'message' => 'eBay 1 order fetch failed: '.$e->getMessage(),
                'upserted' => $upserted,
                'pages' => $pages,
            ];
        }

        if ($import || MarketplaceShopifyImportQueue::shouldDispatchImports('ebay1')) {
            $this->dispatchImportsForNewOrders();
        }

        if (Ebay1OrderPushService::canAutoSyncAddress()) {
            try {
                \App\Jobs\SyncEbay1AddressJob::dispatch(false, 25);
            } catch (\Throwable $e) {
                Log::warning('Ebay1OrderSyncService: could not queue address sync', [
                    'error' => $e->getMessage(),
                ]);
            }
        }
        ...
        return [
            'success' => true,
            'message' => "Synced {$upserted} eBay 1 order line(s) from {$fetched} order(s) ({$pages} page(s)).",
            'upserted' => $upserted,
            'pages' => $pages,
            'fetched' => $fetched,
            'stored' => $upserted,
    ...
    public function dispatchImportsForNewOrders(): int
    {
        ...
        MarketplaceShopifyImportQueue::prepareForDispatch(Ebay1OrderMetric::class, $queue);
        ...
        $dispatched = 0;
        foreach ($orders as $order) {
            if ($paidOnly && ! MarketplaceOrderPaidFilter::isPaid('ebay1', $order)) {
                continue;
            }
            ...
            try {
                ...
                Log::warning('Ebay1OrderSyncService: failed to queue import', [
                    'id' => $order->id,
                    'error' => $e->getMessage(),
                ]);
            }
    ...
    protected function upsertOrder(array $order): int
    {
        ...
        $existingPayload = Ebay1OrderMetric::query()
        ...
        if ($lineItems === []) {
            $existing = Ebay1OrderMetric::query()
                ->where('order_id', $orderId)
                ->where('sku', '__order__')
                ->first();
            Ebay1OrderMetric::updateOrCreate(
        ...
        foreach ($lineItems as $line) {
            ...
            Ebay1OrderMetric::updateOrCreate(
                ...
                array_merge([
                    ...
                    Ebay1OrderMetric::query()->where('order_id', $orderId)->where('sku', $sku)->first()
                ))
            );
            $count++;
        }
    ...
    {
        $clientId = config('services.ebay1.app_id');
        $clientSecret = config('services.ebay1.cert_id');
        $refreshToken = config('services.ebay1.refresh_token');
        ...
        try {
            ...
            // Retry without explicit scope (same as ebay1:daily).
            $response = Http::asForm()
                ->withBasicAuth($clientId, $clientSecret)
                ->timeout(30)
                ->post($this->baseUrl.'/identity/v1/oauth2/token', [
            ...
            Log::error('Ebay1OrderSyncService: token failed', ['error' => $e->getMessage()]);
