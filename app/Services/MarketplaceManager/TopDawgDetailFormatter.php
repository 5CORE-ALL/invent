...
class TopDawgDetailFormatter
{
     ...
     */
    public function formatProduct(?array $aeLive, ?TopDawgProduct $metric, ShopifySku $shopify, array $aeSkuRows = []): array
    {
        $ae = $this->arr($aeLive);
        ...
        return [
            ...
            'link' => [
                ...
                'title' => $metric?->product_title,
                'price' => $metric?->price,
                'l30' => $metric?->r_l30,
                'l60' => $metric?->r_l60,
                'last_order_date' => null,
                'bullet_points' => null,
                'remaining_inventory' => $aeStock,
            ...
            'topdawg' => [
                'product_id' => $this->str($ae['product_id'] ?? $metric?->topdawg_listing_id),
                'title' => $this->extractProductTitle($ae) ?? $metric?->product_title,
     ...
     * @return array<string, mixed>
     */
    public function formatOrder(array $orderRoot, Collection $lines, TopDawgOrderMetric $primaryLine): array
    {
    ...
    /**
     * Build Shopify REST order payload from formatted TopDawg order detail.
     *
    ...
    public function buildShopifyOrderPayload(array $detail, Collection $lines, array $tags): array
    {
        ...
        $noteLines = [
            'Imported from TopDawg Order #'.$orderRef,
        ];
        ...
        $noteAttrs = [
            ['name' => 'topdawg_order_id', 'value' => $orderRef],
        ];
        if ($emailIsPlaceholder) {
            $noteAttrs[] = ['name' => 'topdawg_email_is_placeholder', 'value' => 'true'];
        }
        foreach ([
            'topdawg_buyer_login' => $summary['buyer_login_id'] ?? null,
            'topdawg_payment_method' => $payment['method'] ?? null,
            'topdawg_tracking_number' => $shipment['tracking'] ?? null,
            'topdawg_shipping_method' => $shipment['service'] ?? null,
            'topdawg_tax_number' => $shipping['tax_number'] ?? null,
            'topdawg_buyer_email' => $shipping['email'] ?? null,
            'topdawg_buyer_phone' => $shipping['phone'] ?? null,
            'topdawg_platform_promotion' => isset($funds['platform_promotion']) ? number_format((float) $funds['platform_promotion'], 2, '.', '') : null,
            'topdawg_platform_offer' => isset($funds['platform_offer']) ? number_format((float) $funds['platform_offer'], 2, '.', '') : null,
            'topdawg_order_amount' => isset($funds['order_amount']) ? number_format((float) $funds['order_amount'], 2, '.', '') : null,
            'topdawg_customer_paid' => $customerPaid !== null ? number_format($customerPaid, 2, '.', '') : null,
            'topdawg_seller_amount_paid' => isset($funds['seller_amount_paid']) ? number_format((float) $funds['seller_amount_paid'], 2, '.', '') : null,
            'topdawg_platform_commission' => isset($funds['platform_commission']) ? number_format((float) $funds['platform_commission'], 2, '.', '') : null,
            'topdawg_transaction_service_fee' => isset($funds['transaction_service_fee']) ? number_format((float) $funds['transaction_service_fee'], 2, '.', '') : null,
            'topdawg_tax' => $taxAmount > 0 ? number_format($taxAmount, 2, '.', '') : null,
            'topdawg_platform_offer_tax' => isset($funds['platform_offer_tax']) ? number_format((float) $funds['platform_offer_tax'], 2, '.', '') : null,
        ] as $name => $value) {
            if ($value !== null && $value !== '') {
        ...
        if ($shippingCost > 0) {
            $payload['shipping_lines'] = [[
                'title' => (string) ($shipment['service'] ?? 'TopDawg shipping'),
    ...
    protected function splitShopifyCustomerName(array $detail, string $recipient = ''): array
    {
        ...
        if ($fullName === '') {
            return ['TopDawg', 'Customer'];
        }

        $parts = preg_split('/\s+/u', $fullName, 2) ?: [];
        $firstName = (string) ($parts[0] ?? 'TopDawg');
        if (! isset($parts[1])) {
    ...
    public function resolveShopifySourceAttribution(string $orderRef, ?array $settings = null): array
    {
        ...
        $handle = trim((string) (
            ...
            ?? config('services.topdawg.shopify_source_name')
            ?? 'topdawg'
        ));
        $urlTemplate = (string) (
            $settings['order']['shopify_source_url_template']
            ?? config('services.topdawg.shopify_source_url_template')
            ?? 'https://csp.topdawg.com/m_apps/order-manage/order_detail?orderId={order_id}'
        );

        return [
            'source_name' => $handle !== '' ? $handle : 'topdawg',
            'source_identifier' => $orderRef,
            'source_url' => str_replace('{order_id}', $orderRef, $urlTemplate),
            'referring_site' => 'https://www.topdawg.com/',
        ];
    }
    ...
    public function shopifySourceDisplayName(?array $settings = null): string
    {
        ...
        $name = trim((string) (
            ...
            ?? config('services.topdawg.shopify_source_display_name')
            ?? 'TopDawg'
        ));

        return $name !== '' ? $name : 'TopDawg';
    }
    ...
    public function resolveShopifyCustomerEmail(string $orderRef, ?string $rawEmail): array
    {
        ...
        $domain = (string) env('TOPDAWG_SHOPIFY_PLACEHOLDER_EMAIL_DOMAIN', 'import.5coremanagement.com');
        $slug = preg_replace('/[^a-zA-Z0-9]/', '', $orderRef) ?: 'order';

        return ['topdawg-'.$slug.'@'.$domain, true];
    }
    ...
    /**
     * TopDawg descriptions are often JSON module trees (moduleList / mobileDetail), not plain HTML.
     */
     ...
     */
    protected function formatProductVariants(array $ae, array $aeSkuRows, ?TopDawgProduct $metric = null, ?ShopifySku $shopify = null): array
    {
        $variants = [];
        ...
        if ($variants === [] && $metric && $metric->topdawg_listing_id && $metric->sku && $metric->sku !== $metric->topdawg_listing_id) {
            $variants[] = [
    ...
    /**
     * Prefer live/API variant stock for this SKU, else local remaining_inventory from pricing sync.
     *
     * @param  array<int, array<string, mixed>>  $variants
     */
    protected function resolveProductAeStock(array $variants, ?TopDawgProduct $metric, string $shopifySku): ?int
    {
     ...
     */
    protected function resolveListingLastSyncedAt(?TopDawgProduct $metric, string $shopifySku): ?\Carbon\Carbon
    {
        $candidates = array_filter([
            $metric?->updated_at,
    ...
    {
        $sku = trim($shopifySku);
        if ($sku === '' || ! Schema::hasTable('topdawg_products')) {
            return null;
        }
    ...
    protected function isMetricLinked(?TopDawgProduct $metric, string $shopifySku): bool
    {
     ...
     */
    protected function resolveOrderLineImage(array $product, ?TopDawgOrderMetric $db = null): ?string
    {
        foreach ([
    ...
    protected function normalizeOrderImageUrl(?string $url): ?string
    {
        ...
        if (preg_match('#^https?://#i', $url)
            ...
            && ! str_contains($url, 'topdawg-media.com')
            && ! str_contains($url, 'topdawg.com')) {
            return filter_var($url, FILTER_VALIDATE_URL) ? $url : null;
        }

        $filename = $this->extractOrderImageFilename($url);
        if ($filename) {
            return $this->buildTopDawgMediaImageUrl($filename);
        }
    ...
    protected function buildTopDawgMediaImageUrl(string $filename): string
    {
        return 'https://ae-pic-a1.topdawg-media.com/kf/'.ltrim($filename, '/');
    }
    ...
    protected function resolveOrderLineImageFallback(?string $sku, ?string $productId): ?string
    {
        ...
        if ($productId) {
            $metric = TopDawgProduct::query()->where('topdawg_listing_id', $productId)->first();
