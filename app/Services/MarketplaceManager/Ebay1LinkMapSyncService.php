...
/**
 * Refresh / ensure ebay_metrics link map rows (sku + item_id).
 * item_id is the MM product_id. Linked when item_id is present and ≠ sku.
 */
class Ebay1LinkMapSyncService
{
    private const CACHE_KEY = 'ebay1_link_map_sync';
    ...
    public function syncPage(int $page = 1, int $pageSize = 200, bool $reset = false): array
    {
        if (! Schema::hasTable('ebay_metrics')) {
            return $this->fail('ebay_metrics table missing.');
        }
        ...
        $this->updateProgress([
            'running' => true,
            'page' => $page,
            'message' => "Reading eBay 1 metrics page {$page}…",
        ]);
        ...
        $message = $done
            ? "Validated {$totalUpserted} ebay_metrics row(s) ({$page} page(s), {$totalCount} rows). Linked listings: {$linked}."
    ...
    {
        Log::warning('Ebay1LinkMapSyncService: '.$message);
        $this->updateProgress([
            'running' => false,
            'done' => true,
            'error' => true,
