...
class ReverbOrderSyncService
{
    /** Reverb order list queries are safest in ~30-day windows. */
    ...
    /**
     * Fetch orders from Reverb and upsert into reverb_order_metrics.
     *
    ...
    public function fetchAndStore(int $days = 7, int $pageSize = 50): array
    {
        if (empty($this->aliExpressApi->getAccessToken())) {
            return ['fetched' => 0, 'stored' => 0, 'message' => 'REVERB_TOKEN missing.'];
        }

        if (! Schema::hasTable('reverb_order_metrics')) {
            return ['fetched' => 0, 'stored' => 0, 'message' => 'Run migrations for reverb_order_metrics.'];
        }
        ...
        $dateRange = [
            'updated_start_date' => $start->toIso8601String(),
            'updated_end_date' => $end->toIso8601String(),
        ];
    ...
    public function fetchAndStoreFromDate(string $fromDate, int $pageSize = 50): array
    {
        ...
        while ($chunkEnd->gt($start)) {
            ...
            $dateRange = [
                'updated_start_date' => $chunkStart->toIso8601String(),
                'updated_end_date' => $chunkEnd->toIso8601String(),
            ];
    ...
    protected function fetchOrdersInDateRange(array $dateRange, int $pageSize): array
    {
        ...
        Log::info('[ReverbOrderSync] Date range complete', [
            'start' => $dateRange['updated_start_date'] ?? $dateRange['create_date_start'] ?? null,
            'end' => $dateRange['updated_end_date'] ?? $dateRange['create_date_end'] ?? null,
            'fetched' => $fetched,
            'pages' => $page,
        ]);
    ...
    protected function storeOrder(array $order): int
    {
        ...
        if ($orderDate) {
            try {
                $parsedDate = Carbon::parse($orderDate, 'America/Los_Angeles');
                if ($parsedDate->lt($this->minOrderDate())) {
                    return 0;
                }
            } catch (\Throwable $e) {
                $parsedDate = null;
    ...
    {
        foreach (['total', 'amount_product', 'order_amount', 'total_amount', 'pay_amount'] as $key) {
            $amount = $order[$key] ?? null;
            if (is_array($amount)) {
