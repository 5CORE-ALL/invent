...
class Temu2DetailFormatter
{
     ...
     */
    public function formatProduct(?array $aeLive, ?Temu2Metric $metric, ShopifySku $shopify, array $aeSkuRows = []): array
    {
        $ae = $this->arr($aeLive);
        ...
        $cachedPrice = $this->money($metric?->base_price);
        ...
        return [
            'shopify' => [
                'sku' => $shopify->sku,
                'goods_summary' => $shopify->goods_summary,
            ...
            'link' => [
                ...
                'title' => $metric?->goods_summary,
                'price' => $metric?->base_price,
                'l30' => $metric?->r_l30,
                'l60' => $metric?->r_l60,
            ...
            'temu2' => [
                'product_id' => $this->str($ae['product_id'] ?? $metric?->goods_id),
                'title' => $this->extractProductTitle($ae) ?? $metric?->goods_summary,
     ...
     * @return array<string, mixed>
     */
    public function formatOrder(array $orderRoot, Collection $lines, Temu2Order $primaryLine): array
    {
        ...
        return [
            'summary' => [
                'order_id' => (string) ($order['parent_order_sn'] ?? $order['order_sn'] ?? $primaryLine->parent_order_sn),
                'order_number' => $order['parent_order_sn'] ?? $primaryLine->parent_order_sn ?? null,
                'status' => $order['order_status_text'] ?? $order['parent_order_status_text'] ?? $primaryLine->order_status_text ?? $primaryLine->parent_order_status_text,
                ...
                'created' => $order['parent_order_time'] ?? $order['gmt_create'] ?? $order['create_time'] ?? $primaryLine->parent_order_time,
    ...
    /**
     * Build Shopify REST order payload from formatted Temu 2 order detail.
     *
    ...
    public function buildShopifyOrderPayload(array $detail, Collection $lines, array $tags): array
    {
        ...
        $noteLines = [
            'Imported from Temu 2 Order #'.$orderRef,
        ];
        ...
        $noteAttrs = [
            ['name' => 'temu_order_id', 'value' => $orderRef],
        ];
        if ($emailIsPlaceholder) {
            $noteAttrs[] = ['name' => 'temu_email_is_placeholder', 'value' => 'true'];
        }
        foreach ([
            'temu_buyer_login' => $summary['buyer_login_id'] ?? null,
            'temu_payment_method' => $payment['method'] ?? null,
            'temu_tracking_number' => $shipment['tracking'] ?? null,
            'temu_shipping_method' => $shipment['service'] ?? null,
            'temu_tax_number' => $shipping['tax_number'] ?? null,
            'temu_buyer_email' => $shipping['email'] ?? null,
            'temu_buyer_phone' => $shipping['phone'] ?? null,
            'temu_platform_promotion' => isset($funds['platform_promotion']) ? number_format((float) $funds['platform_promotion'], 2, '.', '') : null,
            'temu_platform_offer' => isset($funds['platform_offer']) ? number_format((float) $funds['platform_offer'], 2, '.', '') : null,
            'temu_order_amount' => isset($funds['order_amount']) ? number_format((float) $funds['order_amount'], 2, '.', '') : null,
            'temu_customer_paid' => $customerPaid !== null ? number_format($customerPaid, 2, '.', '') : null,
            'temu_seller_amount_paid' => isset($funds['seller_amount_paid']) ? number_format((float) $funds['seller_amount_paid'], 2, '.', '') : null,
            'temu_platform_commission' => isset($funds['platform_commission']) ? number_format((float) $funds['platform_commission'], 2, '.', '') : null,
            'temu_transaction_service_fee' => isset($funds['transaction_service_fee']) ? number_format((float) $funds['transaction_service_fee'], 2, '.', '') : null,
            'temu_tax' => $taxAmount > 0 ? number_format($taxAmount, 2, '.', '') : null,
            'temu_platform_offer_tax' => isset($funds['platform_offer_tax']) ? number_format((float) $funds['platform_offer_tax'], 2, '.', '') : null,
        ] as $name => $value) {
            if ($value !== null && $value !== '') {
        ...
        if ($shippingCost > 0) {
            $payload['shipping_lines'] = [[
                'title' => (string) ($shipment['service'] ?? 'Temu shipping'),
    ...
    protected function splitShopifyCustomerName(array $detail, string $recipient = ''): array
    {
        ...
        if ($fullName === '') {
            return ['Temu 2', 'Customer'];
        }

        $parts = preg_split('/\s+/u', $fullName, 2) ?: [];
        $firstName = (string) ($parts[0] ?? 'Temu 2');
        if (! isset($parts[1])) {
    ...
    public function resolveShopifySourceAttribution(string $orderRef, ?array $settings = null): array
    {
        ...
        $handle = trim((string) (
            ...
            ?? config('services.temu2.shopify_source_name')
            ?? 'temu2'
        ));
        $urlTemplate = (string) (
            $settings['order']['shopify_source_url_template']
            ?? config('services.temu2.shopify_source_url_template')
            ?? 'https://csp.temu.com/m_apps/order-manage/order_detail?orderId={order_id}'
        );

        return [
            'source_name' => $handle !== '' ? $handle : 'temu2',
            'source_identifier' => $orderRef,
            'source_url' => str_replace('{order_id}', $orderRef, $urlTemplate),
            'referring_site' => 'https://www.temu.com/',
        ];
    }
    ...
    public function shopifySourceDisplayName(?array $settings = null): string
    {
        ...
        $name = trim((string) (
            ...
            ?? config('services.temu2.shopify_source_display_name')
            ?? 'Temu 2'
        ));

        return $name !== '' ? $name : 'Temu 2';
    }
    ...
    public function resolveShopifyCustomerEmail(string $orderRef, ?string $rawEmail): array
    {
        ...
        $domain = (string) env('TEMU_SHOPIFY_PLACEHOLDER_EMAIL_DOMAIN', 'import.5coremanagement.com');
        $slug = preg_replace('/[^a-zA-Z0-9]/', '', $orderRef) ?: 'order';

        return ['temu-'.$slug.'@'.$domain, true];
    }
    ...
    /**
     * Temu 2 descriptions are often JSON module trees (moduleList / mobileDetail), not plain HTML.
     */
     ...
     */
    protected function formatProductVariants(array $ae, array $aeSkuRows, ?Temu2Metric $metric = null, ?ShopifySku $shopify = null): array
    {
        $variants = [];
        ...
        if ($variants === [] && $metric && $metric->goods_id && $metric->sku && $metric->sku !== $metric->goods_id) {
            $variants[] = [
                'sku' => $this->str($metric->sku),
                'price' => $this->money($metric->base_price),
    ...
    /**
     * Prefer live/API variant stock for this SKU, else local quantity from pricing sync.
     *
     * @param  array<int, array<string, mixed>>  $variants
     */
    protected function resolveProductAeStock(array $variants, ?Temu2Metric $metric, string $shopifySku): ?int
    {
     ...
     */
    protected function resolveListingLastSyncedAt(?Temu2Metric $metric, string $shopifySku): ?\Carbon\Carbon
    {
        $candidates = array_filter([
            $metric?->updated_at,
    ...
    {
        $sku = trim($shopifySku);
        if ($sku === '' || ! Schema::hasTable('temu_products')) {
            return null;
        }
    ...
    protected function isMetricLinked(?Temu2Metric $metric, string $shopifySku): bool
    {
    ...
    protected function localAeStockForSku(string $sku): ?int
    {
        return MarketplaceListingStockResolver::resolveMarketplaceQty(
            MarketplaceListingStockResolver::CHANNEL_TEMU2,
            $sku
        );
    }
     ...
     */
    protected function resolveOrderLineImage(array $product, ?Temu2Order $db = null): ?string
    {
        foreach ([
    ...
    protected function normalizeOrderImageUrl(?string $url): ?string
    {
        ...
        if (preg_match('#^https?://#i', $url)
            ...
            && ! str_contains($url, 'temu-media.com')
            && ! str_contains($url, 'temu.com')) {
            return filter_var($url, FILTER_VALIDATE_URL) ? $url : null;
        }

        $filename = $this->extractOrderImageFilename($url);
        if ($filename) {
            return $this->buildTemuMediaImageUrl($filename);
        }
    ...
    protected function buildTemuMediaImageUrl(string $filename): string
    {
        return 'https://ae-pic-a1.temu-media.com/kf/'.ltrim($filename, '/');
    }
    ...
    protected function resolveOrderLineImageFallback(?string $sku, ?string $productId): ?string
    {
        ...
        if ($productId) {
            $metric = Temu2Metric::query()->where('goods_id', $productId)->first();
    ...
    {
        foreach (['subject', 'product_name', 'title', 'goods_summary'] as $key) {
