...
class ListingManagerPublishDispatcher
{
    ...
    public function publish(ListingManagerChannelDraft $draft, array $details): array
    {
        ...
        if (self::supportsListingApi($channelName)) {
            ...
            if (in_array($key, ['newegg', 'neweggb2c', 'neweggb2b'], true)) {
                $images = is_array($details['images'] ?? null) ? $details['images'] : [];
                $overrides = [
                    'title' => trim((string) $draft->title),
                    ...
                    'manufacturer' => trim((string) ($details['manufacturer'] ?? $details['brand'] ?? '5 Core')),
    ...
    {
        $raw = trim((string) ($details['primary_category_id'] ?? $details['category_uuid'] ?? $details['category_id'] ?? ''));
        if ($raw !== '' && preg_match('/^\d+$/', $raw) && (int) $raw > 0) {
            return (int) $raw;
        }
