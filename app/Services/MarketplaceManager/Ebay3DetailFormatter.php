...
class Ebay3DetailFormatter
{
     ...
     */
    public function formatProduct(?array $aeLive, ?Ebay3Metric $metric, ShopifySku $shopify, array $aeSkuRows = []): array
    {
        $ae = $this->arr($aeLive);
     ...
     * @param  Collection<int, Ebay3OrderMetric>  $lines
     * @return array<string, mixed>
     */
    public function formatOrder(array $orderRoot, Collection $lines, Ebay3OrderMetric $primaryLine): array
    {
    ...
    /**
     * Build Shopify REST order payload from formatted eBay 3 order detail.
     *
    ...
    public function buildShopifyOrderPayload(array $detail, Collection $lines, array $tags): array
    {
        ...
        $noteLines = [
            'Imported from eBay 3 Order #'.$orderRef,
        ];
        ...
        $noteAttrs = [
            ['name' => 'ebay3_order_id', 'value' => $orderRef],
        ];
        if ($emailIsPlaceholder) {
            $noteAttrs[] = ['name' => 'ebay3_email_is_placeholder', 'value' => 'true'];
        }
        foreach ([
            'ebay3_buyer_login' => $summary['buyer_login_id'] ?? null,
            'ebay3_payment_method' => $payment['method'] ?? null,
            'ebay3_tracking_number' => $shipment['tracking'] ?? null,
            'ebay3_shipping_method' => $shipment['service'] ?? null,
            'ebay3_tax_number' => $shipping['tax_number'] ?? null,
            'ebay3_buyer_email' => $shipping['email'] ?? null,
            'ebay3_buyer_phone' => $shipping['phone'] ?? null,
            'ebay3_platform_promotion' => isset($funds['platform_promotion']) ? number_format((float) $funds['platform_promotion'], 2, '.', '') : null,
            'ebay3_platform_offer' => isset($funds['platform_offer']) ? number_format((float) $funds['platform_offer'], 2, '.', '') : null,
            'ebay3_order_amount' => isset($funds['order_amount']) ? number_format((float) $funds['order_amount'], 2, '.', '') : null,
            'ebay3_customer_paid' => $customerPaid !== null ? number_format($customerPaid, 2, '.', '') : null,
            'ebay3_seller_amount_paid' => isset($funds['seller_amount_paid']) ? number_format((float) $funds['seller_amount_paid'], 2, '.', '') : null,
            'ebay3_platform_commission' => isset($funds['platform_commission']) ? number_format((float) $funds['platform_commission'], 2, '.', '') : null,
            'ebay3_transaction_service_fee' => isset($funds['transaction_service_fee']) ? number_format((float) $funds['transaction_service_fee'], 2, '.', '') : null,
            'ebay3_tax' => $taxAmount > 0 ? number_format($taxAmount, 2, '.', '') : null,
            'ebay3_platform_offer_tax' => isset($funds['platform_offer_tax']) ? number_format((float) $funds['platform_offer_tax'], 2, '.', '') : null,
        ] as $name => $value) {
            if ($value !== null && $value !== '') {
        ...
        if ($shippingCost > 0) {
            $payload['shipping_lines'] = [[
                'title' => (string) ($shipment['service'] ?? 'eBay 3 shipping'),
    ...
    protected function splitShopifyCustomerName(array $detail, string $recipient = ''): array
    {
        ...
        if ($fullName === '') {
            return ['eBay 3', 'Customer'];
        }

        $parts = preg_split('/\s+/u', $fullName, 2) ?: [];
        $firstName = (string) ($parts[0] ?? 'eBay 3');
        if (! isset($parts[1])) {
    ...
    public function resolveShopifySourceAttribution(string $orderRef, ?array $settings = null): array
    {
        ...
        $handle = trim((string) (
            ...
            ?? config('services.ebay3.shopify_source_name')
            ?? 'ebay3'
        ));
        $urlTemplate = (string) (
            $settings['order']['shopify_source_url_template']
            ?? config('services.ebay3.shopify_source_url_template')
            ?? 'https://www.ebay.com/mesh/ord/details?orderid={order_id}'
        );

        return [
            'source_name' => $handle !== '' ? $handle : 'ebay3',
            'source_identifier' => $orderRef,
            'source_url' => str_replace('{order_id}', $orderRef, $urlTemplate),
            'referring_site' => 'https://www.ebay.com/',
        ];
    }
    ...
    public function shopifySourceDisplayName(?array $settings = null): string
    {
        ...
        $name = trim((string) (
            ...
            ?? config('services.ebay3.shopify_source_display_name')
            ?? 'eBay 3'
        ));

        return $name !== '' ? $name : 'eBay 3';
    }
    ...
    public function resolveShopifyCustomerEmail(string $orderRef, ?string $rawEmail): array
    {
        ...
        $domain = (string) env('EBAY3_SHOPIFY_PLACEHOLDER_EMAIL_DOMAIN', 'import.5coremanagement.com');
        $slug = preg_replace('/[^a-zA-Z0-9]/', '', $orderRef) ?: 'order';

        return ['ebay3-'.$slug.'@'.$domain, true];
    }
    ...
    /**
     * eBay 3 descriptions are often JSON module trees (moduleList / mobileDetail), not plain HTML.
     */
     ...
     */
    protected function formatProductVariants(array $ae, array $aeSkuRows, ?Ebay3Metric $metric = null, ?ShopifySku $shopify = null): array
    {
        $variants = [];
    ...
    /**
     * Prefer live/API variant stock for this SKU, else local ebay_stock from pricing sync.
     *
     * @param  array<int, array<string, mixed>>  $variants
     */
    protected function resolveProductAeStock(array $variants, ?Ebay3Metric $metric, string $shopifySku): ?int
    {
     ...
     */
    protected function resolveListingLastSyncedAt(?Ebay3Metric $metric, string $shopifySku): ?\Carbon\Carbon
    {
        $candidates = array_filter([
            $metric?->updated_at,
    ...
    {
        $sku = trim($shopifySku);
        if ($sku === '' || ! Schema::hasTable('ebay_3_metrics')) {
            return null;
        }

        $row = Ebay3Metric::query()
    ...
    protected function isMetricLinked(?Ebay3Metric $metric, string $shopifySku): bool
    {
     ...
     */
    protected function resolveOrderLineImage(array $product, ?Ebay3OrderMetric $db = null): ?string
    {
        foreach ([
    ...
    protected function normalizeOrderImageUrl(?string $url): ?string
    {
        ...
        if (preg_match('#^https?://#i', $url)
            ...
            && ! str_contains($url, 'ebayimg.com')
            && ! str_contains($url, 'ebay3.com')) {
            return filter_var($url, FILTER_VALIDATE_URL) ? $url : null;
        }

        $filename = $this->extractOrderImageFilename($url);
        if ($filename) {
            return $this->buildEbay3MediaImageUrl($filename);
        }
    ...
    protected function buildEbay3MediaImageUrl(string $filename): string
    {
        return 'https://ae-pic-a1.ebayimg.com/kf/'.ltrim($filename, '/');
    }
    ...
    protected function resolveOrderLineImageFallback(?string $sku, ?string $productId): ?string
    {
        ...
        if ($productId) {
            $metric = Ebay3Metric::query()->where('product_id', $productId)->first();
