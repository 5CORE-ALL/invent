...
/**
 * Refresh macy_products from Mirakl Connect inventory (or paginate local rows).
 */
class MacyLinkMapSyncService
{
    private const CACHE_KEY = 'macy_link_map_sync';
    ...
    public function syncPage(int $page = 1, int $pageSize = 200, bool $reset = false): array
    {
        if (! Schema::hasTable('macy_products')) {
            return $this->fail('macy_products table missing.');
        }
        ...
        $this->updateProgress([
            'running' => true,
            'page' => $page,
            'message' => "Reading macy_products page {$page}…",
        ]);

        $query = MacyProduct::query()
            ->whereNotNull('sku')
            ->where('sku', '!=', '')
            ->orderBy('id');
        ...
        $message = $done
            ? "Updated {$totalUpserted} SKU link(s) from macy_products ({$page} page(s), {$totalCount} rows)."
    ...
    protected function syncFromApi(): array
    {
        $this->updateProgress([
            'running' => true,
            'page' => 1,
            'message' => 'Refreshing Macy\'s inventory from Mirakl Connect…',
        ]);

        if (! $this->macyApi->isConfigured()) {
            return $this->fail('Macy\'s API credentials missing.');
        }

        try {
            $this->macyApi->getInventory();
        } catch (\Throwable $e) {
            Log::warning('MacyLinkMapSyncService: getInventory failed', ['error' => $e->getMessage()]);

            return $this->fail('Macy\'s inventory refresh failed: '.$e->getMessage());
        }

        $linked = (int) MacyProduct::query()
            ->whereNotNull('sku')
            ->where('sku', '!=', '')
            ->count();

        $message = "Refreshed Macy\'s link map — {$linked} listed SKU(s).";
        $this->updateProgress([
            'running' => false,
            'page' => 1,
            'total_page' => 1,
