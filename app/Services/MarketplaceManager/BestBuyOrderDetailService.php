...
class BestBuyOrderDetailService
{
    /**
    ...
    public function fetchAndPersistOrderDetail(string $orderId): array
    {
        ...
        $result = app(BestBuyOrderSyncService::class)->fetchAndStore(60);
        if (empty($result['success'])) {
            return ['success' => false, 'message' => $result['message'] ?? 'Fetch failed.'];
        }

        $exists = BestBuyOrderMetric::query()
            ->where('order_id', $orderId)
            ->orWhere('channel_order_id', $orderId)
            ->exists();

        if (! $exists) {
            return ['success' => false, 'message' => 'Order not found in Best Buy USA rows after refresh.'];
        }
     ...
     */
    public function resolveOrderRoot(BestBuyOrderMetric $line): array
    {
