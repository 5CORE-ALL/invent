    ...
    public function pullForParents(array $parentOrderSns, bool $refreshExisting = false): array
    {
        ...
        if ($parents === []) {
            return [
                ...
                'missing' => 0,
                'failed' => 0,
                'message' => 'Temu tracking pull: no selected parent orders.',
                'rows' => [],
            ];
        }
        ...
        foreach ($parents as $parentOrderSn) {
            ...
            if (! empty($result['success']) && ! empty($result['updated'])) {
                ...
                $rows[] = [
                    ...
                    'fulfillment_status' => 'Temu API',
                    'shipment_status' => '',
                    'note' => 'Pulled from Temu OpenAPI (selected rows)',
                ];
            } elseif (! empty($result['missing'])) {
                $missing++;
            } elseif (empty($result['success'])) {
        ...
        $message = "Temu tracking pull (selected): checked {$checked}, updated {$updated}, missing {$missing}, failed {$failed}.";
        if ($firstError !== null) {
            $message .= ' First error: '.$firstError;
        }
