...
class Ebay2OrderPushService
{
     ...
     */
    public function previewShopifyPush(Ebay2OrderMetric $order): array
    {
    ...
    /**
     * Push eBay 2 ship-to onto an already-imported Shopify order.
     * Used by "Pull from eBay 2" so address-only refreshes update Shopify.
     *
     * @return array{success: bool, skipped?: bool, message: string, shopify_order_id?: string}
     */
    public function syncShippingAddressToShopify(Ebay2OrderMetric $order): array
    {
        ...
        if ($orderRoot === []) {
            return [
                'success' => false,
                'skipped' => true,
                'message' => 'No eBay 2 order payload available to build an address.',
            ];
        }

        $detail = $this->formatter->formatOrder($orderRoot, $lines, $order);
        $orderPayload = $this->formatter->buildShopifyOrderPayload($detail, $lines, ['ebay2']);
        ...
        if ($address1 === '') {
            return [
                'success' => false,
                'skipped' => true,
                'message' => 'eBay 2 still has no shipping address for this order.',
            ];
        }
    ...
    /**
     * Auto-fill missing Shopify shipping/customer address from eBay 2 for linked orders.
     *
    ...
    public function syncPendingAddressesToShopify(int $limit = 40): array
    {
        ...
        foreach ($unique as $line) {
            ...
            // Refresh eBay 2 ShipTo when possible so address automation stays current.
            try {
            ...
            if (! empty($result['success']) && empty($result['skipped'])) {
                ...
                $failed++;
                Log::warning('Ebay2OrderPushService: auto address sync failed', [
    ...
    public function importToShopify(Ebay2OrderMetric $order): ?string
    {
        ...
        if ($orderId !== '') {
            $localLinked = Ebay2OrderMetric::query()
                ->where('order_id', $orderId)
                ->whereNotNull('shopify_order_id')
                ->where('shopify_order_id', '!=', '')
        ...
        $existing = $this->findExistingShopifyOrderByRefs(
            ...
            ['ebay2-', 'ebay-'],
            ['ebay2_order_id', 'ebay_order_id'],
            'Ebay2OrderPushService'
        );
        ...
        if (! empty($existing['id'])) {
            if ($orderId !== '') {
                $this->linkEbay2OrderToShopify($orderId, (string) $existing['id']);
            } else {
                $order->update([
            ...
            Log::info('Ebay2OrderPushService: linked existing Shopify order (duplicate avoided)', [
                'order_id' => $orderId,
    ...
    protected function linkEbay2OrderToShopify(string $orderId, string $shopifyOrderId): void
    {
        Ebay2OrderMetric::query()
            ->where('order_id', $orderId)
            ->update([
    ...
    /**
     * After Shopify decrements stock for the imported order, push live qty to eBay 2.
     */
    protected function syncInventoryAfterPush(Ebay2OrderMetric $order): void
    {
        $skus = Ebay2OrderMetric::query()
            ->where('order_id', (string) $order->order_id)
            ->pluck('sku')
        ...
        try {
            $result = app(Ebay2InventorySyncService::class)
                ->syncSkusFromShopify($skus, $this->shopifyConfig());

            Log::info('Ebay2OrderPushService: post-push inventory sync', [
                'order_id' => $order->order_id,
                'skus' => $skus,
                'result' => $result,
            ]);
        } catch (\Throwable $e) {
            Log::warning('Ebay2OrderPushService: post-push inventory sync failed', [
                'order_id' => $order->order_id,
                'skus' => $skus,
                'error' => $e->getMessage(),
            ]);
     ...
     * @return array<string, mixed>
     */
    public function buildImportPlan(Ebay2OrderMetric $order): array
    {
        if ($order->shopify_order_id) {
            return [
                'success' => false,
        ...
        if ($orderRoot === [] && $cachedRoot === []) {
            return [
                'success' => false,
                'message' => $detailResult['message'] ?? 'Could not load eBay 2 order details before Shopify push.',
            ];
        }
        ...
        $carrier = (string) (($detail['shipment']['service'] ?? '') ?: 'eBay 2');

        $warnings = $this->buildImportWarnings($detail, $lineResolution, $orderPayload);
        if (empty($detailResult['success'])) {
            $warnings[] = 'Could not refresh live eBay 2 details ('.($detailResult['message'] ?? 'unknown').') — using cached order payload.';
        }

        return [
            'success' => true,
            'ebay2_order_id' => $orderId,
    ...
    protected function buildImportWarnings(array $detail, array $lineResolution, array $orderPayload): array
    {
        ...
        if ($this->payloadUsesPlaceholderEmail($orderPayload)) {
            $warnings[] = 'eBay 2 did not provide a buyer email — a placeholder email will be used for Shopify.';
        } elseif (empty($shipping['email'] ?? null) && empty($orderPayload['customer']['email'] ?? null)) {
            $warnings[] = 'No buyer email on eBay 2 order.';
        }

        $sourceName = trim((string) ($orderPayload['source_name'] ?? ''));
        if ($sourceName === '') {
            $warnings[] = 'No Shopify source_name set — Channel information may show your app name instead of eBay 2.';
        }
        if (empty($shipping['phone'] ?? null)) {
            $warnings[] = 'No buyer phone on eBay 2 order.';
        }
    ...
    protected function resolveLineItemsForShopify(array $orderPayload, Collection $lines): array
    {
        ...
        foreach ($sourceItems as $item) {
            ...
            if ($title === '') {
                $title = $sku !== '' ? $sku : 'eBay 2 order item';
            }
            ...
            $custom = [
                'title' => $title !== '' ? $title : 'eBay 2 order item',
                'price' => $price,
                'quantity' => $quantity,
            ];
            if ($sku !== '') {
        ...
        if ($resolved === []) {
            $line = $lines->first();
            $resolved[] = [
                'title' => (string) ($line?->display_title ?: 'eBay 2 order item'),
                'price' => number_format((float) ($line?->amount ?? 0), 2, '.', ''),
                'quantity' => max(1, (int) ($line?->quantity ?? 1)),
            ];
            $meta[] = [
        ...
        try {
            for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
                ...
                if ($retryable && $attempt < $maxAttempts) {
                    ...
                    Log::warning('Ebay2OrderPushService: Shopify order create retrying', [
                ...
                Log::error('Ebay2OrderPushService: Shopify order create failed', [
                    'status' => $response->status(),
    ...
    protected function addFulfillmentTracking(string $shopifyOrderId, string $trackingNumber, string $carrier): void
    {
        ...
        try {
            ...
            Log::warning('Ebay2OrderPushService: fulfillment tracking failed', [
                'shopify_order_id' => $shopifyOrderId,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    protected function findShopifyVariantIdBySku(string $sku): ?int
    {
        ...
        try {
            ...
            Log::warning('Ebay2OrderPushService: variant lookup failed', ['sku' => $sku, 'error' => $e->getMessage()]);
    ...
    protected function payloadUsesPlaceholderEmail(array $orderPayload): bool
    {
        foreach ($orderPayload['note_attributes'] ?? [] as $attr) {
            if (($attr['name'] ?? '') === 'ebay2_email_is_placeholder' && ($attr['value'] ?? '') === 'true') {
                return true;
            }
        }
    ...
    /**
     * If live eBay 2 refresh blanked ship-to / buyer email, restore from cached root.
     *
