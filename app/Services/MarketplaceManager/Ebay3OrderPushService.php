...
class Ebay3OrderPushService
{
     ...
     */
    public function previewShopifyPush(Ebay3OrderMetric $order): array
    {
    ...
    /**
     * Push eBay 3 ship-to onto an already-imported Shopify order.
     * Used by "Pull from eBay 3" so address-only refreshes update Shopify.
     *
     * @return array{success: bool, skipped?: bool, message: string, shopify_order_id?: string}
     */
    public function syncShippingAddressToShopify(Ebay3OrderMetric $order): array
    {
        ...
        if ($orderRoot === []) {
            return [
                'success' => false,
                'skipped' => true,
                'message' => 'No eBay 3 order payload available to build an address.',
            ];
        }

        $detail = $this->formatter->formatOrder($orderRoot, $lines, $order);
        $orderPayload = $this->formatter->buildShopifyOrderPayload($detail, $lines, ['ebay3']);
        ...
        if ($address1 === '') {
            return [
                'success' => false,
                'skipped' => true,
                'message' => 'eBay 3 still has no shipping address for this order.',
            ];
        }
    ...
    /**
     * Auto-fill missing Shopify shipping/customer address from eBay 3 for linked orders.
     *
    ...
    public function syncPendingAddressesToShopify(int $limit = 40): array
    {
        ...
        foreach ($unique as $line) {
            ...
            // Refresh eBay 3 ShipTo when possible so address automation stays current.
            try {
            ...
            if (! empty($result['success']) && empty($result['skipped'])) {
                ...
                $failed++;
                Log::warning('Ebay3OrderPushService: auto address sync failed', [
    ...
    public function importToShopify(Ebay3OrderMetric $order): ?string
    {
        ...
        if ($orderId !== '') {
            $localLinked = Ebay3OrderMetric::query()
                ->where('order_id', $orderId)
                ->whereNotNull('shopify_order_id')
                ->where('shopify_order_id', '!=', '')
                ->value('shopify_order_id');
            if ($localLinked) {
                $this->linkEbay3OrderToShopify($orderId, (string) $localLinked);
        ...
        $existing = $this->findExistingShopifyOrderByRefs(
            ...
            ['ebay3-', 'ebay-'],
            ['ebay3_order_id', 'ebay_order_id'],
            'Ebay3OrderPushService'
        );
        ...
        if (! empty($existing['id'])) {
            if ($orderId !== '') {
                $this->linkEbay3OrderToShopify($orderId, (string) $existing['id']);
            } else {
                $order->update([
            ...
            Log::info('Ebay3OrderPushService: linked existing Shopify order (duplicate avoided)', [
                'order_id' => $orderId,
    ...
    protected function linkEbay3OrderToShopify(string $orderId, string $shopifyOrderId): void
    {
        Ebay3OrderMetric::query()
            ->where('order_id', $orderId)
            ->update([
    ...
    /**
     * After Shopify decrements stock for the imported order, push live qty to eBay 3.
     */
    protected function syncInventoryAfterPush(Ebay3OrderMetric $order): void
    {
        $skus = Ebay3OrderMetric::query()
            ->where('order_id', (string) $order->order_id)
            ->pluck('sku')
        ...
        try {
            $result = app(Ebay3InventorySyncService::class)
                ->syncSkusFromShopify($skus, $this->shopifyConfig());

            Log::info('Ebay3OrderPushService: post-push inventory sync', [
                'order_id' => $order->order_id,
                'skus' => $skus,
                'result' => $result,
            ]);
        } catch (\Throwable $e) {
            Log::warning('Ebay3OrderPushService: post-push inventory sync failed', [
                'order_id' => $order->order_id,
                'skus' => $skus,
                'error' => $e->getMessage(),
            ]);
     ...
     * @return array<string, mixed>
     */
    public function buildImportPlan(Ebay3OrderMetric $order): array
    {
        if ($order->shopify_order_id) {
            return [
                'success' => false,
        ...
        if ($orderRoot === [] && $cachedRoot === []) {
            return [
                'success' => false,
                'message' => $detailResult['message'] ?? 'Could not load eBay 3 order details before Shopify push.',
            ];
        }
        ...
        $carrier = (string) (($detail['shipment']['service'] ?? '') ?: 'eBay 3');

        $warnings = $this->buildImportWarnings($detail, $lineResolution, $orderPayload);
        if (empty($detailResult['success'])) {
            $warnings[] = 'Could not refresh live eBay 3 details ('.($detailResult['message'] ?? 'unknown').') — using cached order payload.';
        }

        return [
            'success' => true,
            'ebay3_order_id' => $orderId,
    ...
    protected function buildImportWarnings(array $detail, array $lineResolution, array $orderPayload): array
    {
        ...
        if ($this->payloadUsesPlaceholderEmail($orderPayload)) {
            $warnings[] = 'eBay 3 did not provide a buyer email — a placeholder email will be used for Shopify.';
        } elseif (empty($shipping['email'] ?? null) && empty($orderPayload['customer']['email'] ?? null)) {
            $warnings[] = 'No buyer email on eBay 3 order.';
        }

        $sourceName = trim((string) ($orderPayload['source_name'] ?? ''));
        if ($sourceName === '') {
            $warnings[] = 'No Shopify source_name set — Channel information may show your app name instead of eBay 3.';
        }
        if (empty($shipping['phone'] ?? null)) {
            $warnings[] = 'No buyer phone on eBay 3 order.';
        }
    ...
    protected function resolveLineItemsForShopify(array $orderPayload, Collection $lines): array
    {
        ...
        foreach ($sourceItems as $item) {
            ...
            if ($title === '') {
                $title = $sku !== '' ? $sku : 'eBay 3 order item';
            }
            ...
            $custom = [
                'title' => $title !== '' ? $title : 'eBay 3 order item',
                'price' => $price,
                'quantity' => $quantity,
            ];
            if ($sku !== '') {
        ...
        if ($resolved === []) {
            $line = $lines->first();
            $resolved[] = [
                'title' => (string) ($line?->display_title ?: 'eBay 3 order item'),
                'price' => number_format((float) ($line?->amount ?? 0), 2, '.', ''),
                'quantity' => max(1, (int) ($line?->quantity ?? 1)),
            ];
            $meta[] = [
        ...
        try {
            for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
                ...
                if ($retryable && $attempt < $maxAttempts) {
                    ...
                    Log::warning('Ebay3OrderPushService: Shopify order create retrying', [
                ...
                Log::error('Ebay3OrderPushService: Shopify order create failed', [
                    'status' => $response->status(),
            ...
            Log::error('Ebay3OrderPushService: exception', ['error' => $e->getMessage()]);

            return null;
        }
    }

    protected function fulfillShopifyForImportedOrder(Ebay3OrderMetric $order): void
    {
        $id = (int) $order->id;
        if ($id < 1) {
            return;
        ...
        try {
            $result = app(VeeqoShopifyFulfillmentService::class)->fulfillMarketplaceOrder('ebay3', $id);
            if (empty($result['success']) && empty($result['skipped'])) {
                Log::warning('Ebay3OrderPushService: Shopify fulfillment after import failed', [
                    'order_id' => $order->order_id,
                    'result' => $result,
                ]);
            }
        } catch (\Throwable $e) {
            Log::warning('Ebay3OrderPushService: Shopify fulfillment after import exception', [
                'order_id' => $order->order_id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    protected function addFulfillmentTracking(string $shopifyOrderId, string $trackingNumber, string $carrier): void
    {
        ...
        try {
            ...
            Log::warning('Ebay3OrderPushService: fulfillment tracking failed', [
                'shopify_order_id' => $shopifyOrderId,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    protected function findShopifyVariantIdBySku(string $sku): ?int
    {
        ...
        try {
            ...
            Log::warning('Ebay3OrderPushService: variant lookup failed', ['sku' => $sku, 'error' => $e->getMessage()]);
    ...
    protected function payloadUsesPlaceholderEmail(array $orderPayload): bool
    {
        foreach ($orderPayload['note_attributes'] ?? [] as $attr) {
            if (($attr['name'] ?? '') === 'ebay3_email_is_placeholder' && ($attr['value'] ?? '') === 'true') {
                return true;
            }
        }
    ...
    /**
     * If live eBay 3 refresh blanked ship-to / buyer email, restore from cached root.
     *
