...
class FaireOrderPushService
{
     ...
     */
    public function previewShopifyPush(FaireOrderMetric $order): array
    {
    ...
    /**
     * Push Faire ship-to onto an already-imported Shopify order.
     * Used by "Pull from Faire" so address-only refreshes update Shopify.
     *
     * @return array{success: bool, skipped?: bool, message: string, shopify_order_id?: string}
     */
    public function syncShippingAddressToShopify(FaireOrderMetric $order): array
    {
        ...
        if ($orderRoot === []) {
            return [
                'success' => false,
                'skipped' => true,
                'message' => 'No Faire order payload available to build an address.',
            ];
        }

        $detail = $this->formatter->formatOrder($orderRoot, $lines, $order);
        $orderPayload = $this->formatter->buildShopifyOrderPayload($detail, $lines, ['faire']);
        ...
        if ($address1 === '') {
            return [
                'success' => false,
                'skipped' => true,
                'message' => 'Faire still has no shipping address for this order.',
            ];
        }
    ...
    /**
     * Auto-fill missing Shopify shipping/customer address from Faire for linked orders.
     *
    ...
    public function syncPendingAddressesToShopify(int $limit = 40): array
    {
        ...
        foreach ($unique as $line) {
            ...
            // Refresh Faire ShipTo when possible so address automation stays current.
            try {
            ...
            if (! empty($result['success']) && empty($result['skipped'])) {
                ...
                $failed++;
                Log::warning('FaireOrderPushService: auto address sync failed', [
    ...
    public function importToShopify(FaireOrderMetric $order): ?string
    {
        ...
        if ($orderId !== '') {
            $localLinked = FaireOrderMetric::query()
                ->where('order_id', $orderId)
                ->whereNotNull('shopify_order_id')
                ->where('shopify_order_id', '!=', '')
        ...
        if (! empty($existing['id'])) {
            if ($orderId !== '') {
                $this->linkFaireOrderToShopify($orderId, (string) $existing['id']);
            } else {
                $order->update([
            ...
            Log::info('FaireOrderPushService: linked existing Shopify order (duplicate avoided)', [
                'order_id' => $orderId,
    ...
    /**
     * After Shopify decrements stock for the imported order, push live qty to Faire.
     */
    protected function syncInventoryAfterPush(FaireOrderMetric $order): void
    {
        $skus = FaireOrderMetric::query()
            ->where('order_id', (string) $order->order_id)
            ->pluck('sku')
        ...
        try {
            $result = app(FaireInventorySyncService::class)
                ->syncSkusFromShopify($skus, $this->shopifyConfig());

            Log::info('FaireOrderPushService: post-push inventory sync', [
                'order_id' => $order->order_id,
                'skus' => $skus,
                'result' => $result,
            ]);
        } catch (\Throwable $e) {
            Log::warning('FaireOrderPushService: post-push inventory sync failed', [
                'order_id' => $order->order_id,
                'skus' => $skus,
                'error' => $e->getMessage(),
            ]);
     ...
     * @return array<string, mixed>
     */
    public function buildImportPlan(FaireOrderMetric $order): array
    {
        if ($order->shopify_order_id) {
            return [
                'success' => false,
        ...
        if ($orderRoot === [] && $cachedRoot === []) {
            return [
                'success' => false,
                'message' => $detailResult['message'] ?? 'Could not load Faire order details before Shopify push.',
            ];
        }
        ...
        $carrier = (string) (($detail['shipment']['service'] ?? '') ?: 'Faire');

        $warnings = $this->buildImportWarnings($detail, $lineResolution, $orderPayload);
        if (empty($detailResult['success'])) {
            $warnings[] = 'Could not refresh live Faire details ('.($detailResult['message'] ?? 'unknown').') — using cached order payload.';
        }
    ...
    protected function buildImportWarnings(array $detail, array $lineResolution, array $orderPayload): array
    {
        ...
        if ($this->payloadUsesPlaceholderEmail($orderPayload)) {
            $warnings[] = 'Faire did not provide a buyer email — a placeholder email will be used for Shopify.';
        } elseif (empty($shipping['email'] ?? null) && empty($orderPayload['customer']['email'] ?? null)) {
            $warnings[] = 'No buyer email on Faire order.';
        }

        $sourceName = trim((string) ($orderPayload['source_name'] ?? ''));
        if ($sourceName === '') {
            $warnings[] = 'No Shopify source_name set — Channel information may show your app name instead of Faire.';
        }
        if (empty($shipping['phone'] ?? null)) {
            $warnings[] = 'No buyer phone on Faire order.';
        }
    ...
    protected function resolveLineItemsForShopify(array $orderPayload, Collection $lines): array
    {
        ...
        foreach ($sourceItems as $item) {
            ...
            if ($title === '') {
                $title = $sku !== '' ? $sku : 'Faire order item';
            }
            ...
            $custom = [
                'title' => $title !== '' ? $title : 'Faire order item',
                'price' => $price,
                'quantity' => $quantity,
            ];
            if ($sku !== '') {
        ...
        if ($resolved === []) {
            $line = $lines->first();
            $resolved[] = [
                'title' => (string) ($line?->display_title ?: 'Faire order item'),
                'price' => number_format((float) ($line?->amount ?? 0), 2, '.', ''),
                'quantity' => max(1, (int) ($line?->quantity ?? 1)),
            ];
            $meta[] = [
        ...
        try {
            for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
                ...
                if ($retryable && $attempt < $maxAttempts) {
                    ...
                    Log::warning('FaireOrderPushService: Shopify order create retrying', [
                ...
                Log::error('FaireOrderPushService: Shopify order create failed', [
                    'status' => $response->status(),
            ...
            Log::error('FaireOrderPushService: exception', ['error' => $e->getMessage()]);
    ...
    protected function addFulfillmentTracking(string $shopifyOrderId, string $trackingNumber, string $carrier): void
    {
        ...
        try {
            ...
            Log::warning('FaireOrderPushService: fulfillment tracking failed', [
                'shopify_order_id' => $shopifyOrderId,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    protected function findShopifyVariantIdBySku(string $sku): ?int
    {
        ...
        try {
            ...
            Log::warning('FaireOrderPushService: variant lookup failed', ['sku' => $sku, 'error' => $e->getMessage()]);
    ...
    protected function payloadUsesPlaceholderEmail(array $orderPayload): bool
    {
        foreach ($orderPayload['note_attributes'] ?? [] as $attr) {
            if (($attr['name'] ?? '') === 'faire_email_is_placeholder' && ($attr['value'] ?? '') === 'true') {
                return true;
            }
        }
    ...
    /**
     * If live Faire refresh blanked ship-to / buyer email, restore from cached root.
     *
