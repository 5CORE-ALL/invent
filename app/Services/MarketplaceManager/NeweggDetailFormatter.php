...
class NeweggDetailFormatter
{
     ...
     */
    public function formatProduct(?array $aeLive, ?NeweggMetric $metric, ShopifySku $shopify, array $aeSkuRows = []): array
    {
        $ae = $this->arr($aeLive);
     ...
     * @return array<string, mixed>
     */
    public function formatOrder(array $orderRoot, Collection $lines, NeweggOrderMetric $primaryLine): array
    {
    ...
    /**
     * Build Shopify REST order payload from formatted Newegg order detail.
     *
    ...
    public function buildShopifyOrderPayload(array $detail, Collection $lines, array $tags): array
    {
        ...
        $noteLines = [
            'Imported from Newegg Order #'.$orderRef,
        ];
        ...
        $noteAttrs = [
            ['name' => 'newegg_order_id', 'value' => $orderRef],
        ];
        if ($emailIsPlaceholder) {
            $noteAttrs[] = ['name' => 'newegg_email_is_placeholder', 'value' => 'true'];
        }
        foreach ([
            'newegg_buyer_login' => $summary['buyer_login_id'] ?? null,
            'newegg_payment_method' => $payment['method'] ?? null,
            'newegg_tracking_number' => $shipment['tracking'] ?? null,
            'newegg_shipping_method' => $shipment['service'] ?? null,
            'newegg_tax_number' => $shipping['tax_number'] ?? null,
            'newegg_buyer_email' => $shipping['email'] ?? null,
            'newegg_buyer_phone' => $shipping['phone'] ?? null,
            'newegg_platform_promotion' => isset($funds['platform_promotion']) ? number_format((float) $funds['platform_promotion'], 2, '.', '') : null,
            'newegg_platform_offer' => isset($funds['platform_offer']) ? number_format((float) $funds['platform_offer'], 2, '.', '') : null,
            'newegg_order_amount' => isset($funds['order_amount']) ? number_format((float) $funds['order_amount'], 2, '.', '') : null,
            'newegg_customer_paid' => $customerPaid !== null ? number_format($customerPaid, 2, '.', '') : null,
            'newegg_seller_amount_paid' => isset($funds['seller_amount_paid']) ? number_format((float) $funds['seller_amount_paid'], 2, '.', '') : null,
            'newegg_platform_commission' => isset($funds['platform_commission']) ? number_format((float) $funds['platform_commission'], 2, '.', '') : null,
            'newegg_transaction_service_fee' => isset($funds['transaction_service_fee']) ? number_format((float) $funds['transaction_service_fee'], 2, '.', '') : null,
            'newegg_tax' => $taxAmount > 0 ? number_format($taxAmount, 2, '.', '') : null,
            'newegg_platform_offer_tax' => isset($funds['platform_offer_tax']) ? number_format((float) $funds['platform_offer_tax'], 2, '.', '') : null,
        ] as $name => $value) {
            if ($value !== null && $value !== '') {
        ...
        if ($shippingCost > 0) {
            $payload['shipping_lines'] = [[
                'title' => (string) ($shipment['service'] ?? 'Newegg shipping'),
    ...
    protected function splitShopifyCustomerName(array $detail, string $recipient = ''): array
    {
        ...
        if ($fullName === '') {
            return ['Newegg', 'Customer'];
        }

        $parts = preg_split('/\s+/u', $fullName, 2) ?: [];
        $firstName = (string) ($parts[0] ?? 'Newegg');
        if (! isset($parts[1])) {
    ...
    public function resolveShopifySourceAttribution(string $orderRef, ?array $settings = null): array
    {
        ...
        $handle = trim((string) (
            ...
            ?? config('services.newegg.shopify_source_name')
            ?? 'newegg'
        ));
        $urlTemplate = (string) (
            $settings['order']['shopify_source_url_template']
            ?? config('services.newegg.shopify_source_url_template')
            ?? 'https://csp.newegg.com/m_apps/order-manage/order_detail?orderId={order_id}'
        );

        return [
            'source_name' => $handle !== '' ? $handle : 'newegg',
            'source_identifier' => $orderRef,
            'source_url' => str_replace('{order_id}', $orderRef, $urlTemplate),
            'referring_site' => 'https://www.newegg.com/',
        ];
    }
    ...
    public function shopifySourceDisplayName(?array $settings = null): string
    {
        ...
        $name = trim((string) (
            ...
            ?? config('services.newegg.shopify_source_display_name')
            ?? 'Newegg'
        ));

        return $name !== '' ? $name : 'Newegg';
    }
    ...
    public function resolveShopifyCustomerEmail(string $orderRef, ?string $rawEmail): array
    {
        ...
        $domain = (string) env('NEWEGG_SHOPIFY_PLACEHOLDER_EMAIL_DOMAIN', 'import.5coremanagement.com');
        $slug = preg_replace('/[^a-zA-Z0-9]/', '', $orderRef) ?: 'order';

        return ['newegg-'.$slug.'@'.$domain, true];
    }
    ...
    /**
     * Newegg descriptions are often JSON module trees (moduleList / mobileDetail), not plain HTML.
     */
     ...
     */
    protected function formatProductVariants(array $ae, array $aeSkuRows, ?NeweggMetric $metric = null, ?ShopifySku $shopify = null): array
    {
        $variants = [];
    ...
    /**
     * Prefer live/API variant stock for this SKU, else local ne_stock from pricing sync.
     *
     * @param  array<int, array<string, mixed>>  $variants
     */
    protected function resolveProductAeStock(array $variants, ?NeweggMetric $metric, string $shopifySku): ?int
    {
     ...
     */
    protected function resolveListingLastSyncedAt(?NeweggMetric $metric, string $shopifySku): ?\Carbon\Carbon
    {
        $candidates = array_filter([
            $metric?->updated_at,
    ...
    {
        $sku = trim($shopifySku);
        if ($sku === '' || ! Schema::hasTable('newegg_pricing_prices')) {
            return null;
        }

        $row = NeweggPricingPrice::query()
    ...
    protected function isMetricLinked(?NeweggMetric $metric, string $shopifySku): bool
    {
     ...
     */
    protected function resolveOrderLineImage(array $product, ?NeweggOrderMetric $db = null): ?string
    {
        foreach ([
    ...
    protected function normalizeOrderImageUrl(?string $url): ?string
    {
        ...
        if (preg_match('#^https?://#i', $url)
            ...
            && ! str_contains($url, 'newegg-media.com')
            && ! str_contains($url, 'newegg.com')) {
            return filter_var($url, FILTER_VALIDATE_URL) ? $url : null;
        }

        $filename = $this->extractOrderImageFilename($url);
        if ($filename) {
            return $this->buildNeweggMediaImageUrl($filename);
        }
    ...
    protected function buildNeweggMediaImageUrl(string $filename): string
    {
        return 'https://ae-pic-a1.newegg-media.com/kf/'.ltrim($filename, '/');
    }
    ...
    protected function resolveOrderLineImageFallback(?string $sku, ?string $productId): ?string
    {
        ...
        if ($productId) {
            $metric = NeweggMetric::query()->where('product_id', $productId)->first();
