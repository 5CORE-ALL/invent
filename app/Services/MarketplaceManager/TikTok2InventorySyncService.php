...
class TikTok2InventorySyncService
{
    ...
    public function syncSkusFromShopify(array $skus, ?array $shopifyConfig = null, bool $exactShopifyQty = false): array
    {
        ...
        if ($this->tiktokApi->isListingCreateRestricted()) {
            return $this->resultMessage(
                'Stopped: TikTok 2 is blocking listing edits, and the inventory update was rejected too. Updated %d SKU(s).',
                $updated,
                $failed,
                $skipped,
                $errorSamples
    ...
    public function syncFromShopify(bool $dryRun = false): array
    {
        ...
        if (! $dryRun) {
            $priority = app(MarketplaceMismatchInventoryPass::class)->runAndStopIfMore('tiktok2');
            if ($priority !== null) {
                return [
        ...
        if (! $coverage['ok'] && ! $dryRun) {
            ...
            $skus = $metrics->pluck('sku')->unique()->values()->all();
            Log::warning('TikTok2InventorySyncService: Shopify coverage low — pushing confirmed qtys only', $coverage + ['kept' => count($skus)]);
            if ($skus === []) {
                return [
                    'updated' => 0,
                    'failed' => 0,
        ...
        if ($this->tiktokApi->isListingCreateRestricted()) {
            return $this->resultMessage(
                "Stopped: TikTok 2 is blocking listing edits, and the inventory update was rejected too. Updated {$updated}; failed {$failed}; skipped {$skipped}.",
                $updated,
                $failed,
                $skipped,
                $errorSamples
