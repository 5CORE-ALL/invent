...
/**
 * Live listing helpers for Wayfair listings UI.
 */
class WayfairLiveListingsService
{
    ...
    public function liveDetailsByProductIds(array $productIds): array
    {
        ...
        if ($ids === [] || ! Schema::hasTable('wayfair_pricing_prices')) {
            return [];
        }

        $rows = WayfairPricingPrice::query()->whereIn('sku', $ids)->get();
     ...
     */
    protected function mapProduct(WayfairPricingPrice $row): ?array
    {
        ...
        return [
            ...
            'sku' => $sku,
            'state' => 'active',
            'inventory' => $row->wayfair_stock !== null ? (int) $row->wayfair_stock : null,
            'title' => $sku,
