...
class FaireDetailFormatter
{
     ...
     */
    public function formatProduct(?array $aeLive, ?FaireMetric $metric, ShopifySku $shopify, array $aeSkuRows = []): array
    {
        $ae = $this->arr($aeLive);
     ...
     * @param  Collection<int, FaireOrderMetric>  $lines
     * @return array<string, mixed>
     */
    public function formatOrder(array $orderRoot, Collection $lines, FaireOrderMetric $primaryLine): array
    {
    ...
    /**
     * Build Shopify REST order payload from formatted Faire order detail.
     *
    ...
    public function buildShopifyOrderPayload(array $detail, Collection $lines, array $tags): array
    {
        ...
        $noteLines = [
            'Imported from Faire Order #'.$orderRef,
        ];
        ...
        $noteAttrs = [
            ['name' => 'faire_order_id', 'value' => $orderRef],
        ];
        if ($emailIsPlaceholder) {
            $noteAttrs[] = ['name' => 'faire_email_is_placeholder', 'value' => 'true'];
        }
        foreach ([
            'faire_buyer_login' => $summary['buyer_login_id'] ?? null,
            'faire_payment_method' => $payment['method'] ?? null,
            'faire_tracking_number' => $shipment['tracking'] ?? null,
            'faire_shipping_method' => $shipment['service'] ?? null,
            'faire_tax_number' => $shipping['tax_number'] ?? null,
            'faire_buyer_email' => $shipping['email'] ?? null,
            'faire_buyer_phone' => $shipping['phone'] ?? null,
            'faire_platform_promotion' => isset($funds['platform_promotion']) ? number_format((float) $funds['platform_promotion'], 2, '.', '') : null,
            'faire_platform_offer' => isset($funds['platform_offer']) ? number_format((float) $funds['platform_offer'], 2, '.', '') : null,
            'faire_order_amount' => isset($funds['order_amount']) ? number_format((float) $funds['order_amount'], 2, '.', '') : null,
            'faire_customer_paid' => $customerPaid !== null ? number_format($customerPaid, 2, '.', '') : null,
            'faire_seller_amount_paid' => isset($funds['seller_amount_paid']) ? number_format((float) $funds['seller_amount_paid'], 2, '.', '') : null,
            'faire_platform_commission' => isset($funds['platform_commission']) ? number_format((float) $funds['platform_commission'], 2, '.', '') : null,
            'faire_transaction_service_fee' => isset($funds['transaction_service_fee']) ? number_format((float) $funds['transaction_service_fee'], 2, '.', '') : null,
            'faire_tax' => $taxAmount > 0 ? number_format($taxAmount, 2, '.', '') : null,
            'faire_platform_offer_tax' => isset($funds['platform_offer_tax']) ? number_format((float) $funds['platform_offer_tax'], 2, '.', '') : null,
        ] as $name => $value) {
            if ($value !== null && $value !== '') {
        ...
        if ($shippingCost > 0) {
            $payload['shipping_lines'] = [[
                'title' => (string) ($shipment['service'] ?? 'Faire shipping'),
    ...
    protected function splitShopifyCustomerName(array $detail, string $recipient = ''): array
    {
        ...
        if ($fullName === '') {
            return ['Faire', 'Customer'];
        }

        $parts = preg_split('/\s+/u', $fullName, 2) ?: [];
        $firstName = (string) ($parts[0] ?? 'Faire');
        if (! isset($parts[1])) {
    ...
    public function resolveShopifySourceAttribution(string $orderRef, ?array $settings = null): array
    {
        ...
        $handle = trim((string) (
            ...
            ?? config('services.faire.shopify_source_name')
            ?? 'faire'
        ));
        $urlTemplate = (string) (
            $settings['order']['shopify_source_url_template']
            ?? config('services.faire.shopify_source_url_template')
            ?? 'https://csp.faire.com/m_apps/order-manage/order_detail?orderId={order_id}'
        );

        return [
            'source_name' => $handle !== '' ? $handle : 'faire',
            'source_identifier' => $orderRef,
            'source_url' => str_replace('{order_id}', $orderRef, $urlTemplate),
            'referring_site' => 'https://www.faire.com/',
        ];
    }
    ...
    public function shopifySourceDisplayName(?array $settings = null): string
    {
        ...
        $name = trim((string) (
            ...
            ?? config('services.faire.shopify_source_display_name')
            ?? 'Faire'
        ));

        return $name !== '' ? $name : 'Faire';
    }
    ...
    public function resolveShopifyCustomerEmail(string $orderRef, ?string $rawEmail): array
    {
        ...
        $domain = (string) env('FAIRE_SHOPIFY_PLACEHOLDER_EMAIL_DOMAIN', 'import.5coremanagement.com');
        $slug = preg_replace('/[^a-zA-Z0-9]/', '', $orderRef) ?: 'order';

        return ['faire-'.$slug.'@'.$domain, true];
    }
    ...
    /**
     * Faire descriptions are often JSON module trees (moduleList / mobileDetail), not plain HTML.
     */
     ...
     */
    protected function formatProductVariants(array $ae, array $aeSkuRows, ?FaireMetric $metric = null, ?ShopifySku $shopify = null): array
    {
        $variants = [];
     ...
     */
    protected function resolveListingLastSyncedAt(?FaireMetric $metric, string $shopifySku): ?\Carbon\Carbon
    {
        $candidates = array_filter([
            $metric?->updated_at,
    ...
    protected function isMetricLinked(?FaireMetric $metric, string $shopifySku): bool
    {
     ...
     */
    protected function resolveOrderLineImage(array $product, ?FaireOrderMetric $db = null): ?string
    {
        foreach ([
    ...
    protected function normalizeOrderImageUrl(?string $url): ?string
    {
        ...
        if (preg_match('#^https?://#i', $url)
            ...
            && ! str_contains($url, 'faire-media.com')
            && ! str_contains($url, 'faire.com')) {
            return filter_var($url, FILTER_VALIDATE_URL) ? $url : null;
        }

        $filename = $this->extractOrderImageFilename($url);
        if ($filename) {
            return $this->buildFaireMediaImageUrl($filename);
        }
    ...
    protected function buildFaireMediaImageUrl(string $filename): string
    {
        return 'https://ae-pic-a1.faire-media.com/kf/'.ltrim($filename, '/');
    }
    ...
    protected function resolveOrderLineImageFallback(?string $sku, ?string $productId): ?string
    {
        ...
        if ($productId) {
            $metric = FaireMetric::query()->where('product_id', $productId)->first();
