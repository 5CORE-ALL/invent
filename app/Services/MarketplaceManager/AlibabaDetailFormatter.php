...
class AlibabaDetailFormatter
{
     ...
     * @param  Collection<int, AlibabaOrderMetric>  $lines
     * @return array<string, mixed>
     */
    public function formatOrder(array $orderRoot, Collection $lines, AlibabaOrderMetric $primaryLine): array
    {
    ...
    /**
     * Build Shopify REST order payload from formatted Alibaba order detail.
     *
    ...
    public function buildShopifyOrderPayload(array $detail, Collection $lines, array $tags): array
    {
        ...
        $noteLines = [
            'Imported from Alibaba Order #'.$orderRef,
        ];
        ...
        $noteAttrs = [
            ['name' => 'alibaba_order_id', 'value' => $orderRef],
        ];
        if ($emailIsPlaceholder) {
            $noteAttrs[] = ['name' => 'alibaba_email_is_placeholder', 'value' => 'true'];
        }
        foreach ([
            'alibaba_buyer_login' => $summary['buyer_login_id'] ?? null,
            'alibaba_payment_method' => $payment['method'] ?? null,
            'alibaba_tracking_number' => $shipment['tracking'] ?? null,
            'alibaba_shipping_method' => $shipment['service'] ?? null,
            'alibaba_tax_number' => $shipping['tax_number'] ?? null,
            'alibaba_buyer_email' => $shipping['email'] ?? null,
            'alibaba_buyer_phone' => $shipping['phone'] ?? null,
            'alibaba_platform_promotion' => isset($funds['platform_promotion']) ? number_format((float) $funds['platform_promotion'], 2, '.', '') : null,
            'alibaba_platform_offer' => isset($funds['platform_offer']) ? number_format((float) $funds['platform_offer'], 2, '.', '') : null,
            'alibaba_order_amount' => isset($funds['order_amount']) ? number_format((float) $funds['order_amount'], 2, '.', '') : null,
            'alibaba_customer_paid' => $customerPaid !== null ? number_format($customerPaid, 2, '.', '') : null,
            'alibaba_seller_amount_paid' => isset($funds['seller_amount_paid']) ? number_format((float) $funds['seller_amount_paid'], 2, '.', '') : null,
            'alibaba_platform_commission' => isset($funds['platform_commission']) ? number_format((float) $funds['platform_commission'], 2, '.', '') : null,
            'alibaba_transaction_service_fee' => isset($funds['transaction_service_fee']) ? number_format((float) $funds['transaction_service_fee'], 2, '.', '') : null,
            'alibaba_tax' => $taxAmount > 0 ? number_format($taxAmount, 2, '.', '') : null,
            'alibaba_platform_offer_tax' => isset($funds['platform_offer_tax']) ? number_format((float) $funds['platform_offer_tax'], 2, '.', '') : null,
        ] as $name => $value) {
            if ($value !== null && $value !== '') {
        ...
        if ($shippingCost > 0) {
            $payload['shipping_lines'] = [[
                'title' => (string) ($shipment['service'] ?? 'Alibaba shipping'),
    ...
    protected function splitShopifyCustomerName(array $detail, string $recipient = ''): array
    {
        ...
        if ($fullName === '') {
            return ['Alibaba', 'Customer'];
        }

        $parts = preg_split('/\s+/u', $fullName, 2) ?: [];
        $firstName = (string) ($parts[0] ?? 'Alibaba');
        if (! isset($parts[1])) {
    ...
    public function resolveShopifySourceAttribution(string $orderRef, ?array $settings = null): array
    {
        ...
        $handle = trim((string) (
            ...
            ?? config('services.alibaba.shopify_source_name')
            ?? 'alibaba'
        ));
        $urlTemplate = (string) (
            $settings['order']['shopify_source_url_template']
            ?? config('services.alibaba.shopify_source_url_template')
            ?? 'https://csp.alibaba.com/m_apps/order-manage/order_detail?orderId={order_id}'
        );

        return [
            'source_name' => $handle !== '' ? $handle : 'alibaba',
            'source_identifier' => $orderRef,
            'source_url' => str_replace('{order_id}', $orderRef, $urlTemplate),
            'referring_site' => 'https://www.alibaba.com/',
        ];
    }
    ...
    public function shopifySourceDisplayName(?array $settings = null): string
    {
        ...
        $name = trim((string) (
            ...
            ?? config('services.alibaba.shopify_source_display_name')
            ?? 'Alibaba'
        ));

        return $name !== '' ? $name : 'Alibaba';
    }
    ...
    public function resolveShopifyCustomerEmail(string $orderRef, ?string $rawEmail): array
    {
        ...
        $domain = (string) env('ALIBABA_SHOPIFY_PLACEHOLDER_EMAIL_DOMAIN', 'import.5coremanagement.com');
        $slug = preg_replace('/[^a-zA-Z0-9]/', '', $orderRef) ?: 'order';

        return ['alibaba-'.$slug.'@'.$domain, true];
    }
    ...
    /**
     * Alibaba descriptions are often JSON module trees (moduleList / mobileDetail), not plain HTML.
     */
     ...
     */
    protected function formatProductVariants(array $ae, array $aeSkuRows, ?AlibabaMetric $metric = null, ?ShopifySku $shopify = null): array
    {
        $variants = [];
    ...
    /**
     * Prefer live/API variant stock for this SKU, else local ab_stock from pricing sync.
     *
     * @param  array<int, array<string, mixed>>  $variants
     */
    protected function resolveProductAeStock(array $variants, ?AlibabaMetric $metric, string $shopifySku): ?int
    {
     ...
     */
    protected function resolveListingLastSyncedAt(?AlibabaMetric $metric, string $shopifySku): ?\Carbon\Carbon
    {
        $candidates = array_filter([
            $metric?->updated_at,
    ...
    {
        $sku = trim($shopifySku);
        if ($sku === '' || ! Schema::hasTable('alibaba_pricing_prices')) {
            return null;
        }

        $row = AlibabaPricingPrice::query()
    ...
    protected function localAeStockForSku(string $sku): ?int
    {
        ...
        if (! $row || $row->ab_stock === null) {
            return null;
        }

        return (int) $row->ab_stock;
    }
     ...
     */
    protected function resolveOrderLineImage(array $product, ?AlibabaOrderMetric $db = null): ?string
    {
        foreach ([
    ...
    protected function normalizeOrderImageUrl(?string $url): ?string
    {
        ...
        if (preg_match('#^https?://#i', $url)
            ...
            && ! str_contains($url, 'alibaba-media.com')
            && ! str_contains($url, 'alibaba.com')) {
            return filter_var($url, FILTER_VALIDATE_URL) ? $url : null;
        }

        $filename = $this->extractOrderImageFilename($url);
        if ($filename) {
            return $this->buildAlibabaMediaImageUrl($filename);
        }
    ...
    protected function buildAlibabaMediaImageUrl(string $filename): string
    {
        return 'https://ae-pic-a1.alibaba-media.com/kf/'.ltrim($filename, '/');
    }
    ...
    protected function resolveOrderLineImageFallback(?string $sku, ?string $productId): ?string
    {
        ...
        if ($productId) {
            $metric = AlibabaMetric::query()->where('product_id', $productId)->first();
