...
class AlibabaOrderPushService
{
     ...
     */
    public function previewShopifyPush(AlibabaOrderMetric $order): array
    {
    ...
    public function importToShopify(AlibabaOrderMetric $order): ?string
    {
        ...
        if ($orderId !== '') {
            $localLinked = AlibabaOrderMetric::query()
        ...
        if (! empty($existing['id'])) {
            if ($orderId !== '') {
                $this->linkAlibabaOrderToShopify($orderId, (string) $existing['id']);
            } else {
                $order->update([
            ...
            Log::info('AlibabaOrderPushService: linked existing Shopify order (duplicate avoided)', [
                'order_id' => $orderId,
        ...
        if ($this->lastDuplicateLinkMessage === null) {
            ...
            $carrier = (string) ($fulfillment['carrier'] ?? 'Alibaba');
            if ($tracking !== '') {
    ...
    protected function linkAlibabaOrderToShopify(string $orderId, string $shopifyOrderId): void
    {
     ...
     */
    protected function syncInventoryAfterPush(AlibabaOrderMetric $order): void
    {
        $skus = AlibabaOrderMetric::query()
        ...
        try {
            $result = app(AlibabaInventorySyncService::class)
                ->syncSkusFromShopify($skus, $this->shopifyConfig());

            Log::info('AlibabaOrderPushService: post-push inventory sync', [
                'order_id' => $order->order_id,
                'skus' => $skus,
                'result' => $result,
            ]);
        } catch (\Throwable $e) {
            Log::warning('AlibabaOrderPushService: post-push inventory sync failed', [
                'order_id' => $order->order_id,
                'skus' => $skus,
                'error' => $e->getMessage(),
            ]);
     ...
     * @return array<string, mixed>
     */
    public function buildImportPlan(AlibabaOrderMetric $order): array
    {
        if ($order->shopify_order_id) {
            return [
                'success' => false,
        ...
        if (empty($detailResult['success'])) {
            return [
                'success' => false,
                'message' => $detailResult['message'] ?? 'Could not load Alibaba order details before Shopify push.',
            ];
        }
        ...
        $tags = array_values(array_unique(array_merge(
            ['alibaba', 'alibaba-'.($order->order_number ?? $order->order_id)],
        ...
        $carrier = (string) (($detail['shipment']['service'] ?? '') ?: 'Alibaba');

        $warnings = $this->buildImportWarnings($detail, $lineResolution, $orderPayload);

        return [
            'success' => true,
            'alibaba_order_id' => (string) $order->order_id,
    ...
    protected function buildImportWarnings(array $detail, array $lineResolution, array $orderPayload): array
    {
        ...
        if ($this->payloadUsesPlaceholderEmail($orderPayload)) {
            $warnings[] = 'Alibaba did not provide a buyer email — a placeholder email will be used for Shopify.';
        } elseif (empty($shipping['email'] ?? null) && empty($orderPayload['customer']['email'] ?? null)) {
            $warnings[] = 'No buyer email on Alibaba order.';
        }

        $sourceName = trim((string) ($orderPayload['source_name'] ?? ''));
        if ($sourceName === '') {
            $warnings[] = 'No Shopify source_name set — Channel information may show your app name instead of Alibaba.';
        }
        if (empty($shipping['phone'] ?? null)) {
            $warnings[] = 'No buyer phone on Alibaba order.';
        }
    ...
    protected function resolveLineItemsForShopify(array $orderPayload, Collection $lines): array
    {
        ...
        foreach ($sourceItems as $item) {
            ...
            if ($title === '') {
                $title = $sku !== '' ? $sku : 'Alibaba order item';
            }
            ...
            $custom = [
                'title' => $title !== '' ? $title : 'Alibaba order item',
                'price' => $price,
                'quantity' => $quantity,
            ];
            if ($sku !== '') {
        ...
        if ($resolved === []) {
            $line = $lines->first();
            $resolved[] = [
                'title' => (string) ($line?->display_title ?: 'Alibaba order item'),
                'price' => number_format((float) ($line?->amount ?? 0), 2, '.', ''),
                'quantity' => max(1, (int) ($line?->quantity ?? 1)),
            ];
            $meta[] = [
        ...
        try {
            for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
                ...
                if ($retryable && $attempt < $maxAttempts) {
                    ...
                    Log::warning('AlibabaOrderPushService: Shopify order create retrying', [
                ...
                Log::error('AlibabaOrderPushService: Shopify order create failed', [
                    'status' => $response->status(),
            ...
            Log::error('AlibabaOrderPushService: exception', ['error' => $e->getMessage()]);
    ...
    protected function addFulfillmentTracking(string $shopifyOrderId, string $trackingNumber, string $carrier): void
    {
        ...
        try {
            ...
            Log::warning('AlibabaOrderPushService: fulfillment tracking failed', [
                'shopify_order_id' => $shopifyOrderId,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    protected function findShopifyVariantIdBySku(string $sku): ?int
    {
        ...
        try {
            ...
            Log::warning('AlibabaOrderPushService: variant lookup failed', ['sku' => $sku, 'error' => $e->getMessage()]);
    ...
    protected function payloadUsesPlaceholderEmail(array $orderPayload): bool
    {
        foreach ($orderPayload['note_attributes'] ?? [] as $attr) {
            if (($attr['name'] ?? '') === 'alibaba_email_is_placeholder' && ($attr['value'] ?? '') === 'true') {
                return true;
            }
        }
