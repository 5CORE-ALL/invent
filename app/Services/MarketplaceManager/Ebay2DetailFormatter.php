...
class Ebay2DetailFormatter
{
     ...
     */
    public function formatProduct(?array $aeLive, ?Ebay2Metric $metric, ShopifySku $shopify, array $aeSkuRows = []): array
    {
        $ae = $this->arr($aeLive);
     ...
     * @param  Collection<int, Ebay2OrderMetric>  $lines
     * @return array<string, mixed>
     */
    public function formatOrder(array $orderRoot, Collection $lines, Ebay2OrderMetric $primaryLine): array
    {
    ...
    /**
     * Build Shopify REST order payload from formatted eBay 2 order detail.
     *
    ...
    public function buildShopifyOrderPayload(array $detail, Collection $lines, array $tags): array
    {
        ...
        $noteLines = [
            'Imported from eBay 2 Order #'.$orderRef,
        ];
        ...
        $noteAttrs = [
            ['name' => 'ebay2_order_id', 'value' => $orderRef],
        ];
        if ($emailIsPlaceholder) {
            $noteAttrs[] = ['name' => 'ebay2_email_is_placeholder', 'value' => 'true'];
        }
        foreach ([
            'ebay2_buyer_login' => $summary['buyer_login_id'] ?? null,
            'ebay2_payment_method' => $payment['method'] ?? null,
            'ebay2_tracking_number' => $shipment['tracking'] ?? null,
            'ebay2_shipping_method' => $shipment['service'] ?? null,
            'ebay2_tax_number' => $shipping['tax_number'] ?? null,
            'ebay2_buyer_email' => $shipping['email'] ?? null,
            'ebay2_buyer_phone' => $shipping['phone'] ?? null,
            'ebay2_platform_promotion' => isset($funds['platform_promotion']) ? number_format((float) $funds['platform_promotion'], 2, '.', '') : null,
            'ebay2_platform_offer' => isset($funds['platform_offer']) ? number_format((float) $funds['platform_offer'], 2, '.', '') : null,
            'ebay2_order_amount' => isset($funds['order_amount']) ? number_format((float) $funds['order_amount'], 2, '.', '') : null,
            'ebay2_customer_paid' => $customerPaid !== null ? number_format($customerPaid, 2, '.', '') : null,
            'ebay2_seller_amount_paid' => isset($funds['seller_amount_paid']) ? number_format((float) $funds['seller_amount_paid'], 2, '.', '') : null,
            'ebay2_platform_commission' => isset($funds['platform_commission']) ? number_format((float) $funds['platform_commission'], 2, '.', '') : null,
            'ebay2_transaction_service_fee' => isset($funds['transaction_service_fee']) ? number_format((float) $funds['transaction_service_fee'], 2, '.', '') : null,
            'ebay2_tax' => $taxAmount > 0 ? number_format($taxAmount, 2, '.', '') : null,
            'ebay2_platform_offer_tax' => isset($funds['platform_offer_tax']) ? number_format((float) $funds['platform_offer_tax'], 2, '.', '') : null,
        ] as $name => $value) {
            if ($value !== null && $value !== '') {
        ...
        if ($shippingCost > 0) {
            $payload['shipping_lines'] = [[
                'title' => (string) ($shipment['service'] ?? 'eBay 2 shipping'),
    ...
    protected function splitShopifyCustomerName(array $detail, string $recipient = ''): array
    {
        ...
        if ($fullName === '') {
            return ['eBay 2', 'Customer'];
        }

        $parts = preg_split('/\s+/u', $fullName, 2) ?: [];
        $firstName = (string) ($parts[0] ?? 'eBay 2');
        if (! isset($parts[1])) {
    ...
    public function resolveShopifySourceAttribution(string $orderRef, ?array $settings = null): array
    {
        ...
        $handle = trim((string) (
            ...
            ?? config('services.ebay2.shopify_source_name')
            ?? 'ebay2'
        ));
        $urlTemplate = (string) (
            $settings['order']['shopify_source_url_template']
            ?? config('services.ebay2.shopify_source_url_template')
            ?? 'https://www.ebay.com/mesh/ord/details?orderid={order_id}'
        );

        return [
            'source_name' => $handle !== '' ? $handle : 'ebay2',
    ...
    public function shopifySourceDisplayName(?array $settings = null): string
    {
        ...
        $name = trim((string) (
            ...
            ?? config('services.ebay2.shopify_source_display_name')
            ?? 'eBay 2'
        ));

        return $name !== '' ? $name : 'eBay 2';
    }
    ...
    /**
     * eBay 2 descriptions are often JSON module trees (moduleList / mobileDetail), not plain HTML.
     */
     ...
     */
    protected function formatProductVariants(array $ae, array $aeSkuRows, ?Ebay2Metric $metric = null, ?ShopifySku $shopify = null): array
    {
        $variants = [];
     ...
     */
    protected function resolveProductAeStock(array $variants, ?Ebay2Metric $metric, string $shopifySku): ?int
    {
     ...
     */
    protected function resolveListingLastSyncedAt(?Ebay2Metric $metric, string $shopifySku): ?\Carbon\Carbon
    {
        $candidates = array_filter([
            $metric?->updated_at,
    ...
    {
        $sku = trim($shopifySku);
        if ($sku === '' || ! Schema::hasTable('ebay_2_metrics')) {
            return null;
        }

        $row = Ebay2Metric::query()
     ...
     */
    protected function resolveOrderLineImage(array $product, ?Ebay2OrderMetric $db = null): ?string
    {
        foreach ([
    ...
    protected function normalizeOrderImageUrl(?string $url): ?string
    {
        ...
        if (preg_match('#^https?://#i', $url)
            ...
            && ! str_contains($url, 'ebay2.com')) {
            return filter_var($url, FILTER_VALIDATE_URL) ? $url : null;
        }

        $filename = $this->extractOrderImageFilename($url);
        if ($filename) {
            return $this->buildEbay2MediaImageUrl($filename);
        }
    ...
    protected function buildEbay2MediaImageUrl(string $filename): string
    {
