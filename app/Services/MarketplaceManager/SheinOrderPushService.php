...
class SheinOrderPushService
{
     ...
     */
    public function previewShopifyPush(SheinOrderMetric $order): array
    {
    ...
    /**
     * Push Shein ship-to onto an already-imported Shopify order.
     * Used by "Pull from Shein" so address-only refreshes update Shopify.
     *
     * @return array{success: bool, skipped?: bool, message: string, shopify_order_id?: string}
     */
    public function syncShippingAddressToShopify(SheinOrderMetric $order): array
    {
        ...
        if ($orderRoot === []) {
            return [
                'success' => false,
                'skipped' => true,
                'message' => 'No Shein order payload available to build an address.',
            ];
        }
        ...
        if ($address1 === '') {
            return [
                'success' => false,
                'skipped' => true,
                'message' => 'Shein still has no shipping address for this order.',
            ];
        }
    ...
    /**
     * Auto-fill missing Shopify shipping/customer address from Shein for linked orders.
     *
    ...
    public function syncPendingAddressesToShopify(int $limit = 40): array
    {
        ...
        foreach ($unique as $line) {
            ...
            // Refresh Shein ShipTo when possible so address automation stays current.
            try {
            ...
            if (! empty($result['success']) && empty($result['skipped'])) {
                ...
                $failed++;
                Log::warning('SheinOrderPushService: auto address sync failed', [
    ...
    public function importToShopify(SheinOrderMetric $order): ?string
    {
        ...
        if ($order->shopify_order_id) {
            $this->fulfillShopifyForImportedMarketplaceOrder('shein', (int) $order->id, ['order_id' => (string) $order->order_id]);
        ...
        if ($this->lastDuplicateLinkMessage === null) {
            ...
            $carrier = (string) ($fulfillment['carrier'] ?? 'Shein');
            if ($tracking !== '') {
    ...
    /**
     * After Shopify decrements stock for the imported order, push live qty to Shein.
     */
    protected function syncInventoryAfterPush(SheinOrderMetric $order): void
    {
        $skus = SheinOrderMetric::query()
            ->where('order_id', (string) $order->order_id)
            ->pluck('sku')
        ...
        try {
            $result = app(SheinInventorySyncService::class)
                ->syncSkusFromShopify($skus, $this->shopifyConfig());

            Log::info('SheinOrderPushService: post-push inventory sync', [
                'order_id' => $order->order_id,
                'skus' => $skus,
                'result' => $result,
            ]);
        } catch (\Throwable $e) {
            Log::warning('SheinOrderPushService: post-push inventory sync failed', [
                'order_id' => $order->order_id,
                'skus' => $skus,
                'error' => $e->getMessage(),
            ]);
     ...
     * @return array<string, mixed>
     */
    public function buildImportPlan(SheinOrderMetric $order): array
    {
        if ($order->shopify_order_id) {
            return [
                'success' => false,
    ...
    protected function buildImportWarnings(array $detail, array $lineResolution, array $orderPayload): array
    {
        ...
        if ($this->payloadUsesPlaceholderEmail($orderPayload)) {
            $warnings[] = 'Shein did not provide a buyer email — a placeholder email will be used for Shopify.';
        } elseif (empty($shipping['email'] ?? null) && empty($orderPayload['customer']['email'] ?? null)) {
            $warnings[] = 'No buyer email on Shein order.';
        }

        $sourceName = trim((string) ($orderPayload['source_name'] ?? ''));
        if ($sourceName === '') {
            $warnings[] = 'No Shopify source_name set — Channel information may show your app name instead of Shein.';
        }
        if (empty($shipping['phone'] ?? null)) {
            $warnings[] = 'No buyer phone on Shein order.';
        }
    ...
    protected function resolveLineItemsForShopify(array $orderPayload, Collection $lines): array
    {
        ...
        foreach ($sourceItems as $item) {
            ...
            if ($title === '') {
                $title = $sku !== '' ? $sku : 'Shein order item';
            }
            ...
            $custom = [
                'title' => $title !== '' ? $title : 'Shein order item',
                'price' => $price,
                'quantity' => $quantity,
            ];
            if ($sku !== '') {
        ...
        if ($resolved === []) {
            $line = $lines->first();
            $resolved[] = [
                'title' => (string) ($line?->display_title ?: 'Shein order item'),
                'price' => number_format((float) ($line?->amount ?? 0), 2, '.', ''),
                'quantity' => max(1, (int) ($line?->quantity ?? 1)),
            ];
            $meta[] = [
        ...
        try {
            for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
                ...
                if ($retryable && $attempt < $maxAttempts) {
                    ...
                    Log::warning('SheinOrderPushService: Shopify order create retrying', [
                ...
                Log::error('SheinOrderPushService: Shopify order create failed', [
                    'status' => $response->status(),
            ...
            Log::error('SheinOrderPushService: exception', ['error' => $e->getMessage()]);
    ...
    protected function addFulfillmentTracking(string $shopifyOrderId, string $trackingNumber, string $carrier): void
    {
        ...
        try {
            ...
            Log::warning('SheinOrderPushService: fulfillment tracking failed', [
                'shopify_order_id' => $shopifyOrderId,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    protected function findShopifyVariantIdBySku(string $sku): ?int
    {
        ...
        try {
            ...
            Log::warning('SheinOrderPushService: variant lookup failed', ['sku' => $sku, 'error' => $e->getMessage()]);
    ...
    protected function payloadUsesPlaceholderEmail(array $orderPayload): bool
    {
        foreach ($orderPayload['note_attributes'] ?? [] as $attr) {
            if (($attr['name'] ?? '') === 'shein_email_is_placeholder' && ($attr['value'] ?? '') === 'true') {
                return true;
            }
        }
    ...
    /**
     * If live Shein refresh blanked ship-to / buyer email, restore from cached root.
     *
