...
class SheinDetailFormatter
{
     ...
     */
    public function formatProduct(?array $aeLive, ?SheinMmMetric $metric, ShopifySku $shopify, array $aeSkuRows = []): array
    {
        $ae = $this->arr($aeLive);
     ...
     * @return array<string, mixed>
     */
    public function formatOrder(array $orderRoot, Collection $lines, SheinOrderMetric $primaryLine): array
    {
    ...
    /**
     * Build Shopify REST order payload from formatted Shein order detail.
     *
    ...
    public function buildShopifyOrderPayload(array $detail, Collection $lines, array $tags): array
    {
        ...
        $noteLines = [
            'Imported from Shein Order #'.$orderRef,
        ];
        ...
        $noteAttrs = [
            ['name' => 'shein_order_id', 'value' => $orderRef],
        ];
        if ($emailIsPlaceholder) {
            $noteAttrs[] = ['name' => 'shein_email_is_placeholder', 'value' => 'true'];
        }
        foreach ([
            'shein_buyer_login' => $summary['buyer_login_id'] ?? null,
            'shein_payment_method' => $payment['method'] ?? null,
            'shein_tracking_number' => $shipment['tracking'] ?? null,
            'shein_shipping_method' => $shipment['service'] ?? null,
            'shein_tax_number' => $shipping['tax_number'] ?? null,
            'shein_buyer_email' => $shipping['email'] ?? null,
            'shein_buyer_phone' => $shipping['phone'] ?? null,
            'shein_platform_promotion' => isset($funds['platform_promotion']) ? number_format((float) $funds['platform_promotion'], 2, '.', '') : null,
            'shein_platform_offer' => isset($funds['platform_offer']) ? number_format((float) $funds['platform_offer'], 2, '.', '') : null,
            'shein_order_amount' => isset($funds['order_amount']) ? number_format((float) $funds['order_amount'], 2, '.', '') : null,
            'shein_customer_paid' => $customerPaid !== null ? number_format($customerPaid, 2, '.', '') : null,
            'shein_seller_amount_paid' => isset($funds['seller_amount_paid']) ? number_format((float) $funds['seller_amount_paid'], 2, '.', '') : null,
            'shein_platform_commission' => isset($funds['platform_commission']) ? number_format((float) $funds['platform_commission'], 2, '.', '') : null,
            'shein_transaction_service_fee' => isset($funds['transaction_service_fee']) ? number_format((float) $funds['transaction_service_fee'], 2, '.', '') : null,
            'shein_tax' => $taxAmount > 0 ? number_format($taxAmount, 2, '.', '') : null,
            'shein_platform_offer_tax' => isset($funds['platform_offer_tax']) ? number_format((float) $funds['platform_offer_tax'], 2, '.', '') : null,
        ] as $name => $value) {
            if ($value !== null && $value !== '') {
        ...
        if ($shippingCost > 0) {
            $payload['shipping_lines'] = [[
                'title' => (string) ($shipment['service'] ?? 'Shein shipping'),
    ...
    protected function splitShopifyCustomerName(array $detail, string $recipient = ''): array
    {
        ...
        if ($fullName === '') {
            return ['Shein', 'Customer'];
        }

        $parts = preg_split('/\s+/u', $fullName, 2) ?: [];
        $firstName = (string) ($parts[0] ?? 'Shein');
        if (! isset($parts[1])) {
    ...
    public function resolveShopifySourceAttribution(string $orderRef, ?array $settings = null): array
    {
        ...
        $handle = trim((string) (
            ...
            ?? config('services.shein.shopify_source_name')
            ?? 'shein'
        ));
        $urlTemplate = (string) (
            $settings['order']['shopify_source_url_template']
            ?? config('services.shein.shopify_source_url_template')
            ?? 'https://csp.shein.com/m_apps/order-manage/order_detail?orderId={order_id}'
        );

        return [
            'source_name' => $handle !== '' ? $handle : 'shein',
            'source_identifier' => $orderRef,
            'source_url' => str_replace('{order_id}', $orderRef, $urlTemplate),
            'referring_site' => 'https://www.shein.com/',
        ];
    }
    ...
    public function shopifySourceDisplayName(?array $settings = null): string
    {
        ...
        $name = trim((string) (
            ...
            ?? config('services.shein.shopify_source_display_name')
            ?? 'Shein'
        ));

        return $name !== '' ? $name : 'Shein';
    }
    ...
    public function resolveShopifyCustomerEmail(string $orderRef, ?string $rawEmail): array
    {
        ...
        $domain = (string) env('SHEIN_SHOPIFY_PLACEHOLDER_EMAIL_DOMAIN', 'import.5coremanagement.com');
        $slug = preg_replace('/[^a-zA-Z0-9]/', '', $orderRef) ?: 'order';

        return ['shein-'.$slug.'@'.$domain, true];
    }
    ...
    /**
     * Shein descriptions are often JSON module trees (moduleList / mobileDetail), not plain HTML.
     */
     ...
     */
    protected function formatProductVariants(array $ae, array $aeSkuRows, ?SheinMmMetric $metric = null, ?ShopifySku $shopify = null): array
    {
        $variants = [];
    ...
    /**
     * Prefer live/API variant stock for this SKU, else local shein_stock from pricing sync.
     *
     * @param  array<int, array<string, mixed>>  $variants
     */
    protected function resolveProductAeStock(array $variants, ?SheinMmMetric $metric, string $shopifySku): ?int
    {
     ...
     */
    protected function resolveListingLastSyncedAt(?SheinMmMetric $metric, string $shopifySku): ?\Carbon\Carbon
    {
        $candidates = array_filter([
            $metric?->updated_at,
    ...
    {
        $sku = trim($shopifySku);
        if ($sku === '' || ! Schema::hasTable('shein_pricing_prices')) {
            return null;
        }

        $row = SheinPricingPrice::query()
    ...
    protected function isMetricLinked(?SheinMmMetric $metric, string $shopifySku): bool
    {
    ...
    protected function localAeStockForSku(string $sku): ?int
    {
        return MarketplaceListingStockResolver::resolveMarketplaceQty(
            MarketplaceListingStockResolver::CHANNEL_SHEIN,
            $sku
        );
    }
     ...
     */
    protected function resolveOrderLineImage(array $product, ?SheinOrderMetric $db = null): ?string
    {
        foreach ([
    ...
    protected function normalizeOrderImageUrl(?string $url): ?string
    {
        ...
        if (preg_match('#^https?://#i', $url)
            ...
            && ! str_contains($url, 'shein-media.com')
            && ! str_contains($url, 'shein.com')) {
            return filter_var($url, FILTER_VALIDATE_URL) ? $url : null;
        }

        $filename = $this->extractOrderImageFilename($url);
        if ($filename) {
            return $this->buildSheinMediaImageUrl($filename);
        }
    ...
    protected function buildSheinMediaImageUrl(string $filename): string
    {
        return 'https://ae-pic-a1.shein-media.com/kf/'.ltrim($filename, '/');
    }
    ...
    protected function resolveOrderLineImageFallback(?string $sku, ?string $productId): ?string
    {
        ...
        if ($productId) {
            $metric = SheinMmMetric::query()->where('product_id', $productId)->first();
