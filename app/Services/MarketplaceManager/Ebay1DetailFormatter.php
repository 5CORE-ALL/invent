...
class Ebay1DetailFormatter
{
     ...
     */
    public function formatProduct(?array $aeLive, ?EbayMetric $metric, ShopifySku $shopify, array $aeSkuRows = []): array
    {
        $ae = $this->arr($aeLive);
     ...
     * @param  Collection<int, Ebay1OrderMetric>  $lines
     * @return array<string, mixed>
     */
    public function formatOrder(array $orderRoot, Collection $lines, Ebay1OrderMetric $primaryLine): array
    {
    ...
    /**
     * Build Shopify REST order payload from formatted eBay 1 order detail.
     *
    ...
    public function buildShopifyOrderPayload(array $detail, Collection $lines, array $tags): array
    {
        ...
        $noteLines = [
            'Imported from eBay 1 Order #'.$orderRef,
        ];
        ...
        $noteAttrs = [
            ['name' => 'ebay1_order_id', 'value' => $orderRef],
        ];
        if ($emailIsPlaceholder) {
            $noteAttrs[] = ['name' => 'ebay1_email_is_placeholder', 'value' => 'true'];
        }
        foreach ([
            'ebay1_buyer_login' => $summary['buyer_login_id'] ?? null,
            'ebay1_payment_method' => $payment['method'] ?? null,
            'ebay1_tracking_number' => $shipment['tracking'] ?? null,
            'ebay1_shipping_method' => $shipment['service'] ?? null,
            'ebay1_tax_number' => $shipping['tax_number'] ?? null,
            'ebay1_buyer_email' => $shipping['email'] ?? null,
            'ebay1_buyer_phone' => $shipping['phone'] ?? null,
            'ebay1_platform_promotion' => isset($funds['platform_promotion']) ? number_format((float) $funds['platform_promotion'], 2, '.', '') : null,
            'ebay1_platform_offer' => isset($funds['platform_offer']) ? number_format((float) $funds['platform_offer'], 2, '.', '') : null,
            'ebay1_order_amount' => isset($funds['order_amount']) ? number_format((float) $funds['order_amount'], 2, '.', '') : null,
            'ebay1_customer_paid' => $customerPaid !== null ? number_format($customerPaid, 2, '.', '') : null,
            'ebay1_seller_amount_paid' => isset($funds['seller_amount_paid']) ? number_format((float) $funds['seller_amount_paid'], 2, '.', '') : null,
            'ebay1_platform_commission' => isset($funds['platform_commission']) ? number_format((float) $funds['platform_commission'], 2, '.', '') : null,
            'ebay1_transaction_service_fee' => isset($funds['transaction_service_fee']) ? number_format((float) $funds['transaction_service_fee'], 2, '.', '') : null,
            'ebay1_tax' => $taxAmount > 0 ? number_format($taxAmount, 2, '.', '') : null,
            'ebay1_platform_offer_tax' => isset($funds['platform_offer_tax']) ? number_format((float) $funds['platform_offer_tax'], 2, '.', '') : null,
        ] as $name => $value) {
            if ($value !== null && $value !== '') {
        ...
        if ($shippingCost > 0) {
            $payload['shipping_lines'] = [[
                'title' => (string) ($shipment['service'] ?? 'eBay 1 shipping'),
    ...
    protected function splitShopifyCustomerName(array $detail, string $recipient = ''): array
    {
        ...
        if ($fullName === '') {
            return ['eBay 1', 'Customer'];
        }

        $parts = preg_split('/\s+/u', $fullName, 2) ?: [];
        $firstName = (string) ($parts[0] ?? 'eBay 1');
        if (! isset($parts[1])) {
    ...
    public function resolveShopifySourceAttribution(string $orderRef, ?array $settings = null): array
    {
        ...
        $handle = trim((string) (
            ...
            ?? config('services.ebay1.shopify_source_name')
            ?? 'ebay1'
        ));
        $urlTemplate = (string) (
            $settings['order']['shopify_source_url_template']
            ?? config('services.ebay1.shopify_source_url_template')
            ?? 'https://www.ebay.com/mesh/ord/details?orderid={order_id}'
        );

        return [
            'source_name' => $handle !== '' ? $handle : 'ebay1',
    ...
    public function shopifySourceDisplayName(?array $settings = null): string
    {
        ...
        $name = trim((string) (
            ...
            ?? config('services.ebay1.shopify_source_display_name')
            ?? 'eBay 1'
        ));

        return $name !== '' ? $name : 'eBay 1';
    }
    ...
    public function resolveShopifyCustomerEmail(string $orderRef, ?string $rawEmail): array
    {
        ...
        $domain = (string) env('EBAY2_SHOPIFY_PLACEHOLDER_EMAIL_DOMAIN', 'import.5coremanagement.com');
        $slug = preg_replace('/[^a-zA-Z0-9]/', '', $orderRef) ?: 'order';

        return ['ebay1-'.$slug.'@'.$domain, true];
    }
    ...
    /**
     * eBay 1 descriptions are often JSON module trees (moduleList / mobileDetail), not plain HTML.
     */
     ...
     */
    protected function formatProductVariants(array $ae, array $aeSkuRows, ?EbayMetric $metric = null, ?ShopifySku $shopify = null): array
    {
        $variants = [];
     ...
     */
    protected function resolveProductAeStock(array $variants, ?EbayMetric $metric, string $shopifySku): ?int
    {
     ...
     */
    protected function resolveListingLastSyncedAt(?EbayMetric $metric, string $shopifySku): ?\Carbon\Carbon
    {
        $candidates = array_filter([
            $metric?->updated_at,
    ...
    protected function isMetricLinked(?EbayMetric $metric, string $shopifySku): bool
    {
     ...
     */
    protected function resolveOrderLineImage(array $product, ?Ebay1OrderMetric $db = null): ?string
    {
        foreach ([
    ...
    protected function normalizeOrderImageUrl(?string $url): ?string
    {
        ...
        if (preg_match('#^https?://#i', $url)
            ...
            && ! str_contains($url, 'ebay1.com')) {
            return filter_var($url, FILTER_VALIDATE_URL) ? $url : null;
        }

        $filename = $this->extractOrderImageFilename($url);
        if ($filename) {
            return $this->buildEbay1MediaImageUrl($filename);
        }
    ...
    protected function buildEbay1MediaImageUrl(string $filename): string
    {
    ...
    protected function resolveOrderLineImageFallback(?string $sku, ?string $productId): ?string
    {
        ...
        if ($productId) {
            $metric = EbayMetric::query()->where('item_id', $productId)->first();
