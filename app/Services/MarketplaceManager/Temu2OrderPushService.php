...
class Temu2OrderPushService
{
    ...
    public function __construct(
        protected Temu2OrderDetailService $orderDetailService,
        protected Temu2DetailFormatter $formatter
    ) {}
     ...
     */
    public function syncShippingAddressToShopify(Temu2Order $order): array
    {
        ...
        $orderPayload = $this->formatter->buildShopifyOrderPayload($detail, $lines, ['temu2']);
    ...
    public function syncPendingAddressesToShopify(int $limit = 40): array
    {
        ...
        foreach ($unique as $line) {
            ...
            if (! empty($result['success']) && empty($result['skipped'])) {
                ...
                $failed++;
                Log::warning('Temu2OrderPushService: auto address sync failed', [
    ...
    public function importToShopify(Temu2Order $order): ?string
    {
        ...
        if ($parent !== '') {
            $localLinked = Temu2Order::query()
                ->where('parent_order_sn', $parent)
                ->whereNotNull('shopify_order_id')
                ->where('shopify_order_id', '!=', '')
                ->value('shopify_order_id');
            if ($localLinked) {
                $this->linkTemu2ParentToShopify($parent, (string) $localLinked);
        ...
        if (! empty($existing['id'])) {
            if ($parent !== '') {
                $this->linkTemu2ParentToShopify($parent, (string) $existing['id']);
            } else {
                $order->update([
            ...
            Log::info('Temu2OrderPushService: linked existing Shopify order (duplicate avoided)', [
    ...
    protected function linkTemu2ParentToShopify(string $parentOrderSn, string $shopifyOrderId): void
    {
        Temu2Order::query()
     ...
     */
    protected function syncInventoryAfterPush(Temu2Order $order): void
    {
        ...
        try {
            $result = app(Temu2InventorySyncService::class)
                ->syncSkusFromShopify($skus, $this->shopifyConfig());

            Log::info('Temu2OrderPushService: post-push inventory sync', [
                'parent_order_sn' => $order->parent_order_sn,
                'skus' => $skus,
                'result' => $result,
            ]);
        } catch (\Throwable $e) {
            Log::warning('Temu2OrderPushService: post-push inventory sync failed', [
                'parent_order_sn' => $order->parent_order_sn,
                'skus' => $skus,
                'error' => $e->getMessage(),
            ]);
     ...
     * @return array<string, mixed>
     */
    public function buildImportPlan(Temu2Order $order): array
    {
        if ($order->shopify_order_id) {
            return [
                'success' => false,
        ...
        try {
            for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
                ...
                if ($retryable && $attempt < $maxAttempts) {
                    ...
                    Log::warning('Temu2OrderPushService: Shopify order create retrying', [
                ...
                Log::error('Temu2OrderPushService: Shopify order create failed', [
                    'status' => $response->status(),
            ...
            Log::error('Temu2OrderPushService: exception', ['error' => $e->getMessage()]);

            return null;
        }
    }

    protected function fulfillShopifyForImportedOrder(Temu2Order $order): void
    {
        $id = (int) $order->id;
        if ($id < 1) {
            return;
        ...
        try {
            $result = app(VeeqoShopifyFulfillmentService::class)->fulfillMarketplaceOrder('temu2', $id);
            if (empty($result['success']) && empty($result['skipped'])) {
                Log::warning('Temu2OrderPushService: Shopify fulfillment after import failed', [
                    'parent_order_sn' => $order->parent_order_sn,
                    'result' => $result,
                ]);
            }
        } catch (\Throwable $e) {
            Log::warning('Temu2OrderPushService: Shopify fulfillment after import exception', [
                'parent_order_sn' => $order->parent_order_sn,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    protected function addFulfillmentTracking(string $shopifyOrderId, string $trackingNumber, string $carrier): void
    {
        try {
            ...
            Log::warning('Temu2OrderPushService: fulfillment tracking failed', [
                'shopify_order_id' => $shopifyOrderId,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    protected function findShopifyVariantIdBySku(string $sku): ?int
    {
        ...
        try {
            ...
            Log::warning('Temu2OrderPushService: variant lookup failed', ['sku' => $sku, 'error' => $e->getMessage()]);
