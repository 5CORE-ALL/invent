...
/**
 * Push Shopify fulfillment tracking numbers back to AliExpress
 * (declare / modify seller shipment).
 */
class AliexpressTrackingSyncService
{
    ...
    /**
     * Push Shopify tracking for one AliExpress order (by any line row).
     *
     * @return array{
     *   success: bool,
     *   skipped?: bool,
    ...
    public function pushTrackingForOrder(AliexpressOrderMetric $line): array
    {
        ...
        if ($orderId === '') {
            return ['success' => false, 'message' => 'AliExpress order id missing.'];
        }
        ...
        if ($aeTracking !== '' && $this->trackingEquals($aeTracking, $shopifyTracking)) {
            return [
                ...
                'message' => 'AliExpress already has this Shopify tracking number.',
        ...
        if (empty($result['success'])) {
            Log::warning('AliexpressTrackingSyncService: push failed', [
                'order_id' => $orderId,
            ...
            return [
                'success' => false,
                'action' => $action,
                'message' => $result['message'] ?? 'Failed to push tracking to AliExpress.',
                'shopify_tracking' => $shopifyTracking,
                'shopify_carrier' => $shopifyCarrier !== '' ? $shopifyCarrier : null,
        ...
        try {
            ...
            Log::info('AliexpressTrackingSyncService: post-push refresh failed', [
                'order_id' => $orderId,
                'error' => $e->getMessage(),
            ]);
        }

        Log::info('AliexpressTrackingSyncService: tracking pushed', [
            'order_id' => $orderId,
            'shopify_order_id' => $shopifyOrderId,
            'shopify_tracking' => $shopifyTracking,
            'action' => $action,
        ...
        return [
            ...
            'message' => $action === 'modified'
                ? "Updated AliExpress tracking to {$shopifyTracking}."
                : "Declared tracking {$shopifyTracking} on AliExpress.",
            'shopify_tracking' => $shopifyTracking,
            'shopify_carrier' => $shopifyCarrier !== '' ? $shopifyCarrier : null,
            'aliexpress_tracking' => $aeTracking !== '' ? $aeTracking : null,
            'service_name' => $serviceName,
    ...
    protected function fetchShopifyTracking(string $shopifyOrderId): array
    {
        ...
        try {
            ...
            if (! $response->successful()) {
                Log::warning('AliexpressTrackingSyncService: Shopify order fetch failed', [
                    'shopify_order_id' => $shopifyOrderId,
            ...
            Log::warning('AliexpressTrackingSyncService: Shopify tracking exception', [
                'shopify_order_id' => $shopifyOrderId,
                'error' => $e->getMessage(),
            ]);
        }
     ...
     */
    protected function resolveAeShipment(string $orderId, AliexpressOrderMetric $line): array
    {
        try {
            $this->orderDetailService->fetchAndPersistOrderDetail($orderId);
            $line->refresh();
    ...
    protected function tryModifyWithGuessedOldTracking(
        ...
        $ae = $this->resolveAeShipment(
            $orderId,
            AliexpressOrderMetric::query()->where('order_id', $orderId)->orderBy('id')->first()
                ?? new AliexpressOrderMetric(['order_id' => $orderId])
        );
