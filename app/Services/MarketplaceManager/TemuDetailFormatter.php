...
class TemuDetailFormatter
{
     ...
     */
    public function formatProduct(?array $aeLive, ?TemuMetric $metric, ShopifySku $shopify, array $aeSkuRows = []): array
    {
        $ae = $this->arr($aeLive);
     ...
     * @return array<string, mixed>
     */
    public function formatOrder(array $orderRoot, Collection $lines, TemuOrder $primaryLine): array
    {
    ...
    /**
     * Build Shopify REST order payload from formatted Temu order detail.
     *
    ...
    public function buildShopifyOrderPayload(array $detail, Collection $lines, array $tags): array
    {
        ...
        $noteLines = [
            'Imported from Temu Order #'.$orderRef,
        ];
    ...
    protected function splitShopifyCustomerName(array $detail, string $recipient = ''): array
    {
        ...
        if ($fullName === '') {
            return ['Temu', 'Customer'];
        }

        $parts = preg_split('/\s+/u', $fullName, 2) ?: [];
        $firstName = (string) ($parts[0] ?? 'Temu');
        if (! isset($parts[1])) {
    ...
    public function resolveShopifySourceAttribution(string $orderRef, ?array $settings = null): array
    {
        ...
        $handle = trim((string) (
            ...
            ?? config('services.temu.shopify_source_name')
            ?? 'temu'
        ));
        $urlTemplate = (string) (
            $settings['order']['shopify_source_url_template']
            ?? config('services.temu.shopify_source_url_template')
            ?? 'https://csp.temu.com/m_apps/order-manage/order_detail?orderId={order_id}'
        );

        return [
            'source_name' => $handle !== '' ? $handle : 'temu',
    ...
    public function shopifySourceDisplayName(?array $settings = null): string
    {
        ...
        $name = trim((string) (
            ...
            ?? config('services.temu.shopify_source_display_name')
            ?? 'Temu'
        ));

        return $name !== '' ? $name : 'Temu';
    }
    ...
    /**
     * Temu descriptions are often JSON module trees (moduleList / mobileDetail), not plain HTML.
     */
     ...
     */
    protected function formatProductVariants(array $ae, array $aeSkuRows, ?TemuMetric $metric = null, ?ShopifySku $shopify = null): array
    {
        $variants = [];
     ...
     */
    protected function resolveProductAeStock(array $variants, ?TemuMetric $metric, string $shopifySku): ?int
    {
     ...
     */
    protected function resolveListingLastSyncedAt(?TemuMetric $metric, string $shopifySku): ?\Carbon\Carbon
    {
        $candidates = array_filter([
            $metric?->updated_at,
    ...
    protected function isMetricLinked(?TemuMetric $metric, string $shopifySku): bool
    {
     ...
     */
    protected function resolveOrderLineImage(array $product, ?TemuOrder $db = null): ?string
    {
        foreach ([
    ...
    protected function resolveOrderLineImageFallback(?string $sku, ?string $productId): ?string
    {
        ...
        if ($productId) {
            $metric = TemuMetric::query()->where('goods_id', $productId)->first();
