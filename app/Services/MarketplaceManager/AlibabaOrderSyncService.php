...
class AlibabaOrderSyncService
{
    /** Alibaba order list queries are safest in ~30-day windows. */
     ...
     */
    public const MIN_ORDER_DATE = '2026-07-11';
    ...
    /**
     * Fetch orders from Alibaba and upsert into alibaba_order_metrics.
     *
    ...
    public function fetchAndStore(int $days = 7, int $pageSize = 50): array
    {
        if (empty($this->aliExpressApi->getAccessToken())) {
            return ['fetched' => 0, 'stored' => 0, 'message' => 'ALIBABA_ACCESS_TOKEN missing.'];
        }

        if (! Schema::hasTable('alibaba_order_metrics')) {
            return ['fetched' => 0, 'stored' => 0, 'message' => 'Run migrations for alibaba_order_metrics.'];
        }
    ...
    protected function fetchOrdersInDateRange(array $dateRange, int $pageSize): array
    {
        ...
        Log::info('[AlibabaOrderSync] Date range complete', [
    ...
    {
        if (! MarketplaceSyncSettings::canAutoImportToShopify('alibaba')) {
            return 0;
        }
        ...
        foreach ($orders as $order) {
            ...
            $alreadyImported = AlibabaOrderMetric::query()
