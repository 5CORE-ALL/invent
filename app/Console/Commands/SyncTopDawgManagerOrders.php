...
class SyncTopDawgManagerOrders extends Command
{
    ...
    protected $description = 'Fetch TopDawg orders from API and store in topdawg_order_metrics.';

    public function handle(TopDawgOrderSyncService $sync): int
    {
        if (! $this->option('force') && ! MarketplaceSyncSettings::canFetchOrders('topdawg')) {
            $this->info('Skipped: Fetch orders is Off in TopDawg Marketplace Manager settings.');
