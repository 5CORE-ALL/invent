...
class SetTikTokTokens extends Command
{
    ...
    protected $description = 'Store TikTok Shop access/refresh tokens in cache (and optionally .env)';
    ...
    public function handle(TikTokShopService $tiktok): int
    {
        ...
        if ($access === '') {
            ...
            $this->line('Complete OAuth first: open http://127.0.0.1:8000/tiktok/connect');
        ...
        config([
            'services.tiktok.access_token' => $access,
            'services.tiktok.refresh_token' => $refresh !== '' ? $refresh : null,
        ]);

        if ($this->option('write-env')) {
            $this->writeEnv('TIKTOK_ACCESS_TOKEN', $access);
            if ($refresh !== '') {
                $this->writeEnv('TIKTOK_REFRESH_TOKEN', $refresh);
            }
