...
class SyncFaireOrders extends Command
{
    protected $signature = 'faire:sync-orders
    ...
    protected $description = 'Fetch Faire orders from API and store in faire_order_metrics.';

    public function handle(FaireOrderSyncService $sync): int
    {
        if (! $this->option('force') && ! MarketplaceSyncSettings::canFetchOrders('faire')) {
            $this->info('Skipped: Fetch orders is Off in Faire Marketplace Manager settings.');
