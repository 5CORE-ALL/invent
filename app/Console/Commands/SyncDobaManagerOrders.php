...
class SyncDobaManagerOrders extends Command
{
    ...
    protected $description = 'Fetch Doba orders via doba:daily (wraps existing order fetch).';

    public function handle(DobaOrderSyncService $sync): int
    {
        if (! $this->option('force') && ! MarketplaceSyncSettings::canFetchOrders('doba')) {
            $this->info('Skipped: Fetch orders is Off in Doba Marketplace Manager settings.');
