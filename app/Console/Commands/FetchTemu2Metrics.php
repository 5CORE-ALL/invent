...
class FetchTemu2Metrics extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:fetch-temu2-metrics
     ...
     * @var string
     */
    protected $description = 'Fetch Temu 2 SKUs, goods IDs, order qty, stock, base prices, and ads analytics (Open API — no sheet)';
    ...
    {
        return $this->pushedPriceLookup ??= ChannelLivePriceSync::lookupMap('temu2');
    }
    ...
    {       
        Log::info('Starting FetchTemu2Metrics command');
        $this->info('Starting FetchTemu2Metrics command');

        // Verify credentials first
        if (!$this->verifyCredentials()) {
            $this->error('Invalid Temu 2 API credentials. Please check your .env file.');
            return 1;
        }
        ...
        try {
            ...
            Log::info('Completed FetchTemu2Metrics command successfully', ['only' => $only ?: 'all']);
            $this->info('✅ Completed FetchTemu2Metrics command successfully');
            return 0;
        } catch (\Exception $e) {
            Log::error('Error in FetchTemu2Metrics command: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            $this->error('Error in FetchTemu2Metrics command: ' . $e->getMessage());
            return 1;
        }
    }
    ...
    private function verifyCredentials()
    {
        ...
        if (empty($appKey) || empty($appSecret) || empty($accessToken)) {
            $this->error('Missing Temu 2 API credentials in .env file');
            $this->line('Required: TEMU2_APP_KEY, TEMU2_SECRET_KEY, TEMU2_ACCESS_TOKEN');
            return false;
        }
    ...
    private function fetchProductAnalyticsData(){
        ...
        try {
            $goodsIds = Temu2Metric::whereNotNull('goods_id')->pluck('goods_id')->toArray();
     ...
     * (used by temu2-decrease Temu Stock / Map / N Map — API, not sheet).
     */
    private function fetchStock(): void
    {
        Log::info('Starting fetchStock');
        $this->info('Fetching Temu 2 stock from goods list + SKU list APIs...');

        try {
            $service = app(Temu2ApiService::class);
            $items = $service->getInventory();
            $goodsCount = is_array($items) ? count($items) : 0;
            $skuUpdates = $service->syncSkuListStock();
            $withQty = Temu2Metric::where('quantity', '>', 0)->count();
            $this->info("Goods list items: {$goodsCount}; SKU stock updates: {$skuUpdates}; temu2_metrics qty>0: {$withQty}");
    ...
    private function fetchBasePrice()
    {
        ...
        try {
            ...
            $pushedLookup = ChannelLivePriceSync::lookupMap('temu2');
            $idToSku = [];
            foreach ($rows as $row) {
                ...
                foreach ($goodsPriceList as $goodsBlock) {
                    ...
                    if (is_array($skuPriceList)) {
                        foreach ($skuPriceList as $skuPrice) {
                            ...
                            $write = $sku !== ''
                                ? ChannelLivePriceSync::preferIncoming('temu2', $sku, (float) $amount, $pushedLookup)
                                : (float) $amount;
                ...
                foreach ($goodsList as $good) {
                    ...
                    foreach ($good['skuInfoList'] ?? [] as $sku) {
                        ...
                        if ($skuSn && $goodsId) {
                            ...
                            $updated = Temu2Metric::where('sku', $skuSnKey)
    ...
    private function fetchQuantity(){
        ...
        try {
            ...
            foreach ($finalSkuQuantities as $skuId => $data) {
                ...
                $skuKey = (string) $skuId;
                $updated = Temu2Metric::where('sku_id', $skuKey)
                    ->update([
                ...
                if ($updated) {
                    ...
                    $updated = Temu2Metric::where('sku', $skuKey)
                        ->update([
                            'quantity_purchased_l30' => $data['quantity_purchased_l30'],
                            'quantity_purchased_l60' => $data['quantity_purchased_l60'],
                        ]);
    ...
    {
        $sku = trim($sku);
        if ($sku === '' || ! Schema::hasTable('temu2_pricing')) {
            return;
        }
        ...
        Temu2Pricing::updateOrCreate(['sku' => $sku], $data);
    }
    ...
    private function debugSkuStatus()
    {
        ...
        $totalSkus = Temu2Metric::count();
        $skusWithSkuId = Temu2Metric::whereNotNull('sku_id')->count();
        $skusWithGoodsId = Temu2Metric::whereNotNull('goods_id')->count();
        $skusWithPrice = Temu2Metric::whereNotNull('base_price')->count();
        $skusWithQuantity = Temu2Metric::where('quantity_purchased_l30', '>', 0)->count();
        ...
        $incomplete = Temu2Metric::where(function($q) {
