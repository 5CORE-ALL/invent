...
class CollectEbay3Metrics extends Command
{
    use MonitorsCronExecution;
    use ProcessesUpdatesInChunks;

    protected $signature = 'ebay3:collect-metrics
        {--chunk= : Override chunk size (default from cron-monitor config)}';

    protected $description = 'Collect daily eBay 3 metrics (Price, Views, CVR%, INV, OV L30) for historical tracking — same source as /ebay3-tabulator-view (ebay_3_metrics), California date';

    protected string $monitorJobName = 'eBay 3 Collect Metrics';
    ...
    {
        $this->info('Starting eBay 3 metrics collection...');
        $monitor->startFresh()->markLocalOnly();
        // California calendar day — matches /ebay3-tabulator-view SKU chart (PT).
        ...
        $seedYesterday = Ebay3SkuDailyData::where('record_date', $yesterday)->exists() === false;
        if ($seedYesterday) {
            $this->info("No yesterday snapshots found — seeding {$yesterday} as baseline.");
        }

        $totalMetrics = Ebay3Metric::query()->whereNotNull('sku')->count();
        $monitor->setFetched($totalMetrics);
        $monitor->setExpected($totalMetrics);

        $campaignBySku = Ebay3PriorityReport::where('report_range', 'L30')
            ->get(['campaign_name', 'cpc_ad_fees_payout_currency'])
            ->keyBy(function ($item) {
                return strtoupper(trim((string) $item->campaign_name));
            });
        $generalByListing = Ebay3GeneralReport::where('report_range', 'L30')
            ->get(['listing_id', 'ad_fees'])
            ->keyBy(function ($item) {
                return trim((string) $item->listing_id);
            });

        // Shopify INV + OV L30 for tabulator trend dots (same source as /ebay3-tabulator-view columns).
            ...
            function ($rows) use (
                ...
                foreach ($rows as $ebayMetric) {
                    ...
                    try {
                        ...
                        // Same formula as EbayThreeController CVR 30 (SCVR): (eBay L30 / views) × 100
                        ...
                        Ebay3SkuDailyData::updateOrCreate(
                            [
                                'sku' => $sku,
                                'record_date' => $today,
                            ],
                        ...
                        if ($seedYesterday) {
                            ...
                            $yRow = Ebay3SkuDailyData::where('sku', $sku)->where('record_date', $yesterday)->first();
                            if ($yRow) {
                        ...
                        Log::error("Failed to collect eBay3 metrics for SKU: $sku", [
                            'error' => $e->getMessage(),
