...
class UpdateShoppingBudgetCronCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($productMasters as $pm) {
            ...
            if (!isset($campaignUpdates[$budgetId])) {
                try {
                    ...
                    $this->info("Updated SHOPPING campaign {$campaignId} (SKU: {$pm->sku}): Budget=\${$currentBudget} → \${$newBudget} (ACOS={$acos}%, Multiplier={$multiplier})");
                } catch (\Exception $e) {
                    $this->error("Failed to update SHOPPING campaign budget {$campaignId}: " . $e->getMessage());
                    Log::error("SHOPPING Budget Update Failed", [
        ...
        Log::info('SHOPPING Budget Cron Run', ['processed' => $processedCount]);
