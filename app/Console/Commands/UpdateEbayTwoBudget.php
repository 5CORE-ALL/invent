...
/**
 * eBay 2 mirror of {@see UpdateEbayOneBudget} — recalculates each running
 * campaign's daily budget from L30 ACOS and pushes it via
 * `sell/marketing/v1/ad_campaign/{id}/update_campaign_budget`.
 *
 * Source data: `ebay_2_priority_reports` (App\Models\Ebay2PriorityReport)
 * Auth:        EBAY2_APP_ID / EBAY2_CERT_ID / EBAY2_REFRESH_TOKEN
 *              (cached separately under `ebay2_access_token`)
 */
class UpdateEbayTwoBudget extends Command
{
    protected $signature = 'ebay2:update-budget';
    protected $description = 'Update eBay2 campaign budgets based on ACOS rules';
    ...
    public function handle()
    {
        try {
            ...
            $this->info('Starting eBay2 campaign budget update...');

            $accessToken = $this->getEbayAccessToken();
            if (!$accessToken) {
                $this->error('Failed to obtain eBay 2 access token.');
                return 1;
            }

            $this->info('Loading campaign reports (L30)...');

            $campaignReports = Ebay2PriorityReport::where('report_range', 'L30')
            ...
            foreach ($uniqueCampaigns as $campaignId => $reports) {
                ...
                // ACOS = (adFees / sales) * 100  — strip "USD " prefix from stored money strings.
                ...
                // Mirror frontend: 0 ACOS (no sales / no fees) is treated as 100% for the budget tier.
                if ($acos === 0) {
                    $acos = 100;
                }
            ...
            $this->info('eBay2 campaign budget update finished.');
            return 0;
    ...
    /**
     * Get eBay 2 access token using EBAY2_* refresh-token credentials.
     * Cached under `ebay2_access_token` (separate from eBay 1's `ebay_access_token`).
     */
    private function getEbayAccessToken()
    {
        try {
            ...
            if (!$clientId || !$clientSecret || !$refreshToken) {
                throw new Exception('Missing eBay 2 API credentials in environment variables (EBAY2_APP_ID / EBAY2_CERT_ID / EBAY2_REFRESH_TOKEN)');
            }
