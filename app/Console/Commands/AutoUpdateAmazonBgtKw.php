...
class AutoUpdateAmazonBgtKw extends Command
{
    ...
    public function handle()
    {
        try {
            ...
            // Track all skipped campaigns with detailed reasons
            $skippedCampaigns = [];
            ...
            if ($validCampaigns->isEmpty()) {
                ...
                if (!empty($skippedCampaigns)) {
                    $this->newLine();
                    $this->warn("========================================");
                    $this->warn("SKIPPED CAMPAIGNS REPORT");
                    $this->warn("========================================");
            ...
            try {
                ...
                $outcome = $this->applyAmazonBudgetCronUpdates(
                    $validCampaigns,
                    static fn (array $ids, array $bgts) => $updateKwBgts->updateAutoAmazonCampaignBgt($ids, $bgts),
                ...
                $this->newLine();
                $this->info("========================================");
                $this->info("FINAL UPDATE SUMMARY");
                $this->info("========================================");
                $this->info("Total Submitted: " . count($campaigns));
