...
class FetchShopifyB2CMetrics extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:fetch-shopify-b2c-metrics 
     ...
     * @var string
     */
    protected $description = 'Fetch Shopify B2C order data (excludes marketplace orders like TikTok, Amazon, eBay, etc.)';
    ...
    private $totalSkipped = 0; // Orders skipped due to marketplace tags
    ...
    {
        Log::info('Starting FetchShopifyB2CMetrics command');
        ...
        try {
            ...
            Log::info('Completed FetchShopifyB2CMetrics command successfully', [
                'total_fetched' => $this->totalFetched,
                'total_inserted' => $this->totalInserted,
                'total_updated' => $this->totalUpdated,
            ]);
            
            $this->info('✅ Completed FetchShopifyB2CMetrics command successfully');
            return 0;
        } catch (\Exception $e) {
            Log::error('Error in FetchShopifyB2CMetrics command: ' . $e->getMessage(), [
                'trace' => $e->getTraceAsString()
            ]);
            $this->error('Error in FetchShopifyB2CMetrics command: ' . $e->getMessage());
            return 1;
        }
    }
    ...
    private function fetchOrders($fromDate, $toDate)
    {
        ...
        $this->info("   Shopify B2C: Inserted: {$this->totalInserted} | Updated: {$this->totalUpdated}");
        $this->info("   Marketplace Orders Skipped: {$this->totalSkipped} (TikTok, Amazon, eBay, etc.)");
    }
    ...
    private function processOrder($order)
    {
        ...
        // =====================================================
        // FILTER: Only process pure Shopify B2C orders
        // Must have exact "Shopify B2C" tag
        // Exclude blank tags, marketplace orders, and other sources
        // =====================================================
        
        // Excluded marketplace tags
        $excludedTags = [
            'TikTok', 'Amazon', 'eBay', 'Temu', 'Walmart', 'Reverb',
            'Wayfair', 'Shein', 'AliExpress', 'Mercari', 'doba',
        ...
        // Check if order has any marketplace/excluded tag
        $isExcludedOrder = false;
        foreach ($excludedTags as $tag) {
            if (stripos($tags, $tag) !== false) {
                $isExcludedOrder = true;
                break;
            }
        }

        // Skip if:
        // 1. Tags are blank/empty
        // 2. Has excluded marketplace tags
        // 3. Draft orders
        // 4. Tag is NOT exactly "Shopify B2C"
        $isBlankTag = empty(trim($tags));
        $isNotShopifyB2C = (trim($tags) !== 'Shopify B2C');
        
        if ($isBlankTag || $isExcludedOrder || stripos($sourceName, 'draft_order') !== false || $isNotShopifyB2C) {
            $this->totalSkipped++;
        ...
        foreach ($lineItems as $item) {
            ...
            try {
                $existing = ShopifyB2CDailyData::where('order_id', $orderId)
                ...
                if ($existing) {
                    ...
                    ShopifyB2CDailyData::create($data);
                    $this->totalInserted++;
                }
            } catch (\Exception $e) {
    ...
    {
        $totalRecords = ShopifyB2CDailyData::count();
        $l30Count = ShopifyB2CDailyData::where('period', 'l30')->count();
        $l60Count = ShopifyB2CDailyData::where('period', 'l60')->count();
        $uniqueSkus = ShopifyB2CDailyData::distinct('sku')->count('sku');
        $uniqueOrders = ShopifyB2CDailyData::distinct('order_id')->count('order_id');
        
        // Calculate totals
        $totalQuantity = ShopifyB2CDailyData::sum('quantity');
        $totalSales = ShopifyB2CDailyData::sum('total_amount');
        $l30Sales = ShopifyB2CDailyData::where('period', 'l30')->sum('total_amount');
        $l60Sales = ShopifyB2CDailyData::where('period', 'l60')->sum('total_amount');

        // Get date range
        $minDate = ShopifyB2CDailyData::min('order_date');
        $maxDate = ShopifyB2CDailyData::max('order_date');

        $this->line("\n📊 Shopify B2C Order Statistics:");
        $this->line("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        ...
        $topSkus = ShopifyB2CDailyData::where('period', 'l30')
        ...
        Log::info('Debug status complete', [
