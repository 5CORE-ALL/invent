...
class AutoUpdateAmzUnderPtBids extends Command
{
    ...
    protected string $monitorJobName = 'Amazon Bid Sync (PT Under)';
    ...
    protected function executeBidUpdate(CronExecutionContext $monitor): int
    {
        try {
            ...
            if ($campaignBudgetMap === []) {
                $this->warn("No valid campaigns found.");
                $monitor->setExpected(0);
                return self::SUCCESS;
            }
            ...
            $this->info("FINAL Under PT: Total={$stats['total']} | Updated={$stats['updated']} | Skipped={$stats['skipped']} | Failed={$stats['failed']} | Chunks={$stats['chunks']}");
