...
class TiendamiaSalesController extends Controller
{
    public function index()
    {
        return view('sales.tiendamia_daily_sales_data');
    }

    public function getData(Request $request)
    {
        \Log::info('TiendamiaSalesController getData called');

        $orders = MiraklDailyData::tiendamia()
            ->l30()
            ->where('status', '!=', 'CLOSED')
            ->orderBy('order_created_at', 'desc')
            ->get();

        \Log::info('Found ' . $orders->count() . ' Tiendamia orders');
    ...
    {
        try {
            $filePath = storage_path('app/tiendamia_column_visibility.json');
            ...
            Log::error('Error getting Tiendamia column visibility: ' . $e->getMessage());
