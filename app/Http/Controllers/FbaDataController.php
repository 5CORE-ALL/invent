      ...
      $tableData = $fbaData->map(function ($fba, $sku) use ($fbaPriceData, $fbaReportsData, $shopifyData, $productData, $fbaMonthlySales, $fbaManualData, $fbaDispatchDates, $fbaShipCalculations, $amazonDatasheet, $amazonReviews, $fbaShipments, $adsKWDataBySku, $adsPTDataBySku, $overallAvgPrice, $fbaListingStatuses, $amazonLmpLookup, $amazonAdSpendBySku, $amazonPercentage, $fbaAgeData) {
        ...
        $amazonLmpPrice = $amazonLmpRecord ? round(floatval($amazonLmpRecord->price ?? 0), 2) : null;
         ...
         return [
            ...
            'FBA_Quantity' => (int)($fba->getAttribute('quantity_available') ?? 0),
            ...
            'Dil' => ($shopifyInfo ? ($shopifyInfo->quantity ?? 0) : 0) / max($shopifyInfo ? ($shopifyInfo->inv ?? 0) : 1, 1) * 100,
            'FBA_Dil' => ($monthlySales ? ($monthlySales->l30_units ?? 0) : 0) / (($fba->getAttribute('quantity_available') ?? 0) ?: 1) * 100,
            ...
            'ROI' => (function() use ($PRICE, $marginAfterCommission, $tcosPercentage, $LP_FOR_PFT, $FBA_SHIP_FOR_PFT) {
                if ($LP_FOR_PFT <= 0) return 0;
                $tacosDecimal = $tcosPercentage / 100;
                $netMargin = $marginAfterCommission - $tacosDecimal;
                return round((($PRICE * $netMargin) - $LP_FOR_PFT - $FBA_SHIP_FOR_PFT) / $LP_FOR_PFT * 100, 2);
            })(),
