...
use App\Models\MacyCompetitorItem;
...
/**
 * Macy's competitor discovery for the repricer.
 *
 * SerpApi has no dedicated Macy's engine. This controller uses
 * engine=google_shopping with "{query} macys" and keeps rows whose
 * source / link match Macy's — same UX shape as ebay-search / reverb-search.
 */
class MacySearchController extends Controller
{
    public function index()
    {
        return view('repricer.macy_search.index');
    }
    ...
    public function search(Request $request)
    {
        ...
        // Bias Google Shopping toward Macy's without breaking user keywords.
        ...
        try {
            for ($page = 1; $page <= $maxPages; $page++) {
                ...
                if (! $response->successful()) {
                    Log::error('SerpApi Macy shopping error', [
                    ...
                    if ($page === 1) {
                        $cached = MacyCompetitorItem::where('search_query', $searchQuery)
                ...
                foreach ($organicResults as $index => $result) {
                    ...
                    $existing = MacyCompetitorItem::where('search_query', $searchQuery)
                    ...
                    $link = $result['product_link']
                        ?? $result['link']
                        ?? ('https://www.macys.com/shop/featured/'.rawurlencode($searchQuery));
                    $sellerRating = isset($result['rating']) ? (string) $result['rating'] : null;
                    $position = $start + ($index + 1);

                    MacyCompetitorItem::create([
                        ...
                        'seller_name' => $result['source'] ?? "Macy's",
                ...
                // No Macy's hits on this page — further pages unlikely to help.
                if (! $pageHadMacy) {
                    break;
                }
            }

            $results = MacyCompetitorItem::where('search_query', $searchQuery)
                ->orderBy('position', 'asc')
                ->get();
            ...
            Log::error('SerpApi Macy exception', [
    ...
    {
        $searches = MacyCompetitorItem::select('search_query')
    ...
    public function getResults(Request $request)
    {
        ...
        $query = MacyCompetitorItem::where('search_query', $searchQuery);
        ...
        $results = $query->get();
        $priceStats = $this->calculatePriceStats($results);
        $conditions = MacyCompetitorItem::where('search_query', $searchQuery)
    ...
    public function getFilterOptions(Request $request)
    {
        ...
        $results = MacyCompetitorItem::where('search_query', $searchQuery)->get();
    ...
    public function storeCompetitors(Request $request)
    {
        ...
        if ($validator->fails()) {
            Log::warning('MacySearchController storeCompetitors validation failed', [
        ...
        try {
            foreach ($competitors as $competitor) {
                ...
                $result = MacySkuCompetitor::updateOrCreate(
                    [
                        'sku' => $sku,
                        'item_id' => $itemId,
                    ],
                    [
                        'marketplace' => $competitor['marketplace'] ?? 'macy',
            ...
            Log::error('Store Macy Competitors Error', [
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
     ...
     */
    private function isMacysResult(array $result): bool
    {
        $source = (string) ($result['source'] ?? '');
        $link = (string) ($result['product_link'] ?? $result['link'] ?? '');

        return stripos($source, 'macy') !== false || stripos($link, 'macys.com') !== false;
    }
