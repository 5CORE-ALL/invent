...
class CostpriceAnalysisController extends Controller
{
    ...
    public function getViewCostpriceAnalysisData(Request $request)
    {
 ...
 if ($response->getStatusCode() === 200) {
    ...
    public function costpriceAnalysis(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');

        return view('product-master.costpriceAnalysis', [
            'mode' => $mode,
            'demo' => $demo,
        ]);
    }
