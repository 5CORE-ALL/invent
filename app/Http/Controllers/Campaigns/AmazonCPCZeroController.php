...
class AmazonCPCZeroController extends Controller
{
    public function getKwCpcZeroView(){
        return view('campaign.cpc-zero.kw-cpc-zero');
    }

    public function getKwCpcZeroData(){
        ...
        foreach ($productMasters as $pm) {
            ...
            if (($row['l7_cpc'] == 0 || $row['l1_cpc'] == 0) && $row['NRA'] !== 'NRA' && $row['campaignName'] !== '') {
                $result[] = (object) $row;
            }
    ...
    public function getPtCpcZeroView(){
        return view('campaign.cpc-zero.pt-cpc-zero');
    }

    public function getPtCpcZeroData(){
        $productMasters = ProductMaster::orderBy('parent', 'asc')
            ->orderByRaw("CASE WHEN sku LIKE 'PARENT %' THEN 1 ELSE 0 END")
            ->orderBy('sku', 'asc')
            ->get();
