...
class MercariWoShipZeroController extends Controller
{
    public function mercariWoShipZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.mercariWoShipZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    // public function getViewMercariWoShipZeroData(Request $request)
    // {
    ...
    //     $mercariWoShipDataViews = MercariWoShipDataView::whereIn('sku', $skus)->get()->keyBy('sku');
    ...
    //         if ($inv > 0) {
    //             $mercariView = $mercariWoShipDataViews[$sku] ?? null;
    ...
    public function getViewMercariWoShipZeroData(Request $request)
    {
        ...
        $percentage = Cache::remember('mercariwoshop_marketplace_percentage', now()->addDays(30), function () {
        ...
        $temuMetrics = MercariWoShipSheetdata::whereIn(DB::raw('UPPER(TRIM(sku))'), $skus)
            ->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $temuDataViews = MercariWoShipDataView::whereIn(DB::raw('UPPER(TRIM(sku))'), $skus)
            ->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = MercariWoShipDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
    ...
    public function getLivePendingAndZeroViewCounts()
    {
        ...
        $mercariListingStatus = MercariWoShipListingStatus::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $mercariDataViews = MercariWoShipDataView::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $mercariMetrics = MercariWoShipSheetdata::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));
