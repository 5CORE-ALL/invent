...
use App\Models\InstagramShopDataView;
...
class InstagramShopZeroController extends Controller
{
    public function instagramShopZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.instagramShopZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    // public function getViewInstagramShopZeroData(Request $request)
    // {
    ...
    //     $instagramShopDataViews = InstagramShopDataView::whereIn('sku', $skus)->get()->keyBy('sku');
    ...
    //         if ($inv > 0) {
    //             $instaView = $instagramShopDataViews[$sku] ?? null;
    //             $value = $instaView ? $instaView->value : [];
    //             if (is_string($value)) {
    ...
    public function getViewInstagramShopZeroData(Request $request)
    {
        ...
        $percentage = Cache::remember('instagram_marketplace_percentage', now()->addDays(30), function () {
            $marketplaceData = MarketplacePercentage::where('marketplace', 'InstagramShop')->first();
        ...
        $temuMetrics = InstagramShopSheetdata::whereIn(DB::raw('UPPER(TRIM(sku))'), $skus)
            ->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $temuDataViews = InstagramShopDataView::whereIn(DB::raw('UPPER(TRIM(sku))'), $skus)
            ->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = InstagramShopDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
    ...
    public function getLivePendingAndZeroViewCounts()
    {
        ...
        $instagramListingStatus = InstagramShopListingStatus::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $instagramDataViews = InstagramShopDataView::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $instagramsMetrics = InstagramShopSheetdata::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));
        ...
        foreach ($productMasters as $item) {
            ...
            $status = $instagramListingStatus[$sku]->value ?? null;
            if (is_string($status)) {
            ...
            // --- Amazon Live Status ---
            $dataView = $instagramDataViews[$sku]->value ?? null;
            ...
            $metricRecord = $instagramsMetrics[$sku] ?? null;
            $views = null;
