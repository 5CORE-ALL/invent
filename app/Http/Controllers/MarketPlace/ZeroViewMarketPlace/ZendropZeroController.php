...
class ZendropZeroController extends Controller
{
    public function zendropZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.zendropZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewZendropZeroData(Request $request)
    {
        ...
        foreach ($productMasters as $pm) {
            ...
            if ($inv > 0) {
                $zendropView = $zendropDataViews[$sku] ?? null;
                $value = $zendropView ? $zendropView->value : [];
                if (is_string($value)) {
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = ZendropDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
