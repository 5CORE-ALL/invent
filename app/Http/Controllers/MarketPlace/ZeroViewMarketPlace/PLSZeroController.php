...
class PLSZeroController extends Controller
{
    public function plsZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.plsZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewPLSZeroData(Request $request)
    {
        ...
        foreach ($productMasters as $pm) {
            ...
            if ($inv > 0) {
                $plsView = $plsDataViews[$sku] ?? null;
                $value = $plsView ? $plsView->value : [];
                if (is_string($value)) {
    ...
    public function getLivePendingAndZeroViewCounts()
    {
        ...
        $plsListingStatus = PlsListingStatus::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $plsDataViews = PLSDataView::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $plsMetrics = PLSProduct::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));
        ...
        foreach ($productMasters as $item) {
            ...
            $status = $plsListingStatus[$sku]->value ?? null;
            if (is_string($status)) {
            ...
            // --- Amazon Live Status ---
            $dataView = $plsDataViews[$sku]->value ?? null;
            ...
            $metricRecord = $plsMetrics[$sku] ?? null;
            $views = null;
