...
use App\Models\SWGearExchangeDataView;
use Illuminate\Http\Request;

class SWGearExchangeZeroController extends Controller
{
    public function swGearExchangeZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.swGearExchangeZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewSWGearExchangeZeroData(Request $request)
    {
        ...
        $swGearExchangeDataViews = SWGearExchangeDataView::whereIn('sku', $skus)->get()->keyBy('sku');
        ...
        foreach ($productMasters as $pm) {
            ...
            if ($inv > 0) {
                $swGearView = $swGearExchangeDataViews[$sku] ?? null;
                $value = $swGearView ? $swGearView->value : [];
                if (is_string($value)) {
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = SWGearExchangeDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
