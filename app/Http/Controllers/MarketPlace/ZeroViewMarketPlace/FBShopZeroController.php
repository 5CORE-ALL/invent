...
class FBShopZeroController extends Controller
{
    public function fbShopZeroview(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        return view('market-places.zero-market-places.fbShopZeroView', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    // public function getViewFBShopZeroData(Request $request)
    // {
    ...
    //     $fbShopDataViews = FBShopDataView::whereIn('sku', $skus)->get()->keyBy('sku');
    ...
    //         if ($inv > 0) {
    //             $fbShopView = $fbShopDataViews[$sku] ?? null;
    //             $value = $fbShopView ? $fbShopView->value : [];
    //             if (is_string($value)) {
    ...
    public function getViewFBShopZeroData(Request $request)
    {
        ...
        $percentage = Cache::remember('fbshop_marketplace_percentage', now()->addDays(30), function () {
            $marketplaceData = MarketplacePercentage::where('marketplace', 'FBShop')->first();
        ...
        $temuMetrics = FbShopSheetdata::whereIn(DB::raw('UPPER(TRIM(sku))'), $skus)
            ->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $temuDataViews = FBShopDataView::whereIn(DB::raw('UPPER(TRIM(sku))'), $skus)
            ->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));
    ...
    public function updateReasonAction(Request $request)
    {
        ...
        $row = FBShopDataView::firstOrCreate(
            ['sku' => $sku],
            ['value' => json_encode([])]
        );
    ...
    public function getLivePendingAndZeroViewCounts()
    {
        ...
        $fbshopListingStatus = FBShopListingStatus::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $fbshopDataViews = FBShopDataView::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));

        $fbshopsMetrics = FbShopSheetdata::whereIn('sku', $skus)->get()
            ->keyBy(fn($s) => strtoupper(trim($s->sku)));
        ...
        foreach ($productMasters as $item) {
            ...
            $status = $fbshopListingStatus[$sku]->value ?? null;
            if (is_string($status)) {
            ...
            // --- Amazon Live Status ---
            $dataView = $fbshopDataViews[$sku]->value ?? null;
            ...
            $metricRecord = $fbshopsMetrics[$sku] ?? null;
            $views = null;
