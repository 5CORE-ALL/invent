...
class TemuSyncController extends Controller
{
        ...
        if (Schema::hasTable('temu_orders')) {
            if (Schema::hasColumn('temu_orders', 'import_status')) {
                MarketplaceShopifyImportQueue::releaseStuckQueued(
                    TemuOrder::class,
                    MarketplaceManagerRegistry::queueFor('temu')
                );
            }
    ...
    public function fetchOrders(Request $request): JsonResponse
    {
        ...
        if ($request->boolean('import') || MarketplaceSyncSettings::canAutoImportToShopify('temu')) {
    ...
    public function syncMismatchInventoryNow(Request $request): JsonResponse
    {
        ...
        $result = app(TemuInventorySyncService::class)->syncSkusFromShopify($batch, null, true);
