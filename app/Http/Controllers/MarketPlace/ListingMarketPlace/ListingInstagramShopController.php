...
class ListingInstagramShopController extends Controller
{
    use HandlesListingPublishActions;

    public function listingInstagramShop(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        $percentage = Cache::remember('instagramshop_marketplace_percentage', now()->addDays(30), function () {
            return 100;
        });

        return view('market-places.listing-market-places.listingInstagramShop', [
            'instagramShopPercentage' => $percentage,
            'mode' => $mode,
            'demo' => $demo,
        ]);
    }

    public function getViewListingInstagramShopData(Request $request)
    {
        return response()->json([
            'status' => 200,
            'data' => AutomatedListingPage::rows('instagramshop'),
        ]);
    }
    ...
    public function saveStatus(Request $request)
    {
        ...
        $sku = $validated['sku'];
        $status = InstagramShopListingStatus::where('sku', $sku)->first();
        ...
        InstagramShopListingStatus::updateOrCreate(
            ['sku' => $validated['sku']],
            ['value' => $existing]
        );
    ...
    {
        return ChannelListingRegistry::nrReqCountArray('instagramshop');
    }
