...
{
    public function listingShopifyWholesale(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        $percentage = Cache::remember('shopifywholesale_marketplace_percentage', now()->addDays(30), function () {
            return 100;
        });

        return view('market-places.listing-market-places.listingShopifyWholesale', [
            'shopifyWholesalePercentage' => $percentage,
            'mode' => $mode,
            'demo' => $demo,
        ]);
    }

    public function getViewListingShopifyWholesaleData(Request $request)
    {
    ...
    public function saveStatus(Request $request)
    {
        ...
        $sku = $validated['sku'];
        $status = ShopifyWholesaleListingStatus::where('sku', $sku)->first();
        ...
        ShopifyWholesaleListingStatus::updateOrCreate(
            ['sku' => $validated['sku']],
            ['value' => $existing]
        );
