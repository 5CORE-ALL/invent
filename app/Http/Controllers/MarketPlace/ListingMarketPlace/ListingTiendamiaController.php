...
class ListingTiendamiaController extends Controller
{
    public function listingTiendamia(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        $percentage = Cache::remember('tiendamia_marketplace_percentage', now()->addDays(30), function () {
            return 100;
        });

        return view('market-places.listing-market-places.listingTiendamia', [
    ...
    public function getViewListingTiendamiaData(Request $request)
    {
        return response()->json([
            'status' => 200,
            'data' => AutomatedListingPage::rows('tiendamia'),
        ]);
    }
    ...
    {
        return ChannelListingRegistry::nrReqCountArray('tiendamia');
    }
