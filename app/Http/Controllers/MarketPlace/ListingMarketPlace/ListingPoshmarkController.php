...
class ListingPoshmarkController extends Controller
{
    use HandlesListingPublishActions;

    public function listingPoshmark(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        $percentage = Cache::remember('poshmark_marketplace_percentage', now()->addDays(30), function () {
            return 100;
        });

        return view('market-places.listing-market-places.listingPoshmark', [
            'poshmarkPercentage' => $percentage,
            'mode' => $mode,
            'demo' => $demo,
        ]);
    }

    public function getViewListingPoshmarkData(Request $request)
    {
        return response()->json([
            'status' => 200,
            'data' => AutomatedListingPage::rows('poshmark'),
        ]);
    }
    ...
    public function saveStatus(Request $request)
    {
        ...
        $sku = $validated['sku'];
        $status = PoshmarkListingStatus::where('sku', $sku)->first();
        ...
        PoshmarkListingStatus::updateOrCreate(
            ['sku' => $validated['sku']],
            ['value' => $existing]
        );
    ...
    {
        return ChannelListingRegistry::nrReqCountArray('poshmark');
    }
