...
class ListingSynceeController extends Controller
{
    ...
    public function listingSyncee(Request $request)
    {
        ...
        $percentage = Cache::remember('syncee_marketplace_percentage', now()->addDays(30), function () {
            return 100;
        });

        return view('market-places.listing-market-places.listingSyncee', [
            'synceePercentage' => $percentage,
            'mode' => $mode,
            'demo' => $demo,
        ]);
    }

    public function getViewListingSynceeData(Request $request)
    {
        return response()->json([
            'status' => 200,
            'data' => AutomatedListingPage::rows('syncee'),
        ]);
    }
    ...
    public function saveStatus(Request $request)
    {
        ...
        $sku = $validated['sku'];
        $status = SynceeListingStatus::where('sku', $sku)->first();
        ...
        SynceeListingStatus::updateOrCreate(
            ['sku' => $validated['sku']],
            ['value' => $existing]
        );
    ...
    {
        return ChannelListingRegistry::nrReqCountArray('syncee');
    }
