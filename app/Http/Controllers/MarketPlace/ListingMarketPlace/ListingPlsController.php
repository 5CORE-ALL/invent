...
class ListingPlsController extends Controller
{
    use HandlesListingPublishActions;

    public function listingPls(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        $percentage = Cache::remember('pls_marketplace_percentage', now()->addDays(30), function () {
            return 100;
        });

        return view('market-places.listing-market-places.listingPls', [
            'plsPercentage' => $percentage,
            'mode' => $mode,
            'demo' => $demo,
        ]);
    }

    public function getViewListingPlsData(Request $request)
    {
        return response()->json([
            'status' => 200,
            'data' => AutomatedListingPage::rows('pls'),
        ]);
    }
    ...
    public function saveStatus(Request $request)
    {
        ...
        $sku = $validated['sku'];
        $status = PlsListingStatus::where('sku', $sku)->first();
        ...
        PlsListingStatus::updateOrCreate(
            ['sku' => $validated['sku']],
            ['value' => $existing]
        );
    ...
    {
        return ChannelListingRegistry::nrReqCountArray('pls');
    }
