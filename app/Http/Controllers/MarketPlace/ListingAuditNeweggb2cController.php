...
class ListingAuditNeweggb2cController extends Controller
{
    public function listingAuditNeweggb2c(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');

        return view('market-places.listingAuditNeweegb2c', [
            'mode' => $mode,
            'demo' => $demo
        ]);
    }

    public function getViewListingAuditNeweggb2cData()
    {
    ...
    public function saveAuditToDatabase(Request $request)
    {
        ...
        $ebayDataView = Neweegb2cDataView::firstOrNew(['sku' => $sku]);
