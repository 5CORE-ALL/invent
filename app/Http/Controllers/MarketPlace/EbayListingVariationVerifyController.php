...
class EbayListingVariationVerifyController extends Controller
{
    public function index()
    {
        return view('market-places.ebay_listing_variation_verify');
    }
    ...
    public function data(Request $request)
    {
        ...
        return response()->json([
            'data' => $formattedData,
            'meta' => [
                'listings_count' => (int) EbayMetric::query()->whereNotNull('sku')->where('sku', '!=', '')->count(),
                'last_pulled_at' => EbayMetric::query()->max('updated_at'),
                'has_listings_cache' => EbayMetric::query()->whereNotNull('sku')->where('sku', '!=', '')->exists(),
    ...
    public function pullListings(Request $request)
    {
        try {
            ...
            $exitCode = Artisan::call('app:fetch-ebay-reports');
            ...
            if ($exitCode !== 0) {
                return response()->json([
                    ...
                    'message' => $output !== ''
                        ? $output
                        : 'Failed to pull eBay listings (exit code ' . $exitCode . ').',
                ], 422);
            }

            $count = (int) EbayMetric::query()->whereNotNull('sku')->where('sku', '!=', '')->count();

            return response()->json([
                'status' => 200,
                'message' => "Pulled eBay listings. {$count} SKUs in ebay_metrics. Parent Vs Listed SKU updated.",
                'count' => $count,
                'last_pulled_at' => EbayMetric::query()->max('updated_at'),
            ]);
        } catch (\Throwable $e) {
            Log::error('Ebay Listing Variation Verify: pull listings failed', [
                'error' => $e->getMessage(),
            ]);
    ...
    private function buildListedSkuLookup(): array
    {
        ...
        foreach (EbayMetric::query()->whereNotNull('sku')->where('sku', '!=', '')->pluck('sku') as $sku) {
