...
class TiktokShopController extends Controller
{
    ...
    public function overallTiktok(Request $request)
    {
        ...
        $marketplaceData = ChannelMaster::where('channel', 'Tiktok Shop')->first();

        $percentage = $marketplaceData ? $marketplaceData->channel_percentage : 100;
        $adUpdates = $marketplaceData ? $marketplaceData->ad_updates : 0;

        return view('market-places.tiktok_analysis', [
            'mode' => $mode,
            'demo' => $demo,
    ...
    public function getViewTiktokData(Request $request)
    {
        // Get percentage from cache or database
        $percentage = Cache::remember('Tiktok Shop', now()->addDays(30), function () {
            $marketplaceData = MarketplacePercentage::where('marketplace', 'Tiktok Shop')->first();
            return $marketplaceData ? $marketplaceData->percentage : 100;
        });
        ...
        $walmartDataViews = TiktokShopDataView::whereIn('sku', $skus)->get()->keyBy('sku');
        $nrValues = [];
        $listedValues = [];
        $liveValues = [];
    ...
    public function updateListedLive(Request $request)
    {
        ...
        $product = TiktokShopDataView::firstOrCreate(
            ['sku' => $request->sku],
            ['value' => []]
        );
    ...
    public function importTikTokAnalytics(Request $request)
    {
        $request->validate([
        ...
        try {
            ...
            foreach ($rows as $index => $row) {
                ...
                TiktokShopDataView::updateOrCreate(
                    ['sku' => $data['sku']],
                    ['value' => $values]
                );
    ...
    public function exportTiktokShopAnalytics()
    {
        $tiktokShopData = TiktokShopDataView::all();
        ...
        // Data Rows
        $rowIndex = 2;
        foreach ($tiktokShopData as $data) {
        ...
        // Output Download
        $fileName = 'Tiktok_Shop_Analytics_Export_' . date('Y-m-d') . '.xlsx';
    ...
    public function downloadSample()
    {
        ...
        // Output Download
        $fileName = 'Tiktok_Shop_Analytics_Sample.xlsx';
