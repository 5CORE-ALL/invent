...
use App\Jobs\WarmTemu2LiveListingsCache;
use App\Models\Temu2Metric;
use App\Models\Temu2Order;
use App\Models\MarketplaceSyncSettings;
use App\Models\ShopifySku;
use App\Services\Temu2ApiService;
use App\Services\MarketplaceManager\Temu2DetailFormatter;
...
use App\Services\MarketplaceManager\Temu2OrderDetailService;
...
class Temu2SyncController extends Controller
{
    ...
    {
        $appKey = (string) config('services.temu2.app_key', '');
        $secretKey = (string) config('services.temu2.secret_key', '');
        $accessToken = (string) config('services.temu2.access_token', '');
        $credentialsReady = $this->temuApi->isConfigured();
        $apiUrl = (string) config('services.temu2.openapi_router_url', 'https://openapi-b-us.temu.com/openapi/router');

        return view('marketplace.temu2.connect', [
            'title' => 'Temu 2 — Connect',
    ...
    public function showProduct(int $shopifySkuId): View
    {
        ...
        $detail = app(Temu2DetailFormatter::class)->formatProduct(
        ...
        return view('marketplace.temu2.product-show', [
            'title' => 'Temu 2 Listing — '.$sku,
    ...
    /**
     * Push live Shopify qty → Temu 2 for this one SKU immediately (no queue).
     */
    public function pushProductInventory(int $shopifySkuId): JsonResponse
    {
        ...
        if (! ($settings['inventory']['inventory_sync'] ?? false) && ! ($settings['pricing']['price_sync'] ?? false)) {
            return response()->json([
                'success' => false,
                'message' => 'Turn on Inventory sync (or Price sync) in Temu 2 settings first.',
            ], 422);
        }
        ...
        if (! $this->isShopifySkuLinkedOnTemu($metric, $sku)) {
            return response()->json([
                'success' => false,
                'message' => 'This SKU is not linked on Temu 2. Run Sync Temu 2 link map first.',
            ], 422);
        }

        $result = app(Temu2InventorySyncService::class)->syncSkusFromShopify([$sku]);
    ...
    {
        if (! $this->apiConfig->isConfigured('temu2')) {
            return response()->json(['success' => false, 'message' => 'Temu 2 not connected.']);
        }
        ...
        if (! $metric?->goods_id || (string) $metric->goods_id === (string) $metric->sku) {
            return response()->json([
                'success' => false,
                'message' => 'No Temu 2 goods_id mapped for this SKU. Run Sync Temu 2 link map on Listings first.',
            ]);
        }
        ...
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to refresh Temu 2 inventory: '.$e->getMessage(),
            ]);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Refreshed Temu 2 metrics for '.$sku.' from API. Nothing was pushed to Shopify or Temu 2.',
        ]);
    }
    ...
    public function refreshProducts(Request $request): JsonResponse
    {
        ...
        Log::info('Temu 2 link map sync page', ['page' => $page, 'reset' => $reset]);

        $result = app(Temu2LinkMapSyncService::class)->syncPage($page, 50, $reset);
    ...
    {
        $progress = app(Temu2LinkMapSyncService::class)->getProgress();
    ...
    public function syncOrders(Request $request): View
    {
        ...
        return view('marketplace.temu2.orders', [
            'orders' => $orders,
            'title' => 'Temu 2 — Orders',
            'apiError' => $apiError,
            'connected' => $this->apiConfig->isConfigured('temu2'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('temu2'),
        ]);
    }

    public function showOrder(int $id): View
    {
        $line = Temu2Order::query()->findOrFail($id);
        ...
        $aeLiveError = null;
        $aeDataSource = 'cached';
        $detailService = app(Temu2OrderDetailService::class);
        ...
        if ($this->apiConfig->isConfigured('temu2')) {
            ...
            if (! empty($pull['success'])) {
                ...
                $aeLiveError = $pull['message'] ?? 'Could not refresh live Temu 2 order details.';
            }
        }

        $orderRoot = $detailService->resolveOrderRoot($line);
        $detail = app(Temu2DetailFormatter::class)->formatOrder($orderRoot, $lines, $line);

        return view('marketplace.temu2.order-show', [
            'title' => 'Temu 2 Order — '.$orderId,
            'orderId' => $orderId,
            'line' => $line,
            'detail' => $detail,
            'aeLiveError' => $aeLiveError,
            ...
            'connected' => $this->apiConfig->isConfigured('temu2'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('temu2'),
            'orderIsPaid' => MarketplaceOrderPaidFilter::isPaid('temu2', $line),
        ]);
    }
    ...
    public function pullOrderFromTemu(int $id): JsonResponse
    {
        ...
        $line = Temu2Order::query()->findOrFail($id);
        $result = app(Temu2OrderDetailService::class)->fetchAndPersistOrderDetail((string) $line->parent_order_sn);

        // Persist only — Shopify address fill matches AliExpress / Reverb (SyncTemu2AddressJob).
        return response()->json([
            'success' => ! empty($result['success']),
            'message' => $result['message'] ?? ($result['success'] ? 'Order details updated from Temu 2.' : 'Failed to pull order details.'),
        ]);
    }

    /**
     * Push Shopify fulfillment tracking number to Temu 2 (Ship Order).
     */
    public function pushTrackingToTemu(int $id): JsonResponse
    {
        ...
        $line = Temu2Order::query()->findOrFail($id);
        $result = app(Temu2TrackingSyncService::class)->pushTrackingForOrder($line);
    ...
    /**
     * Bulk push Shopify tracking → Temu 2 for linked orders.
     */
    public function syncTrackingNow(): JsonResponse
    {
        ...
        \App\Jobs\SyncTemu2TrackingJob::dispatch(false);

        return response()->json([
            'success' => true,
            'queued' => true,
            'message' => 'Tracking sync queued. It reads Shopify fulfillments and ships orders on Temu 2.',
        ]);
    }
    ...
    public function syncInventoryNow(): JsonResponse
    {
        ...
        \App\Jobs\RunMarketplaceInventorySyncJob::dispatch('temu2');
    ...
    public function pushOrderToShopify(Request $request): JsonResponse
    {
        ...
        if ($request->boolean('dry_run')) {
            $preview = app(Temu2OrderPushService::class)->previewShopifyPush($order);
    ...
    /**
     * Reverse a Shopify push for selected Temu 2 rows (selected SKU only).
    ...
    /**
     * Delete a local Temu 2 order that is still ready for Shopify import
    ...
    public function deleteReadyOrder(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => "Removed Temu 2 order {$orderId} from ready-for-import ({$deleted} row(s)).",
            'deleted' => $deleted,
            'order_id' => $orderId,
        ]);
    }
    ...
    public function markOrderAlreadyImported(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => "Marked Temu 2 order {$orderId} as already imported ({$updated} row(s)).",
            'shopify_order_id' => $shopifyOrderId,
            'updated' => $updated,
        ]);
    }
    ...
    {
        return view('marketplace.temu2.settings', [
            'settings' => MarketplaceSyncSettings::getFor('temu2'),
            'title' => 'Temu 2 — Sync Settings',
            'connected' => $this->apiConfig->isConfigured('temu2'),
        ]);
    }
    ...
    {
        $current = MarketplaceSyncSettings::getFor('temu2');
        ...
        $order = $this->mergeSettingsSection($current['order'] ?? [], $request->input('order', []), [
            ...
            'push_tracking_to_temu2', 'sync_address_to_shopify',
        ]);
        $listings = $this->mergeSettingsSection($current['listings'] ?? [], $request->input('listings', []), [
            'auto_link_by_sku', 'create_products_on_temu2', 'sync_title', 'sync_images',
        ]);
        ...
        MarketplaceSyncSettings::setFor('temu2', [
            'pricing' => $pricing,
            'inventory' => $inventory,
            'order' => $order,
            'listings' => $listings,
    ...
    protected function paginateAeNotInShopify(
        ...
        $rows = $this->aeMetricsWithRealSkuQuery()->orderBy('sku')->get()->filter(function (Temu2Metric $metric) use ($shopifyNormKeys, $searchSku, $searchName) {
            $norm = ShopifySku::normalizeSkuForShopifyLookup((string) $metric->sku);
            if ($norm === '' || isset($shopifyNormKeys[$norm])) {
                return false;
            }
        ...
        $slice = $rows->slice(($page - 1) * $perPage, $perPage)->map(function (Temu2Metric $metric) use ($aeStockMap) {
     ...
     * @return array<string, Temu2Metric>
     */
    protected function temuMetricMapForSkus(array $skus): array
    {
        if ($skus === [] || ! Schema::hasTable('temu2_metrics')) {
            return [];
        }

        $exact = Temu2Metric::query()->whereIn('sku', $skus)->get()->keyBy('sku');
        $byNorm = [];
        foreach (Temu2Metric::query()
            ->whereNotNull('sku')
            ->whereNotNull('goods_id')
            ->where('sku', '!=', '')
            ->where('goods_id', '!=', '')
    ...
    protected function isShopifySkuLinkedOnTemu(?Temu2Metric $metric, string $shopifySku): bool
    {
