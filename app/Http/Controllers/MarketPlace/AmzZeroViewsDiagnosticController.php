...
class AmzZeroViewsDiagnosticController extends Controller
{
    ...
    public function startRun(Request $request, ?string $asin = null): JsonResponse
    {
        ...
        $payload = [
            ...
            'filters' => $filters,
            'zero_only' => $this->truthy($validated['zero_only'] ?? ($filters['zero_only'] ?? true)),
            'all' => $mode === 'all' && ! $this->truthy($filters['zero_only'] ?? true),
        ];
    ...
    private function filtersFromRequest(Request $request): array
    {
        return [
            ...
            'zero_only' => $request->has('zero_only') ? $request->input('zero_only') : 1,
