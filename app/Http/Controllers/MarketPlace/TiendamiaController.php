...
class TiendamiaController extends Controller
{
    ...
    public function __construct(ApiController $apiController)
    {
     ...
     public function overallTiendamia(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');

        $marketplaceData = ChannelMaster::where('channel', 'Tiendamia')->first();

        $percentage = $marketplaceData ? $marketplaceData->channel_percentage : 100;
        $adUpdates = $marketplaceData ? $marketplaceData->ad_updates : 0;

        return view('market-places.tiendamiaAnalysis', [
            'mode' => $mode,
            'demo' => $demo,
    ...
    public function getViewTiendamiaData(Request $request)
    {
        // Get percentage from cache or database
        $percentage = Cache::remember('Tiendamia', now()->addDays(30), function () {
    ...
    public function updateAllTiendamiaSkus(Request $request)
    {
        try {
            ...
            // Store in cache
            Cache::put('Tiendamia', $percent, now()->addDays(30));
    ...
    public function updateListedLive(Request $request)
    {
        ...
        $product = TiendamiaDataView::firstOrCreate(
            ['sku' => $request->sku],
            ['value' => []]
        );
    ...
    public function importTiendamiaAnalytics(Request $request)
    {
        $request->validate([
    ...
    public function exportTiendamiaAnalytics()
    {
        $tiendamiaData = TiendamiaDataView::all();
        ...
        // Output Download
        $fileName = 'Tiendamia_Analytics_Export_' . date('Y-m-d') . '.xlsx';
    ...
    public function downloadSample()
    {
        ...
        // Output Download
        $fileName = 'Tiendamia_Analytics_Sample.xlsx';
