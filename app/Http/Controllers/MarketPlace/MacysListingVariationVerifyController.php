...
class MacysListingVariationVerifyController extends Controller
{
    public function index()
    {
        return view('market-places.macys_listing_variation_verify');
    }

    /**
     * Parent-only rows: Parent, Required, Parent Vs Listed SKU.
     * Listed source: macy_products + macys_price_data (same as /macys-pricing).
     * Missing = CP Master child not listed on Macy's.
     * Extra   = listed Macy's SKU in this parent family that is not a CP Master child.
     */
    public function data(Request $request)
    {
        ...
        $productsCount = (int) MacyProduct::query()->whereNotNull('sku')->where('sku', '!=', '')->count();
        $priceDataCount = (int) MacysPriceData::query()->whereNotNull('sku')->where('sku', '!=', '')->count();
        $lastPulledAt = collect([
            MacyProduct::query()->max('updated_at'),
    ...
    /**
     * Macy's listings come from /macys-pricing (macy_products + macys_price_data upload).
    ...
    public function pullListings(Request $request)
    {
        try {
            ...
            if ($count === 0) {
                return response()->json([
                    'status' => 422,
                    'message' => 'No Macy\'s listings in macy_products / macys_price_data. Update data on Macys - Analytics (/macys-pricing) first.',
                    'count' => 0,
                    'last_pulled_at' => $lastPulledAt,
                ], 422);
            }
            ...
            return response()->json([
                'status' => 200,
                'message' => "Macy's listings ready. {$productsCount} in macy_products, {$priceDataCount} in macys_price_data. Parent Vs Listed SKU updated.",
                'count' => $count,
                'last_pulled_at' => $lastPulledAt,
            ]);
        } catch (\Throwable $e) {
            Log::error('Macys Listing Variation Verify: pull listings failed', [
                'error' => $e->getMessage(),
            ]);
    ...
    private function buildListedSkuLookup(): array
    {
        ...
        foreach (MacyProduct::query()->whereNotNull('sku')->where('sku', '!=', '')->pluck('sku') as $sku) {
            $addSku((string) $sku);
        }

        foreach (MacysPriceData::query()->whereNotNull('sku')->where('sku', '!=', '')->pluck('sku') as $sku) {
            $addSku((string) $sku);
        }
