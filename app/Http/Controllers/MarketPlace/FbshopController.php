...
class FbshopController extends Controller
{
    ...
    public function __construct(ApiController $apiController)
    {
     ...
     public function overallFbshop(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');

        $marketplaceData = ChannelMaster::where('channel', 'FB Shop')->first();

        $percentage = $marketplaceData ? $marketplaceData->channel_percentage : 100;
        $adUpdates = $marketplaceData ? $marketplaceData->ad_updates : 0;

        return view('market-places.fbshopAnalysis', [
            'mode' => $mode,
            'demo' => $demo,
    ...
    public function getViewFbshopData(Request $request)
    {
        // Get percentage from cache or database
        $percentage = Cache::remember('FBShop', now()->addDays(30), function () {
        ...
        $walmartDataViews = FBShopDataView::whereIn('sku', $skus)->get()->keyBy('sku');
        $nrValues = [];
        $listedValues = [];
        $liveValues = [];
    ...
    public function updateAllFbshopSkus(Request $request)
    {
        try {
            ...
            // Store in cache
            Cache::put('FBShop', $percent, now()->addDays(30));
    ...
    public function updateListedLive(Request $request)
    {
        ...
        $product = FBShopDataView::firstOrCreate(
            ['sku' => $request->sku],
            ['value' => []]
        );
    ...
    public function importFbshopAnalytics(Request $request)
    {
        $request->validate([
    ...
    public function exportFbshopAnalytics()
    {
        $fbshopData = FBShopDataView::all();
        ...
        // Output Download
        $fileName = 'FBShop_Analytics_Export_' . date('Y-m-d') . '.xlsx';
    ...
    public function downloadSample()
    {
        ...
        // Output Download
        $fileName = 'FBShop_Analytics_Sample.xlsx';
