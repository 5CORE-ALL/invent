...
use App\Jobs\WarmMacyLiveListingsCache;
use App\Models\MacyProduct;
use App\Models\MacyOrderMetric;
use App\Models\MarketplaceSyncSettings;
use App\Models\ShopifySku;
use App\Services\MacysApiService;
use App\Services\MarketplaceManager\MacyDetailFormatter;
use App\Services\MarketplaceManager\MacyInventorySyncService;
use App\Services\MarketplaceManager\MacyLinkMapSyncService;
use App\Services\MarketplaceManager\MacyLiveListingsService;
use App\Services\MarketplaceManager\MacyOrderDetailService;
...
class MacySyncController extends Controller
{
    ...
    {
        $mcmKey = (string) config('services.macy.mcm_api_key', '');
        $macyClientId = (string) config('services.macy.client_id', '');
        $macySecret = (string) config('services.macy.client_secret', '');
        $credentialsReady = $this->macyApi->isConfigured();
        $mcmBaseUrl = (string) config('services.macy.mcm_base_url', 'https://macysus-prod.mirakl.net');

        return view('marketplace.macy.connect', [
            'title' => "Macy's — Connect",
            'connected' => $this->apiConfig->isConfigured('macy'),
    ...
    public function testConnection(): JsonResponse
    {
        ...
        try {
            $result = $this->macyApi->testConnection();
        } catch (\Throwable $e) {
            return response()->json([
                'success' => false,
    ...
    public function syncProducts(Request $request): View
    {
        ...
        if (! Schema::hasTable('shopify_skus')) {
            ...
            return view('marketplace.macy.products', [
                'products' => $products,
                'title' => 'Macy\'s — Listings',
                'searchSku' => $searchSku,
        ...
        if ($forceLive) {
            WarmMacyLiveListingsCache::dispatch();
        }
        ...
        if ($mismatchQty !== []) {
            ...
            $metricMap = $this->macyMetricMapForSkus($mismatchQty);
            $productIds = [];
            $idToSku = [];
            foreach ($mismatchQty as $sku) {
                $metric = $metricMap[$sku] ?? null;
                if (! $this->isShopifySkuLinkedOnMacy($metric, (string) $sku)) {
                    continue;
                }
        ...
        $aeMap = $this->macyMetricMapForSkus($skus);
        $aeStockMap = $this->macyStockMapForSkus($skus);
        ...
        if (in_array($linkTab, $liveLinkTabs, true)) {
            ...
            foreach ($skus as $sku) {
                ...
                if (! $metric || ! $this->isShopifySkuLinkedOnMacy($metric, (string) $sku)) {
                    continue;
                }
        ...
        $enriched = collect($pageRows)->map(function (ShopifySku $row) use ($aeMap, $aeStockMap, $liveShopifyQty, $stateIndex, $pageLiveByProduct) {
            $sku = (string) $row->sku;
            $metric = $aeMap[$sku] ?? null;
            $linked = $this->isShopifySkuLinkedOnMacy($metric, $sku);
            ...
            return (object) [
                ...
                'macy_title' => $live['title'] ?? ($cached['title'] ?? ($metric->sku ?? null)),
    ...
    public function showProduct(int $shopifySkuId): View
    {
        ...
        $aeMap = $this->macyMetricMapForSkus([$sku]);
        $metric = $aeMap[$sku] ?? null;
        $linked = $this->isShopifySkuLinkedOnMacy($metric, $sku);
        ...
        $detail = app(MacyDetailFormatter::class)->formatProduct(
        ...
        return view('marketplace.macy.product-show', [
            'title' => 'Macy\'s Listing — '.$sku,
    ...
    /**
     * Push live Shopify qty → Macy\'s for this one SKU immediately (no queue).
     */
    public function pushProductInventory(int $shopifySkuId): JsonResponse
    {
        ...
        if (! ($settings['inventory']['inventory_sync'] ?? false) && ! ($settings['pricing']['price_sync'] ?? false)) {
            return response()->json([
                'success' => false,
                'message' => 'Turn on Inventory sync (or Price sync) in Macy\'s settings first.',
            ], 422);
        }
        ...
        $metric = $this->macyMetricMapForSkus([$sku])[$sku] ?? null;
        if (! $this->isShopifySkuLinkedOnMacy($metric, $sku)) {
            return response()->json([
                'success' => false,
                'message' => 'This SKU is not linked on Macy\'s. Run Sync Macy\'s link map first.',
            ], 422);
        }

        $result = app(MacyInventorySyncService::class)->syncSkusFromShopify([$sku]);
    ...
    public function pullProductFromMacy(int $shopifySkuId): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('macy')) {
            return response()->json(['success' => false, 'message' => 'Macy\'s not connected.']);
        }
        ...
        if (! $metric?->sku || (string) $metric->sku === (string) $metric->sku ?? $sku) {
            return response()->json([
                'success' => false,
                'message' => 'No Macy\'s sku mapped for this SKU. Run Sync Macy\'s link map on Listings first.',
            ]);
        }

        try {
            $this->macyApi->getInventory();
        } catch (\Throwable $e) {
            return response()->json([
                'success' => false,
                'message' => 'Failed to refresh Macy\'s inventory: '.$e->getMessage(),
            ]);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Refreshed Macy\'s metrics for '.$sku.' from API. Nothing was pushed to Shopify or Macy\'s.',
        ]);
    }
    ...
    public function refreshProducts(Request $request): JsonResponse
    {
        ...
        Log::info('Macy\'s link map sync page', ['page' => $page, 'reset' => $reset]);

        $result = app(MacyLinkMapSyncService::class)->syncPage($page, 50, $reset);
    ...
    {
        $progress = app(MacyLinkMapSyncService::class)->getProgress();
    ...
    public function syncOrders(Request $request): View
    {
        ...
        return view('marketplace.macy.orders', [
            'orders' => $orders,
            'title' => 'Macy\'s — Orders',
            'apiError' => $apiError,
            'connected' => $this->apiConfig->isConfigured('macy'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('macy'),
        ]);
    }

    public function showOrder(int $id): View
    {
        $line = MacyOrderMetric::query()->findOrFail($id);
        $orderId = $line->displayOrderId();

        $lines = MacyOrderMetric::query()
            ->where(function ($q) use ($orderId) {
                $q->where('order_id', $orderId)->orWhere('channel_order_id', $orderId);
            })
            ->orderBy('id')
            ->get();
        ...
        $aeLiveError = null;
        $aeDataSource = 'cached';
        $detailService = app(MacyOrderDetailService::class);
        ...
        if ($this->apiConfig->isConfigured('macy')) {
            ...
            if (! empty($pull['success'])) {
                ...
                $aeLiveError = $pull['message'] ?? 'Could not refresh live Macy\'s order details.';
            }
        }

        $orderRoot = $detailService->resolveOrderRoot($line);
        $detail = app(MacyDetailFormatter::class)->formatOrder($orderRoot, $lines, $line);

        return view('marketplace.macy.order-show', [
            'title' => 'Macy\'s Order — '.$orderId,
            'orderId' => $orderId,
            'line' => $line,
            'detail' => $detail,
            'aeLiveError' => $aeLiveError,
            ...
            'connected' => $this->apiConfig->isConfigured('macy'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('macy'),
            'orderIsPaid' => MarketplaceOrderPaidFilter::isPaid('macy', $line),
        ]);
    }

    public function pullOrderFromMacy(int $id): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('macy')) {
            return response()->json(['success' => false, 'message' => 'Macy\'s not connected.']);
        }
        ...
        $line = MacyOrderMetric::query()->findOrFail($id);
        $result = app(MacyOrderDetailService::class)->fetchAndPersistOrderDetail($line->displayOrderId());

        // Persist only — Shopify address fill matches AliExpress / Reverb (SyncMacyAddressJob).
        return response()->json([
            'success' => ! empty($result['success']),
            'message' => $result['message'] ?? ($result['success'] ? 'Order details updated from Macy\'s.' : 'Failed to pull order details.'),
        ]);
    }

    /**
     * Push Shopify fulfillment tracking number to Macy\'s (Ship Order).
     */
    public function pushTrackingToMacy(int $id): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('macy')) {
            return response()->json(['success' => false, 'message' => 'Macy\'s not connected.']);
        }
        ...
        $line = MacyOrderMetric::query()->findOrFail($id);
        $result = app(MacyTrackingSyncService::class)->pushTrackingForOrder($line);
    ...
    /**
     * Bulk push Shopify tracking → Macy\'s for linked orders.
     */
    public function syncTrackingNow(): JsonResponse
    {
        ...
        \App\Jobs\SyncMacyTrackingJob::dispatch(false);

        return response()->json([
            'success' => true,
            'queued' => true,
            'message' => 'Tracking sync queued. It reads Shopify fulfillments and ships orders on Macy\'s.',
        ]);
    }
    ...
    public function fetchOrders(Request $request): JsonResponse
    {
        ...
        $sync = app(MacyOrderSyncService::class);
        ...
        if ($request->boolean('import') || MarketplaceSyncSettings::canAutoImportToShopify('macy')) {
    ...
    public function syncInventoryNow(): JsonResponse
    {
        ...
        \App\Jobs\RunMarketplaceInventorySyncJob::dispatch('macy');
    ...
    public function pushOrderToShopify(Request $request): JsonResponse
    {
        ...
        if ($request->boolean('dry_run')) {
            $preview = app(MacyOrderPushService::class)->previewShopifyPush($order);
        ...
        if (MarketplaceOrderPaidFilter::blocksUnpaidPush('macy', $order)) {
            return response()->json([
                'success' => false,
        ...
        $push = app(MacyOrderPushService::class);
        try {
    ...
    /**
     * Delete a local Macy\'s order that is still ready for Shopify import
    ...
    public function deleteReadyOrder(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => "Removed Macy\'s order {$orderId} from ready-for-import ({$deleted} row(s)).",
            'deleted' => $deleted,
            'order_id' => $orderId,
        ]);
    }
    ...
    public function markOrderAlreadyImported(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => "Marked Macy\'s order {$orderId} as already imported ({$updated} row(s)).",
            'shopify_order_id' => $shopifyOrderId,
            'updated' => $updated,
        ]);
    }
    ...
    {
        return view('marketplace.macy.settings', [
            'settings' => MarketplaceSyncSettings::getFor('macy'),
            'title' => 'Macy\'s — Sync Settings',
            'connected' => $this->apiConfig->isConfigured('macy'),
        ]);
    }
    ...
    {
        $current = MarketplaceSyncSettings::getFor('macy');
        ...
        $order = $this->mergeSettingsSection($current['order'] ?? [], $request->input('order', []), [
            ...
            'push_tracking_to_macy', 'sync_address_to_shopify',
        ]);
        $listings = $this->mergeSettingsSection($current['listings'] ?? [], $request->input('listings', []), [
            'auto_link_by_sku', 'create_products_on_macy', 'sync_title', 'sync_images',
        ]);
        ...
        MarketplaceSyncSettings::setFor('macy', [
            'pricing' => $pricing,
            'inventory' => $inventory,
            'order' => $order,
            'listings' => $listings,
        ...
        return response()->json(['success' => true, 'message' => 'Macy\'s sync settings saved.']);
    }
    ...
    protected function aeStateIndexFromCache(
        MacyLiveListingsService $liveService,
    ...
    /**
     * SKUs in macy_products that map to a real Shopify SKU (not product_id placeholders).
     *
     * @return array<int, string>
     */
    protected function linkedMacySkus(): array
    {
        if (! Schema::hasTable('macy_products')) {
            return [];
        }

        return MacyProduct::query()
            ->whereNotNull('sku')
            ->where('sku', '!=', '')
            ->pluck('sku')
            ->unique()
    ...
    protected function paginateAeNotInShopify(
        ...
        $rows = $this->aeMetricsWithRealSkuQuery()->orderBy('sku')->get()->filter(function (MacyProduct $metric) use ($shopifyNormKeys, $searchSku, $searchName) {
            $norm = ShopifySku::normalizeSkuForShopifyLookup((string) $metric->sku ?? $sku);
            if ($norm === '' || isset($shopifyNormKeys[$norm])) {
                return false;
            }
        ...
        $aeStockMap = $this->macyStockMapForSkus($sliceSkus);
        $slice = $rows->slice(($page - 1) * $perPage, $perPage)->map(function (MacyProduct $metric) use ($aeStockMap) {
     ...
     */
    protected function macyMetricMapForSkus(array $skus): array
    {
        if ($skus === [] || ! Schema::hasTable('macy_products')) {
            return [];
        }

        $exact = MacyProduct::query()->whereIn('sku', $skus)->get()->keyBy('sku');
        $byNorm = [];
        foreach (MacyProduct::query()
            ->whereNotNull('sku')
            ->where('sku', '!=', '')
            ->get() as $metric) {
            $norm = ShopifySku::normalizeSkuForShopifyLookup((string) $metric->sku ?? $sku);
     ...
     */
    protected function macyStockMapForSkus(array $skus): array
    {
        return MarketplaceListingStockResolver::stockMapForSkus(
            MarketplaceListingStockResolver::CHANNEL_MACY,
            $skus
        );
    }

    protected function isShopifySkuLinkedOnMacy(?MacyProduct $metric, string $shopifySku): bool
    {
