...
use App\Jobs\WarmEbay2LiveListingsCache;
...
use App\Services\MarketplaceManager\Ebay2OrderDetailService;
...
class Ebay2SyncController extends Controller
{
    ...
    {
        $appId = (string) config('services.ebay2.app_id', env('EBAY_2_APP_ID'));
        $certId = (string) config('services.ebay2.cert_id', env('EBAY_2_CERT_ID'));
        $devId = (string) config('services.ebay2.dev_id', env('EBAY_2_DEV_ID'));
        $refresh = (string) config('services.ebay2.refresh_token', env('EBAY_2_REFRESH_TOKEN'));
        $credentialsReady = $this->ebay2Api->isConfigured();

        return view('marketplace.ebay2.connect', [
            'title' => 'eBay 2 — Connect',
            'connected' => $this->apiConfig->isConfigured('ebay2'),
    ...
    {
        if (! $this->apiConfig->isConfigured('ebay2')) {
            return response()->json([
                'success' => false,
                'message' => 'eBay 2 API credentials missing. Set EBAY_2_APP_ID, EBAY_2_CERT_ID, EBAY_2_DEV_ID, EBAY_2_REFRESH_TOKEN in .env.',
            ]);
        }

        try {
            $result = $this->ebay2Api->testConnection();
        } catch (\Throwable $e) {
            return response()->json([
                'success' => false,
    ...
    public function syncProducts(Request $request): View
    {
        ...
        if (! Schema::hasTable('shopify_skus')) {
            ...
            return view('marketplace.ebay2.products', [
                'products' => $products,
                'title' => 'eBay 2 — Listings',
                'searchSku' => $searchSku,
        ...
        $aeMap = $this->ebay2MetricMapForSkus($skus);
        $aeStockMap = $this->ebay2StockMapForSkus($skus);
        ...
        if (in_array($linkTab, $liveLinkTabs, true)) {
            ...
            foreach ($skus as $sku) {
                ...
                if (! $metric || ! $this->isShopifySkuLinkedOnEbay2($metric, (string) $sku)) {
                    continue;
                }
        ...
        $enriched = collect($pageRows)->map(function (ShopifySku $row) use ($aeMap, $aeStockMap, $liveShopifyQty, $stateIndex, $pageLiveByProduct) {
            ...
            return (object) [
                ...
                'ebay2_title' => $live['title'] ?? ($cached['title'] ?? ($metric->product_name ?? null)),
    ...
    public function showProduct(int $shopifySkuId): View
    {
        ...
        $aeMap = $this->ebay2MetricMapForSkus([$sku]);
        $metric = $aeMap[$sku] ?? null;
        $linked = $this->isShopifySkuLinkedOnEbay2($metric, $sku);
        ...
        $canFetchAe = $productId && $productId !== (string) $metric?->sku && $this->apiConfig->isConfigured('ebay2');

        if ($canFetchAe) {
            $info = $this->ebay2Api->getItem($productId);
            if (! empty($info['success'])) {
                $aeLive = $info['data'] ?? null;
                $aeDataSource = 'api';
            } else {
                $aeLiveError = $info['message'] ?? 'Could not load live eBay 2 product details.';
                $aeDataSource = 'cached';
            }
        ...
        $aeSkuRows = [];
        if (is_array($aeLive)) {
            $aeSkuRows = $this->ebay2Api->extractSkuRowsFromProductInfo(
                $aeLive,
        ...
        $detail = app(Ebay2DetailFormatter::class)->formatProduct(
        ...
        return view('marketplace.ebay2.product-show', [
            'title' => 'eBay 2 Listing — '.$sku,
    ...
    /**
     * Push live Shopify qty → eBay 2 for this one SKU immediately (no queue).
     */
    public function pushProductInventory(int $shopifySkuId): JsonResponse
    {
        ...
        if (! ($settings['inventory']['inventory_sync'] ?? false) && ! ($settings['pricing']['price_sync'] ?? false)) {
            return response()->json([
                'success' => false,
                'message' => 'Turn on Inventory sync (or Price sync) in eBay 2 settings first.',
            ], 422);
        }
        ...
        $metric = $this->ebay2MetricMapForSkus([$sku])[$sku] ?? null;
        if (! $this->isShopifySkuLinkedOnEbay2($metric, $sku)) {
            return response()->json([
                'success' => false,
                'message' => 'This SKU is not linked on eBay 2. Run Sync eBay 2 link map first.',
            ], 422);
        }

        $result = app(Ebay2InventorySyncService::class)->syncSkusFromShopify([$sku]);
    ...
    public function pullProductFromEbay2(int $shopifySkuId): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('ebay2')) {
            return response()->json(['success' => false, 'message' => 'eBay 2 not connected.']);
        }
        ...
        $metric = Ebay2Metric::query()->where('sku', $sku)->orWhere('sku', strtoupper($sku))->first();
        if (! $metric?->item_id || (string) $metric->item_id === (string) $metric->sku) {
            return response()->json([
                'success' => false,
                'message' => 'No eBay 2 item_id mapped for this SKU. Run Sync eBay 2 link map on Listings first.',
            ]);
        }

        try {
            $info = $this->ebay2Api->getItem((string) $metric->item_id);
        } catch (\Throwable $e) {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Pulled live eBay 2 item '.$metric->item_id.' for SKU '.$sku.'.',
    ...
    public function refreshProducts(Request $request): JsonResponse
    {
        ...
        Log::info('eBay 2 link map sync page', ['page' => $page, 'reset' => $reset]);

        $result = app(Ebay2LinkMapSyncService::class)->syncPage($page, 50, $reset);
    ...
    {
        $progress = app(Ebay2LinkMapSyncService::class)->getProgress();
    ...
    public function syncOrders(Request $request): View
    {
        ...
        if (Schema::hasTable('ebay2_order_metrics')) {
            ...
            $statusCounts = Ebay2OrderMetric::query()
        ...
        return view('marketplace.ebay2.orders', [
            'orders' => $orders,
            'statusCounts' => $statusCounts,
            'title' => 'eBay 2 — Orders',
            'apiError' => $apiError,
            'connected' => $this->apiConfig->isConfigured('ebay2'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('ebay2'),
        ]);
    }

    public function showOrder(int $id): View
    {
        $line = Ebay2OrderMetric::query()->findOrFail($id);
        ...
        $aeLiveError = null;
        $aeDataSource = 'cached';
        $detailService = app(Ebay2OrderDetailService::class);
        ...
        if ($this->apiConfig->isConfigured('ebay2')) {
            ...
            if (! empty($pull['success'])) {
                ...
                $aeLiveError = $pull['message'] ?? 'Could not refresh live eBay 2 order details.';
            }
        }

        $orderRoot = $detailService->resolveOrderRoot($line);
        $detail = app(Ebay2DetailFormatter::class)->formatOrder($orderRoot, $lines, $line);

        return view('marketplace.ebay2.order-show', [
            'title' => 'eBay 2 Order — '.$orderId,
            'orderId' => $orderId,
            'line' => $line,
            'detail' => $detail,
            'aeLiveError' => $aeLiveError,
            ...
            'connected' => $this->apiConfig->isConfigured('ebay2'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('ebay2'),
            'orderIsPaid' => MarketplaceOrderPaidFilter::isPaid('ebay2', $line),
        ]);
    }

    public function pullOrderFromEbay2(int $id): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('ebay2')) {
            return response()->json(['success' => false, 'message' => 'eBay 2 not connected.']);
        }
        ...
        $line = Ebay2OrderMetric::query()->findOrFail($id);
        $result = app(Ebay2OrderDetailService::class)->fetchAndPersistOrderDetail((string) $line->order_id);
        ...
        $line->refresh();
        $message = $result['message'] ?? 'Order details updated from eBay 2.';
        $shopifySynced = null;

        // Already-imported orders were often created without ShipTo; push address on pull.
        if (! empty($line->shopify_order_id)) {
            $sync = app(Ebay2OrderPushService::class)->syncShippingAddressToShopify($line);
            $shopifySynced = ! empty($sync['success']);

            if ($shopifySynced) {
                $message = 'Pulled from eBay 2 and updated shipping address on Shopify.';
            } elseif (! empty($sync['skipped'])) {
                $message = 'Pulled from eBay 2. '.($sync['message'] ?? 'Shopify address not updated.');
            } else {
                return response()->json([
                    'success' => false,
                    'pulled' => true,
                    'shopify_synced' => false,
                    'message' => 'Pulled from eBay 2, but Shopify address update failed: '.($sync['message'] ?? 'unknown error'),
                ], 422);
            }
        }
    ...
    /**
     * Push Shopify fulfillment tracking number to eBay 2 (Ship Order).
     */
    public function pushTrackingToEbay2(int $id): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('ebay2')) {
            return response()->json(['success' => false, 'message' => 'eBay 2 not connected.']);
        }
        ...
        $line = Ebay2OrderMetric::query()->findOrFail($id);
        $result = app(Ebay2TrackingSyncService::class)->pushTrackingForOrder($line);
    ...
    /**
     * Bulk push Shopify tracking → eBay 2 for linked orders.
     */
    public function syncTrackingNow(): JsonResponse
    {
        ...
        \App\Jobs\SyncEbay2TrackingJob::dispatch(false);

        return response()->json([
            'success' => true,
            'queued' => true,
            'message' => 'Tracking sync queued. It reads Shopify fulfillments and ships orders on eBay 2.',
        ]);
    }
    ...
    public function fetchOrders(Request $request): JsonResponse
    {
        ...
        $sync = app(Ebay2OrderSyncService::class);
        ...
        if ($request->boolean('import') || MarketplaceSyncSettings::canAutoImportToShopify('ebay2')) {
    ...
    public function syncMismatchInventoryNow(Request $request): JsonResponse
    {
        ...
        try {
            ...
            Log::error('Ebay2 syncMismatchInventoryNow failed', [
                'error' => $e->getMessage(),
     ...
     */
    protected function resolveEbay2MismatchSkuList(string $scope): array
    {
        $catalog = app(ShopifyLiveVerifiedCatalogService::class);
        $liveService = app(Ebay2LiveListingsService::class);
        $linkedSkus = $this->linkedEbay2Skus();
    ...
    public function pushOrderToShopify(Request $request): JsonResponse
    {
        ...
        $order = Ebay2OrderMetric::find($id);
        ...
        if ($request->boolean('dry_run')) {
            $preview = app(Ebay2OrderPushService::class)->previewShopifyPush($order);
        ...
        if (MarketplaceOrderPaidFilter::blocksUnpaidPush('ebay2', $order)) {
            return response()->json([
                'success' => false,
        ...
        $push = app(Ebay2OrderPushService::class);
        try {
    ...
    /**
     * Delete a local eBay 2 order that is still ready for Shopify import
    ...
    public function deleteReadyOrder(Request $request): JsonResponse
    {
        ...
        $deleted = Ebay2OrderMetric::query()
            ->where('order_id', $orderId)
        ...
        return response()->json([
            'success' => true,
            'message' => "Removed eBay 2 order {$orderId} from ready-for-import ({$deleted} row(s)).",
            'deleted' => $deleted,
            'order_id' => $orderId,
        ]);
    }
    ...
    public function markOrderAlreadyImported(Request $request): JsonResponse
    {
        ...
        $updated = Ebay2OrderMetric::query()
            ->where('order_id', $orderId)
            ->whereNull('shopify_order_id')
            ->update([
                'shopify_order_id' => $shopifyOrderId,
        ...
        return response()->json([
            'success' => true,
            'message' => "Marked eBay 2 order {$orderId} as already imported ({$updated} row(s)).",
            'shopify_order_id' => $shopifyOrderId,
            'updated' => $updated,
        ]);
    }
    ...
    {
        return view('marketplace.ebay2.settings', [
            'settings' => MarketplaceSyncSettings::getFor('ebay2'),
            'title' => 'eBay 2 — Sync Settings',
            'connected' => $this->apiConfig->isConfigured('ebay2'),
        ]);
    }
    ...
    public function saveSettings(Request $request): JsonResponse
    {
        ...
        $order = $this->mergeSettingsSection($current['order'] ?? [], $request->input('order', []), [
            ...
            'push_tracking_to_ebay2', 'sync_address_to_shopify',
        ]);
        $listings = $this->mergeSettingsSection($current['listings'] ?? [], $request->input('listings', []), [
            'auto_link_by_sku', 'create_products_on_ebay2', 'sync_title', 'sync_images',
        ]);
        ...
        MarketplaceSyncSettings::setFor('ebay2', [
            'pricing' => $pricing,
            'inventory' => $inventory,
            'order' => $order,
            'listings' => $listings,
    ...
    protected function aeStateIndexFromCache(
        Ebay2LiveListingsService $liveService,
    ...
    /**
     * SKUs in ebay_2_metrics that map to a real Shopify SKU (not product_id placeholders).
     *
     * @return array<int, string>
     */
    protected function linkedEbay2Skus(): array
    {
        if (! Schema::hasTable('ebay_2_metrics')) {
            return [];
        }

        return Ebay2Metric::query()
            ->whereNotNull('sku')
    ...
    protected function paginateAeNotInShopify(
        ...
        $rows = $this->aeMetricsWithRealSkuQuery()->orderBy('sku')->get()->filter(function (Ebay2Metric $metric) use ($shopifyNormKeys, $searchSku, $searchName) {
            $norm = ShopifySku::normalizeSkuForShopifyLookup((string) $metric->sku);
            if ($norm === '' || isset($shopifyNormKeys[$norm])) {
                return false;
            }
        ...
        $aeStockMap = $this->ebay2StockMapForSkus($sliceSkus);
        $slice = $rows->slice(($page - 1) * $perPage, $perPage)->map(function (Ebay2Metric $metric) use ($aeStockMap) {
     ...
     * @return array<string, Ebay2Metric>
     */
    protected function ebay2MetricMapForSkus(array $skus): array
    {
        if ($skus === [] || ! Schema::hasTable('ebay_2_metrics')) {
            return [];
        }

        $exact = Ebay2Metric::query()->whereIn('sku', $skus)->get()->keyBy('sku');
        $byNorm = [];
        foreach (Ebay2Metric::query()
            ->whereNotNull('sku')
            ->whereNotNull('item_id')
            ->where('sku', '!=', '')
            ->where('item_id', '!=', '')
     ...
     */
    protected function ebay2StockMapForSkus(array $skus): array
    {
