...
class Ebay3ListingVariationVerifyController extends Controller
{
    public function index()
    {
        return view('market-places.ebay3_listing_variation_verify');
    }
    ...
    public function data(Request $request)
    {
        ...
        return response()->json([
            'data' => $formattedData,
            'meta' => [
                'listings_count' => (int) Ebay3Metric::query()->whereNotNull('sku')->where('sku', '!=', '')->count(),
                'last_pulled_at' => Ebay3Metric::query()->max('updated_at'),
                'has_listings_cache' => Ebay3Metric::query()->whereNotNull('sku')->where('sku', '!=', '')->exists(),
    ...
    public function pullListings(Request $request)
    {
        try {
            ...
            $exitCode = Artisan::call('app:fetch-ebay-three-metrics');
            ...
            if ($exitCode !== 0) {
                return response()->json([
                    ...
                    'message' => $output !== ''
                        ? $output
                        : 'Failed to pull eBay 3 listings (exit code ' . $exitCode . ').',
                ], 422);
            }

            $count = (int) Ebay3Metric::query()->whereNotNull('sku')->where('sku', '!=', '')->count();

            return response()->json([
                'status' => 200,
                'message' => "Pulled eBay 3 listings. {$count} SKUs in ebay_3_metrics. Parent Vs Listed SKU updated.",
                'count' => $count,
                'last_pulled_at' => Ebay3Metric::query()->max('updated_at'),
            ]);
        } catch (\Throwable $e) {
            Log::error('Ebay 3 Listing Variation Verify: pull listings failed', [
                'error' => $e->getMessage(),
            ]);
    ...
    private function buildListedSkuLookup(): array
    {
        ...
        foreach (Ebay3Metric::query()->whereNotNull('sku')->where('sku', '!=', '')->pluck('sku') as $sku) {
