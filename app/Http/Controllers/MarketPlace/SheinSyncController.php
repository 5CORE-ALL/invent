...
use App\Services\MarketplaceManager\SheinDetailFormatter;
...
class SheinSyncController extends Controller
{
    ...
    {
        $openKeyId = (string) (config('services.shein.open_key_id') ?: config('services.shein.app_id'));
        $secretKey = (string) (config('services.shein.secret_key')
            ?: config('services.shein.app_secret')
            ?: config('services.shein.app_s'));
        $baseUrl = (string) config('services.shein.base_url', 'https://openapi.sheincorp.com');
        $credentialsReady = $this->sheinApi->isConfigured();

        return view('marketplace.shein.connect', [
            'title' => 'Shein — Connect',
            'connected' => $this->apiConfig->isConfigured('shein'),
            'credentialsReady' => $credentialsReady,
            'hasOpenKeyId' => filled($openKeyId),
            'hasSecretKey' => filled($secretKey),
            'maskedOpenKeyId' => $this->maskCredential($openKeyId),
    ...
    public function testConnection(): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('shein')) {
            return response()->json([
                'success' => false,
                'message' => 'Shein API credentials missing. Set SHEIN_OPEN_KEY_ID and SHEIN_SECRET_KEY in .env.',
            ]);
        }

        try {
            $result = $this->sheinApi->testConnection();
        } catch (\Throwable $e) {
            return response()->json([
                'success' => false,
    ...
    public function syncProducts(Request $request): View
    {
        ...
        if (! Schema::hasTable('shopify_skus')) {
            ...
            return view('marketplace.shein.products', [
                'products' => $products,
                'title' => 'Shein — Listings',
                'searchSku' => $searchSku,
        ...
        if ($forceLive) {
            WarmSheinLiveListingsCache::dispatch();
        }
        ...
        if ($mismatchQty !== []) {
            ...
            $metricMap = $this->sheinMetricMapForSkus($mismatchQty);
            $productIds = [];
            $idToSku = [];
            foreach ($mismatchQty as $sku) {
                $metric = $metricMap[$sku] ?? null;
                if (! $this->isShopifySkuLinkedOnShein($metric, (string) $sku)) {
                    continue;
                }
        ...
        $aeMap = $this->sheinMetricMapForSkus($skus);
        $aeStockMap = $this->sheinStockMapForSkus($skus);
        ...
        if (in_array($linkTab, $liveLinkTabs, true)) {
            ...
            foreach ($skus as $sku) {
                ...
                if (! $metric || ! $this->isShopifySkuLinkedOnShein($metric, (string) $sku)) {
                    continue;
                }
        ...
        $enriched = collect($pageRows)->map(function (ShopifySku $row) use ($aeMap, $aeStockMap, $liveShopifyQty, $stateIndex, $pageLiveByProduct) {
            $sku = (string) $row->sku;
            $metric = $aeMap[$sku] ?? null;
            $linked = $this->isShopifySkuLinkedOnShein($metric, $sku);
            ...
            return (object) [
                ...
                'shein_title' => $live['title'] ?? ($cached['title'] ?? ($metric->product_name ?? null)),
    ...
    public function showProduct(int $shopifySkuId): View
    {
        ...
        $aeMap = $this->sheinMetricMapForSkus([$sku]);
        $metric = $aeMap[$sku] ?? null;
        $linked = $this->isShopifySkuLinkedOnShein($metric, $sku);
        ...
        $canFetchAe = $productId && $productId !== (string) $metric?->sku && $this->apiConfig->isConfigured('shein');

        if ($canFetchAe) {
            $info = $this->sheinApi->getProductInfo($productId);
            if (! empty($info['success'])) {
                $aeLive = $info['data'] ?? null;
                $aeDataSource = 'api';
            } else {
                $aeLiveError = $info['message'] ?? 'Could not load live Shein product details.';
                $aeDataSource = 'cached';
            }
        ...
        $aeSkuRows = [];
        if (is_array($aeLive)) {
            $aeSkuRows = $this->sheinApi->extractSkuRowsFromProductInfo(
                $aeLive,
        ...
        $detail = app(SheinDetailFormatter::class)->formatProduct(
        ...
        return view('marketplace.shein.product-show', [
            'title' => 'Shein Listing — '.$sku,
    ...
    /**
     * Push live Shopify qty → Shein for this one SKU immediately (no queue).
     */
    public function pushProductInventory(int $shopifySkuId): JsonResponse
    {
        ...
        if (! ($settings['inventory']['inventory_sync'] ?? false) && ! ($settings['pricing']['price_sync'] ?? false)) {
            return response()->json([
                'success' => false,
                'message' => 'Turn on Inventory sync (or Price sync) in Shein settings first.',
            ], 422);
        }
        ...
        $metric = $this->sheinMetricMapForSkus([$sku])[$sku] ?? null;
        if (! $this->isShopifySkuLinkedOnShein($metric, $sku)) {
            return response()->json([
                'success' => false,
                'message' => 'This SKU is not linked on Shein. Run Sync Shein link map first.',
            ], 422);
        }

        $result = app(SheinInventorySyncService::class)->syncSkusFromShopify([$sku]);
    ...
    public function pullProductFromShein(int $shopifySkuId): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('shein')) {
        ...
        if (! $metric?->product_id || (string) $metric->product_id === (string) $metric->sku) {
            return response()->json([
                'success' => false,
                'message' => 'No Shein product_id mapped for this SKU. Run Sync Shein link map on Listings first.',
            ]);
        }

        $info = $this->sheinApi->getProductInfo((string) $metric->product_id);
        if (empty($info['success'])) {
            return response()->json([
                'success' => false,
                'message' => $info['message'] ?? 'Failed to pull product details from Shein.',
            ]);
        }

        $aeData = is_array($info['data'] ?? null) ? $info['data'] : [];
        $rows = $this->sheinApi->extractSkuRowsFromProductInfo(
            $aeData,
            (string) $metric->product_id,
            $metric->product_name
        );
        ...
        return response()->json([
            'success' => true,
            'message' => 'Pulled latest Shein details for '.$sku.'. Nothing was pushed to Shopify or Shein.',
        ]);
    }
    ...
    public function refreshProducts(Request $request): JsonResponse
    {
        ...
        Log::info('Shein link map sync page', ['page' => $page, 'reset' => $reset]);

        $result = app(SheinLinkMapSyncService::class)->syncPage($page, 50, $reset);
    ...
    {
        $progress = app(SheinLinkMapSyncService::class)->getProgress();
    ...
    public function syncOrders(Request $request): View
    {
        ...
        return view('marketplace.shein.orders', [
            'orders' => $orders,
            'title' => 'Shein — Orders',
            'apiError' => $apiError,
            'connected' => $this->apiConfig->isConfigured('shein'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('shein'),
        ]);
    }
    ...
    public function showOrder(int $id): View
    {
        ...
        $aeLiveError = null;
        $aeDataSource = 'cached';
        $detailService = app(SheinOrderDetailService::class);
        ...
        if ($this->apiConfig->isConfigured('shein')) {
            ...
            if (! empty($pull['success'])) {
                ...
                $aeLiveError = $pull['message'] ?? 'Could not refresh live Shein order details.';
            }
        }

        $orderRoot = $detailService->resolveOrderRoot($line);
        $detail = app(SheinDetailFormatter::class)->formatOrder($orderRoot, $lines, $line);

        return view('marketplace.shein.order-show', [
            'title' => 'Shein Order — '.$orderId,
            'orderId' => $orderId,
            'line' => $line,
            'detail' => $detail,
            'aeLiveError' => $aeLiveError,
            ...
            'connected' => $this->apiConfig->isConfigured('shein'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('shein'),
            'orderIsPaid' => MarketplaceOrderPaidFilter::isPaid('shein', $line),
        ]);
    }

    public function pullOrderFromShein(int $id): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('shein')) {
            return response()->json(['success' => false, 'message' => 'Shein not connected.']);
        }
    ...
    public function pushTrackingToShein(int $id): JsonResponse
    {
        ...
        $line = SheinOrderMetric::query()->findOrFail($id);
        $result = app(SheinTrackingSyncService::class)->pushTrackingForOrder($line);
    ...
    /**
     * Bulk push Shopify tracking → Shein for linked orders.
     */
    public function syncTrackingNow(): JsonResponse
    {
        ...
        \App\Jobs\SyncSheinTrackingJob::dispatch(false);

        return response()->json([
            'success' => true,
            'queued' => true,
            'message' => 'Tracking sync queued. It reads Shopify fulfillments and ships orders on Shein.',
        ]);
    }
    ...
    public function fetchOrders(Request $request): JsonResponse
    {
        ...
        $sync = app(SheinOrderSyncService::class);
        ...
        if ($request->boolean('import') || MarketplaceSyncSettings::canAutoImportToShopify('shein')) {
    ...
    public function syncInventoryNow(): JsonResponse
    {
        ...
        \App\Jobs\RunMarketplaceInventorySyncJob::dispatch('shein');
    ...
    public function pushOrderToShopify(Request $request): JsonResponse
    {
        ...
        $order = SheinOrderMetric::find($id);
        ...
        if ($request->boolean('dry_run')) {
            $preview = app(SheinOrderPushService::class)->previewShopifyPush($order);
        ...
        if (MarketplaceOrderPaidFilter::blocksUnpaidPush('shein', $order)) {
            return response()->json([
                'success' => false,
        ...
        $push = app(SheinOrderPushService::class);
        try {
    ...
    /**
     * Delete a local Shein order that is still ready for Shopify import
    ...
    public function deleteReadyOrder(Request $request): JsonResponse
    {
        ...
        $deleted = SheinOrderMetric::query()
            ->where('order_id', $orderId)
        ...
        return response()->json([
            'success' => true,
            'message' => "Removed Shein order {$orderId} from ready-for-import ({$deleted} row(s)).",
            'deleted' => $deleted,
            'order_id' => $orderId,
        ]);
    }
    ...
    public function markOrderAlreadyImported(Request $request): JsonResponse
    {
        ...
        $updated = SheinOrderMetric::query()
            ->where('order_id', $orderId)
            ->whereNull('shopify_order_id')
            ->update([
                'shopify_order_id' => $shopifyOrderId,
        ...
        return response()->json([
            'success' => true,
            'message' => "Marked Shein order {$orderId} as already imported ({$updated} row(s)).",
            'shopify_order_id' => $shopifyOrderId,
            'updated' => $updated,
        ]);
    }
    ...
    {
        return view('marketplace.shein.settings', [
            'settings' => MarketplaceSyncSettings::getFor('shein'),
            'title' => 'Shein — Sync Settings',
            'connected' => $this->apiConfig->isConfigured('shein'),
        ]);
    }
    ...
    {
        $current = MarketplaceSyncSettings::getFor('shein');
        ...
        MarketplaceSyncSettings::setFor('shein', [
            'pricing' => $pricing,
            'inventory' => $inventory,
            'order' => $order,
            'listings' => $listings,
        ...
        return response()->json(['success' => true, 'message' => 'Shein sync settings saved.']);
    }
    ...
    protected function aeStateIndexFromCache(
        SheinLiveListingsService $liveService,
    ...
    /**
     * SKUs in shein_metric that map to a real Shopify SKU (not product_id placeholders).
     *
    ...
    protected function paginateAeNotInShopify(
        ...
        $rows = $this->aeMetricsWithRealSkuQuery()->orderBy('sku')->get()->filter(function (SheinMmMetric $metric) use ($shopifyNormKeys, $searchSku, $searchName) {
            $norm = ShopifySku::normalizeSkuForShopifyLookup((string) $metric->sku);
            if ($norm === '' || isset($shopifyNormKeys[$norm])) {
                return false;
            }
        ...
        $aeStockMap = $this->sheinStockMapForSkus($sliceSkus);
        $slice = $rows->slice(($page - 1) * $perPage, $perPage)->map(function (SheinMmMetric $metric) use ($aeStockMap) {
     ...
     * @return array<string, SheinMmMetric>
     */
    protected function sheinMetricMapForSkus(array $skus): array
    {
        if ($skus === [] || ! Schema::hasTable('shein_metric')) {
            return [];
        }

        $exact = SheinMmMetric::query()->whereIn('sku', $skus)->get()->keyBy('sku');
        $byNorm = [];
        foreach (SheinMmMetric::query()
            ->whereNotNull('sku')
            ->whereNotNull('product_id')
            ->where('sku', '!=', '')
            ->where('product_id', '!=', '')
     ...
     */
    protected function sheinStockMapForSkus(array $skus): array
    {
