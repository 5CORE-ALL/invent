...
use App\Jobs\WarmDobaLiveListingsCache;
use App\Models\DobaMetric;
use App\Models\DobaDailyData;
use App\Models\MarketplaceSyncSettings;
use App\Models\ShopifySku;
use App\Services\DobaApiService;
use App\Services\MarketplaceManager\DobaDetailFormatter;
...
use App\Services\MarketplaceManager\DobaOrderDetailService;
...
class DobaSyncController extends Controller
{
    ...
    {
        $appKey = (string) config('services.doba.app_key', '');
        $credentialsReady = $this->dobaApi->isConfigured();

        return view('marketplace.doba.connect', [
            'title' => 'Doba — Connect',
            'connected' => $this->apiConfig->isConfigured('doba'),
            'credentialsReady' => $credentialsReady,
            'hasAppKey' => filled($appKey),
            'hasPrivateKey' => filled(config('services.doba.private_key', '')),
    ...
    public function testConnection(): JsonResponse
    {
        if (! $this->dobaApi->isConfigured()) {
            return response()->json([
                'success' => false,
                'message' => 'Doba API credentials missing. Set DOBA_APP_KEY and DOBA_PRIVATE_KEY in .env.',
            ]);
        }

        try {
            $result = $this->dobaApi->testConnection();
        } catch (\Throwable $e) {
            return response()->json([
                'success' => false,
        ...
        $ok = ! isset($result['error'])
            && (
                ...
                || isset($response['businessData'])
            );

        return response()->json([
            'success' => $ok,
            'message' => $ok
                ? 'Doba API connection OK (signed request accepted).'
                : ($response['responseMessage'] ?? $result['error'] ?? 'Connection test failed.'),
        ], $ok ? 200 : 422);
    }
    ...
    public function syncProducts(Request $request): View
    {
        ...
        if (! Schema::hasTable('shopify_skus')) {
            ...
            return view('marketplace.doba.products', [
                'products' => $products,
                'title' => 'Doba — Listings',
                'searchSku' => $searchSku,
        ...
        if ($forceLive) {
            WarmDobaLiveListingsCache::dispatch();
        }
        ...
        $mpStock = MarketplaceListingStockResolver::classifyStockMapFromLiveOrLocal(
            ...
            $this->dobaStockMapForSkus($allLinkedVerified)
        );
        ...
        if ($mismatchQty !== []) {
            ...
            $metricMap = $this->dobaMetricMapForSkus($mismatchQty);
            $productIds = [];
            $idToSku = [];
        ...
        $aeMap = $this->dobaMetricMapForSkus($skus);
        $aeStockMap = $this->dobaStockMapForSkus($skus);
        ...
        if (in_array($linkTab, $liveLinkTabs, true)) {
            ...
            foreach ($skus as $sku) {
                ...
                if (! $metric || ! $this->isShopifySkuLinkedOnDoba($metric, (string) $sku)) {
                    continue;
                }
        ...
        $enriched = collect($pageRows)->map(function (ShopifySku $row) use ($aeMap, $aeStockMap, $liveShopifyQty, $stateIndex, $pageLiveByProduct) {
            $sku = (string) $row->sku;
            $metric = $aeMap[$sku] ?? null;
            $linked = $this->isShopifySkuLinkedOnDoba($metric, $sku);
            ...
            return (object) [
                ...
                'doba_title' => $live['title'] ?? ($cached['title'] ?? ($metric->sku ?? null)),
                'image_src' => $row->image_src ?? null,
                'price' => isset($live['price']) ? $live['price'] : (isset($cached['price']) ? $cached['price'] : ($linked ? ($metric->anticipated_income ?? null) : null)),
    ...
    public function showProduct(int $shopifySkuId): View
    {
        ...
        $aeMap = $this->dobaMetricMapForSkus([$sku]);
        $metric = $aeMap[$sku] ?? null;
        $linked = $this->isShopifySkuLinkedOnDoba($metric, $sku);
        ...
        $detail = app(DobaDetailFormatter::class)->formatProduct(
        ...
        return view('marketplace.doba.product-show', [
            'title' => 'Doba Listing — '.$sku,
    ...
    /**
     * Push live Shopify qty → Doba for this one SKU immediately (no queue).
     */
    public function pushProductInventory(int $shopifySkuId): JsonResponse
    {
        ...
        if (! ($settings['inventory']['inventory_sync'] ?? false) && ! ($settings['pricing']['price_sync'] ?? false)) {
            return response()->json([
                'success' => false,
                'message' => 'Turn on Inventory sync (or Price sync) in Doba settings first.',
            ], 422);
        }
        ...
        $metric = $this->dobaMetricMapForSkus([$sku])[$sku] ?? null;
        if (! $this->isShopifySkuLinkedOnDoba($metric, $sku)) {
            return response()->json([
                'success' => false,
                'message' => 'This SKU is not linked on Doba. Run Sync Doba link map first.',
            ], 422);
        }

        $result = app(DobaInventorySyncService::class)->syncSkusFromShopify([$sku]);
    ...
    public function pullProductFromDoba(int $shopifySkuId): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('doba')) {
            return response()->json(['success' => false, 'message' => 'Doba not connected.']);
        }
        ...
        if (! $metric?->sku || (string) $metric->sku === (string) $metric->sku ?? $sku) {
            return response()->json([
                'success' => false,
                'message' => 'No Doba sku mapped for this SKU. Run Sync Doba link map on Listings first.',
            ]);
        }

        try {
            $this->dobaApi->getInventory();
        } catch (\Throwable $e) {
            return response()->json([
                'success' => false,
                'message' => 'Failed to refresh Doba inventory: '.$e->getMessage(),
            ]);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Refreshed Doba metrics for '.$sku.' from API. Nothing was pushed to Shopify or Doba.',
        ]);
    }
    ...
    public function refreshProducts(Request $request): JsonResponse
    {
        ...
        Log::info('Doba link map sync page', ['page' => $page, 'reset' => $reset]);

        $result = app(DobaLinkMapSyncService::class)->syncPage($page, 50, $reset);
    ...
    {
        $progress = app(DobaLinkMapSyncService::class)->getProgress();
    ...
    public function syncOrders(Request $request): View
    {
        ...
        return view('marketplace.doba.orders', [
            'orders' => $orders,
            'title' => 'Doba — Orders',
            'apiError' => $apiError,
            'connected' => $this->apiConfig->isConfigured('doba'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('doba'),
        ]);
    }

    public function showOrder(int $id): View
    {
        $line = DobaDailyData::query()->findOrFail($id);
        $orderId = (string) $line->order_no;

        $lines = DobaDailyData::query()
        ...
        $aeLiveError = null;
        $aeDataSource = 'cached';
        $detailService = app(DobaOrderDetailService::class);
        ...
        if ($this->apiConfig->isConfigured('doba')) {
            ...
            if (! empty($pull['success'])) {
                ...
                $aeLiveError = $pull['message'] ?? 'Could not refresh live Doba order details.';
            }
        }

        $orderRoot = $detailService->resolveOrderRoot($line);
        $detail = app(DobaDetailFormatter::class)->formatOrder($orderRoot, $lines, $line);

        return view('marketplace.doba.order-show', [
            'title' => 'Doba Order — '.$orderId,
            'orderId' => $orderId,
            'line' => $line,
            'detail' => $detail,
            'aeLiveError' => $aeLiveError,
            ...
            'connected' => $this->apiConfig->isConfigured('doba'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('doba'),
            'orderIsPaid' => MarketplaceOrderPaidFilter::isPaid('doba', $line),
        ]);
    }

    public function pullOrderFromDoba(int $id): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('doba')) {
            return response()->json(['success' => false, 'message' => 'Doba not connected.']);
        }
        ...
        $line = DobaDailyData::query()->findOrFail($id);
        $result = app(DobaOrderDetailService::class)->fetchAndPersistOrderDetail((string) $line->order_no);

        // Persist only — Shopify address fill matches AliExpress / Reverb (SyncDobaAddressJob).
        return response()->json([
            'success' => ! empty($result['success']),
            'message' => $result['message'] ?? ($result['success'] ? 'Order details updated from Doba.' : 'Failed to pull order details.'),
        ]);
    }

    /**
     * Push Shopify fulfillment tracking number to Doba (Ship Order).
     */
    public function pushTrackingToDoba(int $id): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('doba')) {
            return response()->json(['success' => false, 'message' => 'Doba not connected.']);
        }
        ...
        $line = DobaDailyData::query()->findOrFail($id);
        $result = app(DobaTrackingSyncService::class)->pushTrackingForOrder($line);
    ...
    /**
     * Bulk push Shopify tracking → Doba for linked orders.
     */
    public function syncTrackingNow(): JsonResponse
    {
        ...
        \App\Jobs\SyncDobaTrackingJob::dispatch(false);

        return response()->json([
            'success' => true,
            'queued' => true,
            'message' => 'Tracking sync queued. It reads Shopify fulfillments and ships orders on Doba.',
        ]);
    }
    ...
    public function fetchOrders(Request $request): JsonResponse
    {
        ...
        if ($request->boolean('import') || MarketplaceSyncSettings::canAutoImportToShopify('doba')) {
    ...
    public function syncInventoryNow(): JsonResponse
    {
        ...
        \App\Jobs\RunMarketplaceInventorySyncJob::dispatch('doba');
    ...
    public function pushOrderToShopify(Request $request): JsonResponse
    {
        ...
        if ($request->boolean('dry_run')) {
            $preview = app(DobaOrderPushService::class)->previewShopifyPush($order);
        ...
        if (MarketplaceOrderPaidFilter::blocksUnpaidPush('doba', $order)) {
            return response()->json([
                'success' => false,
    ...
    /**
     * Delete a local Doba order that is still ready for Shopify import
     * (not yet imported). Removes all line rows for that AE order_no.
     */
    public function deleteReadyOrder(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => "Removed Doba order {$orderId} from ready-for-import ({$deleted} row(s)).",
            'deleted' => $deleted,
            'order_no' => $orderId,
        ]);
    }
    ...
    public function markOrderAlreadyImported(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => "Marked Doba order {$orderId} as already imported ({$updated} row(s)).",
            'shopify_order_id' => $shopifyOrderId,
            'updated' => $updated,
        ]);
    }
    ...
    {
        return view('marketplace.doba.settings', [
            'settings' => MarketplaceSyncSettings::getFor('doba'),
            'title' => 'Doba — Sync Settings',
            'connected' => $this->apiConfig->isConfigured('doba'),
        ]);
    }
    ...
    {
        $current = MarketplaceSyncSettings::getFor('doba');
        ...
        MarketplaceSyncSettings::setFor('doba', [
            'pricing' => $pricing,
            'inventory' => $inventory,
            'order' => $order,
            'listings' => $listings,
        ...
        return response()->json(['success' => true, 'message' => 'Doba sync settings saved.']);
    }
    ...
    protected function aeStateIndexFromCache(
        DobaLiveListingsService $liveService,
    ...
    /**
     * SKUs in doba_products that map to a real Shopify SKU (not product_id placeholders).
     *
     * @return array<int, string>
     */
    protected function linkedDobaSkus(): array
    {
        if (! Schema::hasTable('doba_metrics')) {
            return [];
        }

        return DobaMetric::query()
            ->whereNotNull('sku')
            ->where('sku', '!=', '')
            ->pluck('sku')
            ->unique()
    ...
    protected function paginateAeNotInShopify(
        ...
        $rows = $this->aeMetricsWithRealSkuQuery()->orderBy('sku')->get()->filter(function (DobaMetric $metric) use ($shopifyNormKeys, $searchSku, $searchName) {
            $norm = ShopifySku::normalizeSkuForShopifyLookup((string) $metric->sku ?? $sku);
            if ($norm === '' || isset($shopifyNormKeys[$norm])) {
                return false;
            }
        ...
        $aeStockMap = $this->dobaStockMapForSkus($sliceSkus);
        $slice = $rows->slice(($page - 1) * $perPage, $perPage)->map(function (DobaMetric $metric) use ($aeStockMap) {
            ...
            return (object) [
                ...
                'doba_title' => null,
                'image_src' => null,
                'price' => $metric->anticipated_income ?? null,
                'shopify_price' => null,
                'stock' => $aeQty,
                'ae_quantity' => $aeQty,
     ...
     * @return array<string, DobaMetric>
     */
    protected function dobaMetricMapForSkus(array $skus): array
    {
        if ($skus === [] || ! Schema::hasTable('doba_metrics')) {
            return [];
        }

        $exact = DobaMetric::query()->whereIn('sku', $skus)->get()->keyBy('sku');
        $byNorm = [];
        foreach (DobaMetric::query()
            ->whereNotNull('sku')
            ->where('sku', '!=', '')
            ->get() as $metric) {
            $norm = ShopifySku::normalizeSkuForShopifyLookup((string) $metric->sku ?? $sku);
     ...
     */
    protected function dobaStockMapForSkus(array $skus): array
    {
        return MarketplaceListingStockResolver::stockMapForSkus(
            MarketplaceListingStockResolver::CHANNEL_DOBA,
            $skus
        );
    }

    protected function isShopifySkuLinkedOnDoba(?DobaMetric $metric, string $shopifySku): bool
    {
