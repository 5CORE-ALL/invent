...
class TikTokSyncController extends Controller
{
    ...
    public function refreshProducts(): JsonResponse
    {
        ...
        $progress = SyncTikTokProductsJob::getProgress('tiktok');
        ...
        if (in_array($status, ['queued', 'running'], true)) {
            return response()->json([
                ...
                'queued' => true,
                'done' => false,
                'message' => $progress['message'] ?? 'TikTok Shop listing sync already running.',
                'progress' => $progress,
            ]);
        }

        SyncTikTokProductsJob::putProgress('tiktok', [
            'status' => 'queued',
            'message' => 'TikTok Shop listing sync queued on mm-tiktok-listings…',
        ...
        SyncTikTokProductsJob::dispatch('tiktok', true);
        ...
        return response()->json([
            ...
            'queued' => true,
            'done' => false,
            'message' => 'TikTok Shop listing sync queued on mm-tiktok-listings (not behind order backlog). Keep this page open…',
            'progress' => SyncTikTokProductsJob::getProgress('tiktok'),
        ]);
    }
    ...
    public function pushTrackingToTikTok(int $id): JsonResponse
    {
        if (! $this->tiktok->isAuthenticated()) {
            return response()->json([
                'success' => false,
        ...
        $line = TiktokOrder::query()->findOrFail($id);
        $result = app(TikTokTrackingSyncService::class)->pushTrackingForOrder($line);
        $ok = ! empty($result['success']);
        ...
        return response()->json([
            ...
            'tracking_pushed_at' => optional(
                TiktokOrder::query()->where('order_id', $line->order_id)->value('tracking_pushed_at')
            )?->toDateTimeString(),
        ], $ok ? 200 : 422);
    }
    ...
    protected function ensureTikTokListingsWorker(): void
    {
        try {
            ...
            if (is_file($script) && is_executable($script)) {
                // Starts missing mm-*-listings workers if not already running.
                exec('bash '.escapeshellarg($script).' >/dev/null 2>&1 &');
            }
        } catch (\Throwable $e) {
            Log::warning('TikTok listings worker ensure failed', ['error' => $e->getMessage()]);
        }
    }
