...
class AliexpressListingVariationVerifyController extends Controller
{
    public function index()
    {
        return view('market-places.aliexpress_listing_variation_verify');
    }

    /**
     * Parent-only rows: Parent, Required, Parent Vs Listed SKU.
     * Listed source: aliexpress_pricing_prices (same Missing rule as /aliexpress-pricing).
     * Missing = CP Master child not listed on AliExpress (no row or price <= 0).
     * Extra   = listed AliExpress SKU in this parent family that is not a CP Master child.
     */
    public function data(Request $request)
    {
        ...
        return response()->json([
            'data' => $formattedData,
            'meta' => [
                'listings_count' => (int) AliexpressPricingPrice::query()->whereNotNull('sku')->where('sku', '!=', '')->count(),
                'last_pulled_at' => AliexpressPricingPrice::query()->max('updated_at'),
                'has_listings_cache' => AliexpressPricingPrice::query()->whereNotNull('sku')->where('sku', '!=', '')->exists(),
    ...
    /**
     * AliExpress listings come from price sheet upload on /aliexpress-pricing (aliexpress_pricing_prices).
     */
    public function pullListings(Request $request)
    {
        try {
            $count = (int) AliexpressPricingPrice::query()->whereNotNull('sku')->where('sku', '!=', '')->count();
            $lastPulledAt = AliexpressPricingPrice::query()->max('updated_at');
            ...
            return response()->json([
                'status' => 200,
                'message' => "AliExpress listings ready. {$count} SKUs in aliexpress_pricing_prices. Parent Vs Listed SKU updated.",
                'count' => $count,
                'last_pulled_at' => $lastPulledAt,
            ]);
        } catch (\Throwable $e) {
            Log::error('AliExpress Listing Variation Verify: pull listings failed', [
                'error' => $e->getMessage(),
            ]);
    ...
    /**
     * Same normalize as AliexpressController::normalizeAeSkuExact.
     */
    ...
    private function buildListedSkuLookup(): array
    {
        ...
        $rows = AliexpressPricingPrice::query()
            ->whereNotNull('sku')
            ->where('sku', '!=', '')
            ->get(['sku', 'price']);
        ...
        foreach ($rows as $row) {
            ...
            // Match /aliexpress-pricing Missing: listed when price > 0
