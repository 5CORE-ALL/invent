...
use App\Jobs\WarmEbay1LiveListingsCache;
use App\Models\EbayMetric;
use App\Models\Ebay1OrderMetric;
use App\Models\MarketplaceSyncSettings;
use App\Models\ShopifySku;
use App\Services\EbayApiService;
use App\Services\MarketplaceManager\Ebay1DetailFormatter;
use App\Services\MarketplaceManager\Ebay1InventorySyncService;
use App\Services\MarketplaceManager\Ebay1LinkMapSyncService;
use App\Services\MarketplaceManager\Ebay1LiveListingsService;
use App\Services\MarketplaceManager\Ebay1OrderDetailService;
...
class Ebay1SyncController extends Controller
{
    ...
    {
        $appId = (string) config('services.ebay1.app_id', env('EBAY_APP_ID'));
        $certId = (string) config('services.ebay1.cert_id', env('EBAY_CERT_ID'));
        $devId = (string) config('services.ebay1.dev_id', env('EBAY_DEV_ID'));
        $refresh = (string) config('services.ebay1.refresh_token', env('EBAY_REFRESH_TOKEN'));
        $credentialsReady = $this->ebay1Api->isConfigured();

        return view('marketplace.ebay1.connect', [
            'title' => 'eBay 1 — Connect',
            'connected' => $this->apiConfig->isConfigured('ebay1'),
    ...
    {
        if (! $this->apiConfig->isConfigured('ebay1')) {
            return response()->json([
                'success' => false,
                'message' => 'eBay 1 API credentials missing. Set EBAY_APP_ID, EBAY_CERT_ID, EBAY_DEV_ID, EBAY_REFRESH_TOKEN in .env.',
            ]);
        }

        try {
            $result = $this->ebay1Api->testConnection();
        } catch (\Throwable $e) {
            return response()->json([
                'success' => false,
    ...
    public function syncProducts(Request $request): View
    {
        ...
        if (! Schema::hasTable('shopify_skus')) {
            ...
            return view('marketplace.ebay1.products', [
                'products' => $products,
                'title' => 'eBay 1 — Listings',
                'searchSku' => $searchSku,
        ...
        $mpStock = MarketplaceListingStockResolver::classifyStockMapFromLiveOrLocal(
            ...
            $this->ebay1StockMapForSkus($allLinkedVerified)
        );
        $classified = $catalog->classifyLinkedInventoryMatch($linkedSkus, $mpStock, marketplace: 'ebay1');
        ...
        if ($mismatchQty !== []) {
            ...
            $metricMap = $this->ebay1MetricMapForSkus($mismatchQty);
            $productIds = [];
            $idToSku = [];
            foreach ($mismatchQty as $sku) {
                $metric = $metricMap[$sku] ?? null;
                if (! $this->isShopifySkuLinkedOnEbay1($metric, (string) $sku)) {
                    continue;
                }
        ...
        $aeMap = $this->ebay1MetricMapForSkus($skus);
        $aeStockMap = $this->ebay1StockMapForSkus($skus);
        ...
        if (in_array($linkTab, $liveLinkTabs, true)) {
            ...
            foreach ($skus as $sku) {
                ...
                if (! $metric || ! $this->isShopifySkuLinkedOnEbay1($metric, (string) $sku)) {
                    continue;
                }
        ...
        $enriched = collect($pageRows)->map(function (ShopifySku $row) use ($aeMap, $aeStockMap, $liveShopifyQty, $stateIndex, $pageLiveByProduct) {
            $sku = (string) $row->sku;
            $metric = $aeMap[$sku] ?? null;
            $linked = $this->isShopifySkuLinkedOnEbay1($metric, $sku);
            ...
            return (object) [
                ...
                'ebay1_title' => $live['title'] ?? ($cached['title'] ?? ($metric->product_name ?? null)),
    ...
    public function showProduct(int $shopifySkuId): View
    {
        ...
        $aeMap = $this->ebay1MetricMapForSkus([$sku]);
        $metric = $aeMap[$sku] ?? null;
        $linked = $this->isShopifySkuLinkedOnEbay1($metric, $sku);
        ...
        $canFetchAe = $productId && $productId !== (string) $metric?->sku && $this->apiConfig->isConfigured('ebay1');

        if ($canFetchAe) {
            $info = $this->ebay1Api->getItem($productId);
            if (! empty($info['success'])) {
                $aeLive = $info['data'] ?? null;
                $aeDataSource = 'api';
            } else {
                $aeLiveError = $info['message'] ?? 'Could not load live eBay 1 product details.';
                $aeDataSource = 'cached';
            }
        ...
        $aeSkuRows = [];
        if (is_array($aeLive)) {
            $aeSkuRows = $this->ebay1Api->extractSkuRowsFromProductInfo(
                $aeLive,
        ...
        $detail = app(Ebay1DetailFormatter::class)->formatProduct(
        ...
        return view('marketplace.ebay1.product-show', [
            'title' => 'eBay 1 Listing — '.$sku,
    ...
    /**
     * Push live Shopify qty → eBay 1 for this one SKU immediately (no queue).
     */
    public function pushProductInventory(int $shopifySkuId): JsonResponse
    {
        ...
        if (! ($settings['inventory']['inventory_sync'] ?? false) && ! ($settings['pricing']['price_sync'] ?? false)) {
            return response()->json([
                'success' => false,
                'message' => 'Turn on Inventory sync (or Price sync) in eBay 1 settings first.',
            ], 422);
        }
        ...
        $metric = $this->ebay1MetricMapForSkus([$sku])[$sku] ?? null;
        if (! $this->isShopifySkuLinkedOnEbay1($metric, $sku)) {
            return response()->json([
                'success' => false,
                'message' => 'This SKU is not linked on eBay 1. Run Sync eBay 1 link map first.',
            ], 422);
        }

        $result = app(Ebay1InventorySyncService::class)->syncSkusFromShopify([$sku]);
    ...
    public function pullProductFromEbay1(int $shopifySkuId): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('ebay1')) {
            return response()->json(['success' => false, 'message' => 'eBay 1 not connected.']);
        }
        ...
        $metric = EbayMetric::query()->where('sku', $sku)->orWhere('sku', strtoupper($sku))->first();
        if (! $metric?->item_id || (string) $metric->item_id === (string) $metric->sku) {
            return response()->json([
                'success' => false,
                'message' => 'No eBay 1 item_id mapped for this SKU. Run Sync eBay 1 link map on Listings first.',
            ]);
        }

        try {
            $info = $this->ebay1Api->getItem((string) $metric->item_id);
        } catch (\Throwable $e) {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Pulled live eBay 1 item '.$metric->item_id.' for SKU '.$sku.'.',
    ...
    public function refreshProducts(Request $request): JsonResponse
    {
        ...
        Log::info('eBay 1 link map sync page', ['page' => $page, 'reset' => $reset]);

        $result = app(Ebay1LinkMapSyncService::class)->syncPage($page, 50, $reset);
    ...
    {
        $progress = app(Ebay1LinkMapSyncService::class)->getProgress();
    ...
    public function syncOrders(Request $request): View
    {
        ...
        if (Schema::hasTable('ebay1_order_metrics')) {
            ...
            $statusCounts = Ebay1OrderMetric::query()
        ...
        return view('marketplace.ebay1.orders', [
            'orders' => $orders,
            'statusCounts' => $statusCounts,
            'title' => 'eBay 1 — Orders',
            'apiError' => $apiError,
            'connected' => $this->apiConfig->isConfigured('ebay1'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('ebay1'),
        ]);
    }

    public function showOrder(int $id): View
    {
        $line = Ebay1OrderMetric::query()->findOrFail($id);
        ...
        $aeLiveError = null;
        $aeDataSource = 'cached';
        $detailService = app(Ebay1OrderDetailService::class);
        ...
        if ($this->apiConfig->isConfigured('ebay1')) {
            ...
            if (! empty($pull['success'])) {
                ...
                $aeLiveError = $pull['message'] ?? 'Could not refresh live eBay 1 order details.';
            }
        }

        $orderRoot = $detailService->resolveOrderRoot($line);
        $detail = app(Ebay1DetailFormatter::class)->formatOrder($orderRoot, $lines, $line);

        return view('marketplace.ebay1.order-show', [
            'title' => 'eBay 1 Order — '.$orderId,
            'orderId' => $orderId,
            'line' => $line,
            'detail' => $detail,
            'aeLiveError' => $aeLiveError,
            ...
            'connected' => $this->apiConfig->isConfigured('ebay1'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('ebay1'),
            'orderIsPaid' => MarketplaceOrderPaidFilter::isPaid('ebay1', $line),
        ]);
    }

    public function pullOrderFromEbay1(int $id): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('ebay1')) {
            return response()->json(['success' => false, 'message' => 'eBay 1 not connected.']);
        }
        ...
        $line = Ebay1OrderMetric::query()->findOrFail($id);
        $result = app(Ebay1OrderDetailService::class)->fetchAndPersistOrderDetail((string) $line->order_id);
        ...
        $line->refresh();
        $message = $result['message'] ?? 'Order details updated from eBay 1.';
        $shopifySynced = null;

        // Already-imported orders were often created without ShipTo; push address on pull.
        if (! empty($line->shopify_order_id)) {
            $sync = app(Ebay1OrderPushService::class)->syncShippingAddressToShopify($line);
            $shopifySynced = ! empty($sync['success']);

            if ($shopifySynced) {
                $message = 'Pulled from eBay 1 and updated shipping address on Shopify.';
            } elseif (! empty($sync['skipped'])) {
                $message = 'Pulled from eBay 1. '.($sync['message'] ?? 'Shopify address not updated.');
            } else {
                return response()->json([
                    'success' => false,
                    'pulled' => true,
                    'shopify_synced' => false,
                    'message' => 'Pulled from eBay 1, but Shopify address update failed: '.($sync['message'] ?? 'unknown error'),
                ], 422);
            }
        }
    ...
    /**
     * Push Shopify fulfillment tracking number to eBay 1 (Ship Order).
     */
    public function pushTrackingToEbay1(int $id): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('ebay1')) {
            return response()->json(['success' => false, 'message' => 'eBay 1 not connected.']);
        }
        ...
        $line = Ebay1OrderMetric::query()->findOrFail($id);
        $result = app(Ebay1TrackingSyncService::class)->pushTrackingForOrder($line);
    ...
    /**
     * Bulk push Shopify tracking → eBay 1 for linked orders.
     */
    public function syncTrackingNow(): JsonResponse
    {
        ...
        \App\Jobs\SyncEbay1TrackingJob::dispatch(false);

        return response()->json([
            'success' => true,
            'queued' => true,
            'message' => 'Tracking sync queued. It reads Shopify fulfillments and ships orders on eBay 1.',
        ]);
    }
    ...
    public function fetchOrders(Request $request): JsonResponse
    {
        ...
        $sync = app(Ebay1OrderSyncService::class);
        ...
        if ($request->boolean('import') || MarketplaceSyncSettings::canAutoImportToShopify('ebay1')) {
    ...
    public function syncInventoryNow(): JsonResponse
    {
        ...
        \App\Jobs\RunMarketplaceInventorySyncJob::dispatch('ebay1');
    ...
    public function syncMismatchInventoryNow(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $cacheKey = 'ebay1_mismatch_sync_list_'.(string) (auth()->id() ?? 'guest').'_'.$scope;
            ...
            if (! is_array($mismatch)) {
                $mismatch = $this->resolveEbay1MismatchSkuList($scope);
            ...
            Log::error('Ebay1 syncMismatchInventoryNow failed', [
                'error' => $e->getMessage(),
     ...
     */
    protected function resolveEbay1MismatchSkuList(string $scope): array
    {
        $catalog = app(ShopifyLiveVerifiedCatalogService::class);
        $liveService = app(Ebay1LiveListingsService::class);
        $linkedSkus = $this->linkedEbay1Skus();
        ...
        $mpStock = MarketplaceListingStockResolver::classifyStockMapFromLiveOrLocal(
            $liveService->peekCached(),
            $this->ebay1StockMapForSkus($verified)
        );
        $classified = $catalog->classifyLinkedInventoryMatch($linkedSkus, $mpStock, marketplace: 'ebay1');
    ...
    public function pushOrderToShopify(Request $request): JsonResponse
    {
        ...
        $order = Ebay1OrderMetric::find($id);
        ...
        if ($request->boolean('dry_run')) {
            $preview = app(Ebay1OrderPushService::class)->previewShopifyPush($order);
        ...
        if (MarketplaceOrderPaidFilter::blocksUnpaidPush('ebay1', $order)) {
            return response()->json([
                'success' => false,
        ...
        $push = app(Ebay1OrderPushService::class);
        try {
    ...
    /**
     * Delete a local eBay 1 order that is still ready for Shopify import
    ...
    public function deleteReadyOrder(Request $request): JsonResponse
    {
        ...
        $deleted = Ebay1OrderMetric::query()
            ->where('order_id', $orderId)
        ...
        return response()->json([
            'success' => true,
            'message' => "Removed eBay 1 order {$orderId} from ready-for-import ({$deleted} row(s)).",
            'deleted' => $deleted,
            'order_id' => $orderId,
        ]);
    }
    ...
    public function markOrderAlreadyImported(Request $request): JsonResponse
    {
        ...
        $updated = Ebay1OrderMetric::query()
            ->where('order_id', $orderId)
            ->whereNull('shopify_order_id')
            ->update([
                'shopify_order_id' => $shopifyOrderId,
        ...
        return response()->json([
            'success' => true,
            'message' => "Marked eBay 1 order {$orderId} as already imported ({$updated} row(s)).",
            'shopify_order_id' => $shopifyOrderId,
            'updated' => $updated,
        ]);
    }
    ...
    {
        return view('marketplace.ebay1.settings', [
            'settings' => MarketplaceSyncSettings::getFor('ebay1'),
            'title' => 'eBay 1 — Sync Settings',
            'connected' => $this->apiConfig->isConfigured('ebay1'),
        ]);
    }
    ...
    {
        $current = MarketplaceSyncSettings::getFor('ebay1');
        ...
        $order = $this->mergeSettingsSection($current['order'] ?? [], $request->input('order', []), [
            ...
            'push_tracking_to_ebay1', 'sync_address_to_shopify',
        ]);
        $listings = $this->mergeSettingsSection($current['listings'] ?? [], $request->input('listings', []), [
            'auto_link_by_sku', 'create_products_on_ebay1', 'sync_title', 'sync_images',
        ]);
        ...
        MarketplaceSyncSettings::setFor('ebay1', [
            'pricing' => $pricing,
            'inventory' => $inventory,
            'order' => $order,
            'listings' => $listings,
        ...
        return response()->json(['success' => true, 'message' => 'eBay 1 sync settings saved.']);
    }
    ...
    protected function aeStateIndexFromCache(
        Ebay1LiveListingsService $liveService,
    ...
    /**
     * SKUs in ebay_metrics that map to a real Shopify SKU (not product_id placeholders).
     *
     * @return array<int, string>
     */
    protected function linkedEbay1Skus(): array
    {
        if (! Schema::hasTable('ebay_metrics')) {
            return [];
        }

        return EbayMetric::query()
            ->whereNotNull('sku')
    ...
    protected function paginateAeNotInShopify(
        ...
        $rows = $this->aeMetricsWithRealSkuQuery()->orderBy('sku')->get()->filter(function (EbayMetric $metric) use ($shopifyNormKeys, $searchSku, $searchName) {
            $norm = ShopifySku::normalizeSkuForShopifyLookup((string) $metric->sku);
            if ($norm === '' || isset($shopifyNormKeys[$norm])) {
                return false;
            }
        ...
        $aeStockMap = $this->ebay1StockMapForSkus($sliceSkus);
        $slice = $rows->slice(($page - 1) * $perPage, $perPage)->map(function (EbayMetric $metric) use ($aeStockMap) {
     ...
     * @return array<string, EbayMetric>
     */
    protected function ebay1MetricMapForSkus(array $skus): array
    {
        if ($skus === [] || ! Schema::hasTable('ebay_metrics')) {
            return [];
        }

        $exact = EbayMetric::query()->whereIn('sku', $skus)->get()->keyBy('sku');
        $byNorm = [];
        foreach (EbayMetric::query()
            ->whereNotNull('sku')
            ->whereNotNull('item_id')
            ->where('sku', '!=', '')
            ->where('item_id', '!=', '')
     ...
     */
    protected function ebay1StockMapForSkus(array $skus): array
    {
    ...
    protected function isShopifySkuLinkedOnEbay1(?EbayMetric $metric, string $shopifySku): bool
    {
