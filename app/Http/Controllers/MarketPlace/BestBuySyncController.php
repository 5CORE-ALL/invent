...
use App\Jobs\WarmBestBuyLiveListingsCache;
use App\Models\BestbuyUsaProduct;
use App\Models\BestBuyOrderMetric;
use App\Models\MarketplaceSyncSettings;
use App\Models\ShopifySku;
use App\Services\BestBuyApiService;
use App\Services\MarketplaceManager\BestBuyDetailFormatter;
...
use App\Services\MarketplaceManager\BestBuyOrderDetailService;
...
class BestBuySyncController extends Controller
{
    ...
    public function connect(Request $request): View
    {
        ...
        return view('marketplace.bestbuy.connect', [
            'title' => 'Best Buy — Connect',
    ...
    public function testConnection(): JsonResponse
    {
        ...
        try {
            $result = $this->bbApi->testConnection();
        } catch (\Throwable $e) {
            return response()->json([
                'success' => false,
    ...
    public function syncProducts(Request $request): View
    {
        ...
        if (! Schema::hasTable('shopify_skus')) {
            ...
            return view('marketplace.bestbuy.products', [
                'products' => $products,
                'title' => 'BestBuy — Listings',
                'searchSku' => $searchSku,
        ...
        if ($forceLive) {
            WarmBestBuyLiveListingsCache::dispatch();
        }
        ...
        if ($mismatchQty !== []) {
            ...
            $metricMap = $this->bestbuyMetricMapForSkus($mismatchQty);
            $productIds = [];
            $idToSku = [];
            foreach ($mismatchQty as $sku) {
                $metric = $metricMap[$sku] ?? null;
                if (! $this->isShopifySkuLinkedOnBestBuy($metric, (string) $sku)) {
                    continue;
                }
        ...
        $aeMap = $this->bestbuyMetricMapForSkus($skus);
        $aeStockMap = $this->bestbuyStockMapForSkus($skus);
        ...
        if (in_array($linkTab, $liveLinkTabs, true)) {
            ...
            foreach ($skus as $sku) {
                ...
                if (! $metric || ! $this->isShopifySkuLinkedOnBestBuy($metric, (string) $sku)) {
                    continue;
                }
        ...
        $enriched = collect($pageRows)->map(function (ShopifySku $row) use ($aeMap, $aeStockMap, $liveShopifyQty, $stateIndex, $pageLiveByProduct) {
            $sku = (string) $row->sku;
            $metric = $aeMap[$sku] ?? null;
            $linked = $this->isShopifySkuLinkedOnBestBuy($metric, $sku);
            ...
            return (object) [
                ...
                'bestbuy_title' => $live['title'] ?? ($cached['title'] ?? ($metric->sku ?? null)),
                ...
                'bestbuy_state' => $state !== '' ? $state : null,
    ...
    public function showProduct(int $shopifySkuId): View
    {
        ...
        $aeMap = $this->bestbuyMetricMapForSkus([$sku]);
        $metric = $aeMap[$sku] ?? null;
        $linked = $this->isShopifySkuLinkedOnBestBuy($metric, $sku);
        ...
        $detail = app(BestBuyDetailFormatter::class)->formatProduct(
        ...
        return view('marketplace.bestbuy.product-show', [
            'title' => 'BestBuy Listing — '.$sku,
    ...
    /**
     * Push live Shopify qty → BestBuy for this one SKU immediately (no queue).
     */
    public function pushProductInventory(int $shopifySkuId): JsonResponse
    {
        ...
        if (! ($settings['inventory']['inventory_sync'] ?? false) && ! ($settings['pricing']['price_sync'] ?? false)) {
            return response()->json([
                'success' => false,
                'message' => 'Turn on Inventory sync (or Price sync) in BestBuy settings first.',
            ], 422);
        }
        ...
        $metric = $this->bestbuyMetricMapForSkus([$sku])[$sku] ?? null;
        if (! $this->isShopifySkuLinkedOnBestBuy($metric, $sku)) {
            return response()->json([
                'success' => false,
                'message' => 'This SKU is not linked on BestBuy. Run Sync BestBuy link map first.',
            ], 422);
        }

        $result = app(BestBuyInventorySyncService::class)->syncSkusFromShopify([$sku]);
    ...
    public function pullProductFromBestBuy(int $shopifySkuId): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('bestbuy')) {
            return response()->json(['success' => false, 'message' => 'BestBuy not connected.']);
        }
        ...
        if (! $metric?->sku || (string) $metric->sku === (string) $metric->sku ?? $sku) {
            return response()->json([
                'success' => false,
                'message' => 'No BestBuy sku mapped for this SKU. Run Sync BestBuy link map on Listings first.',
            ]);
        }
        ...
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to refresh BestBuy inventory: '.$e->getMessage(),
            ]);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Refreshed BestBuy metrics for '.$sku.' from API. Nothing was pushed to Shopify or BestBuy.',
        ]);
    }
    ...
    public function refreshProducts(Request $request): JsonResponse
    {
        ...
        Log::info('BestBuy link map sync page', ['page' => $page, 'reset' => $reset]);

        $result = app(BestBuyLinkMapSyncService::class)->syncPage($page, 50, $reset);
    ...
    {
        $progress = app(BestBuyLinkMapSyncService::class)->getProgress();
    ...
    public function syncOrders(Request $request): View
    {
        ...
        if (Schema::hasTable('mirakl_daily_data')) {
            $orders = BestBuyOrderMetric::query()
        ...
        return view('marketplace.bestbuy.orders', [
            'orders' => $orders,
            'title' => 'Best Buy — Orders',
            'apiError' => $apiError,
            'connected' => $this->apiConfig->isConfigured('bestbuy'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('bestbuy'),
        ]);
    }

    public function showOrder(int $id): View
    {
        $line = BestBuyOrderMetric::query()->findOrFail($id);
        $orderId = $line->displayOrderId();

        $lines = BestBuyOrderMetric::query()
        ...
        $aeLiveError = null;
        $aeDataSource = 'cached';
        $detailService = app(BestBuyOrderDetailService::class);
        ...
        if ($this->apiConfig->isConfigured('bestbuy')) {
            ...
            if (! empty($pull['success'])) {
                ...
                $aeLiveError = $pull['message'] ?? 'Could not refresh live BestBuy order details.';
            }
        }

        $orderRoot = $detailService->resolveOrderRoot($line);
        $detail = app(BestBuyDetailFormatter::class)->formatOrder($orderRoot, $lines, $line);

        return view('marketplace.bestbuy.order-show', [
            'title' => 'BestBuy Order — '.$orderId,
            'orderId' => $orderId,
            'line' => $line,
            'detail' => $detail,
            'aeLiveError' => $aeLiveError,
            ...
            'connected' => $this->apiConfig->isConfigured('bestbuy'),
            'importPaidOrdersOnly' => MarketplaceSyncSettings::importPaidOrdersOnly('bestbuy'),
            'orderIsPaid' => MarketplaceOrderPaidFilter::isPaid('bestbuy', $line),
        ]);
    }

    public function pullOrderFromBestBuy(int $id): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('bestbuy')) {
            return response()->json(['success' => false, 'message' => 'BestBuy not connected.']);
        }
        ...
        $line = BestBuyOrderMetric::query()->findOrFail($id);
        $result = app(BestBuyOrderDetailService::class)->fetchAndPersistOrderDetail($line->displayOrderId());

        // Persist only — Shopify address fill matches AliExpress / Reverb (SyncBestBuyAddressJob).
        return response()->json([
            'success' => ! empty($result['success']),
            'message' => $result['message'] ?? ($result['success'] ? 'Order details updated from BestBuy.' : 'Failed to pull order details.'),
        ]);
    }

    /**
     * Push Shopify fulfillment tracking number to BestBuy (Ship Order).
     */
    public function pushTrackingToBestBuy(int $id): JsonResponse
    {
        if (! $this->apiConfig->isConfigured('bestbuy')) {
            return response()->json(['success' => false, 'message' => 'BestBuy not connected.']);
        }
        ...
        $line = BestBuyOrderMetric::query()->findOrFail($id);
        $result = app(BestBuyTrackingSyncService::class)->pushTrackingForOrder($line);
    ...
    /**
     * Bulk push Shopify tracking → BestBuy for linked orders.
     */
    public function syncTrackingNow(): JsonResponse
    {
        ...
        \App\Jobs\SyncBestBuyTrackingJob::dispatch(false);

        return response()->json([
            'success' => true,
            'queued' => true,
            'message' => 'Tracking sync queued. It reads Shopify fulfillments and ships orders on BestBuy.',
        ]);
    }
    ...
    public function fetchOrders(Request $request): JsonResponse
    {
        ...
        $sync = app(BestBuyOrderSyncService::class);
        ...
        if ($request->boolean('import') || MarketplaceSyncSettings::canAutoImportToShopify('bestbuy')) {
    ...
    public function syncInventoryNow(): JsonResponse
    {
        ...
        \App\Jobs\RunMarketplaceInventorySyncJob::dispatch('bestbuy');
    ...
    public function pushOrderToShopify(Request $request): JsonResponse
    {
        ...
        $order = BestBuyOrderMetric::find($id);
        ...
        if ($request->boolean('dry_run')) {
            $preview = app(BestBuyOrderPushService::class)->previewShopifyPush($order);
        ...
        if (MarketplaceOrderPaidFilter::blocksUnpaidPush('bestbuy', $order)) {
            return response()->json([
                'success' => false,
        ...
        $push = app(BestBuyOrderPushService::class);
        try {
    ...
    /**
     * Delete a local BestBuy order that is still ready for Shopify import
    ...
    public function deleteReadyOrder(Request $request): JsonResponse
    {
        ...
        $deleted = BestBuyOrderMetric::query()
            ->where(function ($q) use ($orderId) {
                $q->where('order_id', $orderId)->orWhere('channel_order_id', $orderId);
            })
        ...
        return response()->json([
            'success' => true,
            'message' => "Removed BestBuy order {$orderId} from ready-for-import ({$deleted} row(s)).",
            'deleted' => $deleted,
            'order_id' => $orderId,
        ]);
    }
    ...
    public function markOrderAlreadyImported(Request $request): JsonResponse
    {
        ...
        $updated = BestBuyOrderMetric::query()
            ->where(function ($q) use ($orderId) {
                $q->where('order_id', $orderId)->orWhere('channel_order_id', $orderId);
            })
            ->whereNull('shopify_order_id')
        ...
        return response()->json([
            'success' => true,
            'message' => "Marked BestBuy order {$orderId} as already imported ({$updated} row(s)).",
            'shopify_order_id' => $shopifyOrderId,
            'updated' => $updated,
        ]);
    }
    ...
    {
        return view('marketplace.bestbuy.settings', [
            'settings' => MarketplaceSyncSettings::getFor('bestbuy'),
            'title' => 'BestBuy — Sync Settings',
            'connected' => $this->apiConfig->isConfigured('bestbuy'),
        ]);
    }
    ...
    {
        $current = MarketplaceSyncSettings::getFor('bestbuy');
        ...
        $order = $this->mergeSettingsSection($current['order'] ?? [], $request->input('order', []), [
            ...
            'push_tracking_to_bestbuy', 'sync_address_to_shopify',
        ]);
        $listings = $this->mergeSettingsSection($current['listings'] ?? [], $request->input('listings', []), [
            'auto_link_by_sku', 'create_products_on_bestbuy', 'sync_title', 'sync_images',
        ]);
        ...
        MarketplaceSyncSettings::setFor('bestbuy', [
            'pricing' => $pricing,
            'inventory' => $inventory,
            'order' => $order,
            'listings' => $listings,
        ...
        return response()->json(['success' => true, 'message' => 'BestBuy sync settings saved.']);
    }
    ...
    protected function aeStateIndexFromCache(
        BestBuyLiveListingsService $liveService,
    ...
    /**
     * SKUs in bestbuy_products that map to a real Shopify SKU (not product_id placeholders).
     *
     * @return array<int, string>
     */
    protected function linkedBestBuySkus(): array
    {
        if (! Schema::hasTable('bestbuy_usa_products')) {
            return [];
        }

        return BestbuyUsaProduct::query()
            ->whereNotNull('sku')
            ->where('sku', '!=', '')
            ->pluck('sku')
            ->unique()
    ...
    protected function paginateAeNotInShopify(
        ...
        $rows = $this->aeMetricsWithRealSkuQuery()->orderBy('sku')->get()->filter(function (BestbuyUsaProduct $metric) use ($shopifyNormKeys, $searchSku, $searchName) {
            $norm = ShopifySku::normalizeSkuForShopifyLookup((string) $metric->sku ?? $sku);
            if ($norm === '' || isset($shopifyNormKeys[$norm])) {
                return false;
            }
        ...
        $aeStockMap = $this->bestbuyStockMapForSkus($sliceSkus);
        $slice = $rows->slice(($page - 1) * $perPage, $perPage)->map(function (BestbuyUsaProduct $metric) use ($aeStockMap) {
     ...
     */
    protected function bestbuyMetricMapForSkus(array $skus): array
    {
        if ($skus === [] || ! Schema::hasTable('bestbuy_usa_products')) {
            return [];
        }

        $exact = BestbuyUsaProduct::query()->whereIn('sku', $skus)->get()->keyBy('sku');
        $byNorm = [];
        foreach (BestbuyUsaProduct::query()
            ->whereNotNull('sku')
            ->where('sku', '!=', '')
            ->get() as $metric) {
            $norm = ShopifySku::normalizeSkuForShopifyLookup((string) $metric->sku ?? $sku);
     ...
     */
    protected function bestbuyStockMapForSkus(array $skus): array
    {
        return MarketplaceListingStockResolver::stockMapForSkus(
            MarketplaceListingStockResolver::CHANNEL_BESTBUY,
            $skus
        );
    }
