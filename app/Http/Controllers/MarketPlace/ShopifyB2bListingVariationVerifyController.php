...
class ShopifyB2bListingVariationVerifyController extends Controller
{
    public function index()
    {
        return view('market-places.shopify_b2b_listing_variation_verify');
    }

    /**
     * Parent-only rows: Parent, Required, Parent Vs Listed SKU.
     * Listed source: shopify_skus (same as /shopify-b2b-pricing Missing column).
    ...
    /**
     * Shopify B2B listings come from shopify_skus (synced for /shopify-b2b-pricing).
    ...
    public function pullListings(Request $request)
    {
        try {
            ...
            if ($count === 0) {
                return response()->json([
                    'status' => 422,
                    'message' => 'No Shopify SKUs in shopify_skus. Sync Shopify data used by Shopify B2B - Analytics (/shopify-b2b-pricing) first.',
                    'count' => 0,
            ...
            return response()->json([
                'status' => 200,
                'message' => "Shopify B2B listings ready. {$count} SKUs in shopify_skus. Parent Vs Listed SKU updated.",
                'count' => $count,
                'last_pulled_at' => $lastPulledAt,
            ]);
        } catch (\Throwable $e) {
            Log::error('Shopify B2B Listing Variation Verify: pull listings failed', [
                'error' => $e->getMessage(),
            ]);
