...
class PlsListingVariationVerifyController extends Controller
{
    public function index()
    {
        return view('market-places.pls_listing_variation_verify');
    }

    /**
     * Parent-only rows: Parent, Required, Parent Vs Listed SKU.
     * Listed source: pls_products (same Missing rule as /pls-pricing).
     * Missing = CP Master child not in pls_products or price <= 0.
     * Extra   = listed PLS SKU in this parent family that is not a CP Master child.
     */
    public function data(Request $request)
    {
        ...
        return response()->json([
            'data' => $formattedData,
            'meta' => [
                'listings_count' => (int) PLSProduct::query()->whereNotNull('sku')->where('sku', '!=', '')->count(),
                'last_pulled_at' => PLSProduct::query()->max('updated_at'),
                'has_listings_cache' => PLSProduct::query()->whereNotNull('sku')->where('sku', '!=', '')->exists(),
    ...
    /**
     * PLS listings come from /pls-pricing (pls_products).
     */
    public function pullListings(Request $request)
    {
        try {
            $count = (int) PLSProduct::query()->whereNotNull('sku')->where('sku', '!=', '')->count();
            $lastPulledAt = PLSProduct::query()->max('updated_at');

            if ($count === 0) {
                return response()->json([
                    'status' => 422,
                    'message' => 'No PLS listings in pls_products. Sync/update data on PLS Pricing (/pls-pricing) first.',
                    'count' => 0,
            ...
            return response()->json([
                'status' => 200,
                'message' => "PLS listings ready. {$count} SKUs in pls_products. Parent Vs Listed SKU updated.",
                'count' => $count,
                'last_pulled_at' => $lastPulledAt,
            ]);
        } catch (\Throwable $e) {
            Log::error('PLS Listing Variation Verify: pull listings failed', [
                'error' => $e->getMessage(),
            ]);
    ...
    /**
     * Same normalize as /pls-pricing (ShopifySku::normalizeSkuForShopifyLookup).
     */
    ...
    private function buildListedSkuLookup(): array
    {
        ...
        foreach ($rows as $row) {
            ...
            // Match /pls-pricing: listed when in pls_products and price > 0
