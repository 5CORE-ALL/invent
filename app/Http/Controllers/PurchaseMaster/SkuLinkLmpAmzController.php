...
use App\Models\LmpAmzSkuMark;
use App\Models\ProductMaster;
use App\Models\ShopifySku;
use App\Services\LmpAmzSkuGroupService;
use App\Services\LmpAmzSkuLinkService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;

class SkuLinkLmpAmzController extends Controller
{
    public function __construct(
        private LmpAmzSkuLinkService $skuLinkService,
        private LmpAmzSkuGroupService $skuGroupService
    ) {
    }

    public function index()
    {
        return view('purchase-master.sku-link-lmp-amz.index');
    }
    ...
    public function getData(Request $request)
    {
        try {
            ...
            $marksBySkuNorm = LmpAmzSkuMark::query()
            ...
            foreach ($products as $product) {
                ...
                $mark = LmpAmzSkuMark::updateOrCreate(
                    ['sku_norm' => $skuNorm],
                    [
            ...
            $data = $products->map(function ($product) use ($shopifyBySku, $lmpDetailsLookup, $marksBySkuNorm, $historySummary) {
                $shopify = $shopifyBySku->get($product->sku);
                $sku = (string) ($product->sku ?? '');
                $skuNorm = strtoupper(trim($sku));
                $linkedLmpSkus = $this->linkedLmpSkusForProduct($sku);
                $lmp = $this->resolveAmazonLmpForSkus($linkedLmpSkus, $lmpDetailsLookup);
                ...
                return [
                    ...
                    'amz_lmp' => $this->computeAmzLmp($lmp['lmp_price'], $m),
    ...
    public function getLmpData(Request $request)
    {
        try {
            ...
            return response()->json([
                ...
                'lowest_price' => $lowestPrice ? (float) $lowestPrice->price : null,
     ...
     */
    private function resolveAmazonLmpForSkus(array $skus, $lmpDetailsLookup): array
    {
        $allEntries = collect();

        foreach ($skus as $sku) {
            $skuLookupKey = AmazonSkuCompetitor::normalizeSkuKey((string) $sku);
            if ($skuLookupKey === '') {
                continue;
            }
        ...
        $allEntries = AmazonSkuCompetitor::dedupeByAsin($allEntries);

        $lowest = $allEntries->first();

        return [
            'lmp_price' => ($lowest && is_numeric($lowest->price))
                ? (float) $lowest->price
                : null,
    ...
    private function computeAmzLmp(?float $lmpPrice, ?string $m): ?float
    {
