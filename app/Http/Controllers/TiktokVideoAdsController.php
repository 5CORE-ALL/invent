...
/**
 * Backs the "TikTok Video Ads" page (/tiktok-video-ads).
 *
 * Independent of the Facebook sheet — its own `tiktok_video_ads` table and
 * its own Sbgt-rule / audit / snapshot tables. The page is a generic sheet
 * importer: users upload any CSV / Excel / TSV file sourced from TikTok and
 * the rows are stored verbatim as JSON in `tiktok_video_ads`. The Tabulator
 * on the front-end then renders whichever columns the upload carried.
 */
class TiktokVideoAdsController extends Controller
{
    /** Allowed ad-type values for each page-type filter (unused — single page). */
    private const TYPE_FILTERS = [];

    /** TikTok Video Ads page (no ad_type filter, full dropdown). */
    public function index()
    {
        return view('tiktok-video-ads', [
            'pageType'       => 'all',
            'pageTitle'      => 'TikTok Video Ads',
            'pageSubtitle'   => 'Generic CSV / Excel / TSV importer — upload TikTok ad sheets and view them as a table',
            'allowedAdTypes' => TiktokVideoAd::AD_TYPES,
        ]);
    }
    ...
    public function getData(Request $request)
    {
        ...
        $query = TiktokVideoAd::query()->orderBy('row_index');
    ...
    private function getMergedView(?array $typeList): ?array
    {
        ...
        try {
            ...
            // log and continue.
            \Log::warning('TikTok snapshot failed: ' . $e->getMessage());
        }
    ...
    {
        $row = DB::table('tiktok_sbgt_rules')->where('key', 'tiktok_all')->first();
        $rule = $row
    ...
    private function snapshotMergedView(array $rows): void
    {
        ...
        foreach ($payloads as $p) {
            DB::table('tiktok_campaign_metric_snapshots')->updateOrInsert(
    ...
    private function latestAuditsFor(array $cids): array
    {
        ...
        // same second.
        $sub = DB::table('tiktok_campaign_audits')
            ->select('campaign_id', DB::raw('MAX(id) AS max_id'))
            ->whereIn('campaign_id', $cids)
            ->groupBy('campaign_id');

        $rows = DB::table('tiktok_campaign_audits AS a')
    ...
    public function getAudit(Request $request)
    {
        ...
        $latest = DB::table('tiktok_campaign_audits')
            ->where('campaign_id', $cid)
            ->orderByDesc('id')
            ->first();

        $history = DB::table('tiktok_campaign_audits')
            ->where('campaign_id', $cid)
            ->orderByDesc('id')
            ->limit(50)
    ...
    public function saveAudit(Request $request)
    {
        ...
        $user = $request->user();
        $id   = DB::table('tiktok_campaign_audits')->insertGetId([
            'campaign_id'      => $cid,
    ...
    public function saveRule(Request $request)
    {
        ...
        DB::table('tiktok_sbgt_rules')->updateOrInsert(
            ['key' => 'tiktok_all'],
    ...
    public function badgeHistory(Request $request)
    {
        ...
        // sees on the badges.
        $base = DB::table('tiktok_campaign_metric_snapshots')
    ...
    private function latestBatchPerType(): array
    {
        ...
        $firstRows = TiktokVideoAd::whereIn('id', $firstIds)
            ->pluck('row_data', 'id');
    ...
    {
        $request->validate([
            'ad_type' => ['nullable', 'string', 'in:' . implode(',', TiktokVideoAd::AD_TYPES)],
        ]);

        $row = TiktokVideoAd::findOrFail($id);
        ...
        if ($cid !== null && $cid !== '') {
            ...
            $propagated = TiktokVideoAd::query()
    ...
    public function batches()
    {
        ...
        $firstIds   = $batches->pluck('first_id')->all();
        $firstRows  = TiktokVideoAd::whereIn('id', $firstIds)->pluck('row_data', 'id');
