...
use App\Models\ShippingHealthChannelData;
use App\Models\ShippingHealthStatusHistory;
use Carbon\Carbon;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

/**
 * Account Health "Shipping Health" grid (Escalated Claims–style).
 * Uses shipping_health_channel_data — separate from Shipping Audit tables.
 */
class ShippingHealthOverviewController extends Controller
{
    private const HISTORY_FRESH_DAYS = 3;

    public function tabulator()
    {
        return view('channels.shipping_health_overview.tabulator');
    }
    ...
    public function statusHistory(Request $request)
    {
        ...
        $rows = ShippingHealthStatusHistory::query()
            ->where('snapshot_date', '>=', $start->toDateString())
            ->orderBy('snapshot_date')
            ->get()
            ->keyBy(function (ShippingHealthStatusHistory $row) {
        ...
        for ($i = 0; $i < $days; $i++) {
            ...
            /** @var ShippingHealthStatusHistory|null $row */
            $row = $rows->get($key);
            $data[] = [
    ...
    public function saveLink(Request $request)
    {
        ...
        $record = ShippingHealthChannelData::query()->updateOrCreate(
    ...
    private function buildChannelRows(): array
    {
        ...
        $linkRows = ShippingHealthChannelData::query()
        ...
        return $channels->map(function (ChannelMaster $c) use ($linkRows) {
            /** @var ShippingHealthChannelData|null $row */
    ...
    {
        ShippingHealthStatusHistory::query()->updateOrCreate(
     ...
     */
    private function historyPayload(?ShippingHealthChannelData $record): array
    {
