...
class MappingChannelCounts
{
    ...
    public const INACTIVE_TOTAL_CACHE_KEY = 'inactive_listings_total_v20';

    public const INACTIVE_MASTER_ROWS_CACHE_KEY = 'inactive_listings_master_rows_v20';

    public const INACTIVE_CP_TOTAL_CACHE_KEY = 'inactive_listings_cp_total_v20';
    ...
    public static function forgetMasterCaches(): void
    {
        try {
            ...
            Cache::forget('inactive_listings_total_v18');
            Cache::forget('inactive_listings_master_rows_v18');
            Cache::forget('inactive_listings_cp_total_v18');
            Cache::forget('inactive_listings_total_v19');
            Cache::forget('inactive_listings_master_rows_v19');
            Cache::forget('inactive_listings_cp_total_v19');
        } catch (\Throwable $e) {
            // ignore
        }
    }
    ...
    public static function cachedCpInactiveTotalOrZero(): int
    {
        try {
            $cached = Cache::get(self::INACTIVE_CP_TOTAL_CACHE_KEY);
            if ($cached !== null) {
                return (int) $cached;
            }
        } catch (\Throwable $e) {
        ...
        try {
            ...
            if (is_array($rows) && $rows !== []) {
                return (int) collect($rows)->sum(fn ($row) => (int) ($row['cp_inactive_child'] ?? 0));
            }
        } catch (\Throwable $e) {
            // ignore
        }
