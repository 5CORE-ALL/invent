@extends('layouts.vertical', ['title' => "Best Buy Competitor Search", 'mode' => 'light'])
...
<div class="container-fluid">
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">Best Buy Competitor Search</li>
                    </ol>
                </div>
                <h4 class="page-title">Best Buy Competitor Discovery</h4>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="search-container">
                <h5 class="mb-3">Search Best Buy Products</h5>
                <p class="text-muted mb-4">Enter a search query to discover Best Buy competitor products and analyze their pricing, condition, and seller information.</p>
                ...
                <form id="searchForm">
                    ...
                    <button type="submit" class="btn btn-primary btn-lg">
                        <i class="mdi mdi-magnify me-2"></i>Search Best Buy
                    </button>
    ...
    <div class="loading-spinner" id="loadingSpinner">
        ...
        <p class="mt-3 text-muted">Searching Best Buy... This may take a moment.</p>
    </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-3">
                                    ...
                                    <select class="form-select" id="sortBy">
                                        <option value="position">Best Buy Position (Default)</option>
...
$(document).ready(function() {
    ...
    let currentMarketplace = 'bestbuy';
    ...
    // Prefill SKU from ?sku= query param (pricing orange-dot links)
    const urlSkuParam = new URLSearchParams(window.location.search).get('sku');
    ...
    function performSearch(query, marketplace) {
        ...
        $.ajax({
            url: '/repricer/bestbuy-search/search',
            method: 'POST',
            data: {
                query: query,
    ...
    function applyFiltersAndSort() {
        ...
        $.ajax({
            url: '/repricer/bestbuy-search/results',
            method: 'GET',
            data: {
    ...
    function loadFilterOptions(query) {
        $.ajax({
            url: '/repricer/bestbuy-search/filter-options',
            method: 'GET',
            data: { query: query },
            success: function(response) {
    ...
    function displayResults(response) {
        ...
        if (results.length === 0) {
            ...
            results.forEach(function(item) {
                ...
                const productLink = item.link || `https://www.bestbuy.com/site/${item.item_id}`;
                    ...
                    <div class="col-xl-3 col-lg-4 col-md-6 col-sm-12 mb-3">
                        <div class="product-card">
                            <input type="checkbox" class="competitor-checkbox product-checkbox form-check-input" 
                                   ...
                                   data-marketplace="${item.marketplace || 'bestbuy'}"
        ...
        // Initialize Select2 after rendering, then preselect ?sku=
        setTimeout(function() {
            initializeSelect2();
            preselectSkuFromQuery();
        }, 100);
    }
    ...
    function loadSearchHistory() {
        $.ajax({
            url: '/repricer/bestbuy-search/history',
            method: 'GET',
            success: function(response) {
    ...
    function loadSkus() {
        $.ajax({
            url: '/repricer/bestbuy-search/skus',
            method: 'GET',
            success: function(response) {
                if (response.success) {
    ...
    function saveSelectedCompetitors() {
        ...
        $.ajax({
            url: '/repricer/bestbuy-search/store-competitors',
            method: 'POST',
    ...
    function preselectSkuFromQuery() {
        if (!urlSkuParam) {
            return;
        }
        const sku = String(urlSkuParam).trim();
        if (!sku) {
            return;
        }

        if ($('#bulkSkuSelect option').filter(function () { return $(this).val() === sku; }).length === 0
            && availableSkus.indexOf(sku) !== -1) {
            $('#bulkSkuSelect').append(new Option(sku, sku, false, false));
        }
        if ($('#bulkSkuSelect option').filter(function () { return $(this).val() === sku; }).length) {
            $('#bulkSkuSelect').val(sku).trigger('change');
        }

        const safeId = 'sku-' + sku.replace(/[^a-zA-Z0-9]/g, '_');
        const $cb = $('#' + safeId);
        if ($cb.length) {
            $cb.prop('checked', true);
            updateSelectedSkuCount();
            $('#skuCheckboxContainer').show();
            $('#toggleSkuList').html('<i class="mdi mdi-chevron-up"></i> Hide SKUs');
        }
    }
    ...
    window.loadHistoryQuery = function(query) {
        ...
        $('#resultsContainer').hide();
        currentSearchQuery = query;
        $.get('/repricer/bestbuy-search/results', { query: query })
