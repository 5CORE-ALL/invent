...
@php
    ...
    $channelPromoUsesSprcDil = in_array($channelPromoChannel, ['ebay1', 'ebay2', 'ebay3', 'temu', 'temu2', 'temu3', 'macys', 'macy', 'purchasing_power', 'wayfair', 'reverb', 'doba', 'doba_withoutship', 'aliexpress', 'shein', 'faire', 'tiktok', 'tiktok2', 'shopify_b2c', 'bestbuy', 'newegg', 'topdawg', 'fb_marketplace', 'mercari_wship', 'mercari_woship'], true);
...
@if($channelPromoPart === 'css' || $channelPromoPart === 'all')
        ...
        .tabulator-row .tabulator-cell[tabulator-field="zero_sold"],
        ...
        #ch-promo-dil-sgroi-table .ch-promo-dil-sgroi-input,
        ...
        #ch-promo-dil-vs-sgroi-btn:hover,
        #ch-promo-dil-vs-sgroi-btn:focus,
...
@if($channelPromoPart === 'buttons' || $channelPromoPart === 'all')
                    ...
                    <div class="btn-group">
                        <button type="button" class="btn btn-sm" id="ch-promo-dil-vs-prmt-btn"
                            title="{{ $channelPromoChannel === 'shopify_b2b' ? 'B2B discount: shared Dil vs PRMT (0.01–3 → 12 … 24–25 → 1). Auto-fills B2B disc; PRMT% auto-applies when Dil slab changes.' : 'Shared Dil vs PRMT (0.01–3 … 21–24, 24–25). PRMT% auto-applies when Dil slab changes. Save updates all marketplaces.' }}">
                    ...
                    @endunless
                    @if(in_array($channelPromoChannel, ['aliexpress', 'shein'], true) && empty($channelPromoUsesSprcDil))
                    <button type="button" class="btn btn-sm" id="ch-promo-dil-vs-sgroi-btn"
                        title="Same Dil slabs as Dil vs PRMT. Maps Dil% to Target SGROI%. S PRC auto-updates so SGROI equals the target when AL30 &gt; 0. If AL30 = 0, use 0 Sold.">
                        <i class="fas fa-sliders-h"></i> SGROI
                    </button>
                    @endif
                    @endunless
                    ...
                    <div class="btn-group">
                        <button type="button" class="btn btn-sm" id="ch-promo-cvr-vs-cpn-btn"
                            title="{{ $channelPromoUsesAmazonCvrDisc ? 'CVR Disc. column rules — same slabs as /amazon-tabulator-view' : 'Map CVR% slabs to CPN%. CPN% auto-applies when CVR slab changes (no coupon push).' }}">
                    ...
                    <button type="button" class="btn btn-sm" id="ch-promo-zero-sold-vs-dil-btn"
                        title="{{ $channelPromoZeroSoldMinRoi
                            ? '0 Sold ('.$channelPromoZeroSoldSoldLabel.' = 0, INV > 0): same as Temu — min Target GROI, or Min ROI% if higher. S PRC auto-applies on load and when slabs / Min ROI change (same as Amazon).'
                            : '0 Sold: Dil color (Red / Green / Pink) → Target GROI%. This page has its own rule table. Apply sets S PRC so SGROI equals the target when 0 sold.' }}">
                        0 Sold
                    </button>
                    @endif
                    @unless(in_array($channelPromoChannel, ['macys', 'macy']))
...
@if($channelPromoPart === 'modals' || $channelPromoPart === 'all')
            ...
            <div class="modal-content">
                ...
                <div class="modal-body py-2">
                    <p class="small text-muted mb-2" id="ch-promo-cvr-cpn-help">
                        @if($channelPromoUsesAmazonCvrDisc)
                            ...
                            <strong>0.01–1% → 9</strong> down to <strong>&gt; 7% → 0</strong> (no 0% slab).
                ...
                <div class="modal-footer py-2 flex-wrap gap-1">
                    ...
                    <button type="button" class="btn btn-sm btn-primary" id="ch-promo-cvr-cpn-save-btn"
                        title="Save CVR→CPN slab values. CPN% also auto-applies when CVR slab changes.">
                        <i class="fas fa-save me-1"></i>Save Rule
                    </button>
                    @endunless
                    @if($channelPromoUsesAmazonCvrDisc)
                    <button type="button" class="btn btn-sm btn-primary" id="ch-promo-cvr-cpn-apply-btn"
                        title="Save CVR Disc rules and refresh the CVR Disc. column (INV = 0 → 0%). Same slabs as /amazon-tabulator-view.">
                        Apply
                    </button>
                    @endif
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body py-2">
                    <p class="small text-muted mb-2" id="ch-promo-dil-prmt-help">
                        ...
                        <strong>PRMT% auto-applies when Dil slab changes</strong>.
                        <strong>Save Rule</strong> stores the slabs.
                ...
                <div class="modal-footer py-2 flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-primary" id="ch-promo-dil-prmt-apply-btn"
                        title="Save Dil→PRMT slabs. PRMT% also auto-applies when Dil slab changes.">
                        <i class="fas fa-save me-1"></i>Save Rule
                    </button>
                </div>
            </div>
    ...
    @if(in_array($channelPromoChannel, ['aliexpress', 'shein'], true) && empty($channelPromoUsesSprcDil))
    <div class="modal fade" id="chPromoDilVsSgroiModal" tabindex="-1" aria-labelledby="chPromoDilVsSgroiModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-md">
            <div class="modal-content">
                <div class="modal-header py-2">
                    <h5 class="modal-title fs-6" id="chPromoDilVsSgroiModalLabel">
                        <i class="fas fa-sliders-h me-1"></i> Dil vs Target SGROI
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body py-2">
                    <p class="small text-muted mb-2" id="ch-promo-dil-sgroi-help">
                        Same Dil slabs as <strong>Dil vs PRMT</strong>. Map Dil% to <strong>Target SGROI%</strong>.
                        Dil is <strong>SKU-wise</strong> (OV L30 ÷ INV).
                        Changing any slab number fills the <strong>Target SGROI%</strong> column.
                        <strong>S PRC auto-applies</strong> so <strong>SGROI = Target</strong>
                        (<code>S PRC = (LP × (1 + SGROI%/100) + Ship) / margin</code>)
                        on selected or visible SKUs with <strong>AL30 &gt; 0</strong> only.
                        If <strong>AL30 = 0</strong>, this rule is skipped (use <strong>0 Sold</strong>).
                        <strong>Save Rule</strong> stores the slabs.
                        If <strong>INV is 0</strong>, this rule is skipped.
                    </p>
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered align-middle mb-0" id="ch-promo-dil-sgroi-table">
                            <thead class="table-light">
                                <tr>
                                    <th style="width:55%;">Dil%</th>
                                    <th style="width:45%;" class="text-end">Target SGROI%</th>
                                </tr>
                            </thead>
                            <tbody id="ch-promo-dil-sgroi-tbody"></tbody>
                        </table>
                    </div>
                    <div class="small text-muted mt-2" id="ch-promo-dil-sgroi-status"></div>
                </div>
                <div class="modal-footer py-2 flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-primary" id="ch-promo-dil-sgroi-apply-btn"
                        title="Save Dil→Target SGROI slabs. S PRC also auto-applies when Dil slab changes.">
                        <i class="fas fa-save me-1"></i>Save Rule
                    </button>
                </div>
            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body py-2">
                    ...
                    <p class="small text-muted mb-2" id="ch-promo-zero-sold-dil-help">
                        @if(!empty($channelPromoZeroSoldMinRoi))
                        Same 0 Sold rule as Temu:
                        when <strong>{{ $channelPromoZeroSoldSoldLabel }} = 0</strong> and <strong>INV &gt; 0</strong>,
                        take the <strong>minimum Target GROI from the slabs</strong>
                        (lowest GROI% in this table — not the Dil-matching color).
                        <strong>Min ROI%</strong> (optional) raises the target when it is higher than that min GROI.
                        <strong>S PRC auto-applies</strong> on page load and when a slab or Min ROI% changes
                        (same as Amazon Sprc Dil), so <strong>SROI = the chosen target</strong>
                        (<code id="ch-promo-zero-sold-dil-formula">S PRC = (LP × (1 + target%/100) + Ship) / margin</code>).
                        <strong>Apply</strong> also saves the table.
                        <span id="ch-promo-zero-sold-dil-owner-line" class="d-none">This page has its own rule table.</span>
                        <span id="ch-promo-zero-sold-dil-gate" class="d-none">{{ $channelPromoZeroSoldSoldLabel }} = 0</span>
                        @else
                        ...
                        <span id="ch-promo-zero-sold-dil-owner-line">This page has its own rule table.</span>
                    ...
                    @if(!empty($channelPromoZeroSoldMinRoi))
                    <div class="d-flex align-items-center gap-2 mt-2" id="ch-promo-zero-sold-min-roi-wrap">
                        <label for="ch-promo-zero-sold-min-roi" class="small fw-semibold mb-0 text-nowrap">Min ROI%</label>
                        <input type="number" id="ch-promo-zero-sold-min-roi" class="form-control form-control-sm"
                            min="0" step="0.1" value="" placeholder="optional" style="width:90px;"
                            title="Optional floor. 0 Sold S PRC uses the higher of min Target GROI and this Min ROI%.">
                        <span class="small text-muted">Blank or 0 = min Target GROI only (Temu). If higher than min GROI, this ROI is used.</span>
                    </div>
                    @endif
                ...
                <div class="modal-footer py-2 flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-primary" id="ch-promo-zero-sold-dil-apply-btn"
                        title="{{ !empty($channelPromoZeroSoldMinRoi)
                            ? 'Save 0 Sold slabs / Min ROI. S PRC also auto-applies on load and when values change.'
                            : 'Save Dil color → Target GROI% and write S PRC on 0 Sold SKUs' }}">
...
@if($channelPromoPart === 'script' || $channelPromoPart === 'all')
        ...
        function chPromoUsesSprcDilInsteadOfPrmt() {
        ...
        function chPromoHideDilPrmt() {
            return !!CHANNEL_PROMO_HIDE_DIL_PRMT || chPromoUsesSprcDilInsteadOfPrmt();
        }
        ...
        const CHANNEL_PROMO_ZERO_SOLD_MIN_ROI = @json(!empty($channelPromoZeroSoldMinRoi));
        function chPromoIsMacysPromoChannel() {
            return CHANNEL_PROMO_CHANNEL === 'macys' || CHANNEL_PROMO_CHANNEL === 'macy';
        }
        function chPromoIsPurchasingPowerPromoChannel() {
            return CHANNEL_PROMO_CHANNEL === 'purchasing_power';
        }
        function chPromoIsWayfairPromoChannel() {
            return CHANNEL_PROMO_CHANNEL === 'wayfair';
        }
        function chPromoIsDobaPromoChannel() {
            return CHANNEL_PROMO_CHANNEL === 'doba';
        }
        function chPromoIsDobaWithoutshipPromoChannel() {
            return CHANNEL_PROMO_CHANNEL === 'doba_withoutship';
        }
        function chPromoIsAliexpressPromoChannel() {
            return CHANNEL_PROMO_CHANNEL === 'aliexpress';
        }
        function chPromoIsSheinPromoChannel() {
            return CHANNEL_PROMO_CHANNEL === 'shein';
        }
        function chPromoIsBestbuyPromoChannel() {
            return CHANNEL_PROMO_CHANNEL === 'bestbuy';
        }
        function chPromoIsNeweggPromoChannel() {
            return CHANNEL_PROMO_CHANNEL === 'newegg';
        }
        function chPromoUsesSprcDilOnlySprice() {
            return chPromoIsMacysPromoChannel()
                || chPromoIsPurchasingPowerPromoChannel()
                || chPromoIsWayfairPromoChannel()
                || chPromoIsDobaPromoChannel()
                || chPromoIsDobaWithoutshipPromoChannel()
                || chPromoIsAliexpressPromoChannel()
                || chPromoIsSheinPromoChannel()
                || chPromoIsBestbuyPromoChannel()
                || chPromoIsNeweggPromoChannel();
        }
        function chPromoZeroSoldUsesTemuMinRoi() {
            return !!CHANNEL_PROMO_ZERO_SOLD_MIN_ROI;
        }
        const CHANNEL_PROMO_CFG = {
            ...
            bestbuy: {
                ...
                skuField: '(Child) sku',
                soldField: 'BB L30',
                soldFieldLabel: 'BB L30',
                saveSpriceMode: 'sku',
            },
            reverb: {
                label: 'Reverb',
                saveSpriceUrl: '/reverb-save-sprice',
                saveSpriceBatchUrl: '/reverb-save-sprice',
                pushPriceUrl: null,
                priceField: 'RV Price',
                cvrField: 'CVR',
                dilField: 'RV Dil%',
            ...
            wayfair: {
                ...
                saveSpriceBatchUrl: '/wayfair/pricing-save-sprice',
                pushPriceUrl: '/cvr-master-push-price',
                priceField: 'price',
            ...
            doba: {
                ...
                saveSpriceBatchUrl: '/doba/save-sprice',
                ...
                skuField: '(Child) sku',
                soldField: 'doba L30',
                soldFieldLabel: 'Doba L30',
                saveSpriceMode: 'sku',
            },
            doba_withoutship: {
                label: 'Doba (no ship)',
                saveSpriceUrl: '/doba/save-sprice-withoutship',
                saveSpriceBatchUrl: '/doba/save-sprice-withoutship',
                pushPriceUrl: '/doba/push-price',
            ...
            tiktok: {
                ...
                saveSpriceBatchUrl: '/tiktok-save-sprice',
                pushPriceUrl: '/cvr-master-push-price',
                priceField: 'TT Price',
                cvrField: 'CVR%',
                dilField: 'TT Dil%',
                ...
                skuField: '(Child) sku',
                soldField: 'TT L30',
                soldFieldLabel: 'TT L30',
                saveSpriceMode: 'sku',
            },
            tiktok2: {
                label: 'TikTok 2',
                saveSpriceUrl: '/tiktok-2-save-sprice',
                saveSpriceBatchUrl: '/tiktok-2-save-sprice',
                pushPriceUrl: '/cvr-master-push-price',
                priceField: 'TT Price',
                cvrField: 'CVR%',
                dilField: 'TT Dil%',
            ...
            purchasing_power: {
                ...
                saveSpriceBatchUrl: '/pp-save-sprice-batch',
                pushPriceUrl: '/cvr-master-push-price',
                priceField: 'PP Price',
                dilField: 'PP Dil%',
                invField: 'INV',
                skuField: '(Child) sku',
                soldField: 'PP L30',
                soldFieldLabel: 'PP L30',
                saveSpriceMode: 'sku',
            },
            aliexpress: {
                label: 'AliExpress',
                saveSpriceUrl: '/aliexpress/save-sprice',
                saveSpriceBatchUrl: '/aliexpress/save-sprice',
                pushPriceUrl: '/cvr-master-push-price',
                priceField: 'price',
                dilField: 'dil_percent',
                invField: 'inv',
            ...
            faire: {
                ...
                saveSpriceBatchUrl: '/faire/pricing-save-sprice',
                pushPriceUrl: '/cvr-master-push-price',
                priceField: 'price',
                cvrField: 'cvr',
                dilField: 'dil_percent',
        ...
        /** eBay 2 / 3 use eBay 1's 0 Sold Dil→GROI table (same Apply path). */
        function chPromoSharesEbay1ZeroSoldRules() {
            return chPromoIsEbayChannel();
        }
        function chPromoZeroSoldRulesUrl() {
            const ch = chPromoSharesEbay1ZeroSoldRules() ? 'ebay1' : CHANNEL_PROMO_CHANNEL;
            return '/channel-promo-pricing/' + encodeURIComponent(ch) + '/zero-sold-prc';
        }
        ...
        function chPromoEbayPrmtUsed(d) {
            if (chPromoHideDilPrmt()) return 0;
        ...
        function chPromoEbayCpnUsed(d) {
            if (CHANNEL_PROMO_HIDE_CVR_CPN) return 0;
            if (CHANNEL_PROMO_CHANNEL === 'shopify_b2c'
                && typeof chPromoIsZeroSoldRow === 'function'
                && chPromoIsZeroSoldRow(d)) {
                if (typeof chPromoKeepZeroSoldPrcSprice === 'function' && chPromoKeepZeroSoldPrcSprice(d)) {
                    return chPromoRound2(Number(d && (d.cpn_pct != null && d.cpn_pct !== ''
            ...
            if (chPromoUsesLiveCvrCpnSlabs() && typeof chPromoEbaySlabCpn === 'function') {
                ...
                if (slab != null) return chPromoRound2(slab);
            }
        ...
        const CH_PEF_CVR_CPN_SKIP_ZERO = true;
        const CH_PEF_CVR_CPN_DEFAULTS = CH_PEF_CVR_CPN_DEFAULTS_ALL;
        ...
        let chPromoZeroSoldMinRoi = 0;
        const CH_PEF_DIL_SGROI_DEFAULTS = (function() {
            const rules = [];
            let sgroi = 70;
            for (let min = 0; min < 24; min += 3) {
                const max = min + 3;
            ...
            rules.push({ key: '24-25', label: '24–25%', sgroi: sgroi });
            return rules;
        })();
        let chPromoDilSgroiRules = CH_PEF_DIL_SGROI_DEFAULTS.map(function(r) { return Object.assign({}, r); });
        ...
        function chPromoIsChildRow(d) {
            return !!(d && !d.is_parent_summary && !d.is_parent && chPromoSku(d) && String(chPromoSku(d)).indexOf('PARENT') === -1);
        }
        ...
        /** Keep in-memory SPRICE / sprice equal to the visible rule price. */
        function chPromoWriteStoredSprice(d, live) {
            if (!d || !(live > 0)) return;
            const patch = chPromoSpricePatch(live);
            Object.assign(d, patch);
            const sku = chPromoSku(d);
            if (sku && typeof chPromoPatchDatasetSprice === 'function') {
        ...
        function chPromoLiveSprice(d) {
            ...
            const dilSgroi = (!zeroSold && typeof chPromoDilSgroiRuleSprice === 'function')
                ...
                : 0;
            const start = zeroSold > 0
                ? zeroSold
                : (dilSgroi > 0
                    ? dilSgroi
                    : (chPromoUsesPrmtCpnDiscountedStart()
                        ? chPromoTemuDiscountedStart(d)
                        : chPromoFloorShopifySpriceToAmz(d, chPromoSpriceFromStdTPromo(d) || 0)));
            const live = (zeroSold > 0 || dilSgroi > 0)
        ...
        function chPromoInvalidateListingDilCache(opts) {
            ...
            if (typeof chPromoScheduleDilSgroiAutoApply === 'function') {
        ...
        function chPromoUsesLiveDilPrmtSlabs() {
            if (chPromoHideDilPrmt()) return false;
            return chPromoIsEbayChannel()
                || CHANNEL_PROMO_CHANNEL === 'aliexpress'
                || CHANNEL_PROMO_CHANNEL === 'shein'
                || CHANNEL_PROMO_CHANNEL === 'newegg'
        ...
        function chPromoUsesLiveCvrCpnSlabs() {
            if (CHANNEL_PROMO_HIDE_CVR_CPN) return false;
            return (chPromoIsEbayChannel()
                || CHANNEL_PROMO_CHANNEL === 'newegg'
                || CHANNEL_PROMO_CHANNEL === 'tiktok'
                || CHANNEL_PROMO_CHANNEL === 'tiktok2'
        ...
        /** S PRC = Std × (1 − (PRMT% + CPN%)/100) — Temu, Faire, and Reverb coupon. */
        ...
        function chPromoIsTemuPromoChannel() {
            return CHANNEL_PROMO_CHANNEL === 'temu' || CHANNEL_PROMO_CHANNEL === 'temu2' || CHANNEL_PROMO_CHANNEL === 'temu3';
        }
        function chPromoIsFairePromoChannel() {
            return CHANNEL_PROMO_CHANNEL === 'faire';
        }
        /** Temu / Faire: 0 Sold Dil→GROI, else Std × (1 − T Promo). Faire never applies EB/Amz/LMP caps. */
        function chPromoUsesPrmtCpnDiscountedStart() {
            return chPromoIsTemuPromoChannel() || chPromoIsFairePromoChannel();
        }
        ...
        function chPromoTemuDiscountedStart(d) {
            ...
            if (typeof chPromoSpriceFromStdTPromo === 'function') {
                const calc = Number(chPromoSpriceFromStdTPromo(d, { skip_lmp_cap: true }));
                if (calc > 0) return chPromoRound2(calc);
            }
            return 0;
        }
        /** Temu: 0 Sold Dil→Target GROI%, else min(Discounted, eBay, Amz, displayed LMP). Faire: same rules, no caps. */
        function chPromoResolveTemuSprice(d, sprice, extra) {
            extra = extra || {};
            if (d && chPromoUsesPrmtCpnDiscountedStart()) {
                const zeroSold = typeof chPromoTemuZeroSoldSprice === 'function'
                    ? Number(chPromoTemuZeroSoldSprice(d))
                    : 0;
                if (zeroSold > 0) return chPromoRound2(zeroSold);
            ...
            if (d && chPromoIsFairePromoChannel()) {
                const start = (extra.use_passed_as_discounted && Number(sprice) > 0)
                    ? Number(sprice)
                    : (chPromoTemuDiscountedStart(d) || Number(sprice) || 0);
                return start > 0 ? chPromoRound2(start) : 0;
            }
            if (d && typeof temuPrepareSpriceForSave === 'function' && chPromoIsTemuPromoChannel()) {
                const start = (extra.use_passed_as_discounted && Number(sprice) > 0)
                    ...
                    : (chPromoTemuDiscountedStart(d) || Number(sprice) || 0);
                const opts = Object.assign({}, extra, {
                    use_passed_as_discounted: extra.use_passed_as_discounted === true || start > 0,
                });
                const v = Number(temuPrepareSpriceForSave(d, start, opts));
                if (v > 0) return chPromoRound2(v);
            }
        ...
        /** Temu discount apply: wipe stored S PRC, then save Discounted → lowest of EB / Amz / LMP. */
        ...
        /** Temu / Faire 0 Sold Dil→Target GROI% owns S PRC. Dil/PRMT/CPN must not overwrite it. */
        function chPromoTemuZeroSoldOwnsSprice(d) {
            if (typeof chPromoUsesSprcDilInsteadOfPrmt === 'function' && chPromoUsesSprcDilInsteadOfPrmt()) {
                return false;
            }
            return chPromoUsesPrmtCpnDiscountedStart()
                && typeof chPromoIsZeroSoldRow === 'function'
                && chPromoIsZeroSoldRow(d);
        }
        ...
        function saveChannelSprice(sku, sprice, silent, extra) {
            ...
            const temuListingPush = chPromoIsTemuPromoChannel()
        ...
        function chPromoDilSgroiEnabled() {
            if (typeof chPromoUsesSprcDilInsteadOfPrmt === 'function' && chPromoUsesSprcDilInsteadOfPrmt()) {
                return false;
            }
            return typeof chPromoIsAeStyleSpriceChannel === 'function' && chPromoIsAeStyleSpriceChannel();
        }
        function chPromoSgroiForRuleKey(key) {
            const rule = chPromoDilSgroiRules.find(function(r) { return r.key === key; });
            if (!rule) return null;
            const n = Number(rule.sgroi);
            return isFinite(n) ? n : null;
        }
        /** Dil slab → Target SGROI% for sold SKUs (AL30 > 0, INV > 0). */
        function chPromoDilSgroiForRow(d) {
            if (!chPromoDilSgroiEnabled() || !d || !chPromoIsChildRow(d)) return null;
            if (chPromoInv(d) <= 0) return null;
            if (typeof chPromoIsZeroSoldRow === 'function' && chPromoIsZeroSoldRow(d)) return null;
            if (typeof chPromoHasSaleQty === 'function' && !chPromoHasSaleQty(d)) return null;
            return chPromoSgroiForRuleKey(chPromoDilSlabKey(chPromoDil(d)));
        }
        function chPromoDilSgroiRuleSprice(d) {
            const roi = chPromoDilSgroiForRow(d);
            if (roi == null) return 0;
            const target = typeof chPromoSpriceFromTargetRoi === 'function'
                ? chPromoSpriceFromTargetRoi(d, roi)
                : 0;
            return (target > 0) ? chPromoRound2(target) : 0;
        }
        ...
        /** cvr % the CVR vs CPN slabs produce for this row. INV=0 or CVR=0 (no 0% slab) → 0. */
        function chPromoEbaySlabCpn(d) {
            if (!d || d.is_parent_summary) return null;
            if (!chPromoIsChildRow(d)) return null;
            if (chPromoInv(d) === 0) return 0;
            if (typeof chPromoIsAeStyleSpriceChannel === 'function' && chPromoIsAeStyleSpriceChannel()
        ...
        function chPromoSyncEbayCpnColumnFromSlabs() {
            if (!chPromoUsesLiveCvrCpnSlabs()) return;
            if (typeof table === 'undefined' || !table) return;
        ...
        /** Lowest Target GROI% in the 0 Sold table (Temu-style). */
        function chPromoZeroSoldMinGroiFromSlabs() {
            let min = null;
            (chPromoZeroSoldDilRules || []).forEach(function(r) {
                const g = Number(r && r.groi);
                if (!isFinite(g)) return;
                if (min === null || g < min) min = g;
            });
            return min;
        }
        function chPromoZeroSoldReadMinRoi() {
            const $inp = $('#ch-promo-zero-sold-min-roi');
            if ($inp.length) {
                const raw = $inp.val();
                if (raw !== '' && raw != null) {
                    const n = Number(raw);
            ...
            const stored = Number(chPromoZeroSoldMinRoi);
            return (isFinite(stored) && stored > 0) ? stored : 0;
        }
        function chPromoZeroSoldTemuStyleTarget() {
            const minGroi = chPromoZeroSoldMinGroiFromSlabs();
            const minRoi = chPromoZeroSoldReadMinRoi();
            if (minGroi == null && !(minRoi > 0)) return null;
            return Math.max(minGroi == null ? 0 : minGroi, minRoi > 0 ? minRoi : 0);
        }
        /** Same Dil as eBay 1 / Dil vs PRMT: listing Dil (Σ OV L30 ÷ Σ INV). */
        function chPromoZeroSoldDilPct(d) {
            return chPromoDil(d);
        }
        function chPromoZeroSoldGroiForRow(d) {
            if (!chPromoIsZeroSoldRow(d)) return null;
            if (typeof chPromoZeroSoldUsesTemuMinRoi === 'function' && chPromoZeroSoldUsesTemuMinRoi()) {
                return chPromoZeroSoldTemuStyleTarget();
            }
        ...
        function chPromoZeroSoldRuleSprice(d) {
            ...
            const roi = typeof chPromoZeroSoldGroiForRow === 'function'
                ...
                : chPromoRoiForZeroSoldDil(chPromoDil(d));
            if (roi == null) return 0;
        ...
        function chPromoIsZeroSoldRow(d) {
            ...
            const isMercari = CHANNEL_PROMO_CHANNEL === 'mercari_wship'
                || CHANNEL_PROMO_CHANNEL === 'mercari_woship';
            // Mercari Dil is 0% when INV is 0; still treat L30 = 0 as 0 Sold.
            if (!(inv > 0) && !(ebayStock > 0) && !isMercari) return false;
            if (chPromoIsEbayChannel()) {
            ...
            if (CHANNEL_PROMO_SHOW_ZERO_SOLD_RULES) {
                return chPromoReverbSoldQty(d) <= 0;
            }
            const q = Number(d && (d.L30 != null ? d.L30 : (d.l30 != null ? d.l30 : d.sold))) || 0;
            return !(q > 0);
        }
        ...
        function chPromoOnDilPrmtNumberChanged(inputEl) {
            ...
            if (typeof chPromoScheduleDilPrmtAutoApply === 'function') {
                chPromoScheduleDilPrmtAutoApply({ force: true, saveRules: true });
            }
        }
        ...
        async function loadChPromoDilPrmtRules() {
            ...
            try {
                ...
                const defaultMsg = 'Using first-time defaults (0.01–3 … 21–24, 24–25). Save Rule stores slabs. PRMT% auto-applies when Dil slab changes.';
        ...
        function saveChPromoDilPrmtRules(opts) {
            opts = opts || {};
            ...
            return $.ajax({
                ...
                if (!opts.fromAutoApply && typeof chPromoScheduleDilPrmtAutoApply === 'function') {
                    chPromoScheduleDilPrmtAutoApply({ force: true });
                }
                return res;
            });
        }

        function cascadeChPromoDilSgroiFromFirst() {
            const $rows = $('#ch-promo-dil-sgroi-tbody tr');
            if (!$rows.length) return;
            const firstVal = parseFloat($rows.eq(0).find('.ch-promo-dil-sgroi-input').val());
            if (!isFinite(firstVal)) return;
            $rows.each(function(i) {
                const sgroi = firstVal - i;
                const $inp = $(this).find('.ch-promo-dil-sgroi-input');
                if (i > 0) $inp.val(sgroi);
                const key = String($(this).attr('data-key') || '');
                const rule = chPromoDilSgroiRules.find(function(r) { return r.key === key; });
                if (rule) rule.sgroi = sgroi;
            });
        }
        function chPromoOnDilSgroiNumberChanged(inputEl) {
            const first = $('#ch-promo-dil-sgroi-tbody .ch-promo-dil-sgroi-input').get(0);
            if (inputEl === first) {
                cascadeChPromoDilSgroiFromFirst();
            } else {
                readChPromoDilSgroiRulesFromModal();
            }
            if (typeof table !== 'undefined' && table) {
                try { table.redraw(true); } catch (e) { /* ignore */ }
            }
            if (typeof chPromoScheduleDilSgroiAutoApply === 'function') {
                chPromoScheduleDilSgroiAutoApply({ force: true, saveRules: true });
            }
        }
        function renderChPromoDilSgroiModalTable() {
            const $tb = $('#ch-promo-dil-sgroi-tbody');
            if (!$tb.length) return;
            $tb.empty();
            chPromoDilSgroiRules.forEach(function(r, idx) {
                const sgroi = isFinite(Number(r.sgroi)) ? Number(r.sgroi) : 0;
                const first = idx === 0;
                $tb.append(
                    '<tr data-key="' + String(r.key).replace(/"/g, '&quot;') + '">'
                    + '<td>' + String(r.label || r.key) + '</td>'
                    + '<td class="text-end">'
                    + '<input type="number" class="form-control form-control-sm ch-promo-dil-sgroi-input" '
                    + 'step="0.1" value="' + sgroi + '" data-idx="' + idx + '"'
                    + (first ? ' title="Changing this sets following slabs to −1 each"' : '')
                    + '>'
                    + '</td></tr>'
                );
            });
        }
        function readChPromoDilSgroiRulesFromModal() {
            $('#ch-promo-dil-sgroi-tbody tr').each(function() {
                const key = String($(this).attr('data-key') || '');
                const val = parseFloat($(this).find('.ch-promo-dil-sgroi-input').val());
                const rule = chPromoDilSgroiRules.find(function(r) { return r.key === key; });
                if (!rule) return;
                rule.sgroi = isFinite(val) ? val : 0;
            });
            return chPromoDilSgroiRules.map(function(r) {
                return { key: r.key, label: r.label, sgroi: Number(r.sgroi) || 0 };
            });
        }
        async function loadChPromoDilSgroiRules() {
            if (!chPromoDilSgroiEnabled() || !$('#ch-promo-dil-sgroi-tbody').length) return;
            $('#ch-promo-dil-sgroi-status').text('Loading…');
            try {
                const res = await $.ajax({
                    url: CH_PROMO_RULES_BASE + '/dil-sgroi',
                    method: 'GET',
                    dataType: 'json',
                });
                if (res && Array.isArray(res.rules) && res.rules.length) {
                    chPromoDilSgroiRules = res.rules.map(function(r) { return Object.assign({}, r); });
                }
                renderChPromoDilSgroiModalTable();
                $('#ch-promo-dil-sgroi-status').text(res && res.is_default
                    ? 'Using first-time defaults (70, then −1 each Dil slab). Save Rule stores slabs. S PRC auto-applies when Dil slab changes.'
                    : 'Loaded Dil vs Target SGROI rules for this page.');
                if (typeof table !== 'undefined' && table) {
                    try { table.redraw(true); } catch (e) { /* ignore */ }
                }
                if (typeof chPromoScheduleDilSgroiAutoApply === 'function') {
                ...
                renderChPromoDilSgroiModalTable();
                $('#ch-promo-dil-sgroi-status').text('Could not load saved rules — showing defaults.');
                if (typeof chPromoScheduleDilSgroiAutoApply === 'function') {
                    chPromoScheduleDilSgroiAutoApply();
                }
            }
        }
        function saveChPromoDilSgroiRules(opts) {
            opts = opts || {};
            const rules = readChPromoDilSgroiRulesFromModal();
            return $.ajax({
                url: CH_PROMO_RULES_BASE + '/dil-sgroi',
                method: 'POST',
                headers: {
                ...
                if (res && Array.isArray(res.rules)) {
                    chPromoDilSgroiRules = res.rules.map(function(r) { return Object.assign({}, r); });
                    if (!opts.silent) renderChPromoDilSgroiModalTable();
                }
                if (!opts.silent) $('#ch-promo-dil-sgroi-status').text('Saved.');
                if (!opts.fromAutoApply && typeof chPromoScheduleDilSgroiAutoApply === 'function') {
                    chPromoScheduleDilSgroiAutoApply({ force: true });
                }
                return res;
            });
        }
        async function saveChPromoDilSgroiRulesOnly() {
            const $btn = $('#ch-promo-dil-sgroi-apply-btn');
            const html = $btn.html();
            $btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin me-1"></i>Saving…');
            try {
                await saveChPromoDilSgroiRules();
                chPromoToast('success', 'Dil vs Target SGROI rules saved');
            } catch (xhr) {
        ...
        let chPromoDilSgroiLastSlab = {};
        let chPromoDilSgroiAutoTimer = null;
        let chPromoDilSgroiAutoBusy = false;
        let chPromoDilSgroiAutoPending = null;
        let chPromoDilSgroiAutoSaveTimer = null;
        function chPromoDilSgroiSlabStoreKey() {
            return 'chPromoDilSgroiSlab:' + CHANNEL_PROMO_CHANNEL;
        }
        function chPromoLoadDilSgroiLastSlabs() {
            try {
                const raw = localStorage.getItem(chPromoDilSgroiSlabStoreKey());
                const obj = raw ? JSON.parse(raw) : {};
                chPromoDilSgroiLastSlab = (obj && typeof obj === 'object') ? obj : {};
            } catch (e) {
                chPromoDilSgroiLastSlab = {};
            }
        }
        function chPromoSaveDilSgroiLastSlabs() {
            try {
                localStorage.setItem(chPromoDilSgroiSlabStoreKey(), JSON.stringify(chPromoDilSgroiLastSlab));
            } catch (e) { /* ignore quota */ }
        }
        function chPromoStampDilSgroiLastSlabs(items) {
            (items || []).forEach(function(item) {
                const d = (item && (item.d || (item.row && item.row.getData()))) || {};
                const sku = chPromoSkuKey(chPromoSku(d));
                if (!sku) return;
                chPromoDilSgroiLastSlab[sku] = chPromoDilSlabKey(chPromoDil(d));
            });
            chPromoSaveDilSgroiLastSlabs();
        }
        function chPromoDilSgroiEligible(d) {
            return chPromoDilSgroiForRow(d) != null;
        }
        function chPromoCollectDilSgroiAutoTargets(forceAll) {
            const seen = new Set();
            const out = [];
            function consider(row, d) {
                if (!chPromoDilSgroiEligible(d)) return;
                const sku = chPromoSkuKey(chPromoSku(d));
                if (!sku || seen.has(sku)) return;
                seen.add(sku);
                const slab = chPromoDilSlabKey(chPromoDil(d));
                const prev = chPromoDilSgroiLastSlab[sku];
            ...
            chPromoDilPrmtDatasetRows().forEach(function(d) {
                if (!d) return;
                consider(chPromoDilPrmtRowAdapter(d), d);
            });
            chPromoSaveDilSgroiLastSlabs();
            return out;
        }
        function chPromoScheduleDilSgroiAutoApply(opts) {
            opts = opts || {};
            if (!chPromoDilSgroiEnabled()) return;
            if (!Object.keys(chPromoDilSgroiLastSlab).length) chPromoLoadDilSgroiLastSlabs();
            if (opts.saveRules) {
                clearTimeout(chPromoDilSgroiAutoSaveTimer);
                chPromoDilSgroiAutoSaveTimer = setTimeout(function() {
                    Promise.resolve(saveChPromoDilSgroiRules({ silent: true, fromAutoApply: true }))
                        .catch(function() { /* keep applying */ });
                }, 600);
            }
            if (chPromoDilSgroiAutoBusy) {
                chPromoDilSgroiAutoPending = opts;
                return;
            }
            clearTimeout(chPromoDilSgroiAutoTimer);
            chPromoDilSgroiAutoTimer = setTimeout(function() {
                chPromoRunDilSgroiAutoApply(opts);
            }, opts.force ? 300 : 500);
        }
        async function applyChPromoDilSgroiToTargets(targets, label, opts) {
            opts = opts || {};
            const silent = !!opts.silent;
            readChPromoDilSgroiRulesFromModal();
            if (!targets.length) {
                if (!silent) chPromoToast('error', 'No sold rows (AL30 > 0, INV > 0) to apply Dil vs Target SGROI');
                return;
            }
            const jobs = [];
            let filled = 0;
            ...
            try {
                for (let i = 0; i < targets.length; i++) {
                    ...
                    const d = item.row.getData();
                    const roi = chPromoDilSgroiForRow(d);
                    const target = (roi != null) ? chPromoSpriceFromTargetRoi(d, roi) : 0;
                    if (!(target > 0)) { skipped++; continue; }
                    const fill = chPromoFinalSpriceToSave(d, target);
                    if (!(fill > 0)) { skipped++; continue; }
                    const prev = chPromoGetSprice(d);
                    const patch = Object.assign(chPromoSpricePatch(fill), {
                        DIL_SGROI_APPLIED: true,
                        DIL_SGROI_TARGET: roi,
                    });
                    item.row.update(patch);
                    filled++;
                    if (!chPromoNearlyEqual(prev, fill)) {
                        jobs.push({ row: item.row, sku: chPromoSku(d), price: fill, sgroi: roi });
                    }
                }
            } finally {
                if (blocked) table.restoreRedraw();
                ...
                await chPromoMapLimit(jobs, 8, async function(job) {
                    ...
                    try {
                        if (chPromoShouldPersistClearThenSprice()) {
                            await chPromoPersistClearThenSprice(job.row, job.price, true, {
                                row: job.row,
                                sku: job.sku,
                            });
                        } else {
                            await saveChannelSpriceAndPromo(job.row, job.price, true, {
                                row: job.row,
                                sku: job.sku,
                            });
                        }
            ...
            chPromoStampDilSgroiLastSlabs(targets);
            if (silent) {
                if (filled || jobs.length) {
                    chPromoToast(
                        'success',
                        'Dil vs Target SGROI: Dil slab changed → S PRC updated on '
                            + (jobs.length || filled) + ' SKU(s).'
                    );
                }
                if (typeof table !== 'undefined' && table) {
                    try { table.redraw(true); } catch (e) { /* ignore */ }
            ...
            chPromoToast(
                ...
                'Dil vs Target SGROI (' + label + '): S PRC from Target SGROI → ' + filled + ' SKU(s)'
                    + (skipped ? ('; skipped ' + skipped) : '') + '.'
            );
            if (typeof table !== 'undefined' && table) {
                try { table.redraw(true); } catch (e) { /* ignore */ }
        ...
        async function chPromoRunDilSgroiAutoApply(opts) {
            opts = opts || {};
            if (!chPromoDilSgroiEnabled()) return;
            if (chPromoDilSgroiAutoBusy) {
                chPromoDilSgroiAutoPending = opts;
            ...
            if ((typeof table === 'undefined' || !table) && !chPromoDilPrmtDatasetRows().length) return;
            const targets = chPromoCollectDilSgroiAutoTargets(!!opts.force);
            if (!targets.length) {
                if (chPromoDilSgroiAutoPending) {
                    const next = chPromoDilSgroiAutoPending;
                    chPromoDilSgroiAutoPending = null;
                    chPromoScheduleDilSgroiAutoApply(next);
                }
                return;
            }
            chPromoDilSgroiAutoBusy = true;
            try {
                await applyChPromoDilSgroiToTargets(targets, 'dil-change', { silent: true });
            } catch (e) {
                /* next Dil change retries */
            } finally {
                chPromoDilSgroiAutoBusy = false;
                if (chPromoDilSgroiAutoPending) {
                    const next = chPromoDilSgroiAutoPending;
                    chPromoDilSgroiAutoPending = null;
                    chPromoScheduleDilSgroiAutoApply(next);
        ...
        function cascadeChPromoCvrCpnFromInput(inputEl) {
            ...
            if (chPromoUsesLiveCvrCpnSlabs()) chPromoSyncEbayCpnColumnFromSlabs();
            else if (chPromoUsesAmazonCvrDisc()) chPromoSyncCvrDiscColumnFromSlabs();
            if (typeof chPromoScheduleCvrCpnAutoApply === 'function') {
                chPromoScheduleCvrCpnAutoApply({ force: true, saveRules: true });
            }
        }
        ...
        async function loadChPromoCvrCpnRules() {
            ...
            try {
                ...
                $('#ch-promo-cvr-cpn-status').text(res && res.is_default
                    ? (chPromoUsesAmazonCvrDisc()
                        ? 'Using Amazon CVR Disc defaults (0.01–1% → 9 … > 7% → 0, no 0% slab). Apply to save.'
                        : 'Using first-time defaults (0.01–1% → 9 … > 7% → 0, no 0% slab). Save Rule to store. CPN% auto-applies when CVR slab changes.')
                    : (chPromoUsesAmazonCvrDisc()
        ...
        function saveChPromoCvrCpnRules(opts) {
            opts = opts || {};
            ...
            return $.ajax({
                ...
                if (res && Array.isArray(res.rules)) {
                    ...
                    if (!opts.silent) renderChPromoCvrCpnModalTable();
                }
                if (!opts.silent) $('#ch-promo-cvr-cpn-status').text('Saved.');
                if (chPromoUsesAmazonCvrDisc()) chPromoSyncCvrDiscColumnFromSlabs();
                else if (chPromoUsesLiveCvrCpnSlabs()) chPromoSyncEbayCpnColumnFromSlabs();
                if (!opts.fromAutoApply && typeof chPromoScheduleCvrCpnAutoApply === 'function') {
                    chPromoScheduleCvrCpnAutoApply({ force: true });
                }
                return res;
            });
        }
        ...
        function renderChPromoZeroSoldDilModalTable() {
            ...
            chPromoZeroSoldDilRules.forEach(function(r, idx) {
                ...
                $tb.append(
                    ...
                    + 'step="0.1" value="' + groi + '" data-idx="' + idx + '" title="Target GROI% for this Dil color">'
                    + '</td></tr>'
                );
            });
        }
        ...
        function chPromoSyncZeroSoldDilHelp() {
            ...
            const $ownerLine = $('#ch-promo-zero-sold-dil-owner-line');
            if ($ownerLine.length) {
                $ownerLine.text(chPromoSharesEbay1ZeroSoldRules()
                    ? 'Uses the same 0 Sold table as eBay 1.'
                    : (owner + ' has its own rule table.'));
            }
            ...
            } else if (typeof chPromoZeroSoldUsesTemuMinRoi === 'function' && chPromoZeroSoldUsesTemuMinRoi()) {
                const m = chPromoZeroSoldTakehomeMargin({});
                $formula.text('S PRC = (LP × (1 + target%/100) + Ship) / ' + (m > 0 ? m.toFixed(2) : 'margin'));
        ...
        async function loadChPromoZeroSoldDilRules() {
            ...
            try {
                ...
                if (chPromoZeroSoldUsesTemuMinRoi()) {
                    const minRoi = Number(res && res.min_roi);
                    chPromoZeroSoldMinRoi = (isFinite(minRoi) && minRoi > 0) ? minRoi : 0;
                    const $minRoi = $('#ch-promo-zero-sold-min-roi');
                    if ($minRoi.length) $minRoi.val(chPromoZeroSoldMinRoi > 0 ? chPromoZeroSoldMinRoi : '');
                }
                renderChPromoZeroSoldDilModalTable();
                const rulesOwner = chPromoSharesEbay1ZeroSoldRules()
                    ? 'eBay 1'
                    : (chPromoCfg.label || CHANNEL_PROMO_CHANNEL);
                $('#ch-promo-zero-sold-dil-status').text(res && res.is_default
                    ...
                    : 'Loaded saved 0 Sold rules for ' + rulesOwner + '.');
                chPromoScheduleZeroSoldDilAutoApply(chPromoZeroSoldUsesTemuMinRoi() ? { force: true } : {});
                if (typeof chPromoMarkEbaySpriceRuleReady === 'function') {
                    chPromoMarkEbaySpriceRuleReady('zeroSold');
                }
            } catch (e) {
                renderChPromoZeroSoldDilModalTable();
        ...
        function saveChPromoZeroSoldDilRules() {
            ...
            const payload = { rules: rules, _token: chPromoCsrf() };
            if (chPromoZeroSoldUsesTemuMinRoi()) {
                payload.min_roi = chPromoZeroSoldReadMinRoi();
                chPromoZeroSoldMinRoi = payload.min_roi;
            }
            return $.ajax({
                url: chPromoZeroSoldRulesUrl(),
                method: 'POST',
                ...
                if (chPromoZeroSoldUsesTemuMinRoi() && res && res.min_roi != null) {
                    const minRoi = Number(res.min_roi);
                    chPromoZeroSoldMinRoi = (isFinite(minRoi) && minRoi > 0) ? minRoi : 0;
                    const $minRoi = $('#ch-promo-zero-sold-min-roi');
                    if ($minRoi.length) $minRoi.val(chPromoZeroSoldMinRoi > 0 ? chPromoZeroSoldMinRoi : '');
                }
                $('#ch-promo-zero-sold-dil-status').text('Saved.');
                chPromoScheduleZeroSoldDilAutoApply({ force: true });
                return res;
            });
        }
        ...
        async function chPromoZeroSoldDilProcessNext() {
            ...
            if (run.index >= run.jobs.length) {
                ...
                chPromoToast(
                    ...
                    title + ' (' + run.label + '): S PRC from Target GROI% → ' + run.ok + ' row(s)'
        ...
        let chPromoZeroSoldLastDilBand = {};
        let chPromoZeroSoldDilAutoTimer = null;
        let chPromoZeroSoldDilAutoBusy = false;
        let chPromoZeroSoldDilAutoPending = null;
        function chPromoZeroSoldDilBandStoreKey() {
            return 'chPromoZeroSoldDilBand:' + CHANNEL_PROMO_CHANNEL;
        }
        function chPromoLoadZeroSoldLastDilBands() {
            try {
                const raw = localStorage.getItem(chPromoZeroSoldDilBandStoreKey());
                const obj = raw ? JSON.parse(raw) : {};
                chPromoZeroSoldLastDilBand = (obj && typeof obj === 'object') ? obj : {};
            } catch (e) {
                chPromoZeroSoldLastDilBand = {};
            }
        }
        function chPromoSaveZeroSoldLastDilBands() {
            try {
                localStorage.setItem(chPromoZeroSoldDilBandStoreKey(), JSON.stringify(chPromoZeroSoldLastDilBand));
            } catch (e) { /* ignore quota */ }
        }
        ...
        function chPromoZeroSoldDilRowAdapter(d) {
            return {
                ...
                update: function(patch) {
                    Object.assign(d, patch || {});
                    if (typeof chPromoPatchDatasetSprice === 'function') {
                        chPromoPatchDatasetSprice(chPromoSku(d), patch);
                    }
                },
                reformat: function() {},
            };
        }
        function chPromoCollectZeroSoldDilAutoTargets(forceAll) {
            const seen = new Set();
            const out = [];
            function consider(row, d) {
                if (!chPromoZeroSoldDilEligible(d)) return;
                const sku = chPromoSkuKey(chPromoSku(d));
                if (!sku || seen.has(sku)) return;
                seen.add(sku);
                const band = chPromoDilColorBand(chPromoZeroSoldDilPct(d));
                const roi = typeof chPromoZeroSoldGroiForRow === 'function'
                    ? chPromoZeroSoldGroiForRow(d)
                    : chPromoRoiForZeroSoldDil(chPromoZeroSoldDilPct(d));
                const target = chPromoSpriceFromTargetRoi(d, roi);
                if (!(target > 0)) return;
                const current = chPromoGetSprice(d);
                if (!forceAll && chPromoNearlyEqual(current, target)) {
                    chPromoZeroSoldLastDilBand[sku] = band;
                    return;
                }
                out.push({ row: row, d: d });
            }
            ...
            extra.forEach(function(d) {
                if (!d) return;
                consider(chPromoZeroSoldDilRowAdapter(d), d);
            });
            chPromoSaveZeroSoldLastDilBands();
            return out;
        }
        function chPromoScheduleZeroSoldDilAutoApply(opts) {
            opts = opts || {};
            if (!CHANNEL_PROMO_SHOW_ZERO_SOLD_DIL_RULE) return;
            if (CHANNEL_PROMO_CHANNEL === 'aliexpress' && chPromoZeroSoldDilRun && chPromoZeroSoldDilRun.active) return;
            if (!Object.keys(chPromoZeroSoldLastDilBand).length) chPromoLoadZeroSoldLastDilBands();
            if (chPromoZeroSoldDilAutoBusy) {
                chPromoZeroSoldDilAutoPending = opts;
                return;
            }
            clearTimeout(chPromoZeroSoldDilAutoTimer);
            chPromoZeroSoldDilAutoTimer = setTimeout(function() {
                chPromoRunZeroSoldDilAutoApply(opts);
            }, opts.force ? 250 : 500);
        }
        async function chPromoRunZeroSoldDilAutoApply(opts) {
            ...
            if ((typeof table === 'undefined' || !table)
                && !(typeof window !== 'undefined' && Array.isArray(window.allTableData) && window.allTableData.length)) {
                return;
            }
            if (chPromoZeroSoldDilRun && chPromoZeroSoldDilRun.active) return;
            const targets = chPromoCollectZeroSoldDilAutoTargets(!!opts.force);
            if (!targets.length) {
                if (chPromoZeroSoldDilAutoPending) {
                    const next = chPromoZeroSoldDilAutoPending;
                    chPromoZeroSoldDilAutoPending = null;
                    chPromoScheduleZeroSoldDilAutoApply(next);
                }
                return;
            }
            chPromoZeroSoldDilAutoBusy = true;
            try {
                await applyChPromoZeroSoldDilToTargets(targets, 'dil-change', { silent: true });
            } catch (e) {
                /* keep last-band map; next Dil change retries */
            } finally {
                chPromoZeroSoldDilAutoBusy = false;
                if (chPromoZeroSoldDilAutoPending) {
                    const next = chPromoZeroSoldDilAutoPending;
                    chPromoZeroSoldDilAutoPending = null;
                    chPromoScheduleZeroSoldDilAutoApply(next);
        ...
        async function applyChPromoZeroSoldDilToTargets(targets, label, opts) {
            opts = opts || {};
            const silent = !!opts.silent;
            ...
            try {
                for (let i = 0; i < targets.length; i++) {
                    ...
                    const zsShip = chPromoIsFairePromoChannel() ? 0 : chPromoShipCost(d);
                    const sgpft = fill > 0
                        ? Math.round(((fill * zsMargin - chPromoLp(d) - zsShip) / fill) * 100)
                        : 0;
                    const sroi = chPromoLp(d) > 0
                        ? Math.round(((fill * zsMargin - chPromoLp(d) - zsShip) / chPromoLp(d)) * 100)
                        : 0;
            ...
            // AliExpress: sequential apply with Pause/Resume
            if (CHANNEL_PROMO_CHANNEL === 'aliexpress' && !silent) {
            ...
            if (silent) {
                if (ok) {
                    chPromoToast(
                        fail && !ok ? 'error' : 'success',
                        '0 Sold: S PRC autofilled from 0 Sold rule on ' + ok + ' SKU(s)'
                            + (fail ? (', ' + fail + ' failed') : '') + '.'
                    );
                }
                chPromoOverwriteStoredSpriceFromRules({ silent: true, force: true });
                return;
            }
            chPromoToast(
                fail && !ok ? 'error' : 'success',
                '0 Sold (' + label + '): S PRC from Dil→Target GROI% → ' + ok + ' row(s)'
                    + (fail ? (', ' + fail + ' failed') : '')
                    + (skipped ? (', ' + skipped + ' skipped') : '') + '.'
            );
            chPromoOverwriteStoredSpriceFromRules({ silent: true, force: true });
        ...
        async function applyChPromoDilPrmtToTargets(targets, label, opts) {
            if (chPromoHideDilPrmt()) return;
            opts = opts || {};
            const silent = !!opts.silent;
            readChPromoDilPrmtRulesFromModal();
            if (!targets.length) {
                if (!silent) chPromoToast('error', 'No rows to apply');
                return;
            }
            ...
            if ((CHANNEL_PROMO_SHOW_ZERO_SOLD_RULES || CHANNEL_PROMO_CHANNEL === 'reverb') && !ebayParentDil) {
                ...
                if (opts.zeroSoldOnly) {
                    ...
                    if (!applyTargets.length) {
                        if (!silent) chPromoToast('error', 'No 0 Sold rows (' + soldLabel + ' = 0) to apply');
                        return;
                    }
                } else {
                    applyTargets = applyTargets.filter(function(item) {
                    ...
                    if (!applyTargets.length) {
                        if (!silent) chPromoToast('error', 'No sold rows (' + soldLabel + ' > 0) to apply Dil vs PRMT');
                        return;
                    }
                }
            }
            if (!ebayParentDil && !opts.zeroSoldOnly) {
                if (chPromoSaleGatedApply() && !chPromoUsesAmazonDilPrmtSlabs()) {
                    ...
                    if (!applyTargets.length) {
                        if (!silent) {
                            chPromoToast('error', CHANNEL_PROMO_CHANNEL === 'shopify_b2c'
                                ? 'No >0 Sold SKUs to apply Dil vs PRMT (0 Sold uses the 0 Sold rules)'
                                : 'No SKUs to apply Dil vs PRMT');
                        }
                        return;
                    }
                }
            ...
            if (ebayParentDil) {
                ...
                applyTargets = targets.filter(function(item) {
                    const d = (item.d || (item.row && item.row.getData())) || {};
                    if (!chPromoIsChildRow(d)) return false;
                    if (typeof chPromoIsZeroSoldRow === 'function' && chPromoIsZeroSoldRow(d)) return false;
                ...
                if (!applyTargets.length) {
                    if (!silent) chPromoToast('error', 'No rows with eBay sale (E L30) > 0 to apply');
                    return;
                }
            ...
            if (!opts.zeroSoldOnly && typeof chPromoStampDilLastSlabs === 'function') {
                chPromoStampDilLastSlabs(applyTargets);
            }
            if (silent) {
                if (!opts.zeroSoldOnly && (filled || jobs.length)) {
                    chPromoToast(
                        'success',
                        'Dil vs PRMT: Dil slab changed → PRMT% updated on ' + (jobs.length || applyTargets.length) + ' SKU(s).'
                    );
                }
        ...
        let chPromoDilLastSlab = {};
        let chPromoDilPrmtAutoTimer = null;
        let chPromoDilPrmtAutoBusy = false;
        let chPromoDilPrmtAutoPending = null;
        let chPromoDilPrmtAutoSaveTimer = null;
        function chPromoDilSlabStoreKey() {
            return 'chPromoDilPrmtSlab:' + CHANNEL_PROMO_CHANNEL;
        }
        function chPromoLoadDilLastSlabs() {
            try {
                const raw = localStorage.getItem(chPromoDilSlabStoreKey());
                const obj = raw ? JSON.parse(raw) : {};
                chPromoDilLastSlab = (obj && typeof obj === 'object') ? obj : {};
            } catch (e) {
                chPromoDilLastSlab = {};
            }
        }
        function chPromoSaveDilLastSlabs() {
            try {
                localStorage.setItem(chPromoDilSlabStoreKey(), JSON.stringify(chPromoDilLastSlab));
            } catch (e) { /* ignore quota */ }
        }
        function chPromoStampDilLastSlabs(items) {
            (items || []).forEach(function(item) {
                const d = (item && (item.d || (item.row && item.row.getData()))) || {};
                const sku = chPromoSkuKey(chPromoSku(d));
                if (!sku) return;
                chPromoDilLastSlab[sku] = chPromoDilSlabKey(chPromoDil(d));
            });
            chPromoSaveDilLastSlabs();
        }
        function chPromoDilPrmtEligible(d) {
            if (!d || !chPromoIsChildRow(d)) return false;
            if (chPromoIsEbayChannel()
                && typeof chPromoIsZeroSoldRow === 'function'
                && chPromoIsZeroSoldRow(d)) {
        ...
        function chPromoDilPrmtDatasetRows() {
        ...
        function chPromoDilPrmtRowAdapter(d) {
            return {
                getData: function() { return d; },
                update: function(patch) {
                    Object.assign(d, patch || {});
        ...
        function chPromoCollectDilPrmtAutoTargets(forceAll) {
            const seen = new Set();
            const out = [];
            function consider(row, d) {
                if (!chPromoDilPrmtEligible(d)) return;
                const sku = chPromoSkuKey(chPromoSku(d));
                if (!sku || seen.has(sku)) return;
                seen.add(sku);
                const slab = chPromoDilSlabKey(chPromoDil(d));
        ...
        function chPromoScheduleDilPrmtAutoApply(opts) {
            if (chPromoHideDilPrmt()) return;
            if (chPromoUsesSprcDilInsteadOfPrmt()) return;
            opts = opts || {};
            if (!Object.keys(chPromoDilLastSlab).length) chPromoLoadDilLastSlabs();
            if (opts.saveRules) {
                clearTimeout(chPromoDilPrmtAutoSaveTimer);
                chPromoDilPrmtAutoSaveTimer = setTimeout(function() {
                    Promise.resolve(saveChPromoDilPrmtRules({ silent: true, fromAutoApply: true }))
                        .catch(function() { /* keep applying */ });
                }, 600);
            }
            if (chPromoDilPrmtAutoBusy) {
                chPromoDilPrmtAutoPending = opts;
                return;
            }
            clearTimeout(chPromoDilPrmtAutoTimer);
            chPromoDilPrmtAutoTimer = setTimeout(function() {
                chPromoRunDilPrmtAutoApply(opts);
            }, opts.force ? 300 : 500);
        }
        async function chPromoRunDilPrmtAutoApply(opts) {
            if (chPromoHideDilPrmt()) return;
            opts = opts || {};
            if (chPromoDilPrmtAutoBusy) {
                chPromoDilPrmtAutoPending = opts;
            ...
            if ((typeof table === 'undefined' || !table) && !chPromoDilPrmtDatasetRows().length) return;
            const targets = chPromoCollectDilPrmtAutoTargets(!!opts.force);
            if (!targets.length) {
                if (chPromoDilPrmtAutoPending) {
                    const next = chPromoDilPrmtAutoPending;
                    chPromoDilPrmtAutoPending = null;
                    chPromoScheduleDilPrmtAutoApply(next);
                }
                return;
            }
        ...
        let chPromoCvrLastSlab = {};
        let chPromoCvrCpnAutoTimer = null;
        let chPromoCvrCpnAutoBusy = false;
        let chPromoCvrCpnAutoPending = null;
        let chPromoCvrCpnAutoSaveTimer = null;
        function chPromoCvrSlabStoreKey() {
            return 'chPromoCvrCpnSlab:' + CHANNEL_PROMO_CHANNEL;
        }
        function chPromoLoadCvrLastSlabs() {
            try {
                const raw = localStorage.getItem(chPromoCvrSlabStoreKey());
                const obj = raw ? JSON.parse(raw) : {};
                chPromoCvrLastSlab = (obj && typeof obj === 'object') ? obj : {};
            } catch (e) {
                chPromoCvrLastSlab = {};
            }
        }
        function chPromoSaveCvrLastSlabs() {
            try {
                localStorage.setItem(chPromoCvrSlabStoreKey(), JSON.stringify(chPromoCvrLastSlab));
            } catch (e) { /* ignore quota */ }
        }
        function chPromoCvrCpnEligible(d) {
            if (!d || !chPromoIsChildRow(d)) return false;
            if (chPromoUsesAmazonCvrDisc()) return false;
            if (chPromoSaleGatedApply() && typeof chPromoHasSaleQty === 'function' && !chPromoHasSaleQty(d)) {
                return false;
            }
            return true;
        }
        function chPromoCvrDatasetRows() {
            if (typeof window !== 'undefined' && Array.isArray(window.allTableData) && window.allTableData.length) {
                return window.allTableData;
            }
            if (typeof allTableData !== 'undefined' && Array.isArray(allTableData) && allTableData.length) {
        ...
        function chPromoCvrRowAdapter(d) {
            return {
                getData: function() { return d; },
                update: function(patch) {
                    Object.assign(d, patch || {});
        ...
        function chPromoCollectCvrCpnAutoTargets(forceAll) {
            const seen = new Set();
            const out = [];
            function consider(row, d) {
                if (!chPromoCvrCpnEligible(d)) return;
                const sku = chPromoSkuKey(chPromoSku(d));
                if (!sku || seen.has(sku)) return;
                seen.add(sku);
            ...
            chPromoCvrDatasetRows().forEach(function(d) {
                if (!d) return;
                consider(chPromoCvrRowAdapter(d), d);
            });
            chPromoSaveCvrLastSlabs();
            return out;
        }
        function chPromoScheduleCvrCpnAutoApply(opts) {
            opts = opts || {};
            if (CHANNEL_PROMO_HIDE_CVR_CPN || chPromoUsesAmazonCvrDisc()) return;
            if (!Object.keys(chPromoCvrLastSlab).length) chPromoLoadCvrLastSlabs();
            if (opts.saveRules) {
                clearTimeout(chPromoCvrCpnAutoSaveTimer);
                chPromoCvrCpnAutoSaveTimer = setTimeout(function() {
                    Promise.resolve(saveChPromoCvrCpnRules({ silent: true, fromAutoApply: true }))
                        .catch(function() { /* keep applying */ });
                }, 600);
            }
            if (chPromoCvrCpnAutoBusy) {
                chPromoCvrCpnAutoPending = opts;
                return;
            }
            clearTimeout(chPromoCvrCpnAutoTimer);
            chPromoCvrCpnAutoTimer = setTimeout(function() {
                chPromoRunCvrCpnAutoApply(opts);
            }, opts.force ? 300 : 500);
        }
        async function chPromoRunCvrCpnAutoApply(opts) {
            opts = opts || {};
            if (chPromoCvrCpnAutoBusy) {
                chPromoCvrCpnAutoPending = opts;
                return;
            }
            if ((typeof table === 'undefined' || !table) && !chPromoCvrDatasetRows().length) return;
            const targets = chPromoCollectCvrCpnAutoTargets(!!opts.force);
            if (!targets.length) {
                if (chPromoCvrCpnAutoPending) {
                    const next = chPromoCvrCpnAutoPending;
                    chPromoCvrCpnAutoPending = null;
                    chPromoScheduleCvrCpnAutoApply(next);
                }
                return;
            }
        ...
        async function applyChPromoCvrCpnToTargets(targets, label, opts) {
            if (CHANNEL_PROMO_HIDE_CVR_CPN) return;
            opts = opts || {};
            const silent = !!opts.silent;
            ...
            if (chPromoSaleGatedApply() && !chPromoUsesAmazonCvrDisc()) {
                ...
                if (!targets.length) {
                    if (!silent) chPromoToast('error', 'No rows with ' + chPromoSaleGateLabel() + ' to apply');
                    return;
                }
            }
            ...
            jobs.forEach(function(job) {
                const skuKey = chPromoSkuKey(job.sku);
                if (!skuKey || !job.row) return;
                const d = job.row.getData() || {};
                chPromoCvrLastSlab[skuKey] = chPromoCvrSlabKey(chPromoCvr(d));
            });
            chPromoSaveCvrLastSlabs();
            if (silent) {
                if (ok || jobs.length) {
                    chPromoToast(
                        ok ? 'success' : 'success',
                        'CVR%: slab changed → CPN% updated on ' + jobs.length + ' SKU(s).'
                    );
                }
                chPromoOverwriteStoredSpriceFromRules({ silent: true, force: true });
                return;
        ...
        function computeChannelPushPrcPlan(d) {
            ...
            const zeroSoldGroi = (chPromoIsEbayChannel() && typeof chPromoZeroSoldRuleSprice === 'function')
                ? Number(chPromoZeroSoldRuleSprice(d)) || 0
                : 0;
            if (!(stdRaw > 0) && !(zeroSoldGroi > 0)) return null;
            const std = stdRaw > 0 ? chPromoRound2(stdRaw) : chPromoRound2(zeroSoldGroi);
            if (zeroSoldGroi > 0) {
                const sale = chPromoRound2(zeroSoldGroi);
                return {
                    std: std,
                    sale: sale,
                    max: std > 0 ? chPromoRound2(std * 1.10) : sale,
                    min: sale,
                    business: sale,
                    prmt: 0,
                    zeroSold: sale,
                    totalDisc: 0,
                    cpn: Math.max(0, Number(d.cpn_pct != null ? d.cpn_pct : d._cpn_pct_applied) || 0),
                    effective: sale,
                };
            }
            const prmt = Math.max(0, Number(d.prmt_pct != null ? d.prmt_pct : d._prmt_pct_applied) || 0);
        ...
        function chPromoSpriceFromStdTPromo(d, extra) {
            ...
            const finish = function(p) {
                if (!(p >= 0.01)) return 0;
                let out = (extra.skip_lmp_cap || chPromoIsFairePromoChannel())
                    ? chPromoRound2(p)
        ...
        function applyChannelSpriceFromStdChange(row, opts) {
            ...
            const start = zeroSold > 0
                ...
                : (chPromoUsesPrmtCpnDiscountedStart()
                    ? chPromoTemuDiscountedStart(d)
                    : chPromoFloorShopifySpriceToAmz(d, chPromoSpriceFromStdTPromo(d)));
            let fill = zeroSold > 0
                ? zeroSold
                : chPromoResolveTemuSprice(d, start, { use_passed_as_discounted: start > 0 });
        ...
        function autopopulateEbaySpriceFromStdPrmtCpn(opts) {
            ...
            const forceSkipPush = opts.skip_push === true;
            const jobs = [];
            const blocked = typeof table.blockRedraw === 'function';
            if (blocked) table.blockRedraw();
                ...
                const start = zeroSold > 0
                    ...
                    : (dilSgroi > 0
                        ...
                        : (chPromoUsesPrmtCpnDiscountedStart()
                            ? chPromoTemuDiscountedStart(d)
                            : chPromoFloorShopifySpriceToAmz(d, chPromoSpriceFromStdTPromo(d))));
                const fill = (zeroSold > 0 || dilSgroi > 0)
                    ? (zeroSold > 0 ? zeroSold : dilSgroi)
                    : chPromoResolveTemuSprice(d, start, { use_passed_as_discounted: start > 0 });
                if (!(fill > 0)) return;
            ...
            if (chPromoIsTemuPromoChannel() && typeof enqueueTemuListingPushAfterSave === 'function') {
                jobs.filter(function(j) { return !j.skip_push; }).forEach(function(j) {
                    enqueueTemuListingPushAfterSave(j.sku, j.price, j.row);
                });
            } else if (livePushOn && pushCount && typeof enqueueChannelPushSprice === 'function') {
        ...
        let chPromoEbaySpriceRulesReady = {
            dil: !!CHANNEL_PROMO_HIDE_DIL_PRMT,
            cvr: !!CHANNEL_PROMO_HIDE_CVR_CPN,
            zeroSold: !CHANNEL_PROMO_SHOW_ZERO_SOLD_DIL_RULE,
        };
        ...
        function chPromoMarkEbaySpriceRuleReady(kind) {
            ...
            if (kind === 'zeroSold') chPromoEbaySpriceRulesReady.zeroSold = true;
        ...
        setTimeout(function() {
            ...
            chPromoEbaySpriceRulesReady.zeroSold = true;
            chPromoTryQueueAfterSlabs();
        }, 4000);
        ...
        const CH_PROMO_RULE_SPRICE_CLEAR_KEY = 'chPromoRuleSpriceClearedOnce:v2:' + CHANNEL_PROMO_CHANNEL;
        function chPromoShouldClearStoredOnce() {
            try { return !localStorage.getItem(CH_PROMO_RULE_SPRICE_CLEAR_KEY); }
            catch (e) { return true; }
        }
        function chPromoMarkStoredClearedOnce() {
            try { localStorage.setItem(CH_PROMO_RULE_SPRICE_CLEAR_KEY, '1'); }
            catch (e) { /* ignore */ }
        }
        function chPromoOverwriteStoredSpriceFromRules(extra) {
            extra = extra || {};
            autopopulateEbaySpriceFromStdPrmtCpn(Object.assign({
                overwrite: true,
                persist: true,
        ...
        function fillSpriceFromTPromo() {
            ...
            function next() {
                ...
                const item = ready[i++];
                const rowData = item.row.getData();
                const start = chPromoUsesPrmtCpnDiscountedStart()
                    ? chPromoTemuDiscountedStart(rowData)
                    : chPromoFloorShopifySpriceToAmz(rowData, chPromoSpriceFromStdTPromo(rowData));
                const fill = chPromoResolveTemuSprice(rowData, start, { use_passed_as_discounted: start > 0 });
                $btn.html('<i class="fas fa-spinner fa-spin"></i> ' + i + '/' + ready.length);
                if (!(fill > 0)) {
                    fail++;
                    next();
        ...
        function channelPromoPricingColumns() {
                    ...
                    sorter: chPromoUsesAmazonCvrDisc()
                        ...
                        : function(a, b, aRow, bRow) {
                            const valOf = function(row) {
                                ...
                                return Number(d.cpn_pct != null && d.cpn_pct !== ''
                                    ? d.cpn_pct : d._cpn_pct_applied) || 0;
                            };
                            return valOf(aRow) - valOf(bRow);
                        },
                    ...
                    formatter: function(cell) {
                        ...
                        if (typeof chPromoIsAeStyleSpriceChannel === 'function' && chPromoIsAeStyleSpriceChannel()
                            ...
                            if (slab != null) {
                                ...
                                const cvr = chPromoCvr(d);
                                const key = chPromoCvrSlabKey(cvr);
                                tip = 'CVR ' + (isFinite(cvr) ? (Math.round(cvr * 100) / 100) : 0)
                                    + '% → ' + key + ' slab → cvr % ' + slab;
                            }
                        }
                        const shown = (val === null || val === undefined || val === '') ? 0 : val;
                        const dot = chPromoHistoryDotHtml(sku, 'cpn', shown);
                        return '<span style="display:inline-flex;align-items:center;justify-content:center;gap:3px;"'
                            + (tip ? (' title="' + chPromoEscAttr(tip) + '"') : '') + '>'
                            + dot + fmtChPromoCell(shown, '%') + '</span>';
                    },
                    cellClick: function(e) {
                        const el = e.target.closest('.view-sku-chart') || e.target.closest('.ch-pef-hist-dot');
                        if (!el) return;
            ...
            if (chPromoHideDilPrmt()) {
                return cols.filter(function(c) {
                    if (!c) return false;
                    if (c.field === 'prmt_pct') return false;
                    if (!chPromoUsesSprcDilInsteadOfPrmt()) return true;
                    return c.field !== 'cvr_up_dn'
                        && c.field !== 't_discounts'
                        && c.field !== 'zero_sold_prmt'
                        && c.field !== 'gt_sold_pct'
                        && c.field !== 'push_prmt';
                });
            }
        ...
        function initChannelPromoPricingUi() {
            ...
            if (!chPromoHideDilPrmt() && typeof loadChPromoDilPrmtRules === 'function') {
                chPromoLoadDilLastSlabs();
                Promise.resolve(loadChPromoDilPrmtRules()).catch(function() { /* defaults */ });
            } else if (chPromoHideDilPrmt() && typeof chPromoMarkEbaySpriceRuleReady === 'function') {
                chPromoMarkEbaySpriceRuleReady('dil');
            }
            if (typeof loadChPromoDilSgroiRules === 'function' && chPromoDilSgroiEnabled()) {
                chPromoLoadDilSgroiLastSlabs();
                Promise.resolve(loadChPromoDilSgroiRules()).catch(function() { /* defaults */ });
            }
            if (CHANNEL_PROMO_SHOW_ZERO_SOLD_DIL_RULE && typeof loadChPromoZeroSoldDilRules === 'function') {
                chPromoLoadZeroSoldLastDilBands();
                Promise.resolve(loadChPromoZeroSoldDilRules()).then(function() {
                    if (typeof table !== 'undefined' && table) {
                        try { table.redraw(true); } catch (e) { /* ignore */ }
                    }
                    chPromoScheduleZeroSoldDilAutoApply(
                        chPromoZeroSoldUsesTemuMinRoi() ? { force: true } : {}
                    );
                }).catch(function() { /* defaults */ });
            }
            if (!CHANNEL_PROMO_HIDE_CVR_CPN && typeof loadChPromoCvrCpnRules === 'function') {
                chPromoLoadCvrLastSlabs();
            ...
            if (!chPromoHideDilPrmt()) $('#ch-promo-dil-vs-prmt-btn').off('click.chpromo').on('click.chpromo', function(e) {
                e.preventDefault();
                ...
                if (chPromoIsEbayChannel()) {
                    ...
                    if (help) {
                        help.innerHTML = 'Shared Dil vs PRMT (<strong>all marketplaces</strong>): '
                            ...
                            + 'immediately fills the <strong>PRMT %</strong> column. '
                            + '<strong>PRMT% auto-applies when Dil slab changes</strong> on SKUs with '
                            + '<strong>eBay sale (E L30) &gt; 0</strong>. '
                            + '<strong>Save Rule</strong> stores the slabs. '
                    ...
                    if (help) {
                        help.innerHTML = 'Shared Dil vs PRMT (<strong>all marketplaces</strong>): '
                            ...
                            + '<strong>PRMT% auto-applies when Dil slab changes</strong> on selected or visible SKUs. '
                            + '<strong>Save Rule</strong> stores the slabs. '
                            + 'If <strong>INV = 0</strong>, PRMT% is <strong>0</strong>.'
                            + (CHANNEL_PROMO_CHANNEL === 'shopify_b2c'
                                ? ' If discounted S PRC is <strong>below Amz price</strong>, S PRC is raised to A Price (Amz label).'
                ...
                } else if (CHANNEL_PROMO_CHANNEL === 'temu' || CHANNEL_PROMO_CHANNEL === 'temu2' || CHANNEL_PROMO_CHANNEL === 'temu3') {
                    const help = document.getElementById('ch-promo-dil-prmt-help');
                    if (help) {
                        help.innerHTML = 'Map Dil% slabs to PRMT%. On Temu, Dil is <strong>SKU-wise</strong> '
                            + '(OV L30 ÷ INV). <strong>PRMT% auto-applies when Dil slab changes</strong> '
                            + 'on each SKU — parent rows are not changed. '
                            + '<strong>Save Rule</strong> stores the slabs. '
                    ...
                    if (help) {
                        help.innerHTML = 'Map Dil% slabs to PRMT%. Dil is <strong>SKU-wise</strong> '
                            ...
                            + '<strong>PRMT% auto-applies when Dil slab changes</strong> on '
                            + '<strong>selected or visible SKUs</strong>'
                            + (CHANNEL_PROMO_CHANNEL === 'aliexpress'
                            ...
                            + ' <strong>S PRC for sold SKUs comes from <em>Dil vs Target SGROI</em></strong> (the SGROI button), not from this PRMT% table. '
                            + '<strong>Save Rule</strong> stores the slabs. If INV is 0, PRMT% is <strong>0</strong>.';
                    }
                    ...
                    if (help) {
                        help.innerHTML = 'Map Dil% slabs to PRMT% for <strong>sold</strong> SKUs ('
                            ...
                            + '. '
                            + '<strong>PRMT% auto-applies when Dil slab changes</strong> on each sold SKU. '
                            + '<strong>Save Rule</strong> stores the slabs. '
                            + 'If INV is 0, PRMT% is <strong>0</strong>.';
                    }
                    ...
                    if (help) {
                        help.innerHTML = 'Shared Dil vs PRMT (<strong>0.01–3%</strong> … <strong>24–25%</strong>) — same table as /ebay-tabulator-view. '
                            + 'Dil is SKU-wise (OV L30 ÷ INV). '
                            + '<strong>PRMT% auto-applies when Dil slab changes</strong> '
                            + 'on SKUs with <strong>RV L30 &gt; 0</strong>. '
                            + '<strong>Save Rule</strong> stores the slabs. '
            ...
            $('#ch-promo-dil-prmt-apply-btn').off('click.chpromo').on('click.chpromo', saveChPromoDilPrmtRulesOnly);
            ...
            $('#ch-promo-dil-vs-sgroi-btn').off('click.chpromo').on('click.chpromo', function(e) {
                e.preventDefault();
                const modalEl = document.getElementById('chPromoDilVsSgroiModal');
                if (!modalEl) return;
                renderChPromoDilSgroiModalTable();
                bootstrap.Modal.getOrCreateInstance(modalEl).show();
            });
            $('#ch-promo-dil-sgroi-apply-btn').off('click.chpromo').on('click.chpromo', saveChPromoDilSgroiRulesOnly);
            $(document).off('input.chPromoDilSgroiCascade change.chPromoDilSgroiCascade', '#ch-promo-dil-sgroi-tbody .ch-promo-dil-sgroi-input')
                .on('input.chPromoDilSgroiCascade change.chPromoDilSgroiCascade', '#ch-promo-dil-sgroi-tbody .ch-promo-dil-sgroi-input', function() {
                    chPromoOnDilSgroiNumberChanged(this);
                });
                ...
                if (CHANNEL_PROMO_CHANNEL === 'reverb') {
                    ...
                    if (help) {
                        help.innerHTML = 'Map CVR% slabs to <strong>cvr %</strong> (same as /ebay-tabulator-view). '
                            + '<strong>cvr % auto-applies when CVR slab changes</strong> on SKUs with '
                    ...
                    if (help) {
                        help.innerHTML = 'Same CVR Disc map as <strong>/amazon-tabulator-view</strong>: '
                            + '<strong>0.01–1% → 9</strong> … '
                            + '<strong>6.5–7% → 1</strong>, <strong>&gt; 7% → 0</strong> (no 0% slab). '
                            + '<strong>INV = 0</strong> or <strong>CVR = 0%</strong> → Disc% is <strong>0</strong>. '
            ...
            if (CHANNEL_PROMO_SHOW_ZERO_SOLD_DIL_RULE) {
                ...
                $(document).on('input.chPromoZeroSoldDilAuto change.chPromoZeroSoldDilAuto', '#ch-promo-zero-sold-dil-tbody .ch-promo-zero-sold-dil-roi-input', function() {
                    readChPromoZeroSoldDilRulesFromModal();
                    chPromoScheduleZeroSoldDilAutoApply({ force: true });
                });
                $(document).off('input.chPromoZeroSoldMinRoiAuto change.chPromoZeroSoldMinRoiAuto', '#ch-promo-zero-sold-min-roi');
                $(document).on('input.chPromoZeroSoldMinRoiAuto change.chPromoZeroSoldMinRoiAuto', '#ch-promo-zero-sold-min-roi', function() {
                    chPromoZeroSoldMinRoi = chPromoZeroSoldReadMinRoi();
                    chPromoScheduleZeroSoldDilAutoApply({ force: true });
                });
                $('#ch-promo-zero-sold-dil-save-btn').off('click.chpromo').on('click.chpromo', async function(e) {
                    ...
                    try {
                        ...
                        chPromoToast('success', '0 Sold rules saved for this page.');
                    } catch (xhr) {
                        chPromoToast('error', 'Save failed: ' + ((xhr && xhr.responseJSON && xhr.responseJSON.message) || 'error'));
                    } finally {
                        $btn.prop('disabled', false).html(html);
        ...
        window.channelPromoAnalyticsColumns = function() {
            ...
            if (chPromoHideDilPrmt()) hide.prmt_pct = 1;
            if (CHANNEL_PROMO_HIDE_CVR_CPN) hide.cpn_pct = 1;
            if (chPromoUsesSprcDilInsteadOfPrmt()) {
                hide.prmt_pct = 1;
                hide.cvr_up_dn = 1;
                hide.t_discounts = 1;
                hide.zero_sold_prmt = 1;
                hide.gt_sold_pct = 1;
                hide.push_prmt = 1;
            }
        ...
        window.chPromoIsFairePromoChannel = chPromoIsFairePromoChannel;
        window.chPromoUsesPrmtCpnDiscountedStart = chPromoUsesPrmtCpnDiscountedStart;
        ...
        window.chPromoEbaySlabCpn = chPromoEbaySlabCpn;
        window.chPromoSyncEbayCpnColumnFromSlabs = chPromoSyncEbayCpnColumnFromSlabs;
        ...
        window.chPromoScheduleZeroSoldDilAutoApply = chPromoScheduleZeroSoldDilAutoApply;
        window.chPromoScheduleCvrCpnAutoApply = chPromoScheduleCvrCpnAutoApply;
        window.chPromoScheduleDilPrmtAutoApply = chPromoScheduleDilPrmtAutoApply;
