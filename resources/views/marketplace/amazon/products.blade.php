...
<div class="row">
    <div class="col-12">
        ...
        <p class="text-muted mb-3">
            ...
            <strong>Inv SKU Match / Inv SKU Mismatch</strong> = Shopify vs Amz quantity. Shopify qty must not be less than marketplace qty. Match allows marketplace to be short by at most max(3 units, 3% of Shopify). <strong>Linked mismatch SKU</strong> is only the SKUs where marketplace qty is higher than Shopify.
            <strong>Active SKU / Inactive SKU</strong> = actual Amz seller portal status (not inventory match).
            <em>Fetch new listings</em> pulls new Shopify SKUs into this table.
            Use <em>Link</em> on an unlinked row to match Shopify inventory to Amz (auto by seller SKU, or paste an ASIN).
            <em>Refresh live</em> warms Amz status.
        </p>
        ...
        @include('marketplace._queue-status', ['slug' => 'amazon'])

        @include('marketplace.amazon._nav', ['active' => 'products'])
            ...
            <div class="card-body">
                ...
                <form method="get" class="mb-3">
                    ...
                    @if(in_array(($linkTab ?? ''), ['matched_inactive', 'mismatch_inactive'], true) && empty($stateCacheReady) && (int) ($counts[$linkTab] ?? 0) === 0)
                        <p class="small text-muted mt-2 mb-0">Active / Inactive counts need the live Amz catalog — click <em>Refresh live</em>, wait a minute, then reload.</p>
                    @endif
                </form>
                    ...
                    <table class="table table-bordered table-hover mb-0 table-sm">
                        ...
                        <tbody>
                            @forelse($products as $p)
                                @php $detailUrl = !empty($p->shopify_sku_id) ? route('marketplace.products.show', ['marketplace' => 'amazon', 'shopifySku' => $p->shopify_sku_id]) : null; @endphp
                                <tr @if($detailUrl) style="cursor: pointer;" onclick="window.location='{{ $detailUrl }}'" @endif>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            {{ Str::limit($p->title ?? '—', 50) }}
                                        @endif
                                        @if(!empty($p->amazon_title) && $p->amazon_title !== $p->title)
                                            <div class="text-muted small">AE: {{ Str::limit($p->amazon_title, 40) }}</div>
                                        @endif
                                    </td>
                                    <td class="small">{{ $p->product_id ?? '—' }}</td>
                                    <td class="small">
                                        @if(!empty($p->amazon_state))
                                            @php $st = strtolower((string)$p->amazon_state); @endphp
                                            <span class="badge {{ $st === 'active' ? 'bg-success-subtle text-success' : ($st === 'inactive' ? 'bg-warning-subtle text-warning' : 'bg-light text-muted') }}">{{ $p->amazon_state }}</span>
                                        @else
                                            —
                                        @endif
                                    </td>
...
document.getElementById('btn-refresh-api')?.addEventListener('click', function () {
    ...
    var url = '{{ route('marketplace.manager.amazon.refresh') }}';
    var page = 1;
...
document.getElementById('btn-sync-mismatch-now')?.addEventListener('click', function () {
    ...
    if (!confirm('Push the actual live Shopify quantity to every Linked mismatch SKU on Amz right now (no queue)? This runs in batches and may take a few minutes.')) {
        return;
    }
    btn.disabled = true;
    var original = btn.innerHTML;
    var url = '{{ route('marketplace.manager.amazon.sync.mismatch.inventory') }}';
    var offset = 0;
