...
@php
    ...
    ob_start();
@endphp
@include('marketplace.amazon._detail-table', ['showEmpty' => true, 'rows' => $shopifyListingRows])
@php $shopifyListingHtml = ob_get_clean(); ob_start(); @endphp
@include('marketplace.amazon._detail-table', ['showEmpty' => true, 'rows' => $aeListingRows])
@php $aeListingHtml = ob_get_clean(); @endphp

<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.products', 'amazon') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Listings</a>

        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-3">
            <div>
                @include('marketplace._page-heading', ['slug' => 'amazon', 'heading' => $displayTitle, 'mt' => ''])
        ...
        @include('marketplace.amazon._compare-row', [
        ...
        @php
            ob_start();
        @endphp
        @include('marketplace.amazon._image-gallery', [
                            ...
                            <tbody>
                                @foreach($ae['variants'] as $v)
                                    <tr class="{{ ($v['sku'] ?? '') === ($s['sku'] ?? '') ? 'table-primary' : '' }}">
                                        <td class="small text-muted">{{ $v['source'] ?? ($aeSource === 'api' ? 'amazon' : 'shopify') }}</td>
                                        <td style="width:72px;">
                                            @if(!empty($v['image']))
...
document.getElementById('btn-pull-ae')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/amazon/products') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-sync-inventory')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/amazon/products') }}/' + id + '/sync-inventory', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
