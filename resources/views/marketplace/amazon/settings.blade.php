            ...
            <div class="settings-section">
                ...
                <div class="settings-section-body">
                    ...
                    <div class="sync-toggle-row">
                        ...
                        <div class="form-text ms-4">ON by default. Every 5 minutes the app reads Shopify fulfillments. If Shopify has no tracking yet, it tries Veeqo / GOFO (4Seller labels), fulfills the Shopify order, then confirms shipment on Amazon. A separate job also fetches tracking onto unfulfilled Shopify copies for every marketplace every 10 minutes.</div>
                    </div>
                    <div class="mt-2">
                    ...
                    <div class="mt-2">
                        ...
                        <input type="text" class="form-control form-control-sm" name="order[shopify_order_tags]" value="{{ implode(', ', $settings['order']['shopify_order_tags'] ?? ['amazon']) }}" style="max-width: 400px;">
                    </div>
                </div>
            </div>
...
<script>
document.getElementById('amazon-settings-form')?.addEventListener('submit', function (e) {
    e.preventDefault();
    ...
    fetch('{{ route('marketplace.settings.save', 'amazon') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-sync-inventory-now')?.addEventListener('click', function () {
    ...
    fetch('{{ route('marketplace.manager.amazon.sync.inventory') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-sync-tracking-now')?.addEventListener('click', function () {
    ...
    status.className = 'ms-2 small text-muted';
    fetch('{{ route('marketplace.manager.amazon.sync.tracking') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
