@extends('layouts.vertical', ['title' => $title ?? 'Newegg — Listings', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        ...
        <p class="text-muted mb-3">
            ...
            <strong>Inv SKU Match / Inv SKU Mismatch</strong> = Shopify vs Newegg quantity. Shopify qty must not be less than marketplace qty. Match allows marketplace to be short by at most max(3 units, 3% of Shopify). <strong>Linked mismatch SKU</strong> is only the SKUs where marketplace qty is higher than Shopify.
            <strong>Active SKU / Inactive SKU</strong> = actual Newegg seller portal status (not inventory match).
            ...
            <div class="card-body">
                ...
                <form method="get" class="mb-3">
                    ...
                    @if(in_array(($linkTab ?? ''), ['matched_inactive', 'mismatch_inactive'], true) && empty($stateCacheReady) && (int) ($counts[$linkTab] ?? 0) === 0)
                        <p class="small text-muted mt-2 mb-0">Active / Inactive counts need the live Newegg catalog — click <em>Refresh live</em>, wait a minute, then reload.</p>
                    @endif
                </form>
                    ...
                    <table class="table table-bordered table-hover mb-0 table-sm">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>SKU</th>
                                <th>{{ ($linkTab ?? '') === 'not_in_shopify' ? 'Title (Newegg)' : 'Title (Shopify)' }}</th>
                                <th>Newegg ID</th>
                                <th>State</th>
                        ...
                        <tbody>
                            @forelse($products as $p)
                                @php $detailUrl = !empty($p->shopify_sku_id) ? route('marketplace.products.show', ['marketplace' => 'newegg', 'shopifySku' => $p->shopify_sku_id]) : null; @endphp
                                <tr @if($detailUrl) style="cursor: pointer;" onclick="window.location='{{ $detailUrl }}'" @endif>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            {{ Str::limit($p->title ?? '—', 50) }}
                                        @endif
                                        @if(!empty($p->newegg_title) && $p->newegg_title !== $p->title)
                                            <div class="text-muted small">AE: {{ Str::limit($p->newegg_title, 40) }}</div>
                                        @endif
                                    </td>
                                    <td class="small">{{ $p->product_id ?? '—' }}</td>
                                    <td class="small">
                                        @if(!empty($p->newegg_state))
                                            @php $st = strtolower((string)$p->newegg_state); @endphp
                                            <span class="badge {{ $st === 'active' ? 'bg-success-subtle text-success' : ($st === 'inactive' ? 'bg-warning-subtle text-warning' : 'bg-light text-muted') }}">{{ $p->newegg_state }}</span>
                                        @else
                                            —
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    <td colspan="11" class="text-center text-muted py-4">
                                        @if(($linkTab ?? 'all') === 'not_in_shopify')
                                            No Newegg listings found without a matching Shopify SKU.
                                        @else
                                            No Shopify SKUs found.
                                        @endif
                                        @if(($linkTab ?? 'all') === 'linked')
                                            None linked yet — click <strong>Sync Newegg link map</strong> after SKUs exist in Newegg.
                                        @elseif(($linkTab ?? 'all') === 'not_in_shopify')
                                            All synced Newegg SKUs appear to exist in your Shopify catalog, or run <strong>Sync Newegg link map</strong> first.
                                        @elseif($connected)
                                            Your Shopify catalog may be empty, or filters excluded all rows.
                                        @else
                                            <a href="{{ route('marketplace.manager.newegg.connect') }}">Connect Newegg</a> first.
                                        @endif
                                    </td>
                                </tr>
                            @endforelse
...
document.getElementById('btn-refresh-api')?.addEventListener('click', function () {
    ...
    var url = '{{ route('marketplace.manager.newegg.refresh') }}';
    var page = 1;
    ...
    function setProgress(pageNum, totalPage, totalUpserted, message, totalCount) {
        ...
        var extra = totalCount ? ' (' + totalCount + ' products on Newegg)' : '';
    ...
    if (!confirm('Sync all Newegg listings and refresh SKU ↔ product_id mappings? This may take a few minutes.')) {
        return;
    }
    ...
    function runPage(reset) {
        syncNext(reset).then(function (data) {
            if (!data.success && data.done) {
                ...
                btn.disabled = false;
                btn.innerHTML = '<i class="ri-refresh-line"></i> Sync Newegg link map';
                return;
            }
...
document.getElementById('btn-sync-mismatch-now')?.addEventListener('click', function () {
    ...
    if (!confirm('Push the actual live Shopify quantity to every Linked mismatch SKU on Newegg right now (no queue)? This runs in batches and may take a few minutes.')) {
        return;
    }
    btn.disabled = true;
