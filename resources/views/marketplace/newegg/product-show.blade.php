@extends('layouts.vertical', ['title' => $title ?? 'Newegg — Listing Detail', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@php
    ...
    $aeListingRows = [
        ...
        'Newegg Qty' => $linked ? ($ae['stock'] ?? $l['ae_stock'] ?? null) : null,
    ...
    ob_start();
@endphp
@include('marketplace.newegg._detail-table', ['showEmpty' => true, 'rows' => $shopifyListingRows])
@php $shopifyListingHtml = ob_get_clean(); ob_start(); @endphp
@include('marketplace.newegg._detail-table', ['showEmpty' => true, 'rows' => $aeListingRows])
@php $aeListingHtml = ob_get_clean(); @endphp

<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.products', 'newegg') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Listings</a>

        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-3">
            <div>
                @include('marketplace._page-heading', ['slug' => 'newegg', 'heading' => $displayTitle, 'mt' => ''])
                <div class="sku-hero"><code>{{ $s['sku'] ?? '—' }}</code></div>
                <div class="mt-2 d-flex flex-wrap gap-2 align-items-center">
                    @if($linked)
                        <span class="badge bg-success-subtle text-success">Linked on Newegg</span>
                    @else
                        <span class="badge bg-light text-muted">Not linked on Newegg</span>
                    @endif
                    @if($aeSource === 'api')
                        <span class="badge bg-info-subtle text-info source-pill">Newegg data: live API</span>
                    @elseif($aeSource === 'cached')
                        <span class="badge bg-warning-subtle text-warning source-pill">Newegg data: cached map</span>
                    @else
                        <span class="badge bg-light text-muted source-pill">Newegg data: not loaded</span>
                    @endif
        ...
        @if(!empty($inventoryMismatch))
            <div class="alert alert-warning py-2 small mb-3">
                Inventory mismatch: Shopify <strong>{{ (int) $shopifyQtyShow }}</strong> vs Newegg <strong>{{ (int) $mpQtyShow }}</strong>.
                Click <strong>Sync Inventory</strong> to push Shopify qty to Newegg now.
            </div>
        @else
            <div class="alert alert-info py-2 small mb-3">
                <strong>Compare view.</strong> Shopify (source) vs Newegg side by side.
            </div>
        @endif
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-md-6">
                <div class="card h-100 {{ !empty($inventoryMismatch) ? 'border-warning' : '' }}">
                    <div class="card-body py-3">
                        <div class="text-muted small">Newegg Qty</div>
        ...
        @if(!$linked)
            <div class="alert alert-secondary small">
                This Shopify SKU is not linked in <code>newegg_metric</code> yet.
                Go to <a href="{{ route('marketplace.products', 'newegg') }}">Listings</a> and click <strong>Sync Newegg link map</strong> to match SKUs.
            </div>
        @endif

        @include('marketplace.newegg._compare-row', [
            'rowTitle' => 'Listing details',
            'left' => $shopifyListingHtml,
            'right' => $aeListingHtml,
            'leftEmpty' => 'No Shopify listing data',
            'rightEmpty' => 'No Newegg listing data — link SKU and pull from Newegg',
        ])

        @php
            ob_start();
        @endphp
        @include('marketplace.newegg._image-gallery', [
        ...
        @include('marketplace.newegg._compare-row', [
            ...
            'rightEmpty' => 'No Newegg images — pull live product details',
        ])
                            ...
                            <tbody>
                                @foreach($ae['variants'] as $v)
                                    <tr class="{{ ($v['sku'] ?? '') === ($s['sku'] ?? '') ? 'table-primary' : '' }}">
                                        <td class="small text-muted">{{ $v['source'] ?? ($aeSource === 'api' ? 'newegg' : 'shopify') }}</td>
                                        <td style="width:72px;">
                                            @if(!empty($v['image']))
        ...
        @if($hasAttributes)
            ...
            @include('marketplace.newegg._compare-row', [
                ...
                'rightEmpty' => 'No Newegg properties',
            ])
        @endif
        ...
        @if(filled($shopifyDescHtml) || filled($aeDescHtml))
            @include('marketplace.newegg._compare-row', [
                ...
                'rightEmpty' => 'No Newegg description — pull live product details',
            ])
        @endif
    </div>
</div>
...
document.getElementById('btn-pull-ae')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/newegg/products') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-sync-inventory')?.addEventListener('click', function () {
    ...
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Push live Shopify quantity to Newegg for this SKU now (no queue)?')) return;
    btn.disabled = true;
    var original = btn.innerHTML;
    btn.innerHTML = '<i class="ri-loader-4-line"></i> Syncing…';
    fetch('{{ url('marketplace/newegg/products') }}/' + id + '/sync-inventory', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
