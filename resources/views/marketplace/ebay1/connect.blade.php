@extends('layouts.vertical', ['title' => $title ?? 'eBay 1 — Connect', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        ...
        @include('marketplace._page-heading', ['slug' => 'ebay1', 'heading' => 'eBay 1 — Connect', 'mb' => 'mb-3'])

        @include('marketplace.ebay1._nav', ['active' => 'connect'])
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <p class="text-muted mb-3">
                            Credentials are read from <code>.env</code> / <code>config/services.php</code> → <code>ebay1</code>.
                        ...
                        <button type="button" class="btn btn-primary" id="ebay1-test-connection">
                            <i class="ri-plink"></i> Test connection
                        </button>
                        <div id="ebay1-test-result" class="mt-3" style="display:none;"></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
(function () {
    const btn = document.getElementById('ebay1-test-connection');
    const box = document.getElementById('ebay1-test-result');
    if (!btn || !box) return;
    btn.addEventListener('click', async function () {
        ...
        try {
            const res = await fetch('{{ route('marketplace.manager.ebay1.test') }}', {
                method: 'POST',
                headers: {
