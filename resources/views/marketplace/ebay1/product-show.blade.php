@extends('layouts.vertical', ['title' => $title ?? 'eBay 1 — Listing Detail', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@php
    ...
    $aeListingRows = [
        ...
        'eBay 1 Qty' => $linked ? ($ae['stock'] ?? $l['ae_stock'] ?? null) : null,
    ...
    ob_start();
@endphp
@include('marketplace.ebay1._detail-table', ['showEmpty' => true, 'rows' => $shopifyListingRows])
@php $shopifyListingHtml = ob_get_clean(); ob_start(); @endphp
@include('marketplace.ebay1._detail-table', ['showEmpty' => true, 'rows' => $aeListingRows])
@php $aeListingHtml = ob_get_clean(); @endphp

<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.products', 'ebay1') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Listings</a>

        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-3">
            <div>
                @include('marketplace._page-heading', ['slug' => 'ebay1', 'heading' => $displayTitle, 'mt' => ''])
                <div class="sku-hero"><code>{{ $s['sku'] ?? '—' }}</code></div>
                <div class="mt-2 d-flex flex-wrap gap-2 align-items-center">
                    @if($linked)
                        <span class="badge bg-success-subtle text-success">Linked on eBay 1</span>
                    @else
                        <span class="badge bg-light text-muted">Not linked on eBay 1</span>
                    @endif
                    @if($aeSource === 'api')
                        <span class="badge bg-info-subtle text-info source-pill">eBay 1 data: live API</span>
                    @elseif($aeSource === 'cached')
                        <span class="badge bg-warning-subtle text-warning source-pill">eBay 1 data: cached map</span>
                    @else
                        <span class="badge bg-light text-muted source-pill">eBay 1 data: not loaded</span>
                    @endif
            ...
            <div class="d-flex gap-2 flex-wrap">
                @if($connected)
                    <button type="button" class="btn btn-sm btn-outline-primary" id="btn-pull-ae" data-id="{{ $shopifySkuId }}">
                        <i class="ri-download-cloud-line"></i> Pull from eBay 1
                    </button>
                @endif
        ...
        @if(!empty($inventoryMismatch))
            <div class="alert alert-warning py-2 small mb-3">
                Inventory mismatch: Shopify <strong>{{ (int) $shopifyQtyShow }}</strong> vs eBay 1 <strong>{{ (int) $mpQtyShow }}</strong>.
                Click <strong>Sync Inventory</strong> to push Shopify qty to eBay 1 now.
            </div>
        @else
            <div class="alert alert-info py-2 small mb-3">
                <strong>Compare view.</strong> Shopify (source) vs eBay 1 side by side.
            </div>
        @endif
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-md-6">
                <div class="card h-100 {{ !empty($inventoryMismatch) ? 'border-warning' : '' }}">
                    <div class="card-body py-3">
                        <div class="text-muted small">eBay 1 Qty</div>
        ...
        @if(!$linked)
            <div class="alert alert-secondary small">
                This Shopify SKU is not linked in <code>ebay_metrics</code> yet.
                Go to <a href="{{ route('marketplace.products', 'ebay1') }}">Listings</a> and click <strong>Sync eBay 1 link map</strong> to match SKUs.
            </div>
        @endif

        @include('marketplace.ebay1._compare-row', [
            'rowTitle' => 'Listing details',
            'left' => $shopifyListingHtml,
            'right' => $aeListingHtml,
            'leftEmpty' => 'No Shopify listing data',
            'rightEmpty' => 'No eBay 1 listing data — link SKU and pull from eBay 1',
        ])

        @php
            ob_start();
        @endphp
        @include('marketplace.ebay1._image-gallery', [
        ...
        @include('marketplace.ebay1._compare-row', [
            ...
            'rightEmpty' => 'No eBay 1 images — pull live product details',
        ])
                            ...
                            <tbody>
                                @foreach($ae['variants'] as $v)
                                    <tr class="{{ ($v['sku'] ?? '') === ($s['sku'] ?? '') ? 'table-primary' : '' }}">
                                        <td class="small text-muted">{{ $v['source'] ?? ($aeSource === 'api' ? 'ebay1' : 'shopify') }}</td>
                                        <td style="width:72px;">
                                            @if(!empty($v['image']))
        ...
        @if($hasAttributes)
            ...
            @include('marketplace.ebay1._compare-row', [
                ...
                'rightEmpty' => 'No eBay 1 properties',
            ])
        @endif
        ...
        @if(filled($shopifyDescHtml) || filled($aeDescHtml))
            @include('marketplace.ebay1._compare-row', [
                ...
                'rightEmpty' => 'No eBay 1 description — pull live product details',
            ])
        @endif
    </div>
</div>
...
document.getElementById('btn-pull-ae')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/ebay1/products') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
    ...
    .finally(function () {
        btn.disabled = false;
        btn.innerHTML = '<i class="ri-download-cloud-line"></i> Pull from eBay 1';
    });
});
...
document.getElementById('btn-sync-inventory')?.addEventListener('click', function () {
    ...
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Push live Shopify quantity to eBay 1 for this SKU now (no queue)?')) return;
    btn.disabled = true;
    var original = btn.innerHTML;
    btn.innerHTML = '<i class="ri-loader-4-line"></i> Syncing…';
    fetch('{{ url('marketplace/ebay1/products') }}/' + id + '/sync-inventory', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
