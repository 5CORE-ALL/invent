...
<div class="row">
    <div class="col-12">
        ...
        @include('marketplace._page-heading', ['slug' => 'temu2', 'heading' => 'Temu Sync Settings'])
        <p class="text-muted mb-3">Configure how Shopify (source) syncs with Temu 2 for pricing, inventory, and orders.</p>

        @include('marketplace.temu2._nav', ['active' => 'settings'])
        ...
        <form id="temu-settings-form">
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync prices from Shopify to Temu 2</span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync stock quantities from Shopify to Temu 2</span>
                        </label>
                    </div>
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Fetch orders from Temu 2 on schedule</span>
                        </label>
                        <div class="form-text ms-4">When on, the 15‑minute schedule pulls Temu 2 orders into our DB. Manual <strong>Fetch from Temu 2</strong> on the Orders page always works.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                    ...
                    <div class="sync-toggle-row">
                        ...
                        <div class="form-text ms-4">When on, unpaid / payment-pending Temu 2 orders stay in our DB and are not queued or manually pushed to Shopify. Turn this off to import unpaid orders.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Automatically sync Temu 2 customer / shipping address to Shopify</span>
                        </label>
                        <div class="form-text ms-4">ON by default. Every 15 minutes the app fills missing Shopify shipping/billing/customer address from Temu 2 — no manual Pull needed.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="order[push_tracking_to_temu2]" value="1" {{ ($settings['order']['push_tracking_to_temu2'] ?? true) ? 'checked' : '' }}>
                            <span class="form-check-label">Automatically push Shopify tracking numbers to Temu 2</span>
                        </label>
                        <div class="form-text ms-4">ON by default. Every 5 minutes the app reads Shopify fulfillments (after you print/download a label) and ships the order on Temu 2 — no manual push needed. You can still push per order from the order detail page.</div>
                    </div>
                    <div class="mt-2">
                        ...
                        <div class="form-text">Temu 2 orders import here. Default is main B2C store (same as <code>shopify_skus</code>).</div>
                    </div>
                    <div class="mt-2">
                        <label class="form-label small">Shopify channel handle (source_name)</label>
                        <input type="text" class="form-control form-control-sm" name="order[shopify_source_name]" value="{{ $settings['order']['shopify_source_name'] ?? 'temu2' }}" style="max-width: 400px;">
                    ...
                    <div class="mt-2">
                        ...
                        <input type="text" class="form-control form-control-sm" name="order[shopify_source_display_name]" value="{{ $settings['order']['shopify_source_display_name'] ?? 'Temu 2' }}" style="max-width: 400px;">
                    ...
                    <div class="mt-2">
                        ...
                        <input type="text" class="form-control form-control-sm" name="order[shopify_order_tags]" value="{{ implode(', ', $settings['order']['shopify_order_tags'] ?? ['temu2']) }}" style="max-width: 400px;">
                    </div>
                </div>
            </div>
            ...
            <div class="settings-section">
                ...
                <div class="settings-section-body">
                    <p class="text-muted small mb-2">Sync Temu 2 link map only reads Temu 2 and saves SKU mappings locally — it never creates listings on Temu 2. When <strong>Auto-link</strong> is on, this also runs hourly on schedule.</p>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                        ...
                        <div class="form-text ms-4">When on, refresh Temu 2 SKU ↔ product_id mappings hourly (same as manual Sync Temu 2 link map). Manual sync on Listings always works.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[create_products_on_temu2]" value="1" {{ ($settings['listings']['create_products_on_temu2'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Create new listings on Temu 2 from Shopify <span class="text-muted">(off for testing — not implemented yet)</span></span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[sync_title]" value="1" {{ ($settings['listings']['sync_title'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Push title updates to existing Temu 2 listings</span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[sync_images]" value="1" {{ ($settings['listings']['sync_images'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Push image updates to existing Temu 2 listings</span>
                        </label>
                    </div>
                </div>
            </div>
...
document.getElementById('temu-settings-form')?.addEventListener('submit', function (e) {
    ...
    fetch('{{ route('marketplace.settings.save', 'temu2') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-sync-inventory-now')?.addEventListener('click', function () {
    ...
    fetch('{{ route('marketplace.manager.temu2.sync.inventory') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-sync-tracking-now')?.addEventListener('click', function () {
    ...
    status.className = 'ms-2 small text-muted';
    fetch('{{ route('marketplace.manager.temu2.sync.tracking') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
