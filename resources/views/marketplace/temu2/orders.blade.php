@extends('layouts.vertical', ['title' => $title ?? 'Temu 2 — Orders', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])

@section('content')
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.manager.show', 'temu2') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Temu 2 Manager</a>
        @include('marketplace._page-heading', ['slug' => 'temu2', 'heading' => 'Temu 2 Orders'])
        <p class="text-muted mb-3">Orders from <strong>July 7, 2026</strong> onward. New orders are queued to Shopify automatically when auto-import is On in <a href="{{ route('marketplace.settings', 'temu2') }}">Settings</a>. If the order already exists in Shopify, it is linked — a second copy is not created. Use Push only for a one-off retry.</p>
