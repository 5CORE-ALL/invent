@extends('layouts.vertical', ['title' => $title ?? 'Temu — Listings', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        ...
        <p class="text-muted mb-3">
            ...
            <strong>Inv SKU Match / Inv SKU Mismatch</strong> = Shopify vs Temu quantity. Shopify qty must not be less than marketplace qty. Match allows marketplace to be short by at most max(3 units, 3% of Shopify). <strong>Linked mismatch SKU</strong> is only the SKUs where marketplace qty is higher than Shopify.
            <strong>Active SKU / Inactive SKU</strong> = actual Temu Seller Center status (not inventory match).
            <em>Refresh live</em> pulls Active/Inactive from Temu. Refresh Shopify from <a href="{{ route('marketplace.manager.index') }}">Marketplace Manager</a>.
        </p>
        ...
        @include('marketplace._queue-status', ['slug' => 'temu2'])

        @include('marketplace.temu2._nav', ['active' => 'products'])
            ...
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                <span class="badge bg-primary">
                    ...
                    @elseif(($linkTab ?? '') === 'unlinked')
                        {{ $products->total() }} not on Temu 2 (in-stock Shopify)
                ...
                <div class="d-flex gap-2 flex-wrap">
                    @if(in_array(($linkTab ?? ''), ['all', 'matched', 'matched_inactive', 'mismatch', 'linked_mismatch', 'mismatch_inactive', 'zero'], true))
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['clear_cache' => 1, 'refresh_live' => null]) }}" class="btn btn-sm btn-outline-secondary" onclick="return confirm('Clear the warm Temu 2 live listings cache? Counts will refresh after Refresh live.');">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary" id="btn-refresh-api">
                        <i class="ri-refresh-line"></i> Sync Temu 2 link map
                    </button>
                </div>
            </div>
            ...
            <div class="card-body">
                ...
                <form method="get" class="mb-3">
                    ...
                    @if(in_array(($linkTab ?? ''), ['matched_inactive', 'mismatch_inactive'], true) && empty($stateCacheReady) && (int) ($counts[$linkTab] ?? 0) === 0)
                        <p class="small text-muted mt-2 mb-0">Active / Inactive counts need the live Temu catalog — click <em>Refresh live</em>, wait a minute, then reload.</p>
                    @endif
                </form>
                ...
                <ul class="nav nav-tabs nav-bordered mb-3" role="tablist">
                    ...
                    <li class="nav-item">
                        <a href="{{ request()->url() }}?link=unlinked&search_name={{ $qName }}&search_sku={{ $qSku }}" class="nav-link {{ ($linkTab ?? '') === 'unlinked' ? 'active' : '' }}">Not on Temu 2 {{ $counts['unlinked'] ?? 0 }}</a>
                    </li>
                </ul>
                    ...
                    <table class="table table-bordered table-hover mb-0 table-sm">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>SKU</th>
                                <th>{{ ($linkTab ?? '') === 'not_in_shopify' ? 'Title (Temu 2)' : 'Title (Shopify)' }}</th>
                                <th>Temu ID</th>
                                <th>State</th>
                        ...
                        <tbody>
                            @forelse($products as $p)
                                @php $detailUrl = !empty($p->shopify_sku_id) ? route('marketplace.products.show', ['marketplace' => 'temu2', 'shopifySku' => $p->shopify_sku_id]) : null; @endphp
                                <tr @if($detailUrl) style="cursor: pointer;" onclick="window.location='{{ $detailUrl }}'" @endif>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            {{ Str::limit($p->title ?? '—', 50) }}
                                        @endif
                                        @if(!empty($p->temu_title) && $p->temu_title !== $p->title)
                                            <div class="text-muted small">AE: {{ Str::limit($p->temu_title, 40) }}</div>
                                        @endif
                                    </td>
                                    <td class="small">{{ $p->product_id ?? '—' }}</td>
                                    <td class="small">
                                        @if(!empty($p->temu_state))
                                            @php $st = strtolower((string)$p->temu_state); @endphp
                                            <span class="badge {{ $st === 'active' ? 'bg-success-subtle text-success' : ($st === 'inactive' ? 'bg-warning-subtle text-warning' : 'bg-light text-muted') }}">{{ $p->temu_state }}</span>
                                        @else
                                            —
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    <td colspan="11" class="text-center text-muted py-4">
                                        @if(($linkTab ?? 'all') === 'not_in_shopify')
                                            No Temu 2 listings found without a matching Shopify SKU.
                                        @elseif(($linkTab ?? '') === 'matched_inactive')
                                            No inactive Temu Seller Center listings found.
                                        @elseif(($linkTab ?? '') === 'mismatch_inactive')
                                            No active Temu Seller Center listings found.
                                        @else
                                            No Shopify SKUs found.
                                        @endif
                                        @if(($linkTab ?? 'all') === 'linked')
                                            None linked yet — click <strong>Sync Temu 2 link map</strong> after SKUs exist in Temu 2.
                                        @elseif(($linkTab ?? 'all') === 'not_in_shopify')
                                            All synced Temu 2 SKUs appear to exist in your Shopify catalog, or run <strong>Sync Temu 2 link map</strong> first.
                                        @elseif(in_array(($linkTab ?? ''), ['matched_inactive', 'mismatch_inactive'], true))
                                            Click <strong>Refresh live</strong> to pull Active / Inactive from Temu Seller Center.
                                        @elseif($connected)
                                            Your Shopify catalog may be empty, or filters excluded all rows.
                                        @else
                                            <a href="{{ route('marketplace.manager.temu2.connect') }}">Connect Temu</a> first.
                                        @endif
                                    </td>
                                </tr>
                            @endforelse
...
document.getElementById('btn-refresh-api')?.addEventListener('click', function () {
    ...
    var url = '{{ route('marketplace.manager.temu2.refresh') }}';
    var page = 1;
    ...
    function setProgress(pageNum, totalPage, totalUpserted, message, totalCount) {
        ...
        var extra = totalCount ? ' (' + totalCount + ' products on Temu)' : '';
    ...
    if (!confirm('Sync all Temu 2 listings and refresh SKU ↔ product_id mappings? This may take a few minutes.')) {
        return;
    }
    ...
    function runPage(reset) {
        syncNext(reset).then(function (data) {
            if (!data.success && data.done) {
                ...
                btn.disabled = false;
                btn.innerHTML = '<i class="ri-refresh-line"></i> Sync Temu 2 link map';
                return;
            }
...
document.getElementById('btn-sync-mismatch-now')?.addEventListener('click', function () {
    ...
    if (!confirm('Push the actual live Shopify quantity to every Linked mismatch SKU on Temu 2 now (no queue)? This runs in batches and may take a few minutes.')) {
        return;
    }
    btn.disabled = true;
    var original = btn.innerHTML;
    var url = '{{ route('marketplace.manager.temu2.sync.mismatch.inventory') }}';
    var offset = 0;
