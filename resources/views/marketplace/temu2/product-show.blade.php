...
@php
    ...
    ob_start();
@endphp
@include('marketplace.temu2._detail-table', ['showEmpty' => true, 'rows' => $shopifyListingRows])
@php $shopifyListingHtml = ob_get_clean(); ob_start(); @endphp
@include('marketplace.temu2._detail-table', ['showEmpty' => true, 'rows' => $aeListingRows])
@php $aeListingHtml = ob_get_clean(); @endphp

<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.products', 'temu2') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Listings</a>

        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-3">
            <div>
                @include('marketplace._page-heading', ['slug' => 'temu2', 'heading' => $displayTitle, 'mt' => ''])
        ...
        @if(!empty($inventoryMismatch))
            <div class="alert alert-warning py-2 small mb-3">
                Inventory mismatch: Shopify <strong>{{ (int) $shopifyQtyShow }}</strong> vs Temu 2 <strong>{{ (int) $mpQtyShow }}</strong>.
                Click <strong>Sync Inventory</strong> to push Shopify qty to Temu 2 now.
            </div>
        @else
            <div class="alert alert-info py-2 small mb-3">
                <strong>Compare view.</strong> Shopify (source) vs Temu 2 side by side.
            </div>
        @endif
        ...
        @if(!$linked)
            <div class="alert alert-secondary small">
                ...
                Go to <a href="{{ route('marketplace.products', 'temu2') }}">Listings</a> and click <strong>Sync Temu 2 link map</strong> to match SKUs.
            </div>
        @endif

        @include('marketplace.temu2._compare-row', [
            'rowTitle' => 'Listing details',
            'left' => $shopifyListingHtml,
            'right' => $aeListingHtml,
            'leftEmpty' => 'No Shopify listing data',
            'rightEmpty' => 'No Temu 2 listing data — link SKU and pull from Temu 2',
        ])

        @php
            ob_start();
        @endphp
        @include('marketplace.temu2._image-gallery', [
        ...
        @include('marketplace.temu2._compare-row', [
            ...
            'rightEmpty' => 'No Temu 2 images — pull live product details',
        ])
                            ...
                            <tbody>
                                @foreach($ae['variants'] as $v)
                                    <tr class="{{ ($v['sku'] ?? '') === ($s['sku'] ?? '') ? 'table-primary' : '' }}">
                                        <td class="small text-muted">{{ $v['source'] ?? ($aeSource === 'api' ? 'temu2' : 'shopify') }}</td>
                                        <td style="width:72px;">
                                            @if(!empty($v['image']))
        ...
        @if($hasAttributes)
            ...
            @include('marketplace.temu2._compare-row', [
                ...
                'rightEmpty' => 'No Temu 2 properties',
            ])
        @endif
        ...
        @if(filled($shopifyDescHtml) || filled($aeDescHtml))
            @include('marketplace.temu2._compare-row', [
                ...
                'rightEmpty' => 'No Temu 2 description — pull live product details',
            ])
        @endif
    </div>
</div>
...
document.getElementById('btn-pull-ae')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/temu2/products') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
    ...
    .finally(function () {
        btn.disabled = false;
        btn.innerHTML = '<i class="ri-download-cloud-line"></i> Pull from Temu 2';
    });
});
...
document.getElementById('btn-sync-inventory')?.addEventListener('click', function () {
    ...
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Push live Shopify quantity to Temu 2 for this SKU now (no queue)?')) return;
    btn.disabled = true;
    var original = btn.innerHTML;
    btn.innerHTML = '<i class="ri-loader-4-line"></i> Syncing…';
    fetch('{{ url('marketplace/temu2/products') }}/' + id + '/sync-inventory', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
