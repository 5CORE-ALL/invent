@extends('layouts.vertical', ['title' => $title ?? 'Best Buy — Settings', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.manager.show', 'bestbuy') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Best Buy Manager</a>
        @include('marketplace._page-heading', ['slug' => 'bestbuy', 'heading' => 'Best Buy Sync Settings'])
        <p class="text-muted mb-3">Configure how Shopify (source) syncs with Best Buy for pricing, inventory, and orders.</p>

        @include('marketplace.bestbuy._nav', ['active' => 'settings'])

        <form id="bestbuy-settings-form">
            @csrf
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync prices from Shopify to Best Buy</span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync stock quantities from Shopify to Best Buy</span>
                        </label>
                    </div>
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Fetch orders from Best Buy on schedule</span>
                        </label>
                        <div class="form-text ms-4">When on, the 15‑minute schedule pulls Best Buy orders into our DB. Manual <strong>Fetch from Best Buy</strong> on the Orders page always works.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Automatically import Best Buy orders to Shopify</span>
                        </label>
                        <div class="form-text ms-4">ON by default. New Best Buy orders are queued to Shopify on the 15‑minute schedule.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                        ...
                        <div class="form-text ms-4">When on, unpaid / payment-pending Best Buy orders stay in our DB and are not queued or manually pushed to Shopify. Turn this off to import unpaid orders.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Automatically sync Best Buy customer / shipping address to Shopify</span>
                        </label>
                        <div class="form-text ms-4">ON by default. Every 15 minutes the app fills missing Shopify shipping/billing/customer address from Best Buy — no manual Pull needed.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="order[push_tracking_to_bestbuy]" value="1" {{ ($settings['order']['push_tracking_to_bestbuy'] ?? true) ? 'checked' : '' }}>
                            <span class="form-check-label">Automatically push Shopify tracking numbers to Best Buy</span>
                        </label>
                        <div class="form-text ms-4">ON by default. Every 5 minutes the app reads Shopify fulfillments (after you print/download a label) and ships the order on Best Buy — no manual push needed. You can still push per order from the order detail page.</div>
                    </div>
                    <div class="mt-2">
                        ...
                        <div class="form-text">Best Buy orders import here. Default is main B2C store (same as <code>shopify_skus</code>).</div>
                    </div>
                    <div class="mt-2">
                        <label class="form-label small">Shopify channel handle (source_name)</label>
                        <input type="text" class="form-control form-control-sm" name="order[shopify_source_name]" value="{{ $settings['order']['shopify_source_name'] ?? 'bestbuy' }}" style="max-width: 400px;">
                        <div class="form-text">Case-sensitive handle registered in your Shopify app’s <strong>Marketplace extension</strong> (Partner Dashboard). Shopify shows this as the channel name (e.g. Best Buy) instead of the app name (5core).</div>
                    </div>
                    <div class="mt-2">
                        <label class="form-label small">Shopify channel display label</label>
                        <input type="text" class="form-control form-control-sm" name="order[shopify_source_display_name]" value="{{ $settings['order']['shopify_source_display_name'] ?? 'Best Buy' }}" style="max-width: 400px;">
                    ...
                    <div class="mt-2">
                        ...
                        <input type="text" class="form-control form-control-sm" name="order[shopify_order_tags]" value="{{ implode(', ', $settings['order']['shopify_order_tags'] ?? ['bestbuy']) }}" style="max-width: 400px;">
                    </div>
                </div>
            </div>
            ...
            <div class="settings-section">
                ...
                <div class="settings-section-body">
                    <p class="text-muted small mb-2">Sync Best Buy link map only reads Best Buy and saves SKU mappings locally — it never creates listings on Best Buy. When <strong>Auto-link</strong> is on, this also runs hourly on schedule.</p>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                        ...
                        <div class="form-text ms-4">When on, refresh Best Buy SKU ↔ product_id mappings hourly (same as manual Sync Best Buy link map). Manual sync on Listings always works.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[create_products_on_bestbuy]" value="1" {{ ($settings['listings']['create_products_on_bestbuy'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Create new listings on Best Buy from Shopify <span class="text-muted">(off for testing — not implemented yet)</span></span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[sync_title]" value="1" {{ ($settings['listings']['sync_title'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Push title updates to existing Best Buy listings</span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[sync_images]" value="1" {{ ($settings['listings']['sync_images'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Push image updates to existing Best Buy listings</span>
                        </label>
                    </div>
                </div>
            </div>
...
<script>
document.getElementById('bestbuy-settings-form')?.addEventListener('submit', function (e) {
    e.preventDefault();
    ...
    fetch('{{ route('marketplace.settings.save', 'bestbuy') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-sync-inventory-now')?.addEventListener('click', function () {
    ...
    fetch('{{ route('marketplace.manager.bestbuy.sync.inventory') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-sync-tracking-now')?.addEventListener('click', function () {
    ...
    status.className = 'ms-2 small text-muted';
    fetch('{{ route('marketplace.manager.bestbuy.sync.tracking') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
