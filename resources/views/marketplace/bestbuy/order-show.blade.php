@extends('layouts.vertical', ['title' => $title ?? 'Best Buy — Order Detail', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.orders', 'bestbuy') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Orders</a>
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-1">
            <div>
                @include('marketplace._page-heading', ['slug' => 'bestbuy', 'heading' => 'Order '.($summary['order_id'] ?? $orderId), 'mt' => ''])
                <p class="text-muted mb-0">
                    ...
                    @if($aeSource === 'api')
                        <span class="badge bg-info-subtle text-info ms-1">Best Buy data: live API</span>
                    @else
                        <span class="badge bg-warning-subtle text-warning ms-1">Best Buy data: cached</span>
                    @endif
                </p>
            </div>
        ...
        @include('marketplace.bestbuy._nav', ['active' => 'orders'])
            ...
            <div class="col-lg-8">
                <div class="card h-100">
                    ...
                    <div class="card-body p-0">
                        @include('marketplace.bestbuy._detail-table', ['showEmpty' => true, 'rows' => [
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                @if($connected && !empty($shopify['shopify_order_id']))
                    <button type="button" class="btn btn-sm btn-outline-warning" id="btn-push-tracking-bestbuy-inline" data-id="{{ $line->id }}">
            ...
            @if($connected && !empty($shopify['shopify_order_id']) && empty($shipment['tracking']))
                <div class="card-footer small text-muted">
                    No tracking on Best Buy yet. After Shopify shows a tracking number from your label, click <strong>Push Shopify tracking</strong>.
                </div>
            @endif
        </div>
                                    ...
                                    <td>
                                        @php
                                            ...
                                            $listingUrl = (!empty($lineSku) && isset($listingSkuIds[$lineSku]))
                                                ? route('marketplace.products.show', ['marketplace' => 'bestbuy', 'shopifySku' => $listingSkuIds[$lineSku]])
                                                : null;
                                        @endphp
                                        @if($listingUrl)
...
document.getElementById('btn-pull-ae-order')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/bestbuy/orders') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
...
function pushTrackingToBest Buy(btn) {
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Read the Shopify tracking number for this order and mark it shipped on Best Buy?')) return;
    var original = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<i class="ri-loader-4-line"></i> Pushing…';
    fetch('{{ url('marketplace/bestbuy/orders') }}/' + id + '/push-tracking', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-push-tracking-bestbuy')?.addEventListener('click', function () {
    pushTrackingToBest Buy(this);
});
document.getElementById('btn-push-tracking-bestbuy-inline')?.addEventListener('click', function () {
    pushTrackingToBest Buy(this);
});
...
document.getElementById('btn-dry-run-shopify')?.addEventListener('click', function () {
    ...
    fetch('{{ route('marketplace.orders.push', 'bestbuy') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
    ...
    .then(function (data) {
        ...
        if (!data.success) {
            ...
            summaryEl.innerHTML = warnHtml
                ...
                + '<p class="small mb-1"><strong>Channel:</strong> ' + ((p.channel && p.channel.display_name) || 'Best Buy')
...
document.getElementById('btn-push-order')?.addEventListener('click', function () {
    ...
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Create a real Shopify order from this Best Buy order?')) return;
    btn.disabled = true;
    fetch('{{ route('marketplace.orders.push', 'bestbuy') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-delete-ready-order')?.addEventListener('click', function () {
    ...
    if (!id) return;
    if (!confirm('Delete Best Buy order ' + orderId + ' from ready-for-import?\n\nThis only removes it from our platform. It does not delete the order on Best Buy or Shopify.')) {
        return;
    }
    btn.disabled = true;
    fetch('{{ route('marketplace.orders.delete-ready', 'bestbuy') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
    ...
    .then(function (data) {
        ...
        if (data.success) {
            window.location = '{{ route('marketplace.orders', 'bestbuy') }}';
        } else {
            btn.disabled = false;
        }
    })
