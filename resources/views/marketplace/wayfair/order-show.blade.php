@extends('layouts.vertical', ['title' => $title ?? 'Wayfair — Order Detail', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.orders', 'wayfair') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Orders</a>
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-1">
            <div>
                @include('marketplace._page-heading', ['slug' => 'wayfair', 'heading' => 'Order '.($summary['order_id'] ?? $orderId), 'mt' => ''])
                <p class="text-muted mb-0">
                    ...
                    @if($aeSource === 'api')
                        <span class="badge bg-info-subtle text-info ms-1">Wayfair data: live API</span>
                    @else
                        <span class="badge bg-warning-subtle text-warning ms-1">Wayfair data: cached</span>
                    @endif
                </p>
            </div>
            <div class="d-flex gap-2">
                @if($connected)
                    <button type="button" class="btn btn-sm btn-outline-primary" id="btn-pull-ae-order" data-id="{{ $line->id }}">
                        <i class="ri-download-cloud-line"></i> Pull from Wayfair
                    </button>
                    @if(!empty($shopify['shopify_order_id']))
                        @include('marketplace._fetch-tracking-button', ['fetchTrackingMarketplace' => 'wayfair'])
                        <button type="button" class="btn btn-sm btn-warning" id="btn-push-tracking-wayfair" data-id="{{ $line->id }}" title="Read Shopify fulfillment tracking and mark shipped on Wayfair">
                            <i class="ri-truck-line"></i> Push tracking to Wayfair
                        </button>
                    @endif
                @endif
            </div>
        ...
        @include('marketplace.wayfair._nav', ['active' => 'orders'])

        <div class="alert alert-info py-2 small mb-3">
            Order details are pulled from Wayfair and sent to Shopify when you push this order.
            After you buy/download a shipping label in Shopify, use <strong>Push tracking to Wayfair</strong> so the tracking number is declared on Wayfair too.@include('marketplace._fetch-tracking-hint')
        </div>
            ...
            <div class="col-lg-8">
                <div class="card h-100">
                    ...
                    <div class="card-body p-0">
                        @include('marketplace.wayfair._detail-table', ['showEmpty' => true, 'rows' => [
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                @if($connected && !empty($shopify['shopify_order_id']))
                    <button type="button" class="btn btn-sm btn-outline-warning" id="btn-push-tracking-wayfair-inline" data-id="{{ $line->id }}">
            ...
            @if($connected && !empty($shopify['shopify_order_id']) && empty($shipment['tracking']))
                <div class="card-footer small text-muted">
                    No tracking on Wayfair yet. After Shopify shows a tracking number from your label, click <strong>Push Shopify tracking</strong>.
                </div>
            @endif
        </div>
                                    ...
                                    <td>
                                        @php
                                            ...
                                            $listingUrl = (!empty($lineSku) && isset($listingSkuIds[$lineSku]))
                                                ? route('marketplace.products.show', ['marketplace' => 'wayfair', 'shopifySku' => $listingSkuIds[$lineSku]])
                                                : null;
                                        @endphp
                                        @if($listingUrl)
...
document.getElementById('btn-pull-ae-order')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/wayfair/orders') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
    ...
    .finally(function () {
        btn.disabled = false;
        btn.innerHTML = '<i class="ri-download-cloud-line"></i> Pull from Wayfair';
    });
});

function pushTrackingToWayfair(btn) {
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Read the Shopify tracking number for this order and mark it shipped on Wayfair?')) return;
    var original = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<i class="ri-loader-4-line"></i> Pushing…';
    fetch('{{ url('marketplace/wayfair/orders') }}/' + id + '/push-tracking', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-push-tracking-wayfair')?.addEventListener('click', function () {
    pushTrackingToWayfair(this);
});
document.getElementById('btn-push-tracking-wayfair-inline')?.addEventListener('click', function () {
    pushTrackingToWayfair(this);
});
...
document.getElementById('btn-dry-run-shopify')?.addEventListener('click', function () {
    ...
    fetch('{{ route('marketplace.orders.push', 'wayfair') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
    ...
    .then(function (data) {
        ...
        if (!data.success) {
            ...
            summaryEl.innerHTML = warnHtml
                ...
                + '<p class="small mb-1"><strong>Channel:</strong> ' + ((p.channel && p.channel.display_name) || 'Wayfair')
...
document.getElementById('btn-push-order')?.addEventListener('click', function () {
    ...
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Create a real Shopify order from this Wayfair order?')) return;
    btn.disabled = true;
    fetch('{{ route('marketplace.orders.push', 'wayfair') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-delete-ready-order')?.addEventListener('click', function () {
    ...
    if (!id) return;
    if (!confirm('Delete Wayfair order ' + orderId + ' from ready-for-import?\n\nThis only removes it from our platform. It does not delete the order on Wayfair or Shopify.')) {
        return;
    }
    btn.disabled = true;
    fetch('{{ route('marketplace.orders.delete-ready', 'wayfair') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
    ...
    .then(function (data) {
        ...
        if (data.success) {
            window.location = '{{ route('marketplace.orders', 'wayfair') }}';
        } else {
            btn.disabled = false;
        }
    })
