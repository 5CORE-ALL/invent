@extends('layouts.vertical', ['title' => $title ?? 'TikTok 2 — Product', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])

@section('content')
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.products', 'tiktok2') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> TikTok 2 Listings</a>
        @include('marketplace._page-heading', ['slug' => 'tiktok2', 'heading' => $sku, 'mb' => 'mb-3'])
...
document.getElementById('btn-sync-inventory')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/tiktok2/products') }}/' + id + '/sync-inventory', {
        method: 'POST',
