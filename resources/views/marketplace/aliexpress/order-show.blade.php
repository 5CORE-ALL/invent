...
<div class="row">
    <div class="col-12">
            ...
            <div class="d-flex gap-2">
                @if($connected)
                    ...
                    @if(!empty($shopify['shopify_order_id']))
                        @include('marketplace._fetch-tracking-button', ['fetchTrackingMarketplace' => 'aliexpress'])
                        <button type="button" class="btn btn-sm btn-warning" id="btn-push-tracking-ae" data-id="{{ $line->id }}" title="Read Shopify fulfillment tracking and declare it on AliExpress">
                            <i class="ri-truck-line"></i> Push tracking to AliExpress
                        </button>
                    @endif
                @endif
            </div>
        ...
        <div class="alert alert-info py-2 small mb-3">
            Order details are pulled from AliExpress and sent to Shopify when you push this order.
            After you buy/download a shipping label in Shopify, use <strong>Push tracking to AliExpress</strong> so the tracking number is declared on AliExpress too.@include('marketplace._fetch-tracking-hint')
        </div>
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                @if($connected && !empty($shopify['shopify_order_id']))
                    <button type="button" class="btn btn-sm btn-outline-warning" id="btn-push-tracking-ae-inline" data-id="{{ $line->id }}">
            ...
            @if($connected && !empty($shopify['shopify_order_id']) && empty($shipment['tracking']))
                <div class="card-footer small text-muted">
                    No tracking on AliExpress yet. After Shopify shows a tracking number from your label, click <strong>Push Shopify tracking</strong>.
                </div>
            @endif
        </div>
...
function pushTrackingToAliExpress(btn) {
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Read the Shopify tracking number for this order and declare/update it on AliExpress?')) return;
    var original = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<i class="ri-loader-4-line"></i> Pushing…';
    fetch('{{ url('marketplace/aliexpress/orders') }}/' + id + '/push-tracking', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-push-tracking-ae')?.addEventListener('click', function () {
    pushTrackingToAliExpress(this);
});
document.getElementById('btn-push-tracking-ae-inline')?.addEventListener('click', function () {
    pushTrackingToAliExpress(this);
});
