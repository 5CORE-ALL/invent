...
<div class="row">
    <div class="col-12">
        ...
        <p class="text-muted mb-3">
            ...
            <strong>Inv SKU Match / Inv SKU Mismatch</strong> = Shopify vs AliExpress quantity. Shopify qty must not be less than marketplace qty. Match allows marketplace to be short by at most max(3 units, 3% of Shopify). <strong>Linked mismatch SKU</strong> is only the SKUs where marketplace qty is higher than Shopify.
            <strong>Active SKU / Inactive SKU</strong> = actual AliExpress seller portal status (not inventory match).
            <em>Refresh live</em> warms AE status. Refresh Shopify from <a href="{{ route('marketplace.manager.index') }}">Marketplace Manager</a>.
        </p>
            ...
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                <span class="badge bg-primary">
                    ...
                    @elseif(($linkTab ?? '') === 'unlinked')
                        {{ $products->total() }} not on AliExpress (in-stock Shopify)
                ...
                <div class="d-flex gap-2 flex-wrap">
                    @if(in_array(($linkTab ?? ''), ['all', 'matched', 'matched_inactive', 'mismatch', 'linked_mismatch', 'mismatch_inactive', 'zero'], true))
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['clear_cache' => 1, 'refresh_live' => null]) }}" class="btn btn-sm btn-outline-secondary" onclick="return confirm('Clear the warm AliExpress live listings cache? Counts will refresh after Refresh live.');">
            ...
            <div class="card-body">
                ...
                <form method="get" class="mb-3">
                    ...
                    @if(in_array(($linkTab ?? ''), ['matched_inactive', 'mismatch_inactive'], true) && empty($stateCacheReady) && (int) ($counts[$linkTab] ?? 0) === 0)
                        <p class="small text-muted mt-2 mb-0">Active / Inactive counts need the live AE catalog — click <em>Refresh live</em>, wait a minute, then reload.</p>
                    @endif
                </form>
                ...
                <ul class="nav nav-tabs nav-bordered mb-3" role="tablist">
                    ...
                    <li class="nav-item">
                        <a href="{{ request()->url() }}?link=unlinked&search_name={{ $qName }}&search_sku={{ $qSku }}" class="nav-link {{ ($linkTab ?? '') === 'unlinked' ? 'active' : '' }}">Not on AliExpress {{ $counts['unlinked'] ?? 0 }}</a>
                    </li>
                </ul>
...
document.getElementById('btn-sync-mismatch-now')?.addEventListener('click', function () {
    ...
    if (!confirm('Push the actual live Shopify quantity to every Linked mismatch SKU on AliExpress right now (no queue)? This runs in batches and may take a few minutes.')) {
        return;
    }
    btn.disabled = true;
    var original = btn.innerHTML;
    var url = '{{ route('marketplace.manager.aliexpress.sync.mismatch.inventory') }}';
    var offset = 0;
