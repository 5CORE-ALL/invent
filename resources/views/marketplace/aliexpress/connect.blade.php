@extends('layouts.vertical', ['title' => $title ?? 'AliExpress — Connect', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        ...
        @include('marketplace._page-heading', ['slug' => 'aliexpress', 'heading' => 'AliExpress — Connect', 'mb' => 'mb-3'])

        @include('marketplace.aliexpress._nav', ['active' => 'connect'])
            ...
            <div class="col-lg-8">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span class="cred-miss">Missing — <code>ALIEXPRESS_APP_KEY</code></span>
                                        @endif
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span class="cred-miss">Missing — <code>ALIEXPRESS_APP_SECRET</code></span>
                                        @endif
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span class="cred-miss">Missing — <code>ALIEXPRESS_ACCESS_TOKEN</code></span>
                                        @endif
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <th>API gateway</th>
                                    <td><code>{{ $gateway ?? 'sync' }}</code> → <code>{{ ($gateway ?? 'sync') === 'rest' ? ($restBase ?? 'https://api-sg.aliexpress.com/rest') : ($apiBase ?? 'https://api-sg.aliexpress.com/sync') }}</code></td>
                                </tr>
                                <tr>
                ...
                @else
                    <div class="card">
                        ...
                        <div class="card-body">
                            <ol class="mb-0">
                                <li class="mb-2">Add <code>ALIEXPRESS_APP_KEY</code> and <code>ALIEXPRESS_APP_SECRET</code> to <code>.env</code>.</li>
                                <li class="mb-2">Click <strong>Re-authorize</strong>, log in as seller, then run:<br>
                                    <code>php artisan aliexpress:auth-url --exchange=CODE_FROM_REDIRECT</code><br>
                                    Copy the new token into <code>ALIEXPRESS_ACCESS_TOKEN</code>.</li>
...
function runConnectionTest(auto) {
    ...
    el.innerHTML = '<span class="text-muted"><i class="ri-loader-4-line"></i> Testing AliExpress API…</span>';

    fetch('{{ route('marketplace.manager.aliexpress.test') }}', {
        method: 'POST',
        headers: {
