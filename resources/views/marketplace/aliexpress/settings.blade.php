@extends('layouts.vertical', ['title' => $title ?? 'AliExpress — Settings', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        ...
        <form id="aliexpress-settings-form">
            @csrf
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync prices from Shopify to AliExpress</span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync stock quantities from Shopify to AliExpress</span>
                        </label>
                    </div>
            ...
            <div class="settings-section">
                ...
                <div class="settings-section-body">
                    ...
                    <div class="sync-toggle-row">
                        ...
                        <div class="form-text ms-4">When on, unpaid AliExpress orders stay in our DB and are not queued or manually pushed to Shopify. Turn this off to import unpaid orders.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="order[push_tracking_to_aliexpress]" value="1" {{ ($settings['order']['push_tracking_to_aliexpress'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Push Shopify tracking numbers to AliExpress</span>
                        </label>
                        <div class="form-text ms-4">When on, a 15‑minute job reads Shopify fulfillments (after you download a label) and declares/updates the tracking on AliExpress. You can also push per order from the order detail page.</div>
                    </div>
                    <div class="mt-2">
...
<script>
document.getElementById('aliexpress-settings-form')?.addEventListener('submit', function (e) {
    e.preventDefault();
    ...
    fetch('{{ route('marketplace.settings.save', 'aliexpress') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-sync-inventory-now')?.addEventListener('click', function () {
    ...
    fetch('{{ route('marketplace.manager.aliexpress.sync.inventory') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-sync-tracking-now')?.addEventListener('click', function () {
    ...
    status.className = 'ms-2 small text-muted';
    fetch('{{ route('marketplace.manager.aliexpress.sync.tracking') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
