@extends('layouts.vertical', ['title' => $title ?? 'Reverb — Order Detail', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.orders', 'reverb') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Orders</a>
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-1">
            <div>
                @include('marketplace._page-heading', ['slug' => 'reverb', 'heading' => 'Order '.($summary['order_id'] ?? $orderId), 'mt' => ''])
                <p class="text-muted mb-0">
                    ...
                    @if($aeSource === 'api')
                        <span class="badge bg-info-subtle text-info ms-1">Reverb data: live API</span>
                    @else
                        <span class="badge bg-warning-subtle text-warning ms-1">Reverb data: cached</span>
                    @endif
                </p>
            </div>
            <div class="d-flex gap-2">
                @if($connected)
                    <button type="button" class="btn btn-sm btn-outline-primary" id="btn-pull-ae-order" data-id="{{ $line->id }}">
                        <i class="ri-download-cloud-line"></i> Pull from Reverb
                    </button>
                    @if(!empty($shopify['shopify_order_id']))
                        @include('marketplace._fetch-tracking-button', ['fetchTrackingMarketplace' => 'reverb'])
                        <button type="button" class="btn btn-sm btn-warning" id="btn-push-tracking-reverb" data-id="{{ $line->id }}" title="Read Shopify fulfillment tracking and mark shipped on Reverb">
                            <i class="ri-truck-line"></i> Push tracking to Reverb
                        </button>
                    @endif
                @endif
            </div>
        ...
        <div class="alert alert-info py-2 small mb-3">
            Order details are pulled from Reverb and sent to Shopify when you push this order.
            After you buy/download a shipping label in Shopify, use <strong>Push tracking to Reverb</strong> so Reverb is marked shipped with that tracking number.@include('marketplace._fetch-tracking-hint')
        </div>
            ...
            <div class="col-lg-8">
                <div class="card h-100">
                    ...
                    <div class="card-body p-0">
                        @include('marketplace.reverb._detail-table', ['showEmpty' => true, 'rows' => [
                            'Order ID' => $summary['order_id'] ?? null,
                            'Order number' => $summary['order_number'] ?? null,
                            'Status' => $summary['status'] ?? null,
                            'Buyer ID' => $summary['buyer_login_id'] ?? null,
        ...
        <div class="card mb-3">
            ...
            <div class="card-body p-0">
                @include('marketplace.reverb._detail-table', ['showEmpty' => true, 'rows' => [
                    ...
                    'Tax' => isset($funds['tax']) ? $fmt($funds['tax']) : (isset($amounts['tax']) ? $fmt($amounts['tax']) : null),
                    'Order amount (buyer paid)' => isset($funds['order_amount']) ? $fmt($funds['order_amount']) : null,
                    'Selling fee' => isset($funds['selling_fee']) ? $fmt($funds['selling_fee']) : (isset($funds['platform_commission']) ? $fmt($funds['platform_commission']) : null),
                    'Bump fee' => isset($funds['bump_fee']) ? $fmt($funds['bump_fee']) : (isset($funds['affiliate_commission']) ? $fmt($funds['affiliate_commission']) : null),
                    'Direct checkout fee' => isset($funds['direct_checkout_fee']) ? $fmt($funds['direct_checkout_fee']) : (isset($funds['transaction_service_fee']) ? $fmt($funds['transaction_service_fee']) : null),
                    'Tax on fees' => isset($funds['tax_on_fees']) ? $fmt($funds['tax_on_fees']) : null,
                    'Seller payout' => isset($funds['seller_amount_paid']) ? $fmt($funds['seller_amount_paid']) : null,
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                @if($connected && !empty($shopify['shopify_order_id']))
                    <button type="button" class="btn btn-sm btn-outline-warning" id="btn-push-tracking-reverb-inline" data-id="{{ $line->id }}">
            ...
            @if($connected && !empty($shopify['shopify_order_id']) && empty($shipment['tracking']))
                <div class="card-footer small text-muted">
                    No tracking on Reverb yet. After Shopify shows a tracking number from your label, click <strong>Push Shopify tracking</strong>.
                </div>
            @endif
        </div>
                                    ...
                                    <td>
                                        @php
                                            ...
                                            $listingUrl = (!empty($lineSku) && isset($listingSkuIds[$lineSku]))
                                                ? route('marketplace.products.show', ['marketplace' => 'reverb', 'shopifySku' => $listingSkuIds[$lineSku]])
                                                : null;
                                        @endphp
                                        @if($listingUrl)
...
document.getElementById('btn-pull-ae-order')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/reverb/orders') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
    ...
    .finally(function () {
        btn.disabled = false;
        btn.innerHTML = '<i class="ri-download-cloud-line"></i> Pull from Reverb';
    });
});

function pushTrackingToReverb(btn) {
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Read the Shopify tracking number for this order and mark it shipped on Reverb?')) return;
    var original = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<i class="ri-loader-4-line"></i> Pushing…';
    fetch('{{ url('marketplace/reverb/orders') }}/' + id + '/push-tracking', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-push-tracking-reverb')?.addEventListener('click', function () {
    pushTrackingToReverb(this);
});
document.getElementById('btn-push-tracking-reverb-inline')?.addEventListener('click', function () {
    pushTrackingToReverb(this);
});
    ...
    .then(function (data) {
        ...
        if (!data.success) {
            ...
            summaryEl.innerHTML = warnHtml
                ...
                + '<p class="small mb-1"><strong>Channel:</strong> ' + ((p.channel && p.channel.display_name) || 'Reverb')
...
document.getElementById('btn-push-order')?.addEventListener('click', function () {
    ...
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Create a real Shopify order from this Reverb order?')) return;
    btn.disabled = true;
    fetch('{{ route('marketplace.orders.push', 'reverb') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-delete-ready-order')?.addEventListener('click', function () {
    ...
    if (!id) return;
    if (!confirm('Delete Reverb order ' + orderId + ' from ready-for-import?\n\nThis only removes it from our platform. It does not delete the order on Reverb or Shopify.')) {
        return;
    }
    btn.disabled = true;
    fetch('{{ route('marketplace.orders.delete-ready', 'reverb') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
    ...
    .then(function (data) {
        ...
        if (data.success) {
            window.location = '{{ route('marketplace.orders', 'reverb') }}';
        } else {
            btn.disabled = false;
        }
    })
