...
<div class="row">
    <div class="col-12">
        ...
        <p class="text-muted mb-3">Shopify catalog from <code>shopify_skus</code> (your synced Shopify inventory), with Reverb link status from <code>reverb_metric</code>. Use <strong>Sync Reverb link map</strong> to refresh SKU ↔ product_id mappings from Reverb.</p>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                <span class="badge bg-primary">
                    @if(($linkTab ?? 'all') === 'not_in_shopify')
                        {{ $products->total() }} on Reverb, not in Shopify
                    @else
            ...
            <div class="card-body">
                ...
                <ul class="nav nav-tabs nav-bordered mb-3" role="tablist">
                    ...
                    <li class="nav-item">
                        <a href="{{ request()->url() }}?link=unlinked&search_name={{ urlencode($searchName) }}&search_sku={{ urlencode($searchSku) }}" class="nav-link {{ ($linkTab ?? '') === 'unlinked' ? 'active' : '' }}">Not on Reverb {{ $counts['unlinked'] ?? 0 }}</a>
                    </li>
                    <li class="nav-item">
                        ...
                        <tbody>
                            @forelse($products as $p)
                                ...
                                <tr @if($detailUrl) style="cursor: pointer;" onclick="window.location='{{ $detailUrl }}'" @endif>
                                    ...
                                    <td>{{ $p->rv_quantity ?? $p->quantity ?? '—' }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="text-center text-muted py-4">
                                        @if(($linkTab ?? 'all') === 'not_in_shopify')
                                            No Reverb listings found without a matching Shopify SKU.
                                        @else
