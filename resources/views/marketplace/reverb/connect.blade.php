@extends('layouts.vertical', ['title' => $title ?? 'Reverb — Connect', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        ...
        @include('marketplace._page-heading', ['slug' => 'reverb', 'heading' => 'Reverb — Connect', 'mb' => 'mb-3'])

        @include('marketplace.reverb._nav', ['active' => 'connect'])
        ...
        @if($credentialsReady ?? false)
            <div class="alert alert-success d-flex align-items-start gap-2">
                ...
                <div>
                    ...
                    <p class="mb-0 small">Client ID/secret or personal token are configured. Click <strong>Test connection</strong> below to verify the API responds (or wait — it runs automatically).</p>
                </div>
            </div>
        @else
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <p class="text-muted mb-3">Credentials are read from <code>.env</code>. Shopify B2C is the source shop. API calls use the Reverb REST API.</p>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span class="cred-miss">Missing — <code>REVERB_CLIENT_ID</code></span>
                                        @endif
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span class="cred-miss">Missing — <code>REVERB_CLIENT_SECRET</code></span>
                                        @endif
                                    </td>
                                </tr>
                                <tr>
                                    <th>Personal Token (optional if client credentials set)</th>
                                    <td>
                                        @if($hasToken)
                                            <span class="cred-ok"><i class="ri-check-line"></i> Set</span>
                                            <span class="cred-mask text-muted ms-2">{{ $maskedAccessToken }}</span>
                                        @else
                                            <span class="cred-miss">Missing — <code>REVERB_TOKEN</code></span>
                                        @endif
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <th>API gateway</th>
                                    <td><code>{{ $gateway ?? 'sync' }}</code> → <code>{{ ($gateway ?? 'sync') === 'rest' ? ($restBase ?? 'https://api.reverb.com/rest') : ($apiBase ?? 'https://api.reverb.com/sync') }}</code></td>
                                </tr>
                                <tr>
                    ...
                    <div class="card border-success">
                        ...
                        <div class="card-body">
                            <ol class="mb-0">
                                ...
                                <li class="mb-2">Go to <a href="{{ route('marketplace.products', 'reverb') }}">Listings</a> → <strong>Sync from Reverb API</strong> to pull your products.</li>
                                <li class="mb-2">Open <a href="{{ route('marketplace.settings', 'reverb') }}">Sync Settings</a> → enable inventory / order sync.</li>
                                <li>Order import jobs use the <code>reverb</code> queue — started automatically by <code>scripts/cron-reverb-worker.sh</code> (see deploy / crontab).</li>
                            </ol>
                        </div>
                    </div>
                @else
                    <div class="card">
                        ...
                        <div class="card-body">
                            <ol class="mb-0">
                                <li class="mb-2">Add <code>REVERB_CLIENT_ID</code> + <code>REVERB_CLIENT_SECRET</code>, or <code>REVERB_TOKEN</code>, to <code>.env</code>.</li>
                                <li class="mb-2">Optional: open <strong>Re-authorize</strong> to create/copy a personal token from Reverb API settings, then set <code>REVERB_TOKEN</code>.</li>
...
function runConnectionTest(auto) {
    ...
    el.innerHTML = '<span class="text-muted"><i class="ri-loader-4-line"></i> Testing Reverb API…</span>';

    fetch('{{ route('marketplace.manager.reverb.test') }}', {
        method: 'POST',
        headers: {
