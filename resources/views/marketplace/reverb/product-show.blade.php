@extends('layouts.vertical', ['title' => $title ?? 'Reverb — Listing Detail', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@php
    ...
    $mpQtyShow = $linked ? ($ae['stock'] ?? $l['rv_stock'] ?? null) : null;
    ...
    ob_start();
@endphp
@include('marketplace.reverb._detail-table', ['showEmpty' => true, 'rows' => $shopifyListingRows])
@php $shopifyListingHtml = ob_get_clean(); ob_start(); @endphp
@include('marketplace.reverb._detail-table', ['showEmpty' => true, 'rows' => $aeListingRows])
@php $aeListingHtml = ob_get_clean(); @endphp

<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.products', 'reverb') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Listings</a>

        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-3">
            <div>
                @include('marketplace._page-heading', ['slug' => 'reverb', 'heading' => $displayTitle, 'mt' => ''])
                <div class="sku-hero"><code>{{ $s['sku'] ?? '—' }}</code></div>
                <div class="mt-2 d-flex flex-wrap gap-2 align-items-center">
                    @if($linked)
                        <span class="badge bg-success-subtle text-success">Linked on Reverb</span>
                    @else
                        <span class="badge bg-light text-muted">Not linked on Reverb</span>
                    @endif
                    @if($aeSource === 'api')
                        <span class="badge bg-info-subtle text-info source-pill">Reverb data: live API</span>
        ...
        @if(!empty($inventoryMismatch))
            <div class="alert alert-warning py-2 small mb-3">
                Inventory mismatch: Shopify <strong>{{ (int) $shopifyQtyShow }}</strong> vs Reverb <strong>{{ (int) $mpQtyShow }}</strong>.
                Click <strong>Sync Inventory</strong> to push Shopify qty to Reverb now.
            </div>
        @else
            <div class="alert alert-info py-2 small mb-3">
                <strong>Compare view.</strong> Shopify (source) vs Reverb side by side.
            </div>
        @endif
        ...
        @if(!$linked)
            <div class="alert alert-secondary small">
                This Shopify SKU is not linked in <code>reverb_metric</code> yet.
                Go to <a href="{{ route('marketplace.products', 'reverb') }}">Listings</a> and click <strong>Sync Reverb link map</strong> to match SKUs.
            </div>
        @endif

        @include('marketplace.reverb._compare-row', [
            'rowTitle' => 'Listing details',
            'left' => $shopifyListingHtml,
            'right' => $aeListingHtml,
            'leftEmpty' => 'No Shopify listing data',
            'rightEmpty' => 'No Reverb listing data — link SKU and pull from Reverb',
        ])

        @php
            ob_start();
        @endphp
        @include('marketplace.reverb._image-gallery', [
        ...
        @include('marketplace.reverb._compare-row', [
            ...
            'rightEmpty' => 'No Reverb images — pull live product details',
        ])
                            ...
                            <tbody>
                                @foreach($ae['variants'] as $v)
                                    <tr class="{{ ($v['sku'] ?? '') === ($s['sku'] ?? '') ? 'table-primary' : '' }}">
                                        <td class="small text-muted">{{ $v['source'] ?? ($aeSource === 'api' ? 'reverb' : 'shopify') }}</td>
                                        <td style="width:72px;">
                                            @if(!empty($v['image']))
        ...
        @if($hasAttributes)
            ...
            @include('marketplace.reverb._compare-row', [
                ...
                'rightEmpty' => 'No Reverb properties',
            ])
        @endif
        ...
        @if(filled($shopifyDescHtml) || filled($aeDescHtml))
            @include('marketplace.reverb._compare-row', [
                ...
                'rightEmpty' => 'No Reverb description — pull live product details',
            ])
        @endif
    </div>
</div>
...
document.getElementById('btn-pull-ae')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/reverb/products') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-sync-inventory')?.addEventListener('click', function () {
    ...
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Push live Shopify quantity to Reverb for this SKU now (no queue)?')) return;
    btn.disabled = true;
    var original = btn.innerHTML;
    btn.innerHTML = '<i class="ri-loader-4-line"></i> Syncing…';
    fetch('{{ url('marketplace/reverb/products') }}/' + id + '/sync-inventory', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
