...
<div class="row">
    <div class="col-12">
        ...
        <p class="text-muted mb-3">
            ...
            Linked here ≈ Inv SKU Match + Linked mismatch SKU + Zero on Shopify (sold-out SKUs are still Active in Seller Center).
            <strong>Inv SKU Match / Linked mismatch SKU</strong> = Shopify vs TikTok quantity. Shopify qty must not be less than marketplace qty. Match allows marketplace to be short by at most max(3 units, 3% of Shopify).
            <strong>Active SKU / Inactive SKU</strong> = actual TikTok seller portal status (not inventory match).
        ...
        @include('marketplace._queue-status', ['slug' => 'tiktok'])
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    @if(in_array(($linkTab ?? ''), ['all', 'matched', 'matched_inactive', 'mismatch', 'mismatch_inactive', 'zero', 'unlinked'], true))
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['clear_cache' => 1, 'refresh_live' => null]) }}" class="btn btn-sm btn-outline-secondary" onclick="return confirm('Clear the warm TikTok live listings cache?');">
            ...
            <div class="card-body">
                ...
                <ul class="nav nav-tabs nav-bordered mb-3" role="tablist">
                    ...
                    <li class="nav-item">
                        <a href="{{ request()->url() }}?link=unlinked&search_name={{ $qName }}&search_sku={{ $qSku }}" class="nav-link {{ ($linkTab ?? '') === 'unlinked' ? 'active' : '' }}">Not on TikTok {{ $counts['unlinked'] ?? 0 }}</a>
                    </li>
                </ul>
                    ...
                    <table class="table table-bordered table-hover mb-0 table-sm">
                        ...
                        <tbody>
                            @forelse($products as $p)
                                @php $detailUrl = !empty($p->shopify_sku_id) ? route('marketplace.products.show', ['marketplace' => 'tiktok', 'shopifySku' => $p->shopify_sku_id]) : null; @endphp
                            ...
                            @empty
                                <tr>
                                    <td colspan="12" class="text-center text-muted py-4">
                                        ...
                                        @if($connected)
                                            Click <strong>Sync TikTok link map</strong> if products are empty, or refresh Shopify.
                                        @else
                                            <a href="{{ route('marketplace.manager.tiktok.connect') }}">Connect TikTok Shop</a> first.
                                        @endif
                                    </td>
                                </tr>
                            @endforelse
...
document.getElementById('btn-refresh-api')?.addEventListener('click', function () {
    ...
    function setProgress(pageNum, totalPage, totalUpserted, message, totalCount) {
        ...
        var extra = totalCount ? ' (' + totalCount + ' products on TikTok)' : '';
...
@include('marketplace._sync-mismatch-now', [
    'url' => route('marketplace.manager.tiktok.sync.mismatch.inventory'),
    'confirm' => "Push the actual live Shopify quantity to every Linked mismatch SKU on TikTok Shop right now (batched, no queue)?",
    'limit' => 1,
])
