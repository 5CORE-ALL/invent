@extends('layouts.vertical', ['title' => $title ?? 'TikTok Shop — Orders', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])

@section('content')
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.manager.show', 'tiktok') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> TikTok Shop Manager</a>
        @include('marketplace._page-heading', ['slug' => 'tiktok', 'heading' => 'TikTok Shop Orders', 'mb' => 'mb-3'])
        <p class="text-muted mb-3">Order lines from <code>tiktok_orders</code>. They auto-push to Shopify when auto-import is On in Settings. If the order already exists in Shopify, it is linked — a second copy is not created.</p>
            ...
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex gap-2 align-items-center flex-wrap">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary" id="btn-fetch-orders">
                        <i class="ri-download-cloud-line"></i> Fetch from TikTok Shop
                    </button>
                </div>
            </div>
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-bordered table-hover mb-0">
                        ...
                        <tbody>
                            @forelse($orders as $o)
                                @php $url = route('marketplace.orders.show', ['marketplace' => 'tiktok', 'order' => $o->id]); @endphp
                            ...
                            @empty
                                <tr><td colspan="8" class="text-muted text-center py-4">No orders. Click Fetch from TikTok Shop.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
...
document.querySelectorAll('.btn-push-order').forEach(btn => {
    btn.addEventListener('click', async function (e) {
        ...
        this.textContent = '…';
        try {
            const res = await fetch('/marketplace-manager/tiktok/push-order/' + id, {
                method: 'POST',
                headers: {
...
document.getElementById('btn-fetch-orders')?.addEventListener('click', async function () {
    ...
    try {
        const res = await fetch(@json(route('marketplace.manager.tiktok.fetch.orders')), {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json',
