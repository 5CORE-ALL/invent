@extends('layouts.vertical', ['title' => $title ?? 'eBay 3 — Order Detail', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.orders', 'ebay3') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Orders</a>
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-1">
            <div>
                @include('marketplace._page-heading', ['slug' => 'ebay3', 'heading' => 'Order '.($summary['order_id'] ?? $orderId), 'mt' => ''])
                <p class="text-muted mb-0">
                    ...
                    @if($aeSource === 'api')
                        <span class="badge bg-info-subtle text-info ms-1">eBay 3 data: live API</span>
                    @else
                        <span class="badge bg-warning-subtle text-warning ms-1">eBay 3 data: cached</span>
                    @endif
                </p>
            </div>
            <div class="d-flex gap-2">
                @if($connected)
                    ...
                    @if(!empty($shopify['shopify_order_id']))
                        @include('marketplace._fetch-tracking-button', ['fetchTrackingMarketplace' => 'ebay3'])
                        <button type="button" class="btn btn-sm btn-warning" id="btn-push-tracking-ebay3" data-id="{{ $line->id }}" title="Read Shopify fulfillment tracking and mark shipped on eBay 3">
                            <i class="ri-truck-line"></i> Push tracking to eBay 3
                        </button>
                    @endif
                @endif
            </div>
        ...
        @include('marketplace.ebay3._nav', ['active' => 'orders'])

        <div class="alert alert-info py-2 small mb-3">
            <strong>Pull from eBay 3</strong> refreshes shipping/buyer details and, if this order is already on Shopify, updates the Shopify address.
            After you buy/download a shipping label in Shopify, use <strong>Push tracking to eBay 3</strong> so eBay 3 is marked shipped with that tracking number.@include('marketplace._fetch-tracking-hint')
        </div>
            ...
            <div class="col-lg-8">
                <div class="card h-100">
                    ...
                    <div class="card-body p-0">
                        @include('marketplace.ebay3._detail-table', ['showEmpty' => true, 'rows' => [
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                @if($connected && !empty($shopify['shopify_order_id']))
                    <button type="button" class="btn btn-sm btn-outline-warning" id="btn-push-tracking-ebay3-inline" data-id="{{ $line->id }}">
            ...
            @if($connected && !empty($shopify['shopify_order_id']) && empty($shipment['tracking']))
                <div class="card-footer small text-muted">
                    No tracking on eBay 3 yet. After Shopify shows a tracking number from your label, click <strong>Push Shopify tracking</strong>.
                </div>
            @endif
        </div>
                                    ...
                                    <td>
                                        @php
                                            ...
                                            $listingUrl = (!empty($lineSku) && isset($listingSkuIds[$lineSku]))
                                                ? route('marketplace.products.show', ['marketplace' => 'ebay3', 'shopifySku' => $listingSkuIds[$lineSku]])
                                                : null;
                                        @endphp
                                        @if($listingUrl)
...
document.getElementById('btn-pull-ae-order')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/ebay3/orders') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
...
function pushTrackingToEbay3(btn) {
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Read the Shopify tracking number for this order and mark it shipped on eBay 3?')) return;
    var original = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<i class="ri-loader-4-line"></i> Pushing…';
    fetch('{{ url('marketplace/ebay3/orders') }}/' + id + '/push-tracking', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-push-tracking-ebay3')?.addEventListener('click', function () {
    pushTrackingToEbay3(this);
});
document.getElementById('btn-push-tracking-ebay3-inline')?.addEventListener('click', function () {
    pushTrackingToEbay3(this);
});
...
document.getElementById('btn-dry-run-shopify')?.addEventListener('click', function () {
    ...
    fetch('{{ route('marketplace.orders.push', 'ebay3') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
    ...
    .then(function (data) {
        ...
        if (!data.success) {
            ...
            summaryEl.innerHTML = warnHtml
                ...
                + '<p class="small mb-1"><strong>Channel:</strong> ' + ((p.channel && p.channel.display_name) || 'eBay 3')
...
document.getElementById('btn-push-order')?.addEventListener('click', function () {
    ...
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Create a real Shopify order from this eBay 3 order?')) return;
    btn.disabled = true;
    fetch('{{ route('marketplace.orders.push', 'ebay3') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-delete-ready-order')?.addEventListener('click', function () {
    ...
    if (!id) return;
    if (!confirm('Delete eBay 3 order ' + orderId + ' from ready-for-import?\n\nThis only removes it from our platform. It does not delete the order on eBay 3 or Shopify.')) {
        return;
    }
    btn.disabled = true;
    fetch('{{ route('marketplace.orders.delete-ready', 'ebay3') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
    ...
    .then(function (data) {
        ...
        if (data.success) {
            window.location = '{{ route('marketplace.orders', 'ebay3') }}';
        } else {
            btn.disabled = false;
        }
    })
