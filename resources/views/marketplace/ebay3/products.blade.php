@extends('layouts.vertical', ['title' => $title ?? 'eBay 3 — Listings', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])

@section('content')
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.manager.show', 'ebay3') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> eBay 3 Manager</a>
        ...
        <p class="text-muted mb-3">
            ...
            <strong>Active SKU / Inactive SKU</strong> = qty matched, split by eBay 3 status.
            <strong>Active SKU Mismatch / Inactive SKU Mismatch</strong> = qty differs, split by eBay 3 status.
        ...
        @include('marketplace._queue-status', ['slug' => 'ebay3'])

        @include('marketplace.ebay3._nav', ['active' => 'products'])
            ...
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                <span class="badge bg-primary">
                    ...
                    @elseif(($linkTab ?? '') === 'unlinked')
                        {{ $products->total() }} not on eBay 3 (in-stock Shopify)
                ...
                <div class="d-flex gap-2 flex-wrap">
                    @if(in_array(($linkTab ?? ''), ['all', 'matched', 'matched_inactive', 'mismatch', 'mismatch_inactive', 'zero'], true))
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['clear_cache' => 1, 'refresh_live' => null]) }}" class="btn btn-sm btn-outline-secondary" onclick="return confirm('Clear the warm eBay 3 live listings cache? Counts will refresh after Refresh live.');">
            ...
            <div class="card-body">
                ...
                <form method="get" class="mb-3">
                    ...
                    @if($isLinkedTab && empty($stateCacheReady))
                        <p class="small text-muted mt-2 mb-0">State counts need the live eBay 3 catalog cache — click <em>Refresh live</em>, wait a minute, then reload.</p>
                    @endif
                </form>
                ...
                <ul class="nav nav-tabs nav-bordered mb-3" role="tablist">
                    ...
                    <li class="nav-item">
                        <a href="{{ request()->url() }}?link=unlinked&search_name={{ $qName }}&search_sku={{ $qSku }}" class="nav-link {{ ($linkTab ?? '') === 'unlinked' ? 'active' : '' }}">Not on eBay 3 {{ $counts['unlinked'] ?? 0 }}</a>
                    </li>
                </ul>
                    ...
                    <table class="table table-bordered table-hover mb-0 table-sm">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>SKU</th>
                                <th>{{ ($linkTab ?? '') === 'not_in_shopify' ? 'Title (eBay 3)' : 'Title (Shopify)' }}</th>
                                <th>eBay 3 ID</th>
                                <th>State</th>
                                <th>Shopify Qty</th>
                                <th>eBay 3 Qty</th>
                        ...
                        <tbody>
                            @forelse($products as $p)
                                @php $detailUrl = !empty($p->shopify_sku_id) ? route('marketplace.products.show', ['marketplace' => 'ebay3', 'shopifySku' => $p->shopify_sku_id]) : null; @endphp
                                <tr @if($detailUrl) style="cursor: pointer;" onclick="window.location='{{ $detailUrl }}'" @endif>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            {{ Str::limit($p->title ?? '—', 50) }}
                                        @endif
                                        @if(!empty($p->ebay3_title) && $p->ebay3_title !== $p->title)
                                            <div class="text-muted small">AE: {{ Str::limit($p->ebay3_title, 40) }}</div>
                                        @endif
                                    </td>
                                    <td class="small">{{ $p->product_id ?? '—' }}</td>
                                    <td class="small">
                                        @if(!empty($p->ebay3_state))
                                            @php $st = strtolower((string)$p->ebay3_state); @endphp
                                            <span class="badge {{ $st === 'active' ? 'bg-success-subtle text-success' : ($st === 'inactive' ? 'bg-warning-subtle text-warning' : 'bg-light text-muted') }}">{{ $p->ebay3_state }}</span>
                                        @else
                                            —
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    <td colspan="10" class="text-center text-muted py-4">
                                        @if(($linkTab ?? 'all') === 'not_in_shopify')
                                            No eBay 3 listings found without a matching Shopify SKU.
                                        @else
                                            No Shopify SKUs found.
                                        @endif
                                        @if(($linkTab ?? 'all') === 'linked')
                                            None linked yet — click <strong>Sync eBay 3 link map</strong> after SKUs exist in eBay 3.
                                        @elseif(($linkTab ?? 'all') === 'not_in_shopify')
                                            All synced eBay 3 SKUs appear to exist in your Shopify catalog, or run <strong>Sync eBay 3 link map</strong> first.
                                        @elseif($connected)
                                            Your Shopify catalog may be empty, or filters excluded all rows.
                                        @else
                                            <a href="{{ route('marketplace.manager.ebay3.connect') }}">Connect eBay 3</a> first.
                                        @endif
                                    </td>
                                </tr>
                            @endforelse
...
document.getElementById('btn-refresh-api')?.addEventListener('click', function () {
    ...
    var url = '{{ route('marketplace.manager.ebay3.refresh') }}';
    var page = 1;
    ...
    function setProgress(pageNum, totalPage, totalUpserted, message, totalCount) {
        ...
        var extra = totalCount ? ' (' + totalCount + ' products on eBay 3)' : '';
    ...
    if (!confirm('Sync all eBay 3 listings and refresh SKU ↔ product_id mappings? This may take a few minutes.')) {
        return;
    }
    ...
    function runPage(reset) {
        syncNext(reset).then(function (data) {
            if (!data.success && data.done) {
                ...
                btn.disabled = false;
                btn.innerHTML = '<i class="ri-refresh-line"></i> Sync eBay 3 link map';
                return;
            }
...
document.getElementById('btn-sync-mismatch-now')?.addEventListener('click', function () {
    ...
    if (!confirm('Sync ' + (scope === 'mismatch_inactive' ? 'Inactive' : 'Active') + ' Mismatch SKUs from live Shopify → eBay 3 right now (no queue)? This runs in batches and may take a few minutes.')) {
        return;
    }
    btn.disabled = true;
