@extends('layouts.vertical', ['title' => $title ?? 'eBay 2 — Orders', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])

@section('css')
<style>
    /* Higher specificity so these win over Bootstrap .badge (loaded after @yield('css')) */
    .badge.ebay2-orders-badge,
    .badge.ebay2-status-badge {
...
<div class="row">
    <div class="col-12">
        ...
        @include('marketplace._page-heading', ['slug' => 'ebay2', 'heading' => 'eBay 2 Orders'])
        <p class="text-muted mb-3">Orders stored locally from eBay 2 API. They auto-push to Shopify when auto-import is On in <a href="{{ route('marketplace.settings', 'ebay2') }}">Settings</a>. If the order already exists in Shopify, it is linked — a second copy is not created.</p>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                <div class="d-flex align-items-center flex-wrap gap-2">
                    <span class="badge bg-primary ebay2-orders-badge">{{ $orders->total() }} orders</span>
                    @foreach(($statusCounts ?? []) as $status => $count)
                        ...
                        @endphp
                        <span class="badge {{ $cls }} ebay2-status-badge" title="{{ $label }}">{{ $label }}: {{ number_format((int) $count) }}</span>
                    @endforeach
                </div>
                <div class="d-flex gap-2 align-items-center flex-wrap">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary" id="btn-fetch-orders">
                        <i class="ri-download-cloud-line"></i> Fetch from eBay 2
                    </button>
                </div>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover mb-0">
                        ...
                        <tbody>
                            @forelse($orders as $o)
                                @php $orderUrl = route('marketplace.orders.show', ['marketplace' => 'ebay2', 'order' => $o->id]); @endphp
                                        ...
                                        @else
                                            @php
                                                $pushBlocked = ($importPaidOrdersOnly ?? false)
                                                    && ! \App\Services\MarketplaceManager\MarketplaceOrderPaidFilter::isPaid('ebay2', $o);
                                            @endphp
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="text-center text-muted py-4">
                                        No orders yet. Click <strong>Fetch from eBay 2</strong>.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
...
document.getElementById('btn-fetch-orders')?.addEventListener('click', function () {
    ...
    btn.disabled = true;
    fetch('{{ route('marketplace.manager.ebay2.fetch.orders') }}', {
        method: 'POST',
        headers: {
