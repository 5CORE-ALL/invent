...
<div class="row">
    <div class="col-12">
        ...
        <p class="text-muted mb-3">
            ...
            <strong>Active SKU / Inactive SKU</strong> = qty matched, split by Faire status.
            <strong>Active SKU Mismatch / Inactive SKU Mismatch</strong> = qty differs, split by Faire status.
            ...
            <div class="card-body">
                ...
                <form method="get" class="mb-3">
                    ...
                    @if($isLinkedTab && empty($stateCacheReady))
                        <p class="small text-muted mt-2 mb-0">State counts need the live Faire catalog cache — click <em>Refresh live</em>, wait a minute, then reload.</p>
                    @endif
                </form>
...
document.getElementById('btn-sync-mismatch-now')?.addEventListener('click', function () {
    ...
    if (!confirm('Sync ' + (scope === 'mismatch_inactive' ? 'Inactive' : 'Active') + ' Mismatch SKUs from live Shopify → Faire right now (no queue)? This runs in batches and may take a few minutes.')) {
        return;
    }
    btn.disabled = true;
