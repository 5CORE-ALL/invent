@extends('layouts.vertical', ['title' => $title ?? 'Faire — Settings', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        ...
        @include('marketplace._page-heading', ['slug' => 'faire', 'heading' => 'Faire Sync Settings'])
        <p class="text-muted mb-3">Configure how Shopify (source) syncs with Faire for pricing, inventory, and orders.</p>

        @include('marketplace.faire._nav', ['active' => 'settings'])

        <form id="faire-settings-form">
            @csrf
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync prices from Shopify to Faire</span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync stock quantities from Shopify to Faire</span>
                        </label>
                    </div>
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Fetch orders from Faire on schedule</span>
                        </label>
                        <div class="form-text ms-4">When on, the 15‑minute schedule pulls Faire orders into our DB. Manual <strong>Fetch from Faire</strong> on the Orders page always works.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Automatically import Faire orders to Shopify</span>
                        </label>
                        <div class="form-text ms-4">ON by default. New Faire orders are queued to Shopify on the 15‑minute schedule.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                        ...
                        <div class="form-text ms-4">When on, unpaid / payment-pending Faire orders stay in our DB and are not queued or manually pushed to Shopify. Turn this off to import unpaid orders.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Automatically sync Faire customer / shipping address to Shopify</span>
                        </label>
                        <div class="form-text ms-4">ON by default. Every 15 minutes the app fills missing Shopify shipping/billing/customer address from Faire — no manual Pull needed.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="order[push_tracking_to_faire]" value="1" {{ ($settings['order']['push_tracking_to_faire'] ?? true) ? 'checked' : '' }}>
                            <span class="form-check-label">Automatically push Shopify tracking numbers to Faire</span>
                        </label>
                        <div class="form-text ms-4">ON by default. Every 5 minutes the app reads Shopify fulfillments (after you print/download a label) and ships the order on Faire — no manual push needed. You can still push per order from the order detail page.</div>
                    </div>
                    <div class="mt-2">
                        ...
                        <div class="form-text">Faire orders import here. Default is main B2C store (same as <code>shopify_skus</code>).</div>
                    </div>
                    <div class="mt-2">
                        <label class="form-label small">Shopify channel handle (source_name)</label>
                        <input type="text" class="form-control form-control-sm" name="order[shopify_source_name]" value="{{ $settings['order']['shopify_source_name'] ?? 'faire' }}" style="max-width: 400px;">
                        <div class="form-text">Case-sensitive handle registered in your Shopify app’s <strong>Marketplace extension</strong> (Partner Dashboard). Shopify shows this as the channel name (e.g. Faire) instead of the app name (5core).</div>
                    </div>
                    <div class="mt-2">
                        <label class="form-label small">Shopify channel display label</label>
                        <input type="text" class="form-control form-control-sm" name="order[shopify_source_display_name]" value="{{ $settings['order']['shopify_source_display_name'] ?? 'Faire' }}" style="max-width: 400px;">
                    ...
                    <div class="mt-2">
                        ...
                        <input type="text" class="form-control form-control-sm" name="order[shopify_order_tags]" value="{{ implode(', ', $settings['order']['shopify_order_tags'] ?? ['faire']) }}" style="max-width: 400px;">
                    </div>
                </div>
            </div>
            ...
            <div class="settings-section">
                ...
                <div class="settings-section-body">
                    ...
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[create_products_on_faire]" value="1" {{ ($settings['listings']['create_products_on_faire'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Create new listings on Faire from Shopify <span class="text-muted">(off for testing — not implemented yet)</span></span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[sync_title]" value="1" {{ ($settings['listings']['sync_title'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Push title updates to existing Faire listings</span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[sync_images]" value="1" {{ ($settings['listings']['sync_images'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Push image updates to existing Faire listings</span>
                        </label>
                    </div>
                </div>
            </div>
...
<script>
document.getElementById('faire-settings-form')?.addEventListener('submit', function (e) {
    e.preventDefault();
    ...
    fetch('{{ route('marketplace.settings.save', 'faire') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-sync-inventory-now')?.addEventListener('click', function () {
    ...
    fetch('{{ route('marketplace.manager.faire.sync.inventory') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-sync-tracking-now')?.addEventListener('click', function () {
    ...
    status.className = 'ms-2 small text-muted';
    fetch('{{ route('marketplace.manager.faire.sync.tracking') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
