@extends('layouts.vertical', ['title' => $title ?? 'Faire — Listing Detail', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@php
    ...
    $aeListingRows = [
        ...
        'Faire Qty' => $linked ? ($ae['stock'] ?? $l['ae_stock'] ?? null) : null,
    ...
    ob_start();
@endphp
@include('marketplace.faire._detail-table', ['showEmpty' => true, 'rows' => $shopifyListingRows])
@php $shopifyListingHtml = ob_get_clean(); ob_start(); @endphp
@include('marketplace.faire._detail-table', ['showEmpty' => true, 'rows' => $aeListingRows])
@php $aeListingHtml = ob_get_clean(); @endphp

<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.products', 'faire') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Listings</a>

        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-3">
            <div>
                @include('marketplace._page-heading', ['slug' => 'faire', 'heading' => $displayTitle, 'mt' => ''])
                <div class="sku-hero"><code>{{ $s['sku'] ?? '—' }}</code></div>
                <div class="mt-2 d-flex flex-wrap gap-2 align-items-center">
                    @if($linked)
                        <span class="badge bg-success-subtle text-success">Linked on Faire</span>
                    @else
                        <span class="badge bg-light text-muted">Not linked on Faire</span>
                    @endif
                    @if($aeSource === 'api')
                        <span class="badge bg-info-subtle text-info source-pill">Faire data: live API</span>
                    @elseif($aeSource === 'cached')
                        <span class="badge bg-warning-subtle text-warning source-pill">Faire data: cached map</span>
                    @else
                        <span class="badge bg-light text-muted source-pill">Faire data: not loaded</span>
                    @endif
        ...
        @include('marketplace.faire._nav', ['active' => 'products'])

        @if(!empty($inventoryMismatch))
            <div class="alert alert-warning py-2 small mb-3">
                Inventory mismatch: Shopify <strong>{{ (int) $shopifyQtyShow }}</strong> vs Faire <strong>{{ (int) $mpQtyShow }}</strong>.
                Click <strong>Sync Inventory</strong> to push Shopify qty to Faire now.
            </div>
        @else
            <div class="alert alert-info py-2 small mb-3">
                <strong>Compare view.</strong> Shopify (source) vs Faire side by side.
            </div>
        @endif
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-md-6">
                <div class="card h-100 {{ !empty($inventoryMismatch) ? 'border-warning' : '' }}">
                    <div class="card-body py-3">
                        <div class="text-muted small">Faire Qty</div>
        ...
        @if(!$linked)
            <div class="alert alert-secondary small">
                This Shopify SKU is not linked in <code>faire_metric</code> yet.
                Go to <a href="{{ route('marketplace.products', 'faire') }}">Listings</a> and click <strong>Sync Faire link map</strong> to match SKUs.
            </div>
        @endif

        @include('marketplace.faire._compare-row', [
            'rowTitle' => 'Listing details',
            'left' => $shopifyListingHtml,
            'right' => $aeListingHtml,
            'leftEmpty' => 'No Shopify listing data',
            'rightEmpty' => 'No Faire listing data — link SKU and pull from Faire',
        ])

        @php
            ob_start();
        @endphp
        @include('marketplace.faire._image-gallery', [
        ...
        @include('marketplace.faire._compare-row', [
            ...
            'rightEmpty' => 'No Faire images — pull live product details',
        ])
                            ...
                            <tbody>
                                @foreach($ae['variants'] as $v)
                                    <tr class="{{ ($v['sku'] ?? '') === ($s['sku'] ?? '') ? 'table-primary' : '' }}">
                                        <td class="small text-muted">{{ $v['source'] ?? ($aeSource === 'api' ? 'faire' : 'shopify') }}</td>
                                        <td style="width:72px;">
                                            @if(!empty($v['image']))
        ...
        @if($hasAttributes)
            ...
            @include('marketplace.faire._compare-row', [
                ...
                'rightEmpty' => 'No Faire properties',
            ])
        @endif
        ...
        @if(filled($shopifyDescHtml) || filled($aeDescHtml))
            @include('marketplace.faire._compare-row', [
                ...
                'rightEmpty' => 'No Faire description — pull live product details',
            ])
        @endif
    </div>
</div>
...
document.getElementById('btn-pull-ae')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/faire/products') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
    ...
    .finally(function () {
        btn.disabled = false;
        btn.innerHTML = '<i class="ri-download-cloud-line"></i> Pull from Faire';
    });
});
...
document.getElementById('btn-sync-inventory')?.addEventListener('click', function () {
    ...
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Push live Shopify quantity to Faire for this SKU now (no queue)?')) return;
    btn.disabled = true;
    var original = btn.innerHTML;
    btn.innerHTML = '<i class="ri-loader-4-line"></i> Syncing…';
    fetch('{{ url('marketplace/faire/products') }}/' + id + '/sync-inventory', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
