@extends('layouts.vertical', ['title' => $title ?? "Macy\'s — Orders", 'mode' => $mode ?? '', 'demo' => $demo ?? ''])

@section('content')
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.manager.show', 'macy') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Macy's Manager</a>
        @include('marketplace._page-heading', ['slug' => 'macy', 'heading' => "Macy\'s Orders"])
        <p class="text-muted mb-3">Orders stored locally from Macy's API. They auto-push to Shopify when auto-import is On in <a href="{{ route('marketplace.settings', 'macy') }}">Settings</a>. If the order already exists in Shopify, it is linked — a second copy is not created.</p>

        @include('marketplace.macy._nav', ['active' => 'orders'])
            ...
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex gap-2 align-items-center flex-wrap">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary" id="btn-fetch-orders">
                        <i class="ri-download-cloud-line"></i> Fetch from Macy's
                    </button>
                </div>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover mb-0">
                        ...
                        <tbody>
                            @forelse($orders as $o)
                                @php $orderUrl = route('marketplace.orders.show', ['marketplace' => 'macy', 'order' => $o->id]); @endphp
                                <tr style="cursor: pointer;" onclick="window.location='{{ $orderUrl }}'">
                                    ...
                                    <td class="small">
                                        @if($o->order_created_at)
                                            {{ $o->order_created_at->format('M d, Y H:i') }}
                                        @else
                                            —
                                        @endif
                                    </td>
                                    ...
                                    <td>{{ Str::limit($o->product_title ?? '—', 40) }}</td>
                                    <td>{{ $o->quantity ?? 1 }}</td>
                                    <td>{{ is_numeric($o->unit_price) ? number_format($o->lineAmount(), 2) : '—' }}</td>
                                    <td>
                                        ...
                                        @else
                                            @php
                                                $pushBlocked = ($importPaidOrdersOnly ?? false)
                                                    && ! \App\Services\MarketplaceManager\MarketplaceOrderPaidFilter::isPaid('macy', $o);
                                            @endphp
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="text-center text-muted py-4">
                                        No orders yet. Click <strong>Fetch from Macy's</strong>.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
...
document.getElementById('btn-fetch-orders')?.addEventListener('click', function () {
    ...
    btn.disabled = true;
    fetch('{{ route('marketplace.manager.macy.fetch.orders') }}', {
        method: 'POST',
        headers: {
...
document.querySelectorAll('.btn-push-order').forEach(function (btn) {
    btn.addEventListener('click', function () {
        ...
        if (!id) return;
        this.disabled = true;
        fetch('{{ route('marketplace.orders.push', 'macy') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json',
...
document.querySelectorAll('.btn-delete-ready-order').forEach(function (btn) {
    btn.addEventListener('click', function () {
        ...
        if (!id) return;
        if (!confirm("Delete Macy's order " + orderId + ' from ready-for-import?\n\nThis only removes it from our platform. It does not delete the order on Macy's or Shopify.')) {
            return;
        }
        this.disabled = true;
        fetch('{{ route('marketplace.orders.delete-ready', 'macy') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json',
