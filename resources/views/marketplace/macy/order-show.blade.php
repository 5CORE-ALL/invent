@extends('layouts.vertical', ['title' => $title ?? "Macy\'s — Order Detail", 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.orders', 'macy') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Orders</a>
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-1">
            <div>
                @include('marketplace._page-heading', ['slug' => 'macy', 'heading' => 'Order '.($summary['order_id'] ?? $orderId), 'mt' => ''])
                <p class="text-muted mb-0">
                    ...
                    @if($aeSource === 'api')
                        <span class="badge bg-info-subtle text-info ms-1">Macy's data: live API</span>
                    @else
                        <span class="badge bg-warning-subtle text-warning ms-1">Macy's data: cached</span>
                    @endif
                </p>
            </div>
            ...
            <div class="col-lg-8">
                <div class="card h-100">
                    ...
                    <div class="card-body p-0">
                        @include('marketplace.macy._detail-table', ['showEmpty' => true, 'rows' => [
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                @if($connected && !empty($shopify['shopify_order_id']))
                    <button type="button" class="btn btn-sm btn-outline-warning" id="btn-push-tracking-macy-inline" data-id="{{ $line->id }}">
            ...
            @if($connected && !empty($shopify['shopify_order_id']) && empty($shipment['tracking']))
                <div class="card-footer small text-muted">
                    No tracking on Macy's yet. After Shopify shows a tracking number from your label, click <strong>Push Shopify tracking</strong>.
                </div>
            @endif
        </div>
                                    ...
                                    <td>
                                        @php
                                            ...
                                            $listingUrl = (!empty($lineSku) && isset($listingSkuIds[$lineSku]))
                                                ? route('marketplace.products.show', ['marketplace' => 'macy', 'shopifySku' => $listingSkuIds[$lineSku]])
                                                : null;
                                        @endphp
                                        @if($listingUrl)
...
document.getElementById('btn-pull-ae-order')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/macy/orders') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
...
function pushTrackingToMacy(btn) {
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm("Read the Shopify tracking number for this order and mark it shipped on Macy's?")) return;
    var original = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<i class="ri-loader-4-line"></i> Pushing…';
    fetch('{{ url('marketplace/macy/orders') }}/' + id + '/push-tracking', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-push-tracking-macy')?.addEventListener('click', function () {
    pushTrackingToMacy(this);
});
document.getElementById('btn-push-tracking-macy-inline')?.addEventListener('click', function () {
    pushTrackingToMacy(this);
});
    ...
    .then(function (data) {
        ...
        if (!data.success) {
            ...
            summaryEl.innerHTML = warnHtml
                ...
                + '<p class="small mb-1"><strong>Channel:</strong> ' + ((p.channel && p.channel.display_name) || "Macy's")
...
document.getElementById('btn-push-order')?.addEventListener('click', function () {
    ...
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm("Create a real Shopify order from this Macy's order?")) return;
    btn.disabled = true;
    fetch('{{ route('marketplace.orders.push', 'macy') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-delete-ready-order')?.addEventListener('click', function () {
    ...
    .then(function (data) {
        ...
        if (data.success) {
            window.location = '{{ route('marketplace.orders', 'macy') }}';
        } else {
            btn.disabled = false;
        }
    })
