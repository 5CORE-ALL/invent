@extends('layouts.vertical', ['title' => $title ?? "Macy\'s — Settings", 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        ...
        @include('marketplace._page-heading', ['slug' => 'macy', 'heading' => "Macy\'s Sync Settings"])
        <p class="text-muted mb-3">Configure how Shopify (source) syncs with Macy's for pricing, inventory, and orders.</p>

        @include('marketplace.macy._nav', ['active' => 'settings'])

        <form id="macy-settings-form">
            @csrf
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync prices from Shopify to Macy's</span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync stock quantities from Shopify to Macy's</span>
                        </label>
                    </div>
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Fetch orders from Macy's on schedule</span>
                        </label>
                        <div class="form-text ms-4">When on, the 15‑minute schedule pulls Macy's orders into our DB. Manual <strong>Fetch from Macy's</strong> on the Orders page always works.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Automatically import Macy's orders to Shopify</span>
                        </label>
                        <div class="form-text ms-4">ON by default. New Macy's orders are queued to Shopify on the 15‑minute schedule.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                        ...
                        <div class="form-text ms-4">When on, unpaid / payment-pending Macy's orders stay in our DB and are not queued or manually pushed to Shopify. Turn this off to import unpaid orders.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Automatically sync Macy's customer / shipping address to Shopify</span>
                        </label>
                        <div class="form-text ms-4">ON by default. Every 15 minutes the app fills missing Shopify shipping/billing/customer address from Macy's — no manual Pull needed.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="order[push_tracking_to_macy]" value="1" {{ ($settings['order']['push_tracking_to_macy'] ?? true) ? 'checked' : '' }}>
                            <span class="form-check-label">Automatically push Shopify tracking numbers to Macy's</span>
                        </label>
                        <div class="form-text ms-4">ON by default. Every 5 minutes the app reads Shopify fulfillments (after you print/download a label) and ships the order on Macy's — no manual push needed. You can still push per order from the order detail page.</div>
                    </div>
                    <div class="mt-2">
                        ...
                        <div class="form-text">Macy's orders import here. Default is main B2C store (same as <code>shopify_skus</code>).</div>
                    </div>
                    <div class="mt-2">
                        <label class="form-label small">Shopify channel handle (source_name)</label>
                        <input type="text" class="form-control form-control-sm" name="order[shopify_source_name]" value="{{ $settings['order']['shopify_source_name'] ?? 'macy' }}" style="max-width: 400px;">
                        <div class="form-text">Case-sensitive handle registered in your Shopify app’s <strong>Marketplace extension</strong> (Partner Dashboard). Shopify shows this as the channel name (e.g. Macy's) instead of the app name (5core).</div>
                    </div>
                    <div class="mt-2">
                        <label class="form-label small">Shopify channel display label</label>
                        <input type="text" class="form-control form-control-sm" name="order[shopify_source_display_name]" value="{{ $settings['order']['shopify_source_display_name'] ?? "Macy's" }} style="max-width: 400px;">
                    ...
                    <div class="mt-2">
                        ...
                        <input type="text" class="form-control form-control-sm" name="order[shopify_order_tags]" value="{{ implode(', ', $settings['order']['shopify_order_tags'] ?? ['macy']) }}" style="max-width: 400px;">
                    </div>
                </div>
            </div>
            ...
            <div class="settings-section">
                ...
                <div class="settings-section-body">
                    <p class="text-muted small mb-2">Sync Macy's link map only reads Macy's and saves SKU mappings locally — it never creates listings on Macy's. When <strong>Auto-link</strong> is on, this also runs hourly on schedule.</p>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                        ...
                        <div class="form-text ms-4">When on, refresh Macy's SKU ↔ product_id mappings hourly (same as manual Sync Macy's link map). Manual sync on Listings always works.</div>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[create_products_on_macy]" value="1" {{ ($settings['listings']['create_products_on_macy'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Create new listings on Macy's from Shopify <span class="text-muted">(off for testing — not implemented yet)</span></span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[sync_title]" value="1" {{ ($settings['listings']['sync_title'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Push title updates to existing Macy's listings</span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[sync_images]" value="1" {{ ($settings['listings']['sync_images'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Push image updates to existing Macy's listings</span>
                        </label>
                    </div>
                </div>
            </div>
...
<script>
document.getElementById('macy-settings-form')?.addEventListener('submit', function (e) {
    e.preventDefault();
    ...
    fetch('{{ route('marketplace.settings.save', 'macy') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-sync-inventory-now')?.addEventListener('click', function () {
    ...
    fetch('{{ route('marketplace.manager.macy.sync.inventory') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
document.getElementById('btn-sync-tracking-now')?.addEventListener('click', function () {
    ...
    status.className = 'ms-2 small text-muted';
    fetch('{{ route('marketplace.manager.macy.sync.tracking') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
