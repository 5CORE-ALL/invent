@extends('layouts.vertical', ['title' => $title ?? 'Alibaba — Settings', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
        ...
        <form id="alibaba-settings-form">
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync prices from Shopify to Alibaba</span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                ...
                <div class="settings-section-body">
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            ...
                            <span class="form-check-label">Sync stock quantities from Shopify to Alibaba</span>
                        </label>
                    </div>
            ...
            <div class="settings-section">
                ...
                <div class="settings-section-body">
                    ...
                    <div class="sync-toggle-row">
                        ...
                        <div class="form-text ms-4">When on, unpaid Alibaba orders stay in our DB but are not queued to Shopify. Manual Push still works.</div>
                    </div>
                    <div class="mt-2">
                        ...
                        <div class="form-text">Alibaba orders import here. Default is main B2C store (same as <code>shopify_skus</code>).</div>
                    </div>
                    <div class="mt-2">
                        <label class="form-label small">Shopify channel handle (source_name)</label>
                        <input type="text" class="form-control form-control-sm" name="order[shopify_source_name]" value="{{ $settings['order']['shopify_source_name'] ?? 'alibaba' }}" style="max-width: 400px;">
                        <div class="form-text">Case-sensitive handle registered in your Shopify app’s <strong>Marketplace extension</strong> (Partner Dashboard). Shopify shows this as the channel name (e.g. Alibaba) instead of the app name (5core).</div>
                    </div>
                    <div class="mt-2">
                        <label class="form-label small">Shopify channel display label</label>
                        <input type="text" class="form-control form-control-sm" name="order[shopify_source_display_name]" value="{{ $settings['order']['shopify_source_display_name'] ?? 'Alibaba' }}" style="max-width: 400px;">
                    ...
                    <div class="mt-2">
                        ...
                        <input type="text" class="form-control form-control-sm" name="order[shopify_order_tags]" value="{{ implode(', ', $settings['order']['shopify_order_tags'] ?? ['alibaba']) }}" style="max-width: 400px;">
                    </div>
                </div>
            </div>
            ...
            <div class="settings-section">
                ...
                <div class="settings-section-body">
                    ...
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[create_products_on_alibaba]" value="1" {{ ($settings['listings']['create_products_on_alibaba'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Create new listings on Alibaba from Shopify <span class="text-muted">(off for testing — not implemented yet)</span></span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[sync_title]" value="1" {{ ($settings['listings']['sync_title'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Push title updates to existing Alibaba listings</span>
                        </label>
                    </div>
                    <div class="sync-toggle-row">
                        <label class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="listings[sync_images]" value="1" {{ ($settings['listings']['sync_images'] ?? false) ? 'checked' : '' }}>
                            <span class="form-check-label">Push image updates to existing Alibaba listings</span>
                        </label>
                    </div>
                </div>
            </div>
...
<script>
document.getElementById('alibaba-settings-form')?.addEventListener('submit', function (e) {
    e.preventDefault();
    ...
    fetch('{{ route('marketplace.settings.save', 'alibaba') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-sync-inventory-now')?.addEventListener('click', function () {
    ...
    fetch('{{ route('marketplace.manager.alibaba.sync.inventory') }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
