@extends('layouts.vertical', ['title' => $title ?? 'Alibaba — Orders', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])

@section('content')
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.manager.show', 'alibaba') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Alibaba Manager</a>
        @include('marketplace._page-heading', ['slug' => 'alibaba', 'heading' => 'Alibaba Orders'])
        <p class="text-muted mb-3">Orders stored locally from Alibaba API. They auto-push to Shopify when auto-import is On in <a href="{{ route('marketplace.settings', 'alibaba') }}">Settings</a>. If the order already exists in Shopify, it is linked — a second copy is not created.</p>
            ...
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex gap-2 align-items-center flex-wrap">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary" id="btn-fetch-orders">
                        <i class="ri-download-cloud-line"></i> Fetch from Alibaba
                    </button>
                </div>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover mb-0">
                        ...
                        <tbody>
                            @forelse($orders as $o)
                                @php $orderUrl = route('marketplace.orders.show', ['marketplace' => 'alibaba', 'order' => $o->id]); @endphp
                                        ...
                                        @else
                                            @php
                                                $pushBlocked = ($importPaidOrdersOnly ?? false)
                                                    && ! \App\Services\MarketplaceManager\MarketplaceOrderPaidFilter::isPaid('alibaba', $o);
                                            @endphp
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="text-center text-muted py-4">
                                        No orders yet. Click <strong>Fetch from Alibaba</strong>.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
...
document.getElementById('btn-fetch-orders')?.addEventListener('click', function () {
    ...
    btn.disabled = true;
    fetch('{{ route('marketplace.manager.alibaba.fetch.orders') }}', {
        method: 'POST',
        headers: {
...
document.querySelectorAll('.btn-push-order').forEach(function (btn) {
    btn.addEventListener('click', function () {
        ...
        if (!id) return;
        this.disabled = true;
        fetch('{{ route('marketplace.orders.push', 'alibaba') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json',
...
document.querySelectorAll('.btn-delete-ready-order').forEach(function (btn) {
    btn.addEventListener('click', function () {
        ...
        if (!id) return;
        if (!confirm('Delete Alibaba order ' + orderId + ' from ready-for-import?\n\nThis only removes it from our platform. It does not delete the order on Alibaba or Shopify.')) {
            return;
        }
        this.disabled = true;
        fetch('{{ route('marketplace.orders.delete-ready', 'alibaba') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json',
