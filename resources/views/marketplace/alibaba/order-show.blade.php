@extends('layouts.vertical', ['title' => $title ?? 'Alibaba — Order Detail', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        <a href="{{ route('marketplace.orders', 'alibaba') }}" class="text-muted small"><i class="ri-arrow-left-line"></i> Back to Orders</a>
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mt-2 mb-1">
            <div>
                @include('marketplace._page-heading', ['slug' => 'alibaba', 'heading' => 'Order '.($summary['order_id'] ?? $orderId), 'mt' => ''])
            ...
            <div class="col-lg-8">
                <div class="card h-100">
                    ...
                    <div class="card-body p-0">
                        @include('marketplace.alibaba._detail-table', ['showEmpty' => true, 'rows' => [
                                    ...
                                    <td>
                                        @php
                                            ...
                                            $listingUrl = (!empty($lineSku) && isset($listingSkuIds[$lineSku]))
                                                ? route('marketplace.products.show', ['marketplace' => 'alibaba', 'shopifySku' => $listingSkuIds[$lineSku]])
                                                : null;
                                        @endphp
                                        @if($listingUrl)
...
document.getElementById('btn-pull-ae-order')?.addEventListener('click', function () {
    ...
    fetch('{{ url('marketplace/alibaba/orders') }}/' + id + '/pull', {
        method: 'POST',
        headers: {
    ...
    .then(function (data) {
        ...
        if (!data.success) {
            ...
            summaryEl.innerHTML = warnHtml
                ...
                + '<p class="small mb-1"><strong>Channel:</strong> ' + ((p.channel && p.channel.display_name) || 'Alibaba')
...
document.getElementById('btn-push-order')?.addEventListener('click', function () {
    ...
    var id = btn.getAttribute('data-id');
    if (!id) return;
    if (!confirm('Create a real Shopify order from this Alibaba order?')) return;
    btn.disabled = true;
    fetch('{{ route('marketplace.orders.push', 'alibaba') }}', {
        method: 'POST',
        headers: {
...
document.getElementById('btn-delete-ready-order')?.addEventListener('click', function () {
    ...
    .then(function (data) {
        ...
        if (data.success) {
            window.location = '{{ route('marketplace.orders', 'alibaba') }}';
        } else {
            btn.disabled = false;
        }
    })
