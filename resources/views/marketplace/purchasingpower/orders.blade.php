@extends('layouts.vertical', ['title' => $title ?? 'PurchasingPower — Orders', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        ...
        @include('marketplace._page-heading', ['slug' => 'purchasingpower', 'heading' => 'PurchasingPower Orders'])
        <p class="text-muted mb-3">Orders stored locally from PurchasingPower API. They auto-push to Shopify when auto-import is On in <a href="{{ route('marketplace.settings', 'purchasingpower') }}">Settings</a>. If the order already exists in Shopify, it is linked — a second copy is not created.</p>
            ...
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex gap-2 align-items-center flex-wrap">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary" id="btn-fetch-orders">
                        <i class="ri-download-cloud-line"></i> Fetch from PurchasingPower
                    </button>
                </div>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover mb-0">
                        ...
                        <tbody>
                            @forelse($orders as $o)
                                @php $orderUrl = route('marketplace.orders.show', ['marketplace' => 'purchasingpower', 'order' => $o->id]); @endphp
                                        ...
                                        @else
                                            @php
                                                $pushBlocked = ($importPaidOrdersOnly ?? false)
                                                    && ! \App\Services\MarketplaceManager\MarketplaceOrderPaidFilter::isPaid('purchasingpower', $o);
                                            @endphp
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="text-center text-muted py-4">
                                        No orders yet. Click <strong>Fetch from PurchasingPower</strong>.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
...
document.getElementById('btn-fetch-orders')?.addEventListener('click', function () {
    ...
    btn.disabled = true;
    fetch('{{ route('marketplace.manager.purchasingpower.fetch.orders') }}', {
        method: 'POST',
        headers: {
