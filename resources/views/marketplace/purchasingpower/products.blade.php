@extends('layouts.vertical', ['title' => $title ?? 'PurchasingPower — Listings', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
<div class="row">
    <div class="col-12">
        ...
        @include('marketplace._page-heading', ['slug' => 'purchasingpower', 'heading' => 'PurchasingPower Listings'])
        <p class="text-muted mb-3">
            Linked tabs: <strong>All</strong> = every Shopify live SKU.
            <strong>Inv SKU Match / Inv SKU Mismatch</strong> = Shopify vs PurchasingPower quantity. Shopify qty must not be less than marketplace qty. Match allows marketplace to be short by at most max(3 units, 3% of Shopify). <strong>Linked mismatch SKU</strong> is only the SKUs where marketplace qty is higher than Shopify.
            <strong>Active SKU / Inactive SKU</strong> = actual PurchasingPower seller portal status (not inventory match).
            <em>Refresh live</em> warms PurchasingPower status. Refresh Shopify from <a href="{{ route('marketplace.manager.index') }}">Marketplace Manager</a>.
        </p>
        ...
        @include('marketplace._queue-status', ['slug' => 'purchasingpower'])
            ...
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                <span class="badge bg-primary">
                    ...
                    @elseif(($linkTab ?? '') === 'unlinked')
                        {{ $products->total() }} not on PurchasingPower (in-stock Shopify)
                ...
                <div class="d-flex gap-2 flex-wrap">
                    @if(in_array(($linkTab ?? ''), ['all', 'matched', 'matched_inactive', 'mismatch', 'linked_mismatch', 'mismatch_inactive', 'zero'], true))
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['clear_cache' => 1, 'refresh_live' => null]) }}" class="btn btn-sm btn-outline-secondary" onclick="return confirm('Clear the warm PurchasingPower live listings cache? Counts will refresh after Refresh live.');">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary" id="btn-refresh-api">
                        <i class="ri-refresh-line"></i> Sync PurchasingPower link map
                    </button>
                </div>
            </div>
            ...
            <div class="card-body">
                ...
                <form method="get" class="mb-3">
                    ...
                    @if(in_array(($linkTab ?? ''), ['matched_inactive', 'mismatch_inactive'], true) && empty($stateCacheReady) && (int) ($counts[$linkTab] ?? 0) === 0)
                        <p class="small text-muted mt-2 mb-0">Active / Inactive counts need the live PurchasingPower catalog — click <em>Refresh live</em>, wait a minute, then reload.</p>
                    @endif
                </form>
                ...
                <ul class="nav nav-tabs nav-bordered mb-3" role="tablist">
                    ...
                    <li class="nav-item">
                        <a href="{{ request()->url() }}?link=unlinked&search_name={{ $qName }}&search_sku={{ $qSku }}" class="nav-link {{ ($linkTab ?? '') === 'unlinked' ? 'active' : '' }}">Not on PurchasingPower {{ $counts['unlinked'] ?? 0 }}</a>
                    </li>
                </ul>
                    ...
                    <table class="table table-bordered table-hover mb-0 table-sm">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>SKU</th>
                                <th>{{ ($linkTab ?? '') === 'not_in_shopify' ? 'Title (PurchasingPower)' : 'Title (Shopify)' }}</th>
                                <th>PurchasingPower ID</th>
                                <th>State</th>
                                <th>Inactive Reason</th>
                                <th>Shopify Qty</th>
                                <th>PurchasingPower Qty</th>
                                <th>Shopify Price</th>
                                <th>PurchasingPower Price</th>
                                <th>Link</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($products as $p)
                                @php $detailUrl = !empty($p->shopify_sku_id) ? route('marketplace.products.show', ['marketplace' => 'purchasingpower', 'shopifySku' => $p->shopify_sku_id]) : null; @endphp
                                <tr @if($detailUrl) style="cursor: pointer;" onclick="window.location='{{ $detailUrl }}'" @endif>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            {{ Str::limit($p->title ?? '—', 50) }}
                                        @endif
                                        @if(!empty($p->purchasingpower_title) && $p->purchasingpower_title !== $p->title)
                                            <div class="text-muted small">AE: {{ Str::limit($p->purchasingpower_title, 40) }}</div>
                                        @endif
                                    </td>
                                    <td class="small">{{ $p->product_id ?? '—' }}</td>
                                    <td class="small">
                                        @if(!empty($p->purchasingpower_state))
                                            @php $st = strtolower((string)$p->purchasingpower_state); @endphp
                                            <span class="badge {{ $st === 'active' ? 'bg-success-subtle text-success' : ($st === 'inactive' ? 'bg-warning-subtle text-warning' : 'bg-light text-muted') }}">{{ $p->purchasingpower_state }}</span>
                                        @else
                                            —
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    <td colspan="11" class="text-center text-muted py-4">
                                        @if(($linkTab ?? 'all') === 'not_in_shopify')
                                            No PurchasingPower listings found without a matching Shopify SKU.
                                        @else
                                            No Shopify SKUs found.
                                        @endif
                                        @if(($linkTab ?? 'all') === 'linked')
                                            None linked yet — click <strong>Sync PurchasingPower link map</strong> after SKUs exist in PurchasingPower.
                                        @elseif(($linkTab ?? 'all') === 'not_in_shopify')
                                            All synced PurchasingPower SKUs appear to exist in your Shopify catalog, or run <strong>Sync PurchasingPower link map</strong> first.
                                        @elseif($connected)
                                            Your Shopify catalog may be empty, or filters excluded all rows.
                                        @else
                                            <a href="{{ route('marketplace.manager.purchasingpower.connect') }}">Connect PurchasingPower</a> first.
                                        @endif
                                    </td>
                                </tr>
                            @endforelse
...
document.getElementById('btn-refresh-api')?.addEventListener('click', function () {
    ...
    var url = '{{ route('marketplace.manager.purchasingpower.refresh') }}';
    var page = 1;
    ...
    function setProgress(pageNum, totalPage, totalUpserted, message, totalCount) {
        ...
        var extra = totalCount ? ' (' + totalCount + ' products on PurchasingPower)' : '';
    ...
    if (!confirm('Sync all PurchasingPower listings and refresh SKU ↔ product_id mappings? This may take a few minutes.')) {
        return;
    }
    ...
    function runPage(reset) {
        syncNext(reset).then(function (data) {
            if (!data.success && data.done) {
                ...
                btn.disabled = false;
                btn.innerHTML = '<i class="ri-refresh-line"></i> Sync PurchasingPower link map';
                return;
            }
...
document.getElementById('btn-sync-mismatch-now')?.addEventListener('click', function () {
    ...
    if (!confirm('Push the actual live Shopify quantity to every Linked mismatch SKU on PurchasingPower right now (no queue)? This runs in batches and may take a few minutes.')) {
        return;
    }
    btn.disabled = true;
    var original = btn.innerHTML;
    var url = '{{ route('marketplace.manager.purchasingpower.sync.mismatch.inventory') }}';
    var offset = 0;
