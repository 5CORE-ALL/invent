...
<style>
    ...
    .tabulator .tabulator-header {
        background: linear-gradient(90deg, #fee2e2 0%, #f4f7fa 100%);
        border-bottom: 2px solid #dc2626; font-weight: 600;
    }
    ...
    .acl-linked-badge {
        ...
        background: #fef2f2; border: 1px solid #fecaca; color: #991b1b; font-size: 12px; max-width: 260px;
    }
                ...
                <div class="card-body">
                    <div class="mb-3 d-flex gap-2 flex-wrap align-items-center">
                        ...
                        <button type="button" id="acl-bulk-push" class="btn btn-sm btn-success ms-auto"
                                title="Import negative keywords into every linked campaign at once">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-muted small mb-2">Link one or more campaigns to <strong id="acl-source"></strong>. All linked campaigns will share negative keywords.</p>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" id="acl-save-btn">
    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-secondary text-white">
                <h5 class="modal-title"><i class="fas fa-ban"></i> Negative Keywords — <span id="acl-kw-title"></span></h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                ...
                <div id="acl-kw-empty" class="alert alert-info mb-0 d-none"><i class="fa fa-info-circle"></i> No negative keywords found for this campaign.</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div id="acl-cmp-body" class="d-none">
                    ...
                    <div id="acl-cmp-empty" class="alert alert-success mb-0 d-none"><i class="fa fa-check-circle"></i> Nothing missing — this campaign already has every negative keyword from its linked campaign(s).</div>
                    <div id="acl-cmp-table-wrap" class="table-responsive d-none" style="max-height: 55vh;">
                        <table class="table table-sm table-bordered table-hover mb-0">
                            <thead class="table-light">
                                <tr><th>#</th><th>Missing Negative</th><th style="width: 120px;">Match</th><th style="width: 110px;">Level</th></tr>
                            </thead>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const dataUrl = @json(route('amazon.ads.negative-link.data'));
    const campaignsUrl = @json(route('amazon.ads.negative-link.campaigns'));
    const keywordsUrl = @json(route('amazon.ads.negative-link.keywords'));
    const linkUrl = @json(route('amazon.ads.negative-link.link'));
    const removeUrl = @json(route('amazon.ads.negative-link.remove'));
    const pushUrl = @json(route('amazon.ads.negative-link.push'));
    const pushAllUrl = @json(route('amazon.ads.negative-link.push-all'));
    const compareUrl = @json(route('amazon.ads.negative-link.compare'));
    ...
    async function openCompareModal(campaign) {
        ...
        try {
            ...
            document.getElementById('acl-cmp-tbody').innerHTML = miss.map(function (m, i) {
                const level = (m.level === 'CAMPAIGN') ? 'Campaign' : (m.level === 'AD_GROUP' ? 'Ad group' : (m.level || '—'));
                return '<tr><td>' + (i + 1) + '</td><td>' + escHtml(m.keywordText) + '</td><td>' + escHtml(m.matchType) + '</td><td>' + escHtml(level) + '</td></tr>';
            }).join('');
    ...
    function keywordCountFormatter(cell) {
        ...
        return '<a href="#" class="acl-kw-open fw-semibold text-danger" data-campaign="' + escAttr(campaign) + '">' + escHtml(String(n)) + '</a>';
    }
    ...
    async function openKeywordsModal(campaign) {
        ...
        try {
            ...
            const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
            const json = await res.json();
            if (!json.success) throw new Error(json.message || 'Failed to load negative keywords.');
            loading.classList.add('d-none');
            const kws = json.keywords || [];
            if (kws.length === 0) { empty.classList.remove('d-none'); return; }
            tbody.innerHTML = kws.map(function (k) {
                const level = (k.level === 'CAMPAIGN') ? 'Campaign' : (k.level === 'AD_GROUP' ? 'Ad group' : (k.level || '—'));
                return '<tr>'
                    + '<td>' + escHtml(k.keyword) + '</td>'
                    + '<td>' + escHtml(k.match_type || '—') + '</td>'
                    + '<td>' + escHtml(level) + '</td>'
                    + '<td>' + escHtml(k.state || '—') + '</td>'
                    + '</tr>';
            }).join('');
    ...
    // ---- Grid events (remove / link buttons / negative count) ----
    ...
    table = new Tabulator('#acl-table', {
        ...
        columns: [
            ...
            { title: 'Negatives', field: 'keyword_count', hozAlign: 'center', width: 110, formatter: keywordCountFormatter },
