@extends('layouts.vertical', ['title' => 'Linked mismatch SKU', 'sidenav' => 'condensed'])
...
@section('content')
    @include('layouts.shared.page-title', [
        'page_title' => 'Linked mismatch SKU',
        'sub_title'  => '',
    ])
        ...
        <div class="card shadow-sm">
            <div class="card-body py-3">
                <div class="d-flex align-items-center flex-wrap gap-2">
                    <span class="badge bg-danger badge-mm-stat" id="stat-linked-mismatch" title="Linked mismatch SKU from Marketplace Manager listings (Shopify qty must not be less than marketplace qty)">
                        Linked mismatch SKU: <span id="total-linked-mismatch">{{ number_format(\App\Support\Marketplace\MappingChannelCounts::cachedLinkedMismatchTotalOrZero()) }}</span>
                    </span>
                </div>
            </div>
            <div class="card-body" style="padding: 0;">
                <div class="p-2 bg-light border-bottom">
                    <input type="text" id="linked-mismatch-search" class="form-control form-control-sm" placeholder="Search by Channel...">
                </div>
                <div id="linked-mismatch-table" style="height: calc(100vh - 280px);"></div>
            </div>
        </div>
    </div>
@endsection
...
<script>
    ...
    function updateSidebarLinkedMismatchCount(n) {
        const $b = $('.linked-mismatch-sku-badge');
        if (!$b.length) return;
        const v = Number(n || 0);
    ...
    function updateStats(rows, totalMismatch) {
        let total;
        if (totalMismatch !== undefined && totalMismatch !== null && !isNaN(Number(totalMismatch))) {
            total = Number(totalMismatch);
        } else {
            total = (rows || []).reduce((sum, r) => sum + Number(r.linked_mismatch_sku || 0), 0);
        }
        $('#total-linked-mismatch').text(total.toLocaleString('en-US'));
        updateSidebarLinkedMismatchCount(total);
    }
    ...
    $(document).ready(function() {
        table = new Tabulator("#linked-mismatch-table", {
            ajaxURL: "{{ url('/linked-mismatch-sku/channels-data') }}",
            ajaxResponse: function(_url, _params, response) {
                const data = (response && response.data) ? response.data : [];
                updateStats(data, response && response.total_linked_mismatch);
                return data;
            },
            ...
            initialSort: [{ column: "linked_mismatch_sku", dir: "desc" }],
            ...
            columns: [
                ...
                },
                {
                    ...
                    formatter: function(cell) {
                        ...
                        return `<a href="${escapeHtml(url)}" class="mm-listings-arrow mm-listings-arrow-on" title="Open ${escapeHtml(name)} Linked mismatch SKU listings" target="_self"><i class="fas fa-arrow-up-right-from-square"></i></a>`;
                    },
                },
                {
                    title: "Linked mismatch SKU",
                    field: "linked_mismatch_sku",
                    width: 230,
                    hozAlign: "center",
                    sorter: "number",
                    headerTooltip: "Linked listings where Shopify qty does not match marketplace qty (Shopify must not be less than marketplace)",
                    formatter: function(cell) {
        ...
        $('#linked-mismatch-search').on('input', function() {
