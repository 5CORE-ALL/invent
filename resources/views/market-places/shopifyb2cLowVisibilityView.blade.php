@extends('layouts.vertical', ['title' => 'Shopify B2C Low view', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    @include('layouts.shared/page-title', [
        'page_title' => 'Shopify B2C Low view',
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">Shopify B2C Low view</h4>
                    ...
                    <div class="table-container">
                        <table class="custom-resizable-table" id="shopifyB2CLowVisibility-table">
                            <thead>
                                <tr>
                    ...
                    <div id="data-loader" class="card-loader-overlay" style="display: none;">
                        <div class="loader-content">
                            ...
                            <div class="loader-text">Loading Shopify B2C Low view data...</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function() {
            $('#editPercentBtn').on('click', function() {
                ...
                if ($icon.hasClass('fa-pen')) {
                    ...
                    $.ajax({
                        // url: '/update-all-shopifyB2CLowVisibility-skus',
                        type: 'POST',
                        data: {
                            percent: percent,
            ...
            // Cache system
            const shopifyB2CLowVisibilityDataCache = {
                cache: {},
            ...
            window.addEventListener('load', function() {
                shopifyB2CLowVisibilityDataCache.clear();
            });
            ...
            function updateZeroViewDiv() {
                ...
                $('#zero-view-div').html(`Low view - ${zeroCount}`);
            }
            ...
            function loadData() {
                showLoader();
                return $.ajax({
                    url: '/shopifyB2C/low-visibility/view-data',
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
            ...
            function renderTable() {
                const $tbody = $('#shopifyB2CLowVisibility-table tbody');
            ...
            window.openModal = function(selectedItem, type) {
                try {
                    ...
                    const cachedData = shopifyB2CLowVisibilityDataCache.get(itemId);
                    const dataToUse = cachedData || itemData;

                    // Store the data in cache if it wasn't already
                    if (!cachedData) {
                        shopifyB2CLowVisibilityDataCache.set(itemId, itemData);
                    }
            ...
            function saveChanges(contentElement, title, slNo, isHyperlink, updatedValue, isCheckbox, rowElement) {
                ...
                shopifyB2CLowVisibilityDataCache.updateField(itemId, title, cacheUpdateValue);
                ...
                if (title === 'S Price') {
                    ...
                    if (cachedData) {
                        ...
                        shopifyB2CLowVisibilityDataCache.updateField(itemId, 'SPFT %', roundedSpft / 100);
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        ...
                        // Revert cache
                        shopifyB2CLowVisibilityDataCache.updateField(itemId, title, originalValue);
                        if (title === 'S Price') {
                            ...
                            if (originalSpft !== undefined) {
                                shopifyB2CLowVisibilityDataCache.updateField(itemId, 'SPFT %', originalSpft);
                            }
                        }
            ...
            function initResizableColumns() {
                const $table = $('#shopifyB2CLowVisibility-table');
            ...
            $('#saveReasonActionBtn').on('click', function() {
                ...
                if (sku) {
                    $.ajax({
                        url: '/shopifyb2c-low-visibility/reason-action/update',
                        method: 'POST',
                        data: {
                            sku: sku,
                            reason: reason,
