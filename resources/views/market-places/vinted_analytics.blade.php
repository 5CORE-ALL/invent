                ...
                <div class="card-body">
                    <div id="summary-stats" class="mb-2 p-3 bg-light rounded">
                        <div class="d-flex flex-wrap gap-2" role="group" aria-label="Summary metrics">
                            ...
                            <span class="badge bg-success fs-6 p-2 dp-filter-badge" id="dp-sold-pct-badge"
                                ...
                                title="Click to filter V L30 &gt; 0 (INV &gt; 0)">
                            ...
                            @include('partials.lmp-missing-badge', ['lmpBadgeId' => 'vinted-analytics-lmp-missing-badge', 'lmpChannelKey' => 'vinted'])
                            @include('partials.price-gt-lmp-badge', ['pglBadgeId' => 'vinted-analytics-price-gt-lmp-badge', 'pglChannelKey' => 'vinted', 'pglPriceField' => 'price'])
                            @include('partials.price-lt80-lmp-badge', ['pltBadgeId' => 'vinted-analytics-price-lt80-lmp-badge', 'pltChannelKey' => 'vinted', 'pltPriceField' => 'price'])
                            <span class="badge fs-6 p-2" id="vinted-analytics-blue-triangle-badge"
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
                        ...
                        <select id="dp-sold-filter" class="form-select form-select-sm" style="width:120px;"
                            title="Filter by Vinted sheet V L30. 0 / &gt; 0 require INV &gt; 0.">
                        ...
                        <a href="{{ route('vinted.analytics.sample') }}" class="btn btn-sm btn-outline-success" title="Download CSV template (parent, sku, price, l30)">
                    ...
                    <div id="vinted-table-wrapper" style="height: calc(100vh - 220px); display: flex; flex-direction: column;">
                        <div id="vinted-analytics-table" style="flex: 1;"></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function syncDpTriangleBadgeState() {
        $('#vinted-analytics-blue-triangle-badge').css({
    ...
    function updateSelectAllHeaderCheckbox() {
        const el = document.getElementById('vinted-select-all');
        if (!el || !table) return;
    ...
    function updateSummary(rowsInput) {
        ...
        if (window.PriceGtLmpBadge) {
            PriceGtLmpBadge.update('#vinted-analytics-price-gt-lmp-badge', lmpRows, 'vinted', 'price');
            if (window.PriceLt80LmpBadge) {
                PriceLt80LmpBadge.update('#vinted-analytics-price-lt80-lmp-badge', lmpRows, 'vinted', 'price');
            }
        }
        if (window.LmpMissingBadge) {
            LmpMissingBadge.update('#vinted-analytics-lmp-missing-badge', lmpRows, 'vinted');
        }
        let blue = 0;
        rows.forEach(function(row) { if (dpHasBlueTriangle(row)) blue++; });
        $('#vinted-analytics-blue-triangle-badge').html('<i class="fas fa-exclamation-triangle"></i> ' + blue.toLocaleString());
    ...
    $(document).ready(function() {
        ...
        table = new Tabulator("#vinted-analytics-table", {
            ajaxURL: "{{ route('vinted.analytics.data') }}",
            ...
            columns: [
                {
                    title: '<div class="vinted-select-header"><input type="checkbox" id="vinted-select-all"></div>',
                    field: "_select",
                    width: 38,
                    headerSort: false,
                    frozen: true,
                    ...
                    formatter: function(cell) {
                        ...
                        return `<input type="checkbox" class="vinted-sku-chk" data-sku="${safe}" ${checked}>`;
                    }
                },
                {
                    title: "Parent",
                ...
                },
                {
                    ...
                    hozAlign: "center",
                    width: 55,
                    headerTooltip: "Vinted units sold from /vinted/sheet last 30 sale days",
                    formatter: function(cell) {
                        return `<span style="font-weight:700;">${parseInt(cell.getValue(), 10) || 0}</span>`;
                    }
                },
        ...
        if (window.PriceGtLmpBadge) {
            PriceGtLmpBadge.bind({
                badge: '#vinted-analytics-price-gt-lmp-badge',
        ...
        if (window.PriceLt80LmpBadge) {
            PriceLt80LmpBadge.bind({
                badge: '#vinted-analytics-price-lt80-lmp-badge',
        ...
        $('#vinted-analytics-blue-triangle-badge').on('click', function() {
        ...
        $(document).on('change', '#vinted-select-all', function() {
            ...
            $('.vinted-sku-chk').each(function() {
                $(this).prop('checked', selectedSkus.has(String($(this).attr('data-sku'))));
            });
            updateSelectedCount();
        });
        $(document).on('change', '.vinted-sku-chk', function() {
        ...
        $('#dp-apply-target-gpft-btn').on('click', function() {
            ...
            if (!(denom > 0)) {
                showToast('Target GPFT% must be less than the Vinted margin', 'error');
                return;
            }
            if (selectedSkus.size === 0) {
                showToast('Select at least one SKU first', 'error');
        ...
        $('#import-form').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('vinted.analytics.import') }}",
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        if (typeof chPromoBatchClearThenSave === 'function' && opts.clearFirst !== false) {
            chPromoBatchClearThenSave(updates, function(next) {
                ...
                wipeFn: function(zeros) {
                    return $.ajax({
                        url: "{{ route('vinted.analytics.save.sprice') }}",
                        method: 'POST',
