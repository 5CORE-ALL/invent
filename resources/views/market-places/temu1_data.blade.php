...
<script>
    ...
    function temuClampSpriceBand2699(sprice) {
        const n = parseFloat(sprice);
        if (!isFinite(n) || !(n > 0)) return n > 0 ? n : 0;
        return +n.toFixed(2);
    }
    window.temuClampSpriceBand2699 = temuClampSpriceBand2699;
    ...
    function temu2RowSpriceValue(rowData, spriceOverride) {
        if (spriceOverride != null) {
            const n = parseFloat(spriceOverride);
            if (isFinite(n) && n > 0) return +n.toFixed(2);
        }
        if (typeof temuDisplayedSprice === 'function') {
            const shown = temuDisplayedSprice(rowData);
            if (shown > 0) return shown;
        }
        const stored = parseFloat(rowData && (rowData.sprice != null ? rowData.sprice : rowData.SPRICE)) || 0;
        return stored > 0 ? +stored.toFixed(2) : 0;
    }
    function temu2SRPriceFromRow(rowData, spriceOverride) {
        const sprice = temu2RowSpriceValue(rowData, spriceOverride);
    ...
    function temu2SProfit(rowData, sprice) {
        const recovery = temu2SRecovery(temu2RowSpriceValue(rowData, sprice));
    ...
    function temu2SPftProfit(rowData, sprice) {
        const s = temu2RowSpriceValue(rowData, sprice);
        if (s <= 0) return null;
        const margin = temuSpriceMargin(rowData);
        const lp = parseFloat(rowData && rowData.lp) || 0;
        const temuShip = parseFloat(rowData && rowData.temu_ship) || 0;
    ...
    function temuPushBaseFromSprice(sprice) {
        ...
        if (s >= 34 - 0.001 && s <= 37 + 0.001) {
            const band = (s * TEMU2_S_RECOVERY_RATE) - 2.99;
            return band > 0 ? +band.toFixed(2) : null;
        }
    ...
    /** Temu Recovery shown in the LMP column: ≤$27 → (Price × 0.85) + 2.99; else Price × 0.85. */
    function temuLmpRecovery(price) {
        const p = temuParseMoney(price);
        if (!(p > 0)) return null;
    ...
    function temuAmzRefPrice(row) {
        return temuParseMoney(row && (row.a_price != null ? row.a_price
            : (row['A Price'] != null ? row['A Price']
                : (row.amazon_price != null ? row.amazon_price : row['Amz Price']))));
    }

    function temuEbayRefPrice(row) {
        const e = temuParseMoney(row && (row.e_price != null ? row.e_price
            : (row['E Price'] != null ? row['E Price'] : row.ebay_price)));
        const e2 = temuParseMoney(row && (row.e2_price != null ? row.e2_price
            : (row['E2 Price'] != null ? row['E2 Price'] : row.ebay2_price)));
    ...
    function temuSpriceCapResult(row, rawSprice, extra) {
        ...
        if (!zeroSoldOwns) {
            if (ebay > 0 && sprice > ebay + 0.0001) sprice = +ebay.toFixed(2);
            if (amz > 0 && sprice > amz + 0.0001) sprice = +amz.toFixed(2);
            if (lmp > 0 && sprice > lmp + 0.0001) sprice = +lmp.toFixed(2);
        }

        const labels = [];
        if (ebay > 0 && Math.abs(sprice - ebay) <= 0.015 && discounted > ebay + 0.0001) labels.push('EB');
        if (amz > 0 && Math.abs(sprice - amz) <= 0.015 && discounted > amz + 0.0001) labels.push('Amz');
        const lmpAlert = lmp > 0 && Math.abs(sprice - lmp) <= 0.015 && discounted + 0.0001 >= lmp;
        return { sprice: +sprice.toFixed(2), labels: labels, lmpAlert: lmpAlert, amz: amz, ebay: ebay, lmp: lmp };
    }

    const TEMU_SPRICE_BATCH_URL = '/temu-pricing/save-sprice-batch';
    const temuSavedSpriceBySku = {};
    const temuShownSpricePending = {};
    let temuShownSpriceTimer = null;
    function temuQueueShownSpriceSave(sku, value) {
        sku = String(sku || '').trim();
        if (!sku) return;
        temuShownSpricePending[sku] = (parseFloat(value) > 0) ? +Number(value).toFixed(2) : 0;
        if (temuShownSpriceTimer) clearTimeout(temuShownSpriceTimer);
        temuShownSpriceTimer = setTimeout(function() {
            if (typeof window.temuFlushShownSpriceSaves === 'function') {
                window.temuFlushShownSpriceSaves();
            }
        }, 600);
    }
    window.temuQueueShownSpriceSave = temuQueueShownSpriceSave;

    /** Exact S PRC shown in the cell — Dil + EB/Amz/LMP, then clamp. Same number we persist. */
    function temuSpriceCapSnapshot(row, extra) {
        extra = extra || {};
        if (!row || (typeof isTemu2ParentRow === 'function' && isTemu2ParentRow(row))) {
            return { sprice: 0, labels: [], lmpAlert: false, lmp: 0, amz: 0, ebay: 0 };
        }
        const cap = typeof temuSpriceCapResult === 'function'
            ? temuSpriceCapResult(row, extra.candidate, extra)
            : { sprice: 0, labels: [], lmpAlert: false, lmp: 0, amz: 0, ebay: 0 };
        let value = (cap && cap.sprice > 0) ? +Number(cap.sprice).toFixed(2) : 0;
        if (!(value > 0) && !extra.use_passed_as_discounted && window.SpriceLmpCap) {
            const discounted = typeof temuDiscountedPrice === 'function' ? temuDiscountedPrice(row) : 0;
            const lmpCap = SpriceLmpCap.apply(row, discounted);
            if (lmpCap && lmpCap.shown > 0) value = +Number(lmpCap.shown).toFixed(2);
        }
        if (typeof temuClampSpriceBand2699 === 'function') {
            value = temuClampSpriceBand2699(value) || 0;
        }
        return {
            sprice: value > 0 ? +Number(value).toFixed(2) : 0,
    ...
    function temuSpriceShownValue(row) {
        return temuSpriceCapSnapshot(row).sprice;
    }
    window.temuSpriceShownValue = temuSpriceShownValue;

    function temuPrepareSpriceForSave(rowData, sprice, extra) {
        extra = extra || {};
        if (extra.use_passed_as_discounted && parseFloat(sprice) > 0) {
            extra.candidate = sprice;
            const ruled = temuSpriceCapSnapshot(rowData, extra).sprice;
            if (ruled > 0) return ruled;
        }
        const shown = temuSpriceShownValue(rowData);
        if (shown > 0) return shown;
        const cap = temuSpriceCapResult(rowData, sprice, extra);
        const out = cap.sprice > 0 ? cap.sprice : (parseFloat(sprice) || sprice);
        return (parseFloat(out) > 0) ? +Number(out).toFixed(2) : 0;
    }

    /** Wipe stored S PRC, then persist Discounted → lowest of EB / Amz / LMP. */
    function temuClearAndPrepareSprice(row, candidate, extra) {
        extra = extra || {};
        ...
        const start = (isFinite(passed) && passed > 0) ? passed : temuDiscountedPrice(data);
        const opts = Object.assign({ use_passed_as_discounted: start > 0 }, extra);
        const v = parseFloat(temuPrepareSpriceForSave(data, start, opts));
        return (isFinite(v) && v > 0) ? +v.toFixed(2) : 0;
    }
    window.temuPrepareSpriceForSave = temuPrepareSpriceForSave;
    window.temuClearAndPrepareSprice = temuClearAndPrepareSprice;

    /** Single rule price used for both display and save. Never listing Full / R Price. */
    function temuRuleSprice(row, extra) {
        extra = extra || {};
        if (extra.candidate || extra.use_passed_as_discounted) {
            return temuSpriceCapSnapshot(row, extra).sprice;
        }
        return temuSpriceShownValue(row);
    }
    window.temuRuleSprice = temuRuleSprice;

    function temuSpriceCellModel(row) {
        if (!row || (typeof isTemu2ParentRow === 'function' && isTemu2ParentRow(row))) {
            return { value: 0, labels: [], lmpAlert: false, lmp: 0, amz: 0, ebay: 0 };
        }
        const cap = temuSpriceCapSnapshot(row);
        const value = cap.sprice;
        const sku = String(row.sku || '').trim();
        const stored = parseFloat(row.SPRICE != null ? row.SPRICE : row.sprice) || 0;
        if (sku && temuSavedSpriceBySku[sku] == null) {
            temuSavedSpriceBySku[sku] = stored > 0 ? +stored.toFixed(2) : 0;
        }
        row.SPRICE = value > 0 ? value : null;
        row.sprice = value > 0 ? value : null;
        row.has_custom_sprice = value > 0;
        const saved = sku ? (Number(temuSavedSpriceBySku[sku]) || 0) : stored;
        const expanded = window.ParentExpand && typeof ParentExpand.isExpanded === 'function' && ParentExpand.isExpanded();
        if (sku && !expanded && Math.abs(saved - value) > 0.009) {
            temuQueueShownSpriceSave(sku, value);
        }
        return {
            value: value,
            labels: cap.labels || [],
            lmpAlert: !!cap.lmpAlert,
            lmp: cap.lmp || 0,
            amz: cap.amz || 0,
            ebay: cap.ebay || 0,
        };
    ...
    function temuSortLinksRow(a, b, aRow, bRow) {
        const count = function(d) {
            return ((d && d.seller_link) ? 1 : 0) + ((d && d.buyer_link) ? 1 : 0);
        };
        return count(aRow.getData()) - count(bRow.getData());
    }
    function temuSortPushBase(a, b, aRow, bRow) {
        const val = function(d) {
            if (typeof temuPushSpriceAndBase === 'function') {
                const cap = temuPushSpriceAndBase(d);
                return (cap && cap.pushBase != null) ? cap.pushBase : '';
            }
            if (typeof temuPushBaseFromSprice !== 'function') return '';
            const sprice = typeof temuDisplayedSprice === 'function' ? temuDisplayedSprice(d) : 0;
            return temuPushBaseFromSprice(sprice);
        };
        return temuSortNum(val(aRow.getData()), val(bRow.getData()));
    }
    function temuSortPushSprice(a, b, aRow, bRow) {
        const val = function(d) {
            return typeof temuDisplayedSprice === 'function' ? temuDisplayedSprice(d) : (parseFloat(d && d.sprice) || 0);
        };
        return temuSortNum(val(aRow.getData()), val(bRow.getData()));
    }
    ...
    function syncTemuCapBadgeState() {
    ...
    $(document).ready(function() {
        ...
        function temuQueueSpricePushAfterSave(sku, sprice, row) {
            if (typeof enqueueTemuListingPushAfterSave === 'function') {
                return enqueueTemuListingPushAfterSave(sku, sprice, row) !== false;
            }
            return false;
        }
            ...
            return new Promise((resolve, reject) => {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        temuSavedSpriceBySku[sku] = newPriceNum > 0 ? +Number(newPriceNum).toFixed(2) : 0;
        ...
        let temuShownSpriceSaving = false;
        function temuFlushShownSpriceSaves() {
            if (temuShownSpriceSaving) {
                setTimeout(temuFlushShownSpriceSaves, 500);
                return;
            }
            const skus = Object.keys(temuShownSpricePending);
            if (!skus.length) return;
            const updates = skus.map(function(sku) {
                return { sku: sku, sprice: temuShownSpricePending[sku] };
            });
            skus.forEach(function(sku) { delete temuShownSpricePending[sku]; });
            temuShownSpriceSaving = true;
            ...
            for (let i = 0; i < updates.length; i += size) {
                ...
                chain = chain.then(function() {
                    return $.ajax({
                        ...
                        ok += parseInt((res && res.ok) || 0, 10) || 0;
                        chunk.forEach(function(u) {
                            temuSavedSpriceBySku[u.sku] = u.sprice > 0 ? +Number(u.sprice).toFixed(2) : 0;
                            if (typeof temuPatchLoadedSprice === 'function') {
                                temuPatchLoadedSprice(u.sku, u.sprice > 0 ? u.sprice : null);
                            }
                        });
                    });
                });
            ...
            chain.then(function() {
                temuShownSpriceSaving = false;
            }).catch(function() {
                temuShownSpriceSaving = false;
            });
        }
        window.temuFlushShownSpriceSaves = temuFlushShownSpriceSaves;

        function temuSyncShownSpriceToSaved() {
            if (!table) return;
            ...
            rows.forEach(function(row) {
                ...
                if (!d || (typeof isTemu2ParentRow === 'function' && isTemu2ParentRow(d))) return;
                const sku = String(d.sku || '').trim();
                if (!sku) return;
                const shown = typeof temuSpriceShownValue === 'function' ? temuSpriceShownValue(d) : 0;
                const stored = parseFloat(d.SPRICE != null ? d.SPRICE : d.sprice) || 0;
                if (temuSavedSpriceBySku[sku] == null) {
                    temuSavedSpriceBySku[sku] = stored > 0 ? +stored.toFixed(2) : 0;
                }
                d.sprice = shown > 0 ? shown : null;
                d.SPRICE = shown > 0 ? shown : null;
                d.has_custom_sprice = shown > 0;
                if (typeof temuPatchLoadedSprice === 'function') {
                    temuPatchLoadedSprice(sku, shown > 0 ? shown : null);
                }
                if (Math.abs((temuSavedSpriceBySku[sku] || 0) - shown) > 0.009) {
                    temuQueueShownSpriceSave(sku, shown);
                }
            });
            temuFlushShownSpriceSaves();
        }
        function temuClearThenApplyAllRules() {
            temuSyncShownSpriceToSaved();
        }
        window.temuClearThenApplyAllRules = temuClearThenApplyAllRules;
        window.temuSyncShownSpriceToSaved = temuSyncShownSpriceToSaved;
                ...
                if (selectedSkus.has(sku)) {
                    ...
                    if (samePriceModeActive || currentPrice > 0) {
                        ...
                        if (typeof temuClearAndPrepareSprice === 'function') {
                            const v = temuClearAndPrepareSprice(tableRow || row, newPriceNum);
                            if (v > 0) ruled = v;
                        } else if (typeof temuPrepareSpriceForSave === 'function') {
                            const v = parseFloat(temuPrepareSpriceForSave(tableRow ? tableRow.getData() : row, newPriceNum, {
                                use_passed_as_discounted: true
                            }));
        ...
        function updateSummary() {
            ...
            (table ? table.getData() : []).forEach(function(row) {
                ...
                if (typeof temuHasAmzCap === 'function' && temuHasAmzCap(row)) amzCapCount++;
                if (typeof temuHasEbCap === 'function' && temuHasEbCap(row)) ebCapCount++;
            });
        ...
        table = new Tabulator("#temu-table", {
            ...
            columns: [
                {
                    title: "<div style='transform: rotate(0deg) !important; display: flex; justify-content: center; align-items: center;'><input type='checkbox' id='select-all-checkbox' title='Select / clear all SKUs on this page for bulk S PRC push' style='transform: rotate(0deg) !important; width: 16px; height: 16px; cursor: pointer;'></div>",
                    field: "_select",
                    hozAlign: "center",
                    headerSort: false,
                ...
                },
                {
                    ...
                    sorter: temuSortBy(function(d) {
                        ...
                        return (base > 0 && typeof temu2FullPriceFromBase === 'function')
                ...
                },
                {
                    ...
                    minWidth: 80,
                    sorter: temuSortBy(function(d) {
                        const pft = typeof temu2PftDollars === 'function' ? temu2PftDollars(d) : null;
                ...
                },
                {
                    ...
                    hozAlign: "center",
                    sorter: temuSortBy(function(d) {
                        const npft = typeof temu2NpftDollars === 'function' ? temu2NpftDollars(d) : null;
                        const temuPrice = typeof temu2FullPriceFromRow === 'function' ? temu2FullPriceFromRow(d) : 0;
                        return (npft != null && temuPrice > 0) ? (npft / temuPrice) * 100 : 0;
                    }),
                    visible: false,
                ...
                },
                {
                    ...
                    sorter: temuSortBy(function(d) {
                        const npft = typeof temu2NpftDollars === 'function' ? temu2NpftDollars(d) : null;
                        const lp = parseFloat(d.lp) || 0;
                        return (npft != null && lp > 0) ? (npft / lp) * 100 : 0;
                    }),
                    visible: false,
                ...
                },
                {
                    ...
                    headerSort: true,
                    sorter: temuSortBy(function(d) {
                        return typeof getTemu2DisplayLmp === 'function' ? getTemu2DisplayLmp(d) : (parseFloat(d.lmp) || 0);
                    }),
                ...
                },
                {
                    ...
                    formatter: function(cell) {
                        ...
                        const zeroSoldOwns = typeof chPromoTemuZeroSoldOwnsSprice === 'function'
                            && chPromoTemuZeroSoldOwnsSprice(rowData);
                        const overLmp = !zeroSoldOwns && (model.lmpAlert || (lmp > 0 && value + 0.0001 >= lmp));
                        ...
                        return `<span style="white-space:nowrap;display:inline-flex;align-items:center;gap:2px;">${priceHtml}${capHtml}${redTri}${blueTri}</span>`;
                    }
                },
                {
                    title: "S Recovery",
                    ...
                    hozAlign: "center",
                    sorter: temuSortBy(function(d) {
                        const sprice = typeof temuDisplayedSprice === 'function' ? temuDisplayedSprice(d) : (parseFloat(d.sprice) || 0);
                        return (sprice > 0 && typeof temu2SRecovery === 'function') ? temu2SRecovery(sprice) : 0;
                    }),
        ...
        function syncLmpModalToTable(entries) {
            if (typeof table === 'undefined' || !table || !lmpModalSku) return;
            const meta = typeof getTemu2LowestLmpMeta === 'function'
                ? getTemu2LowestLmpMeta({ lmp_entries: entries })
                : { raw: 0 };
            const raw = meta.raw > 0 ? meta.raw : null;
            const recovery = (raw != null && typeof temuLmpRecovery === 'function') ? temuLmpRecovery(raw) : raw;
            let lowestLink = null;
            (entries || []).forEach(function(e) {
                if (e.ignored) return;
                const eff = typeof temuLmpEntryEffective === 'function' ? temuLmpEntryEffective(e) : null;
                if (eff && raw != null && Math.abs(eff.raw - raw) < 0.00001 && !lowestLink) {
                    lowestLink = e.link || null;
                }
            });
            const targets = {};
            targets[String(lmpModalSku)] = true;
            try {
                table.getRows().forEach(function(row) {
                    const d = row.getData() || {};
                    const sku = String(d.sku || '');
                    const linked = Array.isArray(d.linked_lmp_skus) ? d.linked_lmp_skus : [];
                    if (targets[sku] || linked.indexOf(lmpModalSku) !== -1 || linked.some(function(s) { return targets[String(s)]; })) {
                        targets[sku] = true;
                        linked.forEach(function(s) { if (s) targets[String(s)] = true; });
                    }
                });
            } catch (e) { /* ignore */ }
            try {
                table.getRows().forEach(function(row) {
                    const d = row.getData() || {};
                    if (!targets[String(d.sku || '')]) return;
                    row.update({
                        lmp_entries: entries,
                        lmp_raw: raw,
                        lmp: recovery,
        ...
        function updateLmpLowestHighlight() {
            ...
            $('#lmpEntriesContainer .lmp-entry-row').each(function() {
                const tr = $(this);
                tr.removeClass('table-dark lmp-lowest-row');
                tr.find('.lmp-lowest-badge').empty();
                updateTemu2LmpPriceD(tr);
                if (tr.hasClass('lmp-ignored-row')) return;
        ...
        $(document).off('click.templmp', '#lmpEntriesContainer .lmp-remove-row').on('click.templmp', '#lmpEntriesContainer .lmp-remove-row', function(e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        $(document).off('input.templmp', '#lmpEntriesContainer .lmp-price, #lmpEntriesContainer .lmp-delivery, #lmpEntriesContainer .lmp-link')
            .on('input.templmp', '#lmpEntriesContainer .lmp-price, #lmpEntriesContainer .lmp-delivery, #lmpEntriesContainer .lmp-link', function() {
                const $tr = $(this).closest('tr');
                updateTemu2LmpPriceD($tr);
                updateLmpLowestHighlight();
                scheduleLmpAutosave();
            });
        $(document).off('click.templmp', '#lmpEntriesContainer .lmp-open-link').on('click.templmp', '#lmpEntriesContainer .lmp-open-link', function(e) {
            e.preventDefault();
        ...
        $('#lmpAddRowBtn').on('click', function() {
            ...
            appendLmpTableRow($('#lmpEntriesContainer'), price || '', delivery || '', link || '', false, lmpModalSku);
            updateLmpLowestHighlight();
            $('#lmpNewPrice').val('');
            $('#lmpNewDelivery').val('');
            $('#lmpNewLink').val('');
        ...
        table.on('cellEdited', function(cell) {
            ...
            // Handle SPRICE edit
            if (field === 'sprice') {
                const typed = parseFloat(cell.getValue());
                if (typed < 0) {
                    showToast('SPRICE cannot be negative', 'error');
                    cell.restoreOldValue();
                    return;
                }
                const newSprice = typeof temuClearAndPrepareSprice === 'function'
                    ? temuClearAndPrepareSprice(row, typed)
        ...
        table.on('dataLoaded', function(data) {
            ...
            setTimeout(function() {
                if (typeof temuClearThenApplyAllRules === 'function') {
                    temuClearThenApplyAllRules();
                }
            }, 400);
        });
