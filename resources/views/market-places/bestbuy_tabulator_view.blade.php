...
@section('content')
    @include('layouts.shared.page-title', [
        'page_title' => 'Best Buy Pricing',
        'sub_title' => 'Best Buy Pricing',
    ])
        ...
        <div class="card shadow-sm">
            <div class="card-body py-2">
                ...
                <div id="summary-stats" class="d-flex align-items-center flex-wrap gap-1">
                        ...
                        <span class="badge fs-6 p-2" id="more-amz-badge" style="background-color: #28a745; color: white; font-weight: bold; cursor: pointer;" title="Click to filter prices greater than Amz">&gt; Amz</span>
    ...
    <!-- LMP Competitors Modal (BestbuySkuCompetitor source) -->
    <div class="modal fade" id="lmpModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">
                        <i class="fa fa-shopping-cart"></i> Best Buy Competitors for SKU: <span id="lmpSku"></span>
                    </h5>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <form id="addCompetitorForm">
                                ...
                                <div class="row g-3">
                                    <div class="col-md-3">
                                        <label class="form-label">Best Buy Item ID *</label>
...
<script>
    ...
    function bestbuyRowLmp(data) {
        const entries = data && Array.isArray(data.lmp_entries) ? data.lmp_entries : [];
        if (entries.length && window.LmpIgnore && typeof LmpIgnore.l1 === 'function') {
            const n = Number(LmpIgnore.l1(entries));
            return (isFinite(n) && n > 0) ? Math.round(n * 100) / 100 : 0;
        }
        if (window.LmpIgnore && typeof LmpIgnore.effectiveLmp === 'function') {
            const n = Number(LmpIgnore.effectiveLmp(data));
    ...
    function bestbuyPriceIsIgnoredLmp(data, price) {
        const p = Math.round((parseFloat(price) || 0) * 100) / 100;
        if (!(p > 0) || !data || !Array.isArray(data.lmp_entries) || !window.LmpIgnore) return false;
        const active = bestbuyRowLmp(data);
        if (active > 0 && Math.abs(active - p) < 0.02) return false;
        return data.lmp_entries.some(function(item) {
            if (!LmpIgnore.isIgnored(item)) return false;
            const total = Math.round((parseFloat(item.total_price)
                || ((parseFloat(item.price) || 0) + (parseFloat(item.shipping_cost) || 0))) * 100) / 100;
            return total > 0 && Math.abs(total - p) < 0.02;
        });
    }
    /** Amazon floor first, then LMP. If LMP is below A Price, keep A Price. */
    function bestbuyCapAfterAmz(data, sprice) {
        let s = bestbuyApplyAmzFloor(data, sprice);
        if (!(s > 0)) return 0;
        const amz = bestbuyAmazonPrice(data);
        const lmp = bestbuyRowLmp(data);
        if (!(lmp > 0) || s + 0.0001 < lmp) return s;
        if (amz > 0 && lmp + 0.0001 < amz) return amz;
        return lmp;
    }
    ...
    function bestbuySavedSprice(data) {
    ...
    function bestbuyHasAmzCap(data) {
        ...
        const saved = bestbuySavedSprice(data);
        if ((discounted > 0 && discounted < amz - 0.0001) || (saved > 0 && saved < amz - 0.0001)) return true;
        const lmp = bestbuyRowLmp(data);
        return lmp > 0 && lmp + 0.0001 < amz;
    }
    ...
    function bestbuyStdPrcChangeDotHtml(stdPrc, comparePrice) {
        const meta = bestbuyStdPrcChangeDotMeta(stdPrc, comparePrice);
        if (!meta) return '';
        return '<span style="display:inline-block;width:8px;height:8px;border-radius:50%;' +
            'background:' + meta.color + ';flex-shrink:0;" title="' + meta.title + ' — Std Prc (shared with Amazon)"></span>';
    }

    function applyBestbuyStandardPriceToLinkedRows(sku, std, appliedSkus) {
    ...
    document.addEventListener('lmp-modal-sp-saved', function(e) {
        ...
        applyBestbuyStandardPriceToLinkedRows(sku, saved, detail.applied_skus);
    });
    ...
    $(document).ready(function() {
        ...
        /*
         * Target ROI% / Target GPFT% bulk apply (BestBuy, margin = row.percentage or 0.80)
         * --------------------------------------------------------------------------------
         * Back-solves SPRICE so the resulting SROI / SGPFT column matches the entered target.
         * BestBuy's SGPFT / SROI formula (used in the cellEdited handler at lines 1311-1313)
         * includes shipping:
         ...
         * Optimistic SGPFT / SPFT / SROI written client-side, then the existing
         * /bestbuy-save-sprice endpoint reconciles them server-side via saveSpriceUpdates().
         ...
         */
        function bestbuyApplyTargetBackSolve(computeFn, labelPrefix) {
            if (selectedSkus.size === 0) {
                showToast('Please select at least one SKU first (turn on Decrease / Increase to reveal checkboxes)', 'error');
                return;
            }
            ...
            if (updates.length === 0) {
                if (skippedHigh.length > 0) {
                    showToast(`${labelPrefix} too high — must be less than each row's take-home margin (typically < 80%).`, 'error');
                } else {
        ...
        $('#apply-target-roi-btn').on('click', function () {
            ...
            bestbuyApplyTargetBackSolve(function (lp, ship, margin) {
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if (response.price_push_success_count !== undefined || response.price_push_failed_count !== undefined) {
                            ...
                            if (pushFail > 0) {
                                let pushMsg = `BestBuy price push: ${pushOk} success, ${pushFail} failed`;
        ...
        // ========== LMP + Sku Link LMP (BestbuySkuCompetitor source) ==========
        ...
        function loadBestbuyCompetitorsModal(sku, linkedLmpSkus) {
            $('#lmpSku').text(sku);
            $('#addCompSku').val(sku);
            $('#addCompItemId').val('');
            $('#addCompPrice').val('');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success && response.competitors && response.competitors.length > 0) {
                        ...
                        renderBestbuyCompetitorsList(response.competitors, response.lowest_price);
                    } else {
                        $('#lmpDataList').html(`
                            <div class="alert alert-warning">
        ...
        function renderBestbuyCompetitorsList(competitors, lowestPrice) {
            ...
            if (l1Price === null && lowestPrice != null && !(window.LmpIgnore && LmpIgnore.l1)) {
                ...
                html += `<tr class="${rowClass}">
                    ...
                    <td>
                        <div class="btn-group btn-group-sm">
                            ...
                            <button class="btn btn-sm btn-danger delete-bestbuy-lmp-btn"
        ...
        function refreshBestbuySpriceAfterLmpIgnore() {
            ...
            (table.getRows('all') || table.getRows() || []).forEach(function(row) {
                const d = row.getData();
                if (!d || isBestbuyParentRow(d)) return;
                const key = String(d['(Child) sku'] || '').replace(/\s+/g, ' ').trim().toUpperCase();
                if (!targets[key]) return;
                const next = Math.round((bestbuyDisplayedSprice(d) || 0) * 100) / 100;
                const saved = Math.round((bestbuySavedSprice(d) || 0) * 100) / 100;
                try { row.reformat(); } catch (e) { /* ignore */ }
                const clearIgnored = !(next > 0) && bestbuyPriceIsIgnoredLmp(d, saved);
                if (!clearIgnored && (!(next > 0) || Math.abs(saved - next) < 0.005)) return;
                const stored = clearIgnored ? 0 : next;
                try { row.update({ SPRICE: stored, sprice: stored, has_custom_sprice: stored > 0 }); } catch (e) { /* ignore */ }
                if (typeof saveChannelSprice === 'function') {
                    saveChannelSprice(d['(Child) sku'], stored, true, {
                        skip_push: true,
                        queue_push: false,
                        row: row
                    });
        ...
        function patchBestbuyGridLmp() {
            if (!window.LmpIgnore) return;
            LmpIgnore.patchGrid({
            ...
            refreshBestbuySpriceAfterLmpIgnore();
        }
        LmpIgnore.bind({
            marketplace: 'bestbuy',
        ...
        $(document).on('click', '.view-lmp-competitors', function(e) {
            ...
            loadBestbuyCompetitorsModal(sku, linkedSkus);
        });
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $('#addCompLink').val('');
                        $('#addCompTitle').val('');
                        loadBestbuyCompetitorsModal($('#addCompSku').val(), currentLmpData.linkedLmpSkus);
        ...
        $(document).on('click', '.delete-bestbuy-lmp-btn', function() {
            const $btn = $(this);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        loadBestbuyCompetitorsModal(currentLmpData.sku, currentLmpData.linkedLmpSkus);
                        if (table) table.replaceData();
                    } else {
            ...
            columns: [
                ...
                },
                {
                    ...
                    formatter: function(cell) {
                        ...
                        const lmpPrice = bestbuyRowLmp(rowData);
                        const sku = rowData['(Child) sku'] || '';
                        const entryCount = Array.isArray(rowData.lmp_entries) ? rowData.lmp_entries.length : 0;
                        const totalCompetitors = entryCount || rowData.lmp_entries_total || 0;
                        const ignoredOnly = (!lmpPrice && window.LmpIgnore && entryCount)
                            ? (Number(LmpIgnore.ignoredPrice(rowData.lmp_entries)) || 0)
                            : 0;
                        ...
                        const bbPrice = parseFloat(rowData['BB Price']) || 0;
                        ...
                        if (!lmpPrice && totalCompetitors === 0) {
                            ...
                            const priceColor = (bbPrice > 0 && finalPrice < bbPrice) ? '#dc3545' : '#28a745';
                            html += `<span style="color: ${priceColor}; font-weight: 600; font-size: 14px;">$${finalPrice.toFixed(2)}</span>`;
                        } else if (ignoredOnly > 0) {
                            html += '<span style="text-decoration:line-through;color:#94a3b8;font-weight:600;font-size:14px;" title="Ignored — not used for S PRC">$'
                                + Number(ignoredOnly).toFixed(2) + '</span>';
                        }
                ...
                },
                {
                    ...
                    formatter: function(cell) {
                        ...
                        const lmp = bestbuyRowLmp(rowData);
                        ...
                        if (!(value > 0)) return '';
                        const amzNow = bestbuyAmazonPrice(rowData);
                        const lmpBelowAmz = amzNow > 0 && lmp > 0 && lmp + 0.0001 < amzNow;
                        const overLmp = !lmpBelowAmz && lmp > 0 && value + 0.0001 >= lmp;
        ...
        $(document).on('change', '.nr-req-dropdown', function() {
            ...
            $.ajax({
                url: '{{ url("/bestbuy-update-listed-live") }}',
                method: 'POST',
                data: {
                    sku: sku,
                    nr_req: newValue,
            ...
            if (field === 'STANDARD_PRICE') {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        applyBestbuyStandardPriceToLinkedRows(sku, saved, response.applied_skus);
        ...
        function updateSummary() {
            ...
            data.forEach(row => {
                ...
                const dil = parseFloat(row['BB Dil%']) || 0;
                if (dil > 0) {
                    totalDil += dil;
                    dilCount++;
                }
                ...
                const l30 = parseFloat(row['BB L30']) || 0;
                totalCogs += lp * l30;
        ...
        function saveColumnVisibilityToServer() {
            ...
            $.ajax({
                url: '/bestbuy-pricing-column-visibility',
                method: 'POST',
                data: {
