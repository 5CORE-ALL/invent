...
@section('content')
    ...
    <!-- LMP Competitors Modal (same as Amz page) -->
...
@section('script-bottom')
    ...
    <script>
        ...
        function sheinStdPrcChangeDotHtml(stdPrc, comparePrice) {
            const meta = sheinStdPrcChangeDotMeta(stdPrc, comparePrice);
            if (!meta) return '';
            return '<span style="display:inline-block;width:8px;height:8px;border-radius:50%;' +
                'background:' + meta.color + ';flex-shrink:0;" title="' + meta.title + ' — Std Prc (shared with Amazon)"></span>';
        }

        function applySheinStandardPriceToLinkedRows(sku, std, appliedSkus) {
        ...
        document.addEventListener('lmp-modal-sp-saved', function(e) {
            ...
            applySheinStandardPriceToLinkedRows(sku, saved, detail.applied_skus);
        });
            ...
            table = new Tabulator("#shein-pricing-table", {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        hozAlign: "center",
                        headerTooltip: "Standard Price (Std Prc) — same shared value as /amazon-tabulator-view. Editable; saves to all Sku Link LMP siblings. Dot vs Sp. Price / LMP.",
                        editor: "input",
                        width: 70,
                        sorter: "number",
                        ...
                        formatter: function(cell) {
                            ...
                            const lmpPrice = parseFloat(d.lmp_price || 0) || 0;
                ...
                if (field === 'STANDARD_PRICE') {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            applySheinStandardPriceToLinkedRows(sku, saved, response.applied_skus);
                        ...
                        error: function() {
                            if (window.toastr) toastr.error('Failed to save Std Prc');
                            else if (typeof showToast === 'function') showToast('Failed to save Std Prc', 'error');
                        }
                    });
                    return;
                }
