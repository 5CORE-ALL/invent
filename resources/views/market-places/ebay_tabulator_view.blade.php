                ...
                <div class="modal-header bg-primary text-white">
                    ...
                    <div class="d-flex align-items-center gap-2 ms-auto">
                        <button type="button" id="lmpPullApiBtn" class="btn btn-sm btn-light"
                            title="Pull live prices. If the competitor listing has variations, every matching family SKU is updated (4PCS, 2PCS, WH, BLK).">
                ...
                <div class="modal-footer py-2 d-flex justify-content-between">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button type="button" class="btn btn-sm btn-success" id="sbid-slab-apply-btn"
                                title="Autopush is on. Changing a slab saves the rule and pushes the new S Bid to eBay.">
    ...
    <script>
        ...
        function ebaySpriceAmount(rowData) {
            if (typeof ebayDisplayedSprice === 'function') {
                const shown = ebayDisplayedSprice(rowData);
        ...
        function ebayShouldCapSpriceToLmp(rowData, sprice) {
            ...
            const sgroiAtLmp = ebaySgroiAtPrice(rowData, lmp);
        ...
        window.ebayCapSpriceToLmp = ebayCapSpriceToLmp;
        ...
        $(document).ready(function() {
            ...
            function ebay1RowSpriceForAlert(data) {
                ...
                if (typeof ebayDisplayedSprice === 'function') {
                    const shown = ebayDisplayedSprice(data);
                    if (shown > 0) return shown;
                }
                if (typeof chPromoSavedOrLiveSprice === 'function') {
            ...
            $('#ebay1-ended-listing-badge').on('click', function() {
            ...
            function saveSpriceWithRetry(sku, sprice, row, retryCount = 0, skipClear) {
                if (row && Number(sprice) > 0) {
                    sprice = (typeof chPromoFinalSpriceToSave === 'function')
                        ...
                        : (typeof ebayCapSpriceToLmp === 'function'
                            ? ebayCapSpriceToLmp(row.getData(), sprice)
                ...
                ajaxResponse: function(url, params, response) {
                    ...
                    if (Array.isArray(payload)) {
                        payload.forEach(function(row) {
                            if (typeof ebayApplyL1FromEntries === 'function') ebayApplyL1FromEntries(row);
                        });
                ...
                columns: [{
...

                    {
                        ...
                        formatter: function(cell) {
                            const value = parseFloat(cell.getValue() || 0);
                            const rowData = cell.getRow().getData();
                            const lmpPrice = (typeof ebayEffectiveLmp === 'function')
...

                    {
                        ...
                        formatter: function(cell) {
                            ...
                            const lmpPrice = entries.length
                                ? ebayL1FromCompetitors(entries)
                                : cell.getValue();
                            const sku = rowData['(Child) sku'];
                    ...
                    },
                    {
                        ...
                        headerSort: true,
                        sorter: function(a, b, aRow, bRow) {
                            const av = (typeof ebaySpriceAmount === 'function')
                                ? (ebaySpriceAmount(aRow.getData()) || 0)
                                : (parseFloat(a) || 0);
                            const bv = (typeof ebaySpriceAmount === 'function')
                                ? (ebaySpriceAmount(bRow.getData()) || 0)
                                : (parseFloat(b) || 0);
                            return av - bv;
                        },
                        editable: false,
                        ...
                        formatter: function(cell) {
                            ...
                            const sgroiAtLmp = (typeof ebaySgroiAtPrice === 'function')
                                ? ebaySgroiAtPrice(rowData, lmpNow)
                                : null;
            ...
            function applyFilters() {
                ...
                if (endedListingFilterActive) {
                    table.addFilter(function(data) {
                        return ebay1IsEndedListing(data);
                    });
                }
                if (redTriangleFilterActive) {
                    table.addFilter(function(data) {
            ...
            function classifyColumnDefault(field, title) {
                ...
                // Pricing
                if (
                    /^(eBay Price|STANDARD_PRICE|GPFT%|PFT %|ROI%|NROI|lmp_price|linked_lmp_skus|linked_lmp_sku_add|SPRICE|SGPFT|SPFT|SGROI|SROI|E Dil%|SCVR|CVR_45|CVR_60|prmt_pct|cpn_pct|zero_sold|cvr_up_dn|t_discounts|dsc|appr|push_prc)$/i.test(f) ||
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success && response.competitors && response.competitors.length > 0) {
                        ...
                        patchEbayGridLmp(response.lowest_price, response.competitors);
        ...
        function renderEbayCompetitorsList(competitors, lowestPrice) {
            ...
            let l1Price = ebayL1FromCompetitors(competitors);
            if (l1Price === null && lowestPrice != null && ebayActiveLmpCount(competitors) > 0) {
            ...
            const rows = competitors.map(function(item) {
                const ignored = ebayCompetitorIsIgnored(item);
        ...
        function ebayNormalizeSkuKey(s) {
            return String(s || '').replace(/\s+/g, ' ').trim().toUpperCase();
        }

        function patchEbayGridLmp(lowestPrice, competitors) {
            const entries = Array.isArray(competitors) ? competitors : null;
            const l1 = entries ? ebayL1FromCompetitors(entries) : (lowestPrice != null ? parseFloat(lowestPrice) : null);
            const activeCount = entries ? ebayActiveLmpCount(entries) : null;

            const targets = new Set();
            const add = function(s) {
                const t = ebayNormalizeSkuKey(s);
                if (t) targets.add(t);
            };
            add(currentLmpData.sku);
            (currentLmpData.linkedLmpSkus || []).forEach(add);
        ...
        function applyEbayLmpIgnoreLocal(id, ignored) {
            ...
            patchEbayGridLmp(l1, currentLmpData.competitors);
            renderLmpModalStats(currentLmpData.sku);
        }

        // Toggle LMP ignore — excluded from L1, vs LMP, and the LMP column (N)
        $(document).on('change', '#lmpModal .lmp-ignore-cb', function() {
            ...
            applyEbayLmpIgnoreLocal(id, ignored);
