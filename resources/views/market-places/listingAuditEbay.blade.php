@extends('layouts.vertical', ['title' => 'Listing Audit eBay', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    @include('layouts.shared/page-title', ['page_title' => 'Listing Audit eBay', 'sub_title' => 'eBay'])
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function loadData() {
                showLoader();
                return $.ajax({
                    url: '/listing_audit_ebay/view-data',
                    type: 'GET',
                    dataType: 'json',
            ...
            function calculateTotals() {
                try {
                    ...
                    filteredData.forEach(item => {
                        // Only count child rows (not parent rows) and INV > 0
                        if (!item.is_parent) {
            ...
            function initNAEditHandlers() {
                ...
                $(document).on('change', '.na-checkbox', function(e) {
                    ...
                    const sku = $row.find('td:eq(2)').text(); // Assuming SKU is in the 3rd column
                    ...
                    $.ajax({
                        url: '/listing_audit_ebay/save-na',
                        type: 'POST',
                        data: {
                            sku: sku,
                            field: field,
