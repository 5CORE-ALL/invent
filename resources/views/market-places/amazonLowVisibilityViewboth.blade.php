            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-container">
                        <table class="custom-resizable-table" id="amazonLowVisibility-table">
                            <thead>
                                <tr>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function loadData() {
                showLoader();
                return $.ajax({
                    url: '/amazon/low-visibility/view-data-both',
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
            ...
            function renderTable() {
                const $tbody = $('#amazonLowVisibility-table tbody');
                ...
                const $headers = $('#amazonLowVisibility-table thead th');
                ...
                filteredData.forEach(item => {
                    ...
                    if (item.FBA === 'FBM') {
                        $row.addClass('fba-hide');
                    }

                    if (item.FBA === 'BOTH') {
                        $row.addClass('both-hide');
                    }
                    ...
                    // Modified FBA display - showing just text instead of select
                   if (item.is_parent) {
    $row.append($('<td>')); // Empty cell for parent
} else {
    let currentFBA = 'BOTH'; // Default value
    
    // Check if item has value property and extract FBA value
    if (item.value && typeof item.value === 'object') {
        currentFBA = item.value.FBA === 'BOTH' ? 'BOTH' : 
                    (item.value.FBA === 'FBA' ? 'FBA' : 'FBM');
    } else if (typeof item.value === 'string') {
        try {
            const valueObj = JSON.parse(item.value);
            currentFBA = valueObj.FBA === 'BOTH' ? 'BOTH' : 
                        (valueObj.FBA === 'FBA' ? 'FBA' : 'FBM');
        } catch (e) {
    ...
    const $fbaCell = $('<td>');
    const $fbaSpan = $('<span>').text(currentFBA);

    // Set background color based on value
    if (currentFBA === 'BOTH') {
        $fbaSpan.css({
            'background-color': '#ffc107', // Yellow for BOTH
            'color': '#000000',
            'padding': '4px 8px',
            'border-radius': '4px',
            'display': 'inline-block'
        });
    } else if (currentFBA === 'FBA') {
        $fbaSpan.css({
            'background-color': '#dc3545', // Red for FBA
            'color': '#ffffff',
            'padding': '4px 8px',
            'border-radius': '4px',
            'display': 'inline-block'
        ...
        $fbaSpan.css({
            'background-color': '#28a745', // Green for FBM
            'color': '#ffffff',
            'padding': '4px 8px',
            'border-radius': '4px',
            'display': 'inline-block'
    ...
    $row.append($fbaCell);
            ...
            function initColumnToggle() {
                ...
                let amazonZeroViewBoth = JSON.parse(localStorage.getItem('amazonZeroViewBoth')) || {};
                ...
                $headers.each(function() {
                    ...
                    const isChecked = amazonZeroViewBoth.hasOwnProperty(field) ? amazonZeroViewBoth[field] : true;
                ...
                $menu.on('change', '.column-toggle-checkbox', function() {
                    ...
                    amazonZeroViewBoth[field] = isVisible;
                    localStorage.setItem('amazonZeroViewBoth', JSON.stringify(amazonZeroViewBoth));
