@extends('layouts.vertical', ['title' => 'Walmart', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    @include('layouts.shared/page-title', ['page_title' => 'walmart', 'sub_title' => 'Market Place'])
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">walmart Product Analysis</h4>
                            ...
                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h4 class="modal-title" id="createTaskModalLabel">📝 Create New Task Walmart to
                                            Task
                                            Manager</h4>
                    ...
                    <div class="table-container">
                        <table class="custom-resizable-table" id="walmart-table">
                            <thead>
                                <tr>
                    ...
                    <div id="data-loader" class="card-loader-overlay" style="display: none;">
                        <div class="loader-content">
                            ...
                            <div class="loader-text">Loading walmart data...</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function renderTable() {
                const $tbody = $('#walmart-table tbody');
                $tbody.empty();
            ...
            function initNREditHandlers() {
                ...
                $(document).on('change', '.nr-checkbox', function() {
                    ...
                    const $row = $checkbox.closest('tr');
                    const slNo = $row.find('td:eq(0)').text();
                    const rowData = filteredData.find(item => item['SL No.'] == slNo || item['sl_no'] ==
                        slNo);
            ...
            window.openModal = function(selectedItem, type) {
                try {
                    ...
                    const cachedData = walmartDataCache.get(itemId);
                    const dataToUse = cachedData || itemData;

                    // Store the data in cache if it wasn't already
                    if (!cachedData) {
                        walmartDataCache.set(itemId, itemData);
                    }
            ...
            function saveChanges(contentElement, title, slNo, isHyperlink, updatedValue, isCheckbox, rowElement) {
                ...
                walmartDataCache.updateField(itemId, title, cacheUpdateValue);
                ...
                if (index !== -1) {
                    ...
                    if (title === 'SPRICE' && filteredData[index].raw_data) {
                        ...
                        walmartDataCache.updateField(itemId, 'Spft%', Spft);
                ...
                $.ajax({
                    ...
                    url: window.location.origin + (window.location.pathname.includes('/public') ?
                        '/public' : '') + '/api/update-walmart-column',
                    ...
                    error: function(xhr, status, error) {
                        ...
                        // Revert cache
                        walmartDataCache.updateField(itemId, title, originalValue);
