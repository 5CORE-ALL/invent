            ...
            <div class="modal-content" style="overflow: hidden;">
                <div class="modal-header bg-info text-white py-1 px-3">
                    <h6 class="modal-title mb-0" style="font-size: 13px;">
                        ...
                        <span id="pricingMasterChartModalTitle">Sold Master - Inv (Rolling L30)</span>
                    </h6>
                ...
                <div class="modal-body p-2">
                    ...
                    <div id="pricingMasterChartNoData" class="text-center py-3" style="display: none;">
                        ...
                        <p class="text-muted small mb-0"><strong>Refresh this page (Sold Master)</strong> once so today’s SKU data is saved, then open the graph again.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    $(document).ready(function() {
        ...
        function renderMarketplaceData(data) {
            ...
            data.forEach(item => {
                ...
                const isEditable = ['amazon', 'doba', 'ebay', 'ebaytwo', 'ebaythree', 'temu', 'temu2', 'tiktok', 'tiktok2', 'tiktok 2', 'bestbuy', 'macy', 'reverb', 'sb2c', 'shopifyb2c', 'sb2b', 'shopifyb2b', 'fba', 'tiktok2', 'shein', 'faire', 'aliexpress', 'purchasingpower'].includes((item.marketplace || '').toLowerCase());
            ...
            columns: [
                ...
                },
                {
                    ...
                    sorter: function(a, b, aRow, bRow, column, dir, sorterParams) {
                        // Calculate inv_value for each row
                        const aData = aRow.getData();
                        const bData = bRow.getData();
                        
                        const aInv = parseFloat(aData.inventory || 0);
                        const aPrice = parseFloat(aData.amazon_price || 0);
                        const aValue = aInv * aPrice;
                        
                        const bInv = parseFloat(bData.inventory || 0);
                        const bPrice = parseFloat(bData.amazon_price || 0);
                        const bValue = bInv * bPrice;
                        
                        // If comparing a parent row with a child row
                        if (aData.is_parent_summary && !bData.is_parent_summary) {
                            // Check if b is a child of a
                            if (bData.parent === aData.parent) {
                                return 1; // Parent comes after its children in display order
                            }
                        }
                        if (!aData.is_parent_summary && bData.is_parent_summary) {
                            // Check if a is a child of b
                            if (aData.parent === bData.parent) {
                                return -1; // Children come before their parent
                            }
                        }
                        
                        // For parent rows or rows from different parents, sort by inv_value
                        if (aValue > bValue) return -1; // Highest to lowest
                        if (aValue < bValue) return 1;
                        return 0;
                    },
                    formatter: function(cell) {
                        const rowData = cell.getRow().getData();
                        const inventory = parseFloat(rowData.inventory || 0);
                        const amazonPrice = parseFloat(rowData.amazon_price || 0);
                        const invValue = inventory * amazonPrice;
                        
                        if (invValue === 0 || amazonPrice === 0) {
                            return '<span class="text-muted">-</span>';
                        }
                        
                        return '<span style="font-weight: 600;">$' + Math.round(invValue).toLocaleString() + '</span>';
                    }
                },
                {
                    title: "OV L30",
...
title: "Dil %",
        ...
        function getParentRows() {
            ...
            // Calculate total Inv Value for each parent (sum of all children's Inv Values)
            parents.forEach(parent => {
                let totalInvValue = 0;
                fullDataset.forEach(row => {
                    // Sum up Inv Value for all children of this parent
                    if (row.parent === parent.parent && row.is_parent_summary !== true) {
                        const inv = parseFloat(row.inventory || 0);
                        const price = parseFloat(row.amazon_price || 0);
                        totalInvValue += (inv * price);
                    }
                });
                parent._totalInvValue = totalInvValue;
            });
            
            // Sort parents by total Inv Value from highest to lowest
            parents.sort((a, b) => {
                return (b._totalInvValue || 0) - (a._totalInvValue || 0);
            });
        ...
        $(document).on('click', '.summary-chart-badge', function(e) {
            ...
            $('#pricingMasterChartModalTitle').text('Sold Master - All (Summary) - ' + label + ' (Rolling ' + pricingChartRangeLabel(30) + ')');
        ...
        $(document).on('click', '.pricing-master-chart-link', function(e) {
            ...
            $('#pricingMasterChartRangeSelect').val('30');
            const label = pricingChartMetricLabels[metric] || metric;
            $('#pricingMasterChartModalTitle').text('Sold Master - ' + displayName + (isParentChart ? ' (Parent)' : '') + ' - ' + label + ' (Rolling ' + pricingChartRangeLabel(30) + ')');
            $('#pricingMasterChartContainer').hide();
            $('#pricingMasterChartNoData').hide();
            $('#pricingMasterChartLoading').show();
            const modal = new bootstrap.Modal(document.getElementById('pricingMasterChartModal'));
