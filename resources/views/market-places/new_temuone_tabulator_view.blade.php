            ...
            <div class="card-body py-2">
                <div id="summary-stats" class="d-flex align-items-center flex-wrap gap-1">
                    ...
                    <select id="cvr-filter" class="form-select form-select-sm"
                        style="width: auto; display: inline-block;"
                        title="Filter by CVR% = (Temu L30 / Views) × 100.">
                    ...
                    <select id="gpft-filter" class="form-select form-select-sm"
                        style="width: auto; display: inline-block;"
                        title="Filter by GPFT% = Gpft ÷ T Price.">
                ...
                <div id="newtemuone-badge-row" class="d-flex flex-wrap gap-1 mt-2" role="group" aria-label="Summary metrics">
                    ...
                    <span class="badge bg-info fs-6 p-2" id="total-views-badge"
                        ...
                        title="Σ Views once per Parent. Variations of the same parent share one listing.">Views: 0</span>
                ...
                <div class="modal-body">
                    <form id="uploadLmpForm" method="POST" action="{{ route('temu.lmp.upload') }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-text">
                                Writes to shared <code>temu_lmp</code>.
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    /** Live SNROI at S PRC: SGROI − (S PRC × Ads%) / LP. Dil + CVR target this, not SGROI. */
    function chPromoTemuInvertSnroiAtSprice(sprice, lp, ship) {
        const sgroi = chPromoTemuInvertSgroiAtSprice(sprice, lp, ship);
        if (sgroi == null) return null;
    ...
    function temuSpriceCapCompute(row, rawSprice, extra) {
        ...
        const labels = [];
        if (ebay > 0 && Math.abs(sprice - ebay) < 0.015 && discounted > ebay + 0.004) labels.push('EB');
        if (amz > 0 && Math.abs(sprice - amz) < 0.015 && discounted > amz + 0.004) labels.push('Amz');
    ...
    function temuSpriceCellModel(row) {
        ...
        const value = (cap && cap.sprice > 0) ? cap.sprice : ntoSavedSprice(row);
        return {
            value: value,
            labels: temuRowCapLabels(row),
    ...
    function temuDisplayedSprice(row) {
        ...
        if (cap && cap.sprice > 0) return cap.sprice;
        return ntoSavedSprice(row);
    }
    ...
    async function ntoPersistDisplayedSprice() {
        ...
        chPromoEachTableRow(function(row, d) {
            ...
            const cap = typeof temuSpriceCapCompute === 'function'
                ? temuSpriceCapCompute(d, live, { use_passed_as_discounted: true })
                : null;
            const price = (cap && cap.sprice > 0)
                ? cap.sprice
                : chPromoFinalSpriceToSave(d, live);
            if (!(price > 0)) return;
            const current = ntoSavedSprice(d);
            if (Math.abs(current - price) < 0.015) return;
            const labels = (cap && cap.labels) || (typeof temuRowCapLabels === 'function' ? temuRowCapLabels(d) : []);
            updates.push({
                sku: sku,
                sprice: price,
            ...
            rows.push({ row: row, price: price, labels: labels, sprcDil: live });
        });
    ...
    function temuSgroiFromRule(row) {
        ...
        return (rule == null || !isFinite(rule)) ? null : rule;
    }
    /** SGROI% = live SPFT ÷ LP at the displayed S PRC (not the Dil target). */
    function temuSgroiPercent(row) {
        return temuSroiPercent(row);
    }
    ...
    function temuRowCapLabels(row) {
        ...
        ((cap && cap.labels) || []).forEach(add);
        (Array.isArray(row.sprice_labels) ? row.sprice_labels : []).forEach(add);
        const sprice = (cap && cap.sprice > 0) ? cap.sprice : ntoSavedSprice(row);
        const dil = typeof temuDiscountedPrice === 'function' ? temuDiscountedPrice(row) : 0;
        const ebay = (cap && cap.ebay) || temuEbayRefPrice(row);
        const amz = (cap && cap.amz) || temuAmzRefPrice(row);
        if (ebay > 0 && sprice > 0 && Math.abs(sprice - ebay) < 0.015 && dil > ebay + 0.004) add('EB');
        if (amz > 0 && sprice > 0 && Math.abs(sprice - amz) < 0.015 && dil > amz + 0.004) add('Amz');
        return out;
    }
    ...
    function temuLiveListingPrice(row) {
        const t = parseFloat(row && row.temu_price);
        if (isFinite(t) && t > 0) return t;
        const r = parseFloat(row && row.t_price);
        return (isFinite(r) && r > 0) ? r : 0;
    }

    /** Blue triangle: same test the S PRC cell paints — S PRC ≠ T Price. */
    function temuHasBlueTriangle(row) {
        if (!row || row.is_parent_summary) return false;
        const sprice = typeof temuSpriceCellModel === 'function'
            ? (temuSpriceCellModel(row).value || 0)
            : temuDisplayedSprice(row);
        const live = temuLiveListingPrice(row);
    ...
    function ntoMoneyEq(a, b) {
        return Math.round((Number(a) || 0) * 100) === Math.round((Number(b) || 0) * 100);
    }
    function ntoRowPushState(d) {
        if (!d || d.is_parent_summary) return '';
        const queued = String(d.nto_push_status || '');
        if (queued === 'pushing') return 'pushing';
        if (queued === 'error') return 'error';
        const sBase = ntoRowSBase(d);
        const pushed = parseFloat(d.nto_pushed_base);
        if (sBase > 0 && pushed > 0 && ntoMoneyEq(sBase, pushed)) return 'pushed';
        return '';
    }
    function ntoPushCellHtml(d) {
        const sBase = ntoRowSBase(d);
        if (!(sBase > 0)) return '';
        const sku = ntoRowSku(d);
        const esc = String(sku).replace(/"/g, '&quot;');
        const state = ntoRowPushState(d);
        if (state === 'pushing') {
            return '<i class="fas fa-spinner fa-spin" style="color:#ffc107;" title="Pushing S Base Prc $'
                + sBase.toFixed(2) + '…"></i>';
        }
        if (state === 'pushed') {
            const when = d.nto_pushed_at ? String(d.nto_pushed_at) : '';
            const tip = 'Pushed S Base Prc $' + sBase.toFixed(2)
                + (when ? ' at ' + when : '');
            return '<i class="fa-solid fa-check-double nto-push-ok" title="'
                + tip.replace(/"/g, '&quot;') + '"></i>';
        }
        if (state === 'error') {
            return '<button type="button" class="nto-push-single-btn" data-sku="' + esc
                + '" data-price="' + sBase + '" title="Push failed — click to retry">'
                + '<i class="fas fa-times nto-push-fail"></i></button>';
        }
        return '<button type="button" class="nto-push-single-btn" data-sku="' + esc
            + '" data-price="' + sBase + '" title="Push S Base Prc $' + sBase.toFixed(2)
            + ' — or all checked SKUs"><i class="fas fa-upload"></i></button>';
    }
    function ntoPatchPushRows(skus, patch) {
        if (!table || typeof table.getRows !== 'function' || !skus || !skus.length) return;
        const want = {};
        skus.forEach(function(sku) { want[String(sku).toUpperCase()] = true; });
        table.getRows().forEach(function(row) {
            const d = row.getData() || {};
            if (!want[ntoRowSku(d).toUpperCase()]) return;
            row.update(patch);
            try { row.reformat(); } catch (e) { /* ignore */ }
        });
    }
    function ntoApplyPushTaskResults(tasks) {
        if (!table || typeof table.getRows !== 'function' || !Array.isArray(tasks)) return;
        const bySku = {};
        tasks.forEach(function(task) {
            const sku = String((task && task.sku) || '').trim();
            if (sku) bySku[sku.toUpperCase()] = task;
        });
        table.getRows().forEach(function(row) {
            const d = row.getData() || {};
            const task = bySku[ntoRowSku(d).toUpperCase()];
            if (!task) return;
            const st = String(task.status || '');
            const price = parseFloat(task.price) || 0;
            const next = {};
            if (st === 'ok' || st === 'pushed') {
                next.nto_push_status = 'pushed';
                if (price > 0) next.nto_pushed_base = +price.toFixed(2);
                next.nto_pushed_at = new Date().toISOString().slice(0, 19).replace('T', ' ');
            } else if (st === 'pushing') {
                next.nto_push_status = 'pushing';
            } else if (st === 'failed' || st === 'error') {
                next.nto_push_status = 'error';
            }
    ...
    function pollNtoPushSpriceStatus() {
        $.ajax({
            ...
            if (Array.isArray(resp.tasks) && resp.tasks.length) {
                ntoApplyPushTaskResults(resp.tasks);
            }
            if (!active) {
    ...
    function initNtoReloadPushUi() {
        ...
        $(document).off('click.ntoSinglePush', '.nto-push-single-btn').on('click.ntoSinglePush', '.nto-push-single-btn', function(e) {
            ...
            if (selected.length) {
                ntoPatchPushRows(selected.map(function(i) { return i.sku; }), { nto_push_status: 'pushing' });
            ...
            ntoPatchPushRows([sku], { nto_push_status: 'pushing' });
    ...
    function updateSummary() {
        ...
        rows.forEach(function(row) {
            ...
            const sprice = (cap && cap.sprice > 0) ? cap.sprice : ntoSavedSprice(row);
            if (sprice > 0) {
                if (cap && (cap.lmpAlert || (cap.lmp > 0 && sprice >= cap.lmp))) lmpCapped++;
                const labels = temuRowCapLabels(row);
        ...
        table = new Tabulator('#new-temuone-table', {
            ...
            columns: [
                ...
                },
                {
                    ...
                    hozAlign: 'center',
                    sorter: 'number',
                    headerTooltip: 'Dil% = OV L30 ÷ INV. Same Dil Sprc Dil uses for the Target NROI slab.',
                    formatter: function(cell) {
                        const rowData = cell.getRow().getData();
                ...
                @include('partials.analytics-sku-reviews-column', ['marketplace' => 'temu'])
                {
                    ...
                    width: 78,
                    sorter: 'number',
                    headerTooltip: 'CVR = (Temu L30 / Views) × 100 — same as /temu1-data. Arrow compares CVR 30 against CVR 60 (prior 30 days), the same up/down rule as the Amazon tabulator. Down + CVR < 7% subtracts 10 from the Sprc Dil Target NROI; Up + CVR > 10% adds 10.',
                    formatter: function(cell) {
                ...
                },
                {
                    ...
                    formatter: function(cell) {
                        ...
                        const live = typeof temuLiveListingPrice === 'function'
                            ? temuLiveListingPrice(rowData)
                            : (parseFloat(rowData.temu_price || rowData.t_price) || 0);
                        ...
                        const redTri = overLmp
                            ...
                            : '';
                        const blueTri = (typeof temuHasBlueTriangle === 'function'
                            ? temuHasBlueTriangle(rowData)
                            : (live > 0 && Math.round(value * 100) !== Math.round(live * 100)))
                ...
                },
                {
                    ...
                    formatter: function(cell) {
                        ...
                        const tip = 'S Base Prc from S PRC $' + sprice.toFixed(2)
                            + ' → $' + sBase.toFixed(2);
                        return '<span style="font-weight: 600;" title="' + tip.replace(/"/g, '&quot;') + '">$' + sBase.toFixed(2) + '</span>';
                    }
                },
                {
                    title: 'Push',
                    ...
                    sorter: function(a, b, aRow, bRow) {
                        const rank = function(d) {
                            const st = ntoRowPushState(d);
                            if (st === 'pushed') return 4;
                            if (st === 'pushing') return 3;
                            if (st === 'error') return 2;
                            return ntoRowSBase(d) > 0 ? 1 : 0;
                        };
                        return rank(aRow.getData()) - rank(bRow.getData());
                    },
                    headerTooltip: 'Push S Base Prc to Temu. Double tick = this S Base was already pushed. Arrow = not pushed, or S Base changed after the last push. Checked rows push together.',
                    formatter: function(cell) {
                        return ntoPushCellHtml(cell.getRow().getData());
                    }
                },
                {
                    title: 'S R Prc',
                ...
                },
                {
                    ...
                    width: 65,
                    sorter: 'number',
                    headerTooltip: 'SGROI% = SPFT ÷ LP. Live ROI at S PRC: (S R Prc × 0.95 − Temu Ship − LP) / LP. Dil + CVR set SNROI, not this column.',
                    formatter: function(cell) {
                        const row = cell.getRow().getData();
