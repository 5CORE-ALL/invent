@extends('layouts.vertical', ['title' => 'Instagram Shop - Analytics', 'sidenav' => 'condensed'])
...
@section('css')
    ...
    <style>
        ...
        .instagram-select-header { display: flex; align-items: center; justify-content: center; }
        @include('partials.channel-pef-promo', ['channelPromoPart' => 'css', 'channelPromoChannel' => 'instagram'])
        @include('partials.ebay-sprc-dil', ['ebaySprcDilPart' => 'css', 'ebaySprcDilChannel' => 'instagram'])
    </style>
@endsection

@section('content')
    @include('layouts.shared.page-title', [
        'page_title' => 'Instagram Shop - Analytics',
        'sub_title'  => '',
    ])
                ...
                <div class="card-body">
                    <div id="summary-stats" class="mb-2 p-3 bg-light rounded">
                        <div class="d-flex flex-wrap gap-2" role="group" aria-label="Summary metrics">
                            ...
                            <span class="badge bg-primary fs-6 p-2" id="dp-total-sales-badge"
                                title="Σ Instagram sheet Sale AMT (item price × qty) in last 30 sale days">Sales: $0</span>
                            <span class="badge bg-info fs-6 p-2" id="dp-avg-gpft-badge"
                                title="L30 GPFT % = PFT ÷ Sales. PFT = (sheet sales × margin) − (LP × I L30). Price $0 is skipped (not −LP). No ship.">GPFT: 0%</span>
                            <span class="badge bg-success fs-6 p-2" id="dp-total-profit-badge"
                                title="PFT = Σ ((Instagram sheet sales × marketplace margin) − LP × I L30). Unpriced SKUs do not subtract LP. No ship.">PFT: $0</span>
                            <span class="badge fs-6 p-2" id="dp-avg-roi-badge"
                                style="background-color:#6f42c1;color:#fff;"
                                title="L30 GROI % = PFT ÷ COGS. COGS = Σ (LP × I L30) only on SKUs with sheet sales. No ship.">GROI: 0%</span>
                            @include('partials.analytics-dil-badge', ['dilChannel' => 'instagram'])
                            <span class="badge bg-success fs-6 p-2 dp-filter-badge" id="dp-sold-pct-badge"
                                data-filter="more_sold" style="cursor:pointer;"
                                title="Click to filter I L30 &gt; 0 (INV &gt; 0)">
                            ...
                            @include('partials.lmp-missing-badge', ['lmpBadgeId' => 'instagram-lmp-missing-badge', 'lmpChannelKey' => 'instagram'])
                            @include('partials.price-gt-lmp-badge', ['pglBadgeId' => 'instagram-price-gt-lmp-badge', 'pglChannelKey' => 'instagram', 'pglPriceField' => 'price'])
                            @include('partials.price-lt80-lmp-badge', ['pltBadgeId' => 'instagram-price-lt80-lmp-badge', 'pltChannelKey' => 'instagram', 'pltPriceField' => 'price'])
                            <span class="badge fs-6 p-2" id="instagram-blue-triangle-badge"
                                style="background-color:#0d6efd;color:#fff;font-weight:700;cursor:pointer;"
                                title="Blue triangle: S PRC ≠ Price. Click to filter.">
                                <i class="fas fa-exclamation-triangle"></i> 0</span>
                            <span class="badge bg-secondary fs-6 p-2"
                                title="marketplace_percentages.percentage (marketplace = Instagram Shop). Ship is not used.">
                                Margin: {{ number_format((float) ($marginPercent ?? 100), 2) }}%
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
                        ...
                        <select id="dp-sold-filter" class="form-select form-select-sm" style="width:120px;"
                            title="Filter by Instagram sheet I L30. 0 / &gt; 0 require INV &gt; 0.">
                        ...
                        <a href="{{ route('instagram.analytics.export') }}" class="btn btn-sm btn-success" id="export-btn">
                            <i class="fa fa-file-csv"></i>
                        </a>
                        <a href="{{ route('instagram.analytics.sample') }}" class="btn btn-sm btn-outline-success" title="Download CSV template (parent, sku, price, l30)">
                            <i class="fa fa-download"></i> Template
                        </a>
                        @include('partials.ebay-sprc-dil', ['ebaySprcDilPart' => 'buttons', 'ebaySprcDilChannel' => 'instagram'])
                        @include('partials.channel-pef-promo', ['channelPromoPart' => 'buttons', 'channelPromoChannel' => 'instagram'])
                    ...
                    <div id="instagram-table-wrapper" style="height: calc(100vh - 220px); display: flex; flex-direction: column;">
                        <div id="instagram-analytics-table" style="flex: 1;"></div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('partials.channel-pef-promo', ['channelPromoPart' => 'modals', 'channelPromoChannel' => 'instagram'])
    @include('partials.ebay-sprc-dil', ['ebaySprcDilPart' => 'modals', 'ebaySprcDilChannel' => 'instagram'])
@endsection
...
<script>
    @include('partials.channel-pef-promo', ['channelPromoPart' => 'script', 'channelPromoChannel' => 'instagram'])
    @include('partials.ebay-sprc-dil', ['ebaySprcDilPart' => 'script', 'ebaySprcDilChannel' => 'instagram'])

    const DP_MARGIN = {{ (float) ($marginPercent ?? 100) }} / 100;
    let table = null;
    let allTableData = [];
    ...
    function syncDpTriangleBadgeState() {
        $('#instagram-blue-triangle-badge').css({
    ...
    function updateSelectAllHeaderCheckbox() {
        const el = document.getElementById('instagram-select-all');
        if (!el || !table) return;
    ...
    function updateSummary(rowsInput) {
        ...
        rows.forEach(function(row) {
            ...
            // L30 PFT from the Instagram sheet: (sales × margin) − (LP × qty).
        ...
        if (window.PriceGtLmpBadge) {
            PriceGtLmpBadge.update('#instagram-price-gt-lmp-badge', lmpRows, 'instagram', 'price');
            if (window.PriceLt80LmpBadge) {
                PriceLt80LmpBadge.update('#instagram-price-lt80-lmp-badge', lmpRows, 'instagram', 'price');
            }
        }
        if (window.LmpMissingBadge) {
            LmpMissingBadge.update('#instagram-lmp-missing-badge', lmpRows, 'instagram');
        }
        let blue = 0;
    ...
    $(document).ready(function() {
        ...
        table = new Tabulator("#instagram-analytics-table", {
            ajaxURL: "{{ route('instagram.analytics.data') }}",
            ...
            columns: [
                {
                    title: '<div class="instagram-select-header"><input type="checkbox" id="instagram-select-all"></div>',
                    field: "_select",
                    width: 38,
                    headerSort: false,
                    frozen: true,
                    ...
                    formatter: function(cell) {
                        ...
                        return `<input type="checkbox" class="instagram-sku-chk" data-sku="${safe}" ${checked}>`;
                    }
                },
                {
                    title: "Parent",
                ...
                },
                {
                    ...
                    hozAlign: "center",
                    width: 55,
                    headerTooltip: "Instagram units sold (sheet I L30 / uploaded template)",
                    formatter: function(cell) {
                        return `<span style="font-weight:700;">${parseInt(cell.getValue(), 10) || 0}</span>`;
                    }
                },
                ...
                },
                {
                    ...
                    sorter: "number",
                    hozAlign: "right",
                    headerTooltip: "GROI = ((Price × margin) − LP) ÷ LP. No ship. Margin from marketplace Instagram Shop.",
                    formatter: function(cell) {
                        const d = cell.getRow().getData();
                        if (dpIsParentRow(d)) return '<span style="color:#6c757d;">–</span>';
                ...
                },
                {
                    ...
                    sorter: "number",
                    hozAlign: "right",
                    headerTooltip: "Σ item_price × qty from Instagram shop sheet (L30 window)",
        ...
        if (window.PriceGtLmpBadge) {
            PriceGtLmpBadge.bind({
                badge: '#instagram-price-gt-lmp-badge',
        ...
        if (window.PriceLt80LmpBadge) {
            PriceLt80LmpBadge.bind({
                badge: '#instagram-price-lt80-lmp-badge',
        ...
        $('#instagram-blue-triangle-badge').on('click', function() {
        ...
        $(document).on('change', '#instagram-select-all', function() {
            ...
            $('.instagram-sku-chk').each(function() {
                $(this).prop('checked', selectedSkus.has(String($(this).attr('data-sku'))));
            });
            updateSelectedCount();
        });
        $(document).on('change', '.instagram-sku-chk', function() {
        ...
        $('#dp-apply-target-gpft-btn').on('click', function() {
            ...
            if (!(denom > 0)) {
                showToast('Target GPFT% must be less than the Instagram Shop margin', 'error');
                return;
            }
            if (selectedSkus.size === 0) {
                showToast('Select at least one SKU first', 'error');
        ...
        $('#import-form').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('instagram.analytics.import') }}",
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        if (typeof chPromoBatchClearThenSave === 'function' && opts.clearFirst !== false) {
            chPromoBatchClearThenSave(updates, function(next) {
                ...
                wipeFn: function(zeros) {
                    return $.ajax({
                        url: "{{ route('instagram.analytics.save.sprice') }}",
                        method: 'POST',
