@extends('layouts.vertical', ['title' => 'doba analytics', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    @include('layouts.shared/page-title', ['page_title' => 'doba', 'sub_title' => 'doba Analysis'])
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">Doba Product Analysis</h4>
                            ...
                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h4 class="modal-title" id="createTaskModalLabel">📝 Create New Task doba to Task
                                            Manager</h4>
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th data-field="cvr" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center" style="gap: 4px">
                                            <div class="d-flex align-items-center">
                                                Pickup Price <span class="sort-arrow">↓</span>
                                            </div>
                                            <div style="width: 100%; height: 5px; background-color: #9ec7f4;"></div>
                                        </div>
                                    </th>
                    ...
                    <div id="data-loader" class="card-loader-overlay" style="display: none;">
                        <div class="loader-content">
                            ...
                            <div class="loader-text">Loading doba data...</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    `);
            ...
            function updateSoldCounts() {
                ...
                filteredData.forEach(item => {
                    if (!item.is_parent) {
                        const l30 = parseFloat(item['doba L30']) || 0;
                        const inv = parseFloat(item.INV) || 0;
                        // 0 SOLD: L30 == 0, INV > 0
                        if (l30 === 0 && inv > 0) zeroSold++;
                        // SOLD: count all SKUs (not parent) with any INV
                        totalSku++;
                    }
                });
            ...
            function renderTable() {
                ...
                filteredData.forEach(item => {
                    ...
                    $row.append($('<td>').html(
                        ...
                        `<span class="dil-percent-value ${getRoiColor(item.pickup_price)}">${item.pickup_price.toFixed(2)}</span>` :
                        ''
                    ));
            ...
            window.openModal = function(selectedItem, type) {
                try {
                    ...
                    const cachedData = dobaViewDataCache.get(itemId);
                    const dataToUse = cachedData || itemData;

                    // Store the data in cache if it wasn't already
                    if (!cachedData) {
                        dobaViewDataCache.set(itemId, itemData);
                    }
                    ...
                    switch (type.toLowerCase()) {
                        ...
                        case 'price view':
                            fieldsToDisplay = [{
                                    title: 'doba LIVE Price',
                                    content: selectedItem['doba LIVE Price']
                                },
                                {
                                    title: 'doba Price',
            ...
            function saveChanges(contentElement, title, slNo, isHyperlink, updatedValue, isCheckbox, rowElement) {
                ...
                dobaViewDataCache.updateField(itemId, title, cacheUpdateValue);
                ...
                if (index !== -1) {
                    ...
                    if (title === 'sprice' && filteredData[index].raw_data) {
                        ...
                        dobaViewDataCache.updateField(itemId, 'Spft%', Spft);
                ...
                // If doba Price, use a separate route
                let ajaxUrl = '/api/update-doba-column';
                let ajaxData = JSON.stringify(data);
                let ajaxContentType = 'application/json';

                if (title === 'doba Price') {
                    // Find doba_item_id from filteredData or cache
                    let dobaItemId = null;
                    const found = filteredData.find(item => String(item['Sl']) == String(itemId));
                    if (found && found['doba_item_id']) {
                        dobaItemId = found['doba_item_id'];
                    }
                    if (!dobaItemId && dobaViewDataCache.get(itemId) && dobaViewDataCache.get(itemId)[
                            'doba_item_id']) {
                        dobaItemId = dobaViewDataCache.get(itemId)['doba_item_id'];
                    }

                    if (!dobaItemId) {
                        showNotification('danger', 'doba Item ID is not available for this SKU.');
                        if (saveIcon) {
                    ...
                    ajaxUrl = '/doba-product-price-update';
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // --- Update doba Price in all data and re-render table ---
                        if (title === 'doba Price') {
                    ...
                    error: function(xhr, status, error) {
                        ...
                        // Revert cache
                        dobaViewDataCache.updateField(itemId, title, originalValue);
            ...
            function getColorForColumn(column, rowData) {
                ...
                // For Click 90, use the raw value (not percentage)
                if (column === 'Click 90') {
                ...
                if (column === 'SCVR') {
                    let scvr = 0;
                    if (Number(rowData['Click 90']) > 0) {
                        scvr = Number(rowData['doba L30']) / Number(rowData['Click 90']);
                    }
            ...
            $(document).on('click', '.openPricingBtn', function() {
                ...
                $sprInput.off('input').on('input', function() {
                    ...
                    if (SPRICE > 0) {
                        const SPFT = ((SPRICE * 0.95) - LP - SHIP) / SPRICE;
                        const SROI = ((SPRICE * 0.95) - LP - SHIP) / LP;
