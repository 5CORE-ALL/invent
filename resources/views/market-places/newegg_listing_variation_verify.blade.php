...
@section('css')
    ...
    <style>
        ...
        .newegg-stat-badge--mismatch-inv { background: #dc2626; cursor: pointer; }
        .newegg-stat-badge--mismatch-inv:hover { filter: brightness(0.92); }
        .newegg-stat-badge--mismatch { cursor: pointer; }
        .newegg-stat-badge--mismatch:hover { filter: brightness(0.92); }
        .newegg-stat-badge.is-active { outline: 2px solid #0f172a; outline-offset: 2px; }
                ...
                <div class="card-body">
                    <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
                        <div class="d-flex align-items-center flex-wrap gap-2 py-1">
                            ...
                            <span class="newegg-stat-badge newegg-stat-badge--mismatch" id="newegg-lvv-badge-mismatch-btn" role="button" tabindex="0" title="Filter: mismatch only">MISMATCH:<span id="newegg-lvv-badge-mismatch">0</span></span>
                            <span class="newegg-stat-badge newegg-stat-badge--mismatch-inv" id="newegg-lvv-badge-mismatch-inv-btn" role="button" tabindex="0" title="Filter: mismatch parents with Shopify INV &gt; 0">MISMATCH INV&gt;0:<span id="newegg-lvv-badge-mismatch-inv">0</span></span>
                        </div>
...
@section('script')
    ...
    <script>
        ...
        function neweggLvvUpdateMeta(meta) {
            ...
            $('#newegg-lvv-badge-mismatch-inv').text((meta.mismatch_inv_gt0_count || 0).toLocaleString());
        ...
        function neweggLvvSyncBadgeActive() {
            const v = $('#newegg-lvv-listed-filter').val();
            $('#newegg-lvv-badge-mismatch-btn').toggleClass('is-active', v === 'mismatch');
            $('#newegg-lvv-badge-mismatch-inv-btn').toggleClass('is-active', v === 'mismatch_inv');
        }

        function neweggLvvApplyFilters() {
            if (!neweggLvvTable) return;
            if (window.ParentExpand) {
                ParentExpand.beforeFilters(function () { neweggLvvApplyFiltersBody(); });
                return;
            }
            neweggLvvApplyFiltersBody();
        }

        function neweggLvvApplyFiltersBody() {
            if (!neweggLvvTable) return;
            ...
            if (listedFilter === 'mismatch') {
                ...
                neweggLvvTable.addFilter(d => d.match_status === false && (parseFloat(d.INV) || 0) > 0);
        ...
        $(document).ready(function () {
            neweggLvvTable = new Tabulator('#newegg-listing-variation-verify-table', {
                ...
                rowFormatter: function (row) {
                    ...
                    if (isParent) {
                        el.classList.add('newegg-lvv-parent-row', 'parent-row', 'pm-parent-row');
                        el.classList.remove('newegg-lvv-child-row');
                    } else {
                        el.classList.remove('newegg-lvv-parent-row', 'parent-row', 'pm-parent-row');
                        el.classList.add('newegg-lvv-child-row');
                    }
                },
                columns: [
                    {
                        ...
                        formatter: function (cell) {
                            ...
                            if (!isParent) {
                                const sku = d.sku || '';
                                if (!sku) return neweggLvvDash(null);
                                return `<span class="fw-semibold text-primary">${neweggLvvEscapeHtml(sku)}</span>`;
                            }
                        ...
                        : { title: 'P', field: '_parent_expand', width: 36, headerSort: false, hozAlign: 'center' }),
                    {
                        ...
                        formatter: function (cell) {
                            ...
                            if (!isFinite(value)) return neweggLvvDash(null);
            ...
            if (window.ParentExpand) {
                ParentExpand.configure({
                    parentField: 'parent',
                    skuField: 'sku',
                    getTable: function () { return neweggLvvTable; },
                    getDataset: function () { return neweggLvvAllData; },
                    setDataset: function (rows) { neweggLvvAllData = rows; },
                    onCollapse: function () { neweggLvvApplyFilters(); },
                    onAfterExpand: function () { neweggLvvUpdateRowCount(); },
                });
                ParentExpand.bind();
            }
            ...
            $('#newegg-lvv-badge-mismatch-btn').on('click keypress', function (e) {
                if (e.type === 'keypress' && e.which !== 13 && e.which !== 32) return;
                e.preventDefault();
                const cur = $('#newegg-lvv-listed-filter').val();
                $('#newegg-lvv-listed-filter').val(cur === 'mismatch' ? 'all' : 'mismatch');
                neweggLvvApplyFilters();
            });
            $('#newegg-lvv-badge-mismatch-inv-btn').on('click keypress', function (e) {
                if (e.type === 'keypress' && e.which !== 13 && e.which !== 32) return;
                e.preventDefault();
                const cur = $('#newegg-lvv-listed-filter').val();
                $('#newegg-lvv-listed-filter').val(cur === 'mismatch_inv' ? 'all' : 'mismatch_inv');
                neweggLvvApplyFilters();
            });
