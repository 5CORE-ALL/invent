...
@section('css')
    ...
    <style>
        ...
        .tiktok-stat-badge--mismatch-inv { background: #dc2626; cursor: pointer; }
        .tiktok-stat-badge--mismatch-inv:hover { filter: brightness(0.92); }
        .tiktok-stat-badge--mismatch { cursor: pointer; }
        .tiktok-stat-badge--mismatch:hover { filter: brightness(0.92); }
        .tiktok-stat-badge.is-active { outline: 2px solid #0f172a; outline-offset: 2px; }
                ...
                <div class="card-body">
                    <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
                        <div class="d-flex align-items-center flex-wrap gap-2 py-1">
                            ...
                            <span class="tiktok-stat-badge tiktok-stat-badge--mismatch" id="tiktok-lvv-badge-mismatch-btn" role="button" tabindex="0" title="Filter: mismatch only">MISMATCH:<span id="tiktok-lvv-badge-mismatch">0</span></span>
                            <span class="tiktok-stat-badge tiktok-stat-badge--mismatch-inv" id="tiktok-lvv-badge-mismatch-inv-btn" role="button" tabindex="0" title="Filter: mismatch parents with Shopify INV &gt; 0">MISMATCH INV&gt;0:<span id="tiktok-lvv-badge-mismatch-inv">0</span></span>
                        </div>
...
@section('script')
    ...
    <script>
        ...
        function tiktokLvvUpdateMeta(meta) {
            ...
            $('#tiktok-lvv-badge-mismatch-inv').text((meta.mismatch_inv_gt0_count || 0).toLocaleString());
        ...
        function tiktokLvvSyncBadgeActive() {
            const v = $('#tiktok-lvv-listed-filter').val();
            $('#tiktok-lvv-badge-mismatch-btn').toggleClass('is-active', v === 'mismatch');
            $('#tiktok-lvv-badge-mismatch-inv-btn').toggleClass('is-active', v === 'mismatch_inv');
        }

        function tiktokLvvApplyFilters() {
            if (!tiktokLvvTable) return;
            if (window.ParentExpand) {
                ParentExpand.beforeFilters(function () { tiktokLvvApplyFiltersBody(); });
                return;
            }
            tiktokLvvApplyFiltersBody();
        }

        function tiktokLvvApplyFiltersBody() {
            if (!tiktokLvvTable) return;
            ...
            if (listedFilter === 'mismatch') {
                ...
                tiktokLvvTable.addFilter(d => d.match_status === false && (parseFloat(d.INV) || 0) > 0);
        ...
        $(document).ready(function () {
            tiktokLvvTable = new Tabulator('#tiktok-listing-variation-verify-table', {
                ...
                rowFormatter: function (row) {
                    ...
                    if (isParent) {
                        el.classList.add('tiktok-lvv-parent-row', 'parent-row', 'pm-parent-row');
                        el.classList.remove('tiktok-lvv-child-row');
                    } else {
                        el.classList.remove('tiktok-lvv-parent-row', 'parent-row', 'pm-parent-row');
                        el.classList.add('tiktok-lvv-child-row');
                    }
                },
                columns: [
                    {
                        ...
                        formatter: function (cell) {
                            ...
                            if (!isParent) {
                                const sku = d.sku || '';
                                if (!sku) return tiktokLvvDash(null);
                                return `<span class="fw-semibold text-primary">${tiktokLvvEscapeHtml(sku)}</span>`;
                            }
                        ...
                        : { title: 'P', field: '_parent_expand', width: 36, headerSort: false, hozAlign: 'center' }),
                    {
                        ...
                        formatter: function (cell) {
                            ...
                            if (!isFinite(value)) return tiktokLvvDash(null);
            ...
            if (window.ParentExpand) {
                ParentExpand.configure({
                    parentField: 'parent',
                    skuField: 'sku',
                    getTable: function () { return tiktokLvvTable; },
                    getDataset: function () { return tiktokLvvAllData; },
                    setDataset: function (rows) { tiktokLvvAllData = rows; },
                    onCollapse: function () { tiktokLvvApplyFilters(); },
                    onAfterExpand: function () { tiktokLvvUpdateRowCount(); },
                });
                ParentExpand.bind();
            }
            ...
            $('#tiktok-lvv-badge-mismatch-btn').on('click keypress', function (e) {
                if (e.type === 'keypress' && e.which !== 13 && e.which !== 32) return;
                e.preventDefault();
                const cur = $('#tiktok-lvv-listed-filter').val();
                $('#tiktok-lvv-listed-filter').val(cur === 'mismatch' ? 'all' : 'mismatch');
                tiktokLvvApplyFilters();
            });
            $('#tiktok-lvv-badge-mismatch-inv-btn').on('click keypress', function (e) {
                if (e.type === 'keypress' && e.which !== 13 && e.which !== 32) return;
                e.preventDefault();
                const cur = $('#tiktok-lvv-listed-filter').val();
                $('#tiktok-lvv-listed-filter').val(cur === 'mismatch_inv' ? 'all' : 'mismatch_inv');
                tiktokLvvApplyFilters();
            });
