@extends('layouts.vertical', ['title' => 'eBay Sprice×CVR Auto Push', 'sidenav' => 'condensed'])
...
<style>
    ...
    .btn-clear-sprice {
        background: #c0392b; border-color: #c0392b; color: #fff; font-weight: 600;
    }
    .btn-clear-sprice:hover { background: #a93226; color: #fff; }
    .btn-sprice-cvr {
        background: #ffd448; border-color: #e6bf2e; color: #1a1a1a; font-weight: 700;
    }
    .btn-sprice-cvr:hover { background: #f5c842; color: #1a1a1a; }
    .btn-push-ebay {
        background: #0f766e; border-color: #0f766e; color: #fff; font-weight: 600;
    }
    .btn-push-ebay:hover { background: #0d9488; color: #fff; }
    #sprice-auto-output {
    ...
    .channel-chip {
        ...
        border: 1px solid #cbd5e1; background: #f8fafc; margin-right: .4rem; margin-bottom: .35rem;
...
<div class="container-fluid sprice-auto-wrap py-3">
    <div class="sprice-auto-hero">
        <h3><i class="fas fa-robot me-2"></i>eBay Sprice×CVR Auto Push</h3>
        <p>Separate command pipeline for eBay 1, eBay 2, and eBay 3 — Clear SPRICE, apply % Sprice×CVR, then push to eBay.</p>
                ...
                <div class="card-body">
                    ...
                    <div class="sprice-step">
                        ...
                        <div>
                            ...
                            <div class="text-muted small">Remove existing SPRICE / SPFT / SROI fields for listed SKUs.</div>
                        </div>
                    </div>
                    <div class="sprice-step">
                        ...
                        <div>
                            ...
                            <div class="text-muted small">Same rule as tabulator (CVR slabs × Down/Same/Up signed % of eBay price).</div>
                        </div>
                    </div>
                    <div class="sprice-step">
                        <div class="sprice-step-num">3</div>
                        <div>
                            <div class="fw-semibold">Push SPRICE to eBay</div>
                            <div class="text-muted small">ReviseFixedPriceItem on eBay 1 / 2 / 3 with rate-limited API calls.</div>
                        </div>
                    </div>

                    <hr>
                    <div class="mb-2 fw-semibold">Channels</div>
                    <label class="channel-chip"><input type="checkbox" class="channel-cb" value="ebay1" checked> eBay 1</label>
                    <label class="channel-chip"><input type="checkbox" class="channel-cb" value="ebay2" checked> eBay 2</label>
                    <label class="channel-chip"><input type="checkbox" class="channel-cb" value="ebay3" checked> eBay 3</label>
                    ...
                    <div class="d-flex flex-wrap gap-2 mt-3">
                        <button type="button" id="btn-dry-run" class="btn btn-outline-secondary btn-sm"
                            title="Clear + Apply SPRICE in DB; does not push to eBay">
                            <i class="fas fa-eye"></i> Dry Run (Apply, no push)
                        </button>
                        <button type="button" id="btn-run-now" class="btn btn-push-ebay btn-sm">
                            <i class="fas fa-play"></i> Run Now (Clear → Apply → Push)
                        </button>
                        <a href="{{ url('/ebay-tabulator-view') }}" class="btn btn-sprice-cvr btn-sm">
                            <i class="fas fa-percentage"></i> % Sprice×CVR (eBay1)
                        </a>
        ...
        <div class="col-lg-5">
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-body">
                    ...
                    <p class="small text-muted mb-2">Shared key <code>ebay_sprice_cvr</code> (same as tabulator modal).</p>
                    <ul class="small mb-0">
                    ...
                    <a href="{{ url('/ebay-tabulator-view') }}" class="btn btn-link btn-sm px-0 mt-2">Edit rule on eBay1 tabulator →</a>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm mt-3">
                <div class="card-body">
                    ...
                    <code class="small d-block mb-2">php artisan ebay:sprice-cvr-auto-push</code>
                    <code class="small d-block mb-2">php artisan ebay:sprice-cvr-auto-push --dry-run</code>
                    <div class="text-muted small mb-2">Dry-run = Clear + Apply in DB, no eBay push. Refresh tabulator to see S PRC.</div>
                    <code class="small d-block">php artisan ebay:sprice-cvr-auto-push --channels=ebay1 --limit=10 --dry-run</code>
                </div>
            </div>
        </div>
    </div>
...
(function() {
    ...
    function selectedChannels() {
        return Array.from(document.querySelectorAll('.channel-cb:checked')).map(el => el.value);
    }
    ...
    function run(extra, confirmMsg) {
        ...
        fetch(@json(route('ebay.sprice-cvr-auto.run')), {
            method: 'POST',
            headers: {
