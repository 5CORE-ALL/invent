@php
    /**
     * YouTube Video Ads — independent sheet importer. Mirrors the layout of
     * the Meta Ads page but reads its own `youtube_video_ads_sheet` data and
     * posts to the /youtube-video-ads-sheet/* endpoints.
     */
    $pageType        = $pageType        ?? 'all';
    $pageTitle       = $pageTitle       ?? 'YouTube Video Ads';
    $pageSubtitle    = $pageSubtitle    ?? 'Generic CSV / Excel / TSV importer — upload YouTube ad sheets and view them as a table';
    $allowedAdTypes  = $allowedAdTypes  ?? ['SKIPPABLE', 'NON-SKIPPABLE', 'BUMPER', 'IN-FEED', 'SHORTS'];
@endphp
...
@section('css')
    ...
    <style>
        ...
        #youtube-video-ads-table .tabulator-header { background: #f9fafb; }
        #youtube-video-ads-table .tabulator-col-title { font-weight: 600; color: #1f2937; }
        ...
        #youtube-video-ads-table .tabulator-col-sorter,
        #youtube-video-ads-table .tabulator-arrow { display: none !important; }
        /* Reclaim the space the indicator used so titles don't look
           awkwardly off-centre. */
        #youtube-video-ads-table .tabulator-col-content { padding-right: 8px; }
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body" style="padding: 0;">
                <div id="faas-table-wrapper" style="height: calc(100vh - 230px); display: flex; flex-direction: column;">
                    <div id="youtube-video-ads-table" style="flex: 1;"></div>
                </div>
            </div>
        </div>
    </div>
...
{{-- ── Badge history chart modal ───────────────────────────────────
     ...
     opens this modal, /youtube-video-ads-sheet/badge-history feeds it,
...
<script>
    (function () {
        ...
        // Ad-Type dropdown choices for the YouTube Video Ads page.
        const AD_TYPES  = @json($allowedAdTypes);
        const AD_TYPE_COLORS = {
            'SKIPPABLE':     { bg: '#fee2e2', fg: '#991b1b' },
            'NON-SKIPPABLE': { bg: '#fef3c7', fg: '#92400e' },
            'BUMPER':        { bg: '#dbeafe', fg: '#1e40af' },
            'IN-FEED':       { bg: '#dcfce7', fg: '#166534' },
            'SHORTS':        { bg: '#fce7f3', fg: '#9d174d' },
        };
        // No short-label rewriting needed for YouTube ad types — show
        // the value verbatim.
        ...
        const FAAS_COLUMN_CHANNEL    = 'youtube_video_ads';
        ...
        function onAdTypeEdited(cell) {
            ...
            fetch(`/youtube-video-ads-sheet/${id}/ad-type`, {
                method:      'POST',
        ...
        function loadBatches(selectedId = '__merged__') {
            return fetch('/youtube-video-ads-sheet/batches', { credentials: 'same-origin' })
                .then(r => r.json())
                .catch(() => ({}));
        }
        ...
        function loadTable(batchId = '__merged__') {
            ...
            const url = '/youtube-video-ads-sheet/data?' + params.toString();
            ...
            return fetch(url, { credentials: 'same-origin' })
                ...
                .then(resp => {
                    ...
                    tabulator = new Tabulator('#youtube-video-ads-table', {
                        data:                   resp.data || [],
                        columns:                cols,
        ...
        function submitUpload() {
            ...
            fetch('/youtube-video-ads-sheet/upload', {
                method:      'POST',
                credentials: 'same-origin',
                headers:     { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' },
                body:        fd,
        ...
        const SBGT_RULE_GET_URL  = '/youtube-video-ads-sheet/rule';
        const SBGT_RULE_SAVE_URL = '/youtube-video-ads-sheet/rule';
        ...
        const BADGE_HISTORY_URL = '/youtube-video-ads-sheet/badge-history';
        ...
        const AUDIT_GET_URL  = '/youtube-video-ads-sheet/audit';
        const AUDIT_SAVE_URL = '/youtube-video-ads-sheet/audit';
        let auditCurrentCid  = '';
        ...
        function faasExportFilename(ext) {
            ...
            return `youtube_video_ads_${dateStr}.${ext}`;
        }
