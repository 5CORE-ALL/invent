    ...
    <div id="ai-chat-panel" class="ai-chat-panel" aria-hidden="true">
        <div class="ai-chat-header">
            ...
            <div class="ai-chat-header-actions">
                <a href="{{ route('ai.download.sample') }}" id="ai-sample-csv-btn" class="ai-header-link" title="Download sample CSV format">📥 Sample CSV</a>
        ...
        <div class="ai-chat-input-wrap">
            ...
            <div class="ai-chat-attach">
                ...
                <button type="button" id="ai-attach-btn" class="ai-attach-btn" title="Upload CSV to train AI">📎</button>
            </div>
...
.ai-chat-toggle { position: relative; width: 56px; height: 56px; border-radius: 50%;
...
.ai-chat-notification-badge { position: absolute; top: -5px; right: -5px; min-width: 18px; height: 18px; padding: 0 5px;
    background: #dc3545; color: #fff; font-size: 0.7rem; font-weight: 600; border-radius: 10px;
...
.ai-chat-header-actions { display: flex; align-items: center; gap: 12px; }
.ai-header-link { color: rgba(255,255,255,0.95); font-size: 0.75rem; text-decoration: none; padding: 4px 8px; border-radius: 4px; }
.ai-header-link:hover { color: #fff; background: rgba(255,255,255,0.15); }
...
.ai-chat-escalation-info { margin-top: 6px; font-size: 0.8rem; color: #6c757d; }
.ai-chat-input-wrap { padding: 12px; border-top: 1px solid #e9ecef; background: #fff; display: flex; gap: 8px; align-items: flex-end; flex-wrap: wrap; }
.ai-chat-input { flex: 1; min-width: 0; resize: none; border: 1px solid #ced4da; border-radius: 8px; padding: 10px 12px; font-size: 0.9rem; }
.ai-chat-input:focus { outline: none; border-color: #405189; box-shadow: 0 0 0 2px rgba(64,81,137,0.2); }
.ai-chat-attach { display: flex; align-items: center; }
.ai-attach-btn { background: #e9ecef; border: 1px solid #dee2e6; border-radius: 8px; padding: 10px 12px; cursor: pointer; font-size: 1.1rem; }
.ai-attach-btn:hover { background: #dee2e6; }
...
(function() {
    ...
    function appendMessage(role, content, recordId, isEscalation) {
        ...
        if (isEscalation) {
            inner += '<div class="ai-chat-escalation-info">Your question has been escalated. You will be notified when a senior responds.</div>';
        }
    ...
    function sendFeedback(id, helpful, msgWrap) {
        ...
        fetch(routes.feedback, { method: 'POST', body: fd, headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }, credentials: 'same-origin' })
    ...
    function fetchPendingReplies() {
        fetch(routes.pending, { headers: { 'Accept': 'application/json' }, credentials: 'same-origin' })
            .then(function(r) { return r.json(); })
            .then(function(data) {
    ...
    function markRepliesAsRead(ids) {
        ...
        fetch(routes.markRead, { method: 'POST', body: fd, headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }, credentials: 'same-origin' })
            .then(function() { checkNotifications(); })
            .catch(function() {});
    }

    function checkNotifications() {
        fetch(routes.check, { headers: { 'Accept': 'application/json' }, credentials: 'same-origin' })
            .then(function(r) { return r.json(); })
            .then(function(data) {
    ...
    fileInput.addEventListener('change', function() {
        ...
        messagesEl.scrollTop = messagesEl.scrollHeight;
        fetch(routes.upload, { method: 'POST', body: fd, headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }, credentials: 'same-origin' })
            .then(function(r) { return r.json(); })
            .then(function(data) {
                var el = messagesEl.querySelector('[data-uploading="1"]');
                if (el) el.remove();
                var msg = data.success ? 'File uploaded and processed successfully.' : (data.message || 'Upload failed.');
    ...
    function submitQuestion() {
        ...
        fetch(routes.chat, { method: 'POST', body: fd, headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }, credentials: 'same-origin' })
            .then(function(r) { return r.json(); })
            .then(function(data) {
                setLoading(false);
