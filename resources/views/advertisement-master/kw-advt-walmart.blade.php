@extends('layouts.vertical', ['title' => 'KW Walmart', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    ...
    @include('layouts.shared/page-title', [
        'page_title' => 'KW Walmart',
        'sub_title' => 'Walmart master Analysis',
    ])
...
@section('script')
    ...
    <script>
        const campaignSheetData = @json($sheetData['walmart_kw_l30']);
        const walmartKwL1 = @json($sheetData['walmart_kw_l2']);
        ...
        function findSheetBySKU(sku) {
            ...
            return campaignSheetData.find(item => {
                const campaignName = (item['Campaign name'] || '').trim().toUpperCase();
                return campaignName === normalizedSKU;
            });
        }

        function findSheetL2BySKU(sku) {
            if (!sku || !Array.isArray(walmartKwL1)) return null;

            const normalizedSKU = sku.trim().toUpperCase();

            return walmartKwL1.find(item => {
                const campaignName = (item['Campaign name'] || '').trim().toUpperCase();
                return campaignName === normalizedSKU;
            });
        }
            ...
            function renderTable(data) {
                ...
                displayItems.forEach(item => {
                    ...
                    if (isParentRow) {
                        ...
                        let impl30 = '-', clicksl30 = '-', spendl30 = '-', adSalesl30 = '-', adSoldl30 = '-', bgtCrnt = '-', cpcl2 = '-', acosl30 = '-', ctrL30 = '-', cvrl30 = '-';

                        if (sheet) {
                            impl30 = sheet.Impressions ?? '-';
                            clicksl30 = sheet.Clicks ?? '-';
                            spendl30 = sheet['Ad Spend'] ?? '-';
                            adSalesl30 = sheet['Total Attributed Sales'] ?? '-';
                            adSoldl30 = sheet['Units Sold'] ?? '-';
                            bgtCrnt = sheet['Daily budget'] ?? '-';

                            if (spendl30 > 0 && adSalesl30 === 0) {
                                acosl30 = '100%';
                            } else if (spendl30 > 0 && adSalesl30 > 0) {
                                acosl30 = Math.round((spendl30 / adSalesl30) * 100) + '%';
                            }

                            if (clicksl30 && impl30 && impl30 !== 0) {
                                ctrL30 = ((clicksl30 / impl30) * 100).toFixed(1) + '%';
                            }
                        ...
                        row.innerHTML = `
                            ...
                            <td class="text-center">${ctrL30}</td>
