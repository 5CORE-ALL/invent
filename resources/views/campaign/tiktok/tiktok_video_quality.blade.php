@extends('layouts.vertical', ['title' => 'TikTok Video Quality', 'sidenav' => 'condensed'])
...
@section('css')
    ...
    <style>
        ...
        #stat-tt-video-quality.badge,
        .badge-tt-video-quality {
...
@section('content')
    @include('layouts.shared.page-title', [
        'page_title' => 'TikTok Video Quality',
        'sub_title'  => 'All rows from /tiktok-1-ads-raw-data (tiktok_campaign_reports) — no filters',
    ])
        ...
        <div class="card shadow-sm">
            <div class="card-body py-3">
                <div class="d-flex align-items-center flex-wrap gap-2">
                    <span class="badge badge-tt-video-quality bg-primary" id="stat-tt-video-quality">
                        Rows <span id="total-tt-video-quality">0</span>
                    </span>
                    <a href="{{ route('tiktok1.ads.raw') }}" class="btn btn-sm btn-outline-primary">
                        Tiktok 1 Ads Raw Data
                    </a>
                    <button type="button" class="btn btn-sm btn-outline-secondary ms-auto" id="tt-video-quality-csv">
            ...
            <div class="card-body" style="padding: 0;">
                <div class="p-2 bg-light border-bottom">
                    <input type="text" id="tt-video-quality-search" class="form-control form-control-sm"
                           placeholder="Search campaign, product, video, status…">
                </div>
                <div id="tt-video-quality-table" style="height: calc(100vh - 280px);"></div>
            </div>
        </div>
    </div>
@endsection
...
<script>
    ...
    $(document).ready(function() {
        const table = new Tabulator('#tt-video-quality-table', {
            ajaxURL: "{{ route('tiktok.video.quality.data') }}",
            ajaxResponse: function(_url, _params, response) {
                const data = (response && response.data) ? response.data : [];
                $('#total-tt-video-quality').text(Number(response && response.count != null ? response.count : data.length).toLocaleString('en-US'));
                return data;
            },
            layout: 'fitData',
            pagination: true,
        ...
        $('#tt-video-quality-search').on('input', function() {
        ...
        $('#tt-video-quality-csv').on('click', function() {
            table.download('csv', 'tiktok_video_quality.csv');
        });
    });
</script>
@endsection
