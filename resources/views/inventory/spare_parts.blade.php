...
@section('script')
    ...
    <script>
        ...
        const SP = {
            summary: "{{ route('inventory.spare-parts.api.summary') }}",
            searchParts: "{{ route('inventory.spare-parts.api.search-parts') }}",
            partSkus: "{{ route('inventory.spare-parts.api.part-skus') }}",
            parts: "{{ route('inventory.spare-parts.api.parts') }}",
            lowStock: "{{ route('inventory.spare-parts.api.low-stock') }}",
            tree: "{{ route('inventory.spare-parts.api.tree') }}",
            suppliers: "{{ route('inventory.spare-parts.api.suppliers') }}",
            reqIndex: "{{ route('inventory.spare-parts.api.requisitions.index') }}",
            reqStore: "{{ route('inventory.spare-parts.api.requisitions.store') }}",
            issuesPending: "{{ route('inventory.spare-parts.api.issues.pending') }}",
            issueStore: "{{ route('inventory.spare-parts.api.issues.store') }}",
            poIndex: "{{ route('inventory.spare-parts.api.purchase-orders.index') }}",
            poStore: "{{ route('inventory.spare-parts.api.purchase-orders.store') }}",
            poReceive0: "{{ route('inventory.spare-parts.api.purchase-orders.receive', ['sparePartPurchaseOrder' => 0]) }}",
            partUpdate0: "{{ route('inventory.spare-parts.api.parts.update', ['id' => 0]) }}",
            sparePartDetailsStore: "{{ route('inventory.spare-parts.api.spare-part-details.store') }}",
            supplierForSku: "{{ route('refunds.supplier-for-sku') }}",
        };
