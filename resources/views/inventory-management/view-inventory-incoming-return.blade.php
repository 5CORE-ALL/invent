@extends('layouts.vertical', ['title' => 'View Inventory' . ($viewTitleSuffix ?? ''), 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('css')
    ...
    <style>
        ...
        /* DIL: compact colored text (AliExpress pricing style — no heavy pills) */
        .dil-percent-value {
            font-weight: 600;
            font-size: 12px;
        }

        /* Incoming return grid: warehouse column (same hues as Incoming Return page) */
        ...
        .incoming-return-grid-wh-text--main { color: #0d4d2e; font-weight: 600; }
        .incoming-return-grid-wh-text--trash { color: #9b1c1c; font-weight: 600; }
        .incoming-return-grid-wh-text--openbox { color: #7a5f00; font-weight: 600; }
        ...
        /* ── Incoming return grid: AliExpress-style shell ── */
        .ir-grid-toolbar {
            display: flex;
            flex-wrap: wrap;
            align-items: center;
        ...
        .ir-grid-table-wrap .custom-resizable-table {
            font-size: 12px;
        }
        .ir-grid-table-wrap .custom-resizable-table tbody td {
            padding: 8px 10px;
            border-bottom: 1px solid #eef1f4;
            vertical-align: middle;
        }
        .ir-grid-table-wrap .custom-resizable-table th.ir-grid-th-main {
            background: #3ac0c3 !important;
            border-bottom: 2px solid #2fb0b3;
            font-size: 11px;
            font-weight: 600;
            text-transform: uppercase;
        ...
        .ir-grid-table-wrap .custom-resizable-table th.ir-grid-th-main .sort-arrow {
            color: #0a3f42;
            font-weight: 700;
        }
        /* No hover light/dark change — same as default (th also gets .sortable from JS) */
        .ir-grid-table-wrap .custom-resizable-table th.ir-grid-th-main:hover,
        .ir-grid-table-wrap .custom-resizable-table th.ir-grid-th-main.sortable:hover {
            background: #3ac0c3 !important;
        }
        .ir-grid-table-wrap th.ir-grid-th-main .sortable:hover,
        .ir-grid-table-wrap th.ir-grid-th-main .sortable-header:hover {
        ...
        .ir-header-search {
            ...
            border: 1px solid rgba(10, 63, 66, 0.25) !important;
            color: #1e293b;
        }
        .ir-header-search::placeholder {
            color: #64748b;
        }
        ...
        /* Single sticky header row (summary totals moved above table) */
        #ebay-table thead tr th.ir-grid-th-main {
            position: sticky;
            top: 0;
            z-index: 12;
            box-shadow: 0 2px 4px rgba(58, 192, 195, 0.35);
        }
        ...
        #ebay-table .img-wrapper {
            position: relative;
            display: inline-block;
        }

        #ebay-table .thumbnail-img {
            width: 44px;
            height: 44px;
            object-fit: cover;
            border-radius: 6px;
        ...
        #ebay-table .popup-img img {
            width: 280px;
            height: auto;
            border-radius: 6px;
        }

        #ebay-table .img-wrapper:hover .popup-img {
            display: block;
        }
...
@section('content')
    <script>
        window.incomingReturnGridScope = @json($incomingReturnGridScope ?? null);
    </script>
    @include('layouts.shared/page-title', [
        'page_title' => 'Inventory Management' . ($viewTitleSuffix ?? ''),
        'sub_title' => 'View Inventory' . ($viewTitleSuffix ?? ''),
    ])
    <!-- <div class="row">
        <div class="col-12">
            <div class="card">
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title fw-semibold mb-2">View Inventory{{ $viewTitleSuffix ?? '' }}</h4>
                    <div class="alert alert-light border py-2 px-3 small mb-3 d-flex align-items-start gap-2" role="note">
                        <span class="rounded-circle flex-shrink-0 mt-1" style="width:0.5rem;height:0.5rem;background:{{ $incomingReturnGridDotColor ?? '#6c757d' }};"></span>
                        <span class="text-secondary mb-0">Only SKUs with an <code>incoming_return</code> in <strong>{{ $incomingReturnGridLabel ?? '' }}</strong> are shown (latest per SKU). Metrics match the main inventory sheet for those rows.</span>
                    </div>
                    ...
                    <!-- Controls row (AliExpress-style toolbar) -->
                    <div class="ir-grid-toolbar justify-content-between">
                        <div class="d-flex flex-wrap align-items-center gap-2">
                            <label for="row-data-type" class="small text-muted mb-0">Data type</label>
                            <select id="row-data-type" class="form-select form-select-sm" style="width:auto; min-width:7rem;">
                        ...
                        <span id="zeroInvLabel" class="badge bg-secondary" style="font-weight:600;">0 Inv SKUs: 0</span>

                        <div class="d-flex align-items-center gap-2 ms-auto">
                            <label for="search-input" class="small text-muted mb-0 d-none d-md-inline">Search</label>
                            <input type="text" id="search-input" class="form-control form-control-sm"
                                placeholder="Search all columns…" style="min-width: 12rem; max-width: 18rem;">
                        </div>
                    </div>

                    <div id="ir-grid-summary" class="ir-grid-summary-bar" aria-label="Column totals">
                        <span class="ir-sum-badge"><span class="ir-sum-label">INV</span><strong id="inv-total">0</strong></span>
                        <span class="ir-sum-badge"><span class="ir-sum-label">L30</span><strong id="ovl30-total">0</strong></span>
                        <span class="ir-sum-badge"><span class="ir-sum-label">DIL</span><strong id="ovdil-total">0%</strong></span>
                        <span class="ir-sum-badge"><span class="ir-sum-label">Qty</span><strong id="return-qty-total">0</strong></span>
                        <span class="ir-sum-badge"><span class="ir-sum-label">On hand</span><strong id="onhand-total">0</strong></span>
                        <span class="ir-sum-badge"><span class="ir-sum-label">Committed</span><strong id="committed-total">0</strong></span>
                        <span class="ir-sum-badge"><span class="ir-sum-label">Avl sell</span><strong id="avltosell-total">0</strong></span>
                    </div>

                    <div class="table-container ir-grid-table-wrap">
                        <table class="custom-resizable-table" id="ebay-table">
                            <thead>
                                <tr>
                                     <th data-field="images" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;">
                                    ...
                                    <th data-field="parent" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center sortable-header">
                                                Parent <span class="sort-arrow">↓</span>
                                            </div>
                                            <div class="mt-1 dropdown-search-container w-100">
                                                <input type="text" class="form-control form-control-sm parent-search ir-header-search"
                                                    placeholder="Filter parent…" id="parentSearch" title="Buscar parent">
                                                <div class="dropdown-search-results" id="parentSearchResults"></div>
                                            </div>
                                        </div>
                                    </th>
                                    <th data-field="sku" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center w-100">
                                            <div class="d-flex align-items-center sortable-header">
                                                SKU <span class="sort-arrow">↓</span>
                                            </div>
                                            <div class="mt-1 dropdown-search-container w-100">
                                                <input type="text" class="form-control form-control-sm sku-search ir-header-search"
                                                    placeholder="Filter SKU…" id="skuSearch" title="Buscar SKU">
                                                <div class="dropdown-search-results" id="skuSearchResults"></div>
                                            </div>
                                        </div>
                                    </th>
                                    <th data-field="r&a" class="hide-column ir-grid-th-main"
                                    ...
                                    <th data-field="inv" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                    ...
                                    <th data-field="ov_l30" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                    ...
                                    <th data-field="ov_dil" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                    ...
                                    <th data-field="RETURN_QTY" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;" title="Cantidad">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                    ...
                                    <th data-field="WAREHOUSE_NAME" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;" title="Almacén">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                                Warehouse <span class="sort-arrow">↓</span>
                                            </div>
                                        </div>
                                    </th>
                                    <th data-field="el_30" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                    ...
                                    <th data-field="e_dil" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                    ...
                                    <th data-field="views" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                                Avl sell <span class="sort-arrow">↓</span>
                                            </div>
                                        </div>
                                    </th>
                                    <th data-field="views" class="ir-grid-th-main" style="vertical-align: middle; white-space: nowrap;" title="Our history / Shopify">
                                        <div class="d-flex flex-column align-items-center">
        ...
        $(document).ready(function() {
            ...
            function loadData() {
                showLoader();
                    ...
                    // Incoming return inventory for Trash / Open Box godown (latest per SKU), same shape as get-verified-stock merge
                    const inventoryAjax = $.ajax({
                        url: '/incoming-return-grid-inventory',
                        type: 'GET',
                        dataType: 'json',
                        data: {
                            warehouse_scope: window.incomingReturnGridScope || ''
                        }
                    });
                    ...
                    return $.when(sheetAjax, inventoryAjax).done(function(sheetRes, inventoryRes) {
                        ...
                        // Only list SKUs that have at least one incoming_return row in this godown (API returns latest per SKU).
                        // The full verification sheet is still fetched so INV / L30 / parent columns stay populated for those SKUs.
                        const skusWithReturn = new Set(Object.keys(inventoryMap));

                        // Map over sheet data and merge fields from inventory data if SKU matches (API may send sku lowercase)
                        tableData = sheetDataRaw
                            .filter(item => {
                                const sku = (item.SKU || item.sku || '').toUpperCase().trim();
                                return sku !== '' && skusWithReturn.has(sku);
                            })
                            .map((item, index) => {
                            const sku = (item.SKU || item.sku || '').toUpperCase().trim();
                            ...
                            return {
                                ...
                                L30: L30,
                                DIL: DIL,
                                RETURN_QTY: invItem.quantity !== undefined && invItem.quantity !== null && invItem.quantity !== ''
                                    ? invItem.quantity
                                    : '',
                                WAREHOUSE_NAME: invItem.warehouse_name !== undefined && invItem.warehouse_name !== null
                                    ? String(invItem.warehouse_name)
                                    : '',
                                WAREHOUSE_THEME: invItem.warehouse_theme ? String(invItem.warehouse_theme) : '',
            ...
            function renderTable() {
                ...
                if (isLoading) {
                    $tbody.append('<tr><td colspan="13" class="text-center">Loading data...</td></tr>');
                    return;
                }

                if (filteredData.length === 0) {
                    $tbody.append('<tr><td colspan="13" class="text-center">No matching records found</td></tr>');
                    return;
                }

                const zeroInvCountAll = filteredData.filter(r => parseFloat(r.AVAILABLE_TO_SELL) === 0).length;
                $('#zeroInvLabel').text(`0 Inv SKUs: ${zeroInvCountAll}`);
                ...
                filteredData.forEach((item, rowIndex) => {
                    ...
                    if (item.is_parent || ((item.SKU || item.sku || '').toUpperCase().startsWith('PARENT'))) {
                    ...
                    $row.append($('<td>').addClass(item.is_parent ? 'fw-bold' : '').text(item.Parent));
                    ...
                    $row.append($('<td>').addClass('ir-grid-num').text(item.INV));
                    $row.append($('<td>').addClass('ir-grid-num').text(item.L30));

                    const dilValue = parseFloat(item.DIL) || 0;
                    const dilPercent = Math.round(dilValue * 100);
                    const dilColor = dilTextColor(dilValue);
                    const dilContent = `<span class="dil-percent-value" style="color:${dilColor};">${dilPercent}%</span>`;
                    $row.append($('<td>').addClass('ir-grid-num').html(dilContent));

                    const retQty = item.RETURN_QTY;
                    $row.append($('<td>').addClass('ir-grid-num').text(
                        retQty !== '' && retQty !== undefined && retQty !== null ? retQty : ''
                    ));

                    const $whCell = $('<td>').addClass('incoming-return-grid-wh-cell');
                    const whTheme = item.WAREHOUSE_THEME || '';
                    const whName = item.WAREHOUSE_NAME || '';
                    const $whWrap = $('<span>').addClass('d-inline-flex align-items-center');
                    if (whTheme) {
                        $whWrap.append(
                            $('<span>').addClass('incoming-wh-dd-item-dot incoming-wh-dd-item-dot--' + whTheme)
                                .attr('aria-hidden', 'true')
                        );
                    }
                    const textCls = whTheme ? 'incoming-return-grid-wh-text--' + whTheme : '';
                    $whWrap.append($('<span>').addClass(textCls).text(whName || '—'));
                    $whCell.append($whWrap);
                    $row.append($whCell);

                    $row.append($('<td>').addClass('on-hand ir-grid-num').text(item.ON_HAND));
                    $row.append($('<td>').addClass('ir-grid-num').text(item.COMMITTED));
                    $row.append($('<td>').addClass('ir-grid-num').text(item.AVAILABLE_TO_SELL));
            ...
            function initSorting() {
                $('th[data-field]').addClass('sortable').on('click', function(e) {
                    ...
                    freshData.sort((a, b) => {
                        ...
                        if (dataField === 'INV' || dataField === 'L30' || dataField === 'RETURN_QTY') {
            ...
            function calculateTotals() {
                try {
                    ...
                    filteredData.forEach(item => {
                        ...
                        metrics.returnQtyTotal += parseFloat(item.RETURN_QTY) || 0;
                    ...
                    $('#return-qty-total').text(Math.round(metrics.returnQtyTotal).toLocaleString());
