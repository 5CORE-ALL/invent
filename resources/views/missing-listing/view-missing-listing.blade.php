...
document.addEventListener("DOMContentLoaded", function() {
    ...
    table = new Tabulator("#budget-under-table", {
        ...
        ajaxResponse: function(url, params, response) {
            ...
            setTimeout(() => {
                if (zeroInventoryFilterEnabled && !zeroInvFilterFunc) {
                    zeroInvFilterFunc = function(data) {
                        let z = data.is_zero_inventory || {};
                        return !(z && z.shopify === true);
                    };
                    table.addFilter(zeroInvFilterFunc);
                    const btn = document.getElementById("zeroInvToggle");
                    if (btn) {
    ...
    let zeroInventoryFilterEnabled = true;
    ...
    table.on("tableBuilt", function() {
        ...
        $("#ad-status-filter, #inv-filter, #nrl-filter, #nra-filter, #fba-filter").on("change",
            function() {
                table.setFilter(combinedFilter);
            });
    ...
    $(document).on('click', '#confirmImportBtn', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#importFile').val('');
                showNotification('success', 'Import successful! Processed: ' + response
                    .processed);
                location.reload();
            },
            error: function(xhr) {
