        ...
        },
        {
            ...
            formatter: function(cell) {
                ...
                if (value === true || value === "true" || value === 1) {
                    // Checked checkbox if true
                    return `<input type="checkbox" class="mark-done-checkbox" checked title="Mark as Done">`;
                } else if (value) {
                    // If value is a date, show badge
                    const dateObj = new Date(value);
                    if (!isNaN(dateObj.getTime())) {
                        const options = { day: '2-digit', month: 'short', year: 'numeric' };
                        return `<span class="badge bg-success">${dateObj.toLocaleDateString('en-US', options)}</span>`;
                    }
                }
                // Default: unchecked checkbox
                return `<input type="checkbox" class="mark-done-checkbox" title="Mark as Done">`;
            },
            cellClick: function(e, cell) {
                if (e.target.classList.contains('mark-done-checkbox')) {
                    const row = cell.getRow();
                    const sku = row.getData().SKU;
                    const parent = row.getData().Parent;
                    const checked = e.target.checked;

                    if (checked) {
                        const today = new Date().toISOString().slice(0, 10);
                        $.ajax({
                            url: '/product-master/wmp-mark-as-done',
                            type: 'POST',
                            data: {
