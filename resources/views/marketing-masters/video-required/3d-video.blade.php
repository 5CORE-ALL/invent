@extends('layouts.vertical', ['title' => '3D Video Required'])
...
@section('content')
@include('layouts.shared.page-title', ['page_title' => '3D Video Required', 'sub_title' => '3D Video Required'])
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <div class="mb-3 d-flex flex-wrap align-items-center gap-3">
                    ...
                    <!-- Toggle NR -->
                    <button id="toggle-nr-rows" class="btn btn-outline-primary">
                        Show NR &nbsp;<span id="total-nr" class="badge bg-primary text-white"></span>
                    </button>

                    <!-- Toggle REQ -->
                    <button id="toggle-req-rows" class="btn btn-info">
                        Show REQ &nbsp;<span id="total-req" class="badge bg-white text-info"></span>
                    </button>
                    ...
                    <button id="total-pending-video" class="btn btn-danger">Pending Videos &nbsp;<span id="pending-video-count" class="badge bg-white text-danger">0</span></button>
                    <button id="clear-filter" class="btn btn-secondary">Clear Filter</button>

                </div>
                <div id="threed-video-req"></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const table = new Tabulator("#threed-video-req", {
            index: "Sku",
            ajaxURL: "/3d-video-req/view-data",
            ajaxConfig: "GET",
            layout: "fitDataFill",
            pagination: true,
            paginationSize: 50,
            ...
            columns: [
                ...
                },
                {
                    ...
                    titleFormatter: function() {
                        return `<div>
                            New<br>
                            <span id="total-new-header" style="font-size:13px;color:white;font-weight:600;"></span>
                        </div>`;
                    },
                    formatter: function (cell) {
                ...
                },
                {
                    ...
                    titleFormatter: function() {
                        return `<div>
                            2BDC<br>
                            <span id="total-2bdc-header" style="font-size:13px;color:white;font-weight:600;"></span>
                        </div>`;
                    },
                    formatter: function (cell) {
                        const row = cell.getRow();
                ...
                },
                {
                    ...
                    titleFormatter: function() {
                        return `<div>
                            Approved<br>
                            <span id="total-approved-header" style="font-size:13px;color:white;font-weight:600;"></span>
                        </div>`;
                    },
                    formatter: function (cell) {
                        const row = cell.getRow();
                ...
                },
                {
                    ...
                    titleFormatter: function () {
                        return `<div>
                            Avl Video <i class='fa fa-link'></i>
                        </div>`;
                    },
                    hozAlign: "center",
                },
                {
                    title: "Avl Footage Link",
                    field: "AvlFootageLink",
                    editor: "input",
                    formatter: function(cell) {
                        const value = cell.getValue();
                        if (value && value.trim() !== "") {
                    ...
                    titleFormatter: function () {
                        return `<div>
                            Avl Footage <i class='fa fa-link'></i>
                        </div>`;
                    },
                    hozAlign: "center",
                },
                {
                    ...
                    titleFormatter: function () {
                        return `<div>
                            3D Pictures <i class='fa fa-link'></i>
                        </div>`;
                    },
                    hozAlign: "center",
                },
        ...
        table.on("cellEdited", function(cell) {
            ...
            const payload = {
                sku: rowData.Sku,
                value: {
                    New: rowData.New || false,
                    "2BDC": rowData["2BDC"] || false,
                    NR: rowData.NR || false,
                    Req: rowData.Req || false,
                    AvlFootageLink: rowData.AvlFootageLink,
                    AvlVideoLink: rowData.AvlVideoLink,
                    PicturesLink: rowData.PicturesLink,
                    Assigned: rowData.Assigned || '',
                }
            };

            fetch("/3d-video-req/save", {
                method: "POST",
                headers: {
        ...
        buildColumnDropdown();
        applyNrReqAndClearFilter(table);
        ...
        function updateTotalInvAndL30(table) {
            ...
            const totalNew = data.filter(row => row["New"] === true).length;
            const total2BDC = data.filter(row => row["2BDC"] === true).length;
            const totalAvl = data.filter(row => row["AvlVideoLink"] === true).length;
            const totalApproved = data.filter(row => row["Approved"] === true).length;

            const totalNR = data.filter(row => row["NR"] === "NR").length;
            const totalREQ = data.filter(row => row["NR"] === "REQ").length;
            // total pending video count
            const totalSKUs = data.length;
            const pendingVideos = totalSKUs - totalNR - totalAvl;
            ...
            document.getElementById("total-new-header").textContent = totalNew.toLocaleString();
            document.getElementById("total-2bdc-header").textContent = total2BDC.toLocaleString();
            
            document.getElementById("total-nr").textContent = totalNR.toLocaleString();
            document.getElementById("total-req").textContent = totalREQ.toLocaleString();

            document.getElementById("total-approved-header").textContent = totalApproved.toLocaleString();

            // Update pending video count
            const pendingEl = document.getElementById("pending-video-count");
            if (pendingEl) pendingEl.textContent = pendingVideos.toLocaleString();
        ...
        // show nr, req and clear filter
        function applyNrReqAndClearFilter(tableInstance) {
            const clearFilterBtn = document.getElementById("clear-filter");
            clearFilterBtn.style.display = "none";

            document.getElementById("toggle-nr-rows").addEventListener("click", () => {
                tableInstance.setFilter("NR", "=", "NR");
                clearFilterBtn.style.display = "inline-block";
            });

            document.getElementById("toggle-req-rows").addEventListener("click", () => {
                tableInstance.setFilter("NR", "=", "REQ");
                clearFilterBtn.style.display = "inline-block";
            });
        ...
        document.addEventListener("change", function (e) {
            ...
            if (field === "Approved" && value === true && (!rowData.AvlVideoLink || rowData.AvlVideoLink.trim() === "")) {
                alert("Please update Available video link first.");
