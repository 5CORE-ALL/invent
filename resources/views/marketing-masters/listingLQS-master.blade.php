@extends('layouts.vertical', ['title' => 'Listing LQS Master', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">Listing LQS MAster</h4>
                    ...
                    <div class="table-container">
                        <table class="custom-resizable-table" id="listingLQS-table">
                            <thead>
                                <tr>
                                    ...
                                    <th data-field="lqs_jungle" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center">
                                            ...
                                            <!-- <div style="width: 100%; height: 5px; background-color: #9ec7f4;"></div> -->
                                            <!-- <div class="metric-total" id="lqs-total">0</div> -->
                                        </div>
                                    </th>
                                    <th data-field="lqs" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                                LQS <span class="sort-arrow">↓</span>
                                            </div>
                                            <div style="width: 100%; height: 5px; background-color: #9ec7f4;"></div>
                                            <div class="metric-total" id="lqs-total">0</div>
                                        </div>
                                    </th>
                                    <th data-field="lqsc" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                                LQS C <span class="sort-arrow">↓</span>
                                            </div>
                                            <div style="width: 100%; height: 5px; background-color: #9ec7f4;"></div>
                                    ...
                                    <th data-field="link"
                                        ...
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                                C LINK <span class="sort-arrow">↓</span>
                                            </div>
                                        </div>
                                    </th>
                                    <th data-field="issue"
                                        ...
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center">
                                                ISSUE <span class="sort-arrow">↓</span>
                                            </div>
                                        </div>
                                    </th>
                    ...
                    <div id="data-loader" class="card-loader-overlay" style="display: none;">
                        <div class="loader-content">
                            ...
                            <div class="loader-text">Loading Listing LQS data...</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function() {
            $('#editPercentBtn').on('click', function() {
                ...
                if ($icon.hasClass('fa-pen')) {
                    ...
                    $.ajax({
                        url: '/update-all-listingLQS-skus',
                        type: 'POST',
                        data: {
                            percent: percent,
            ...
            function renderTable() {
                const $tbody = $('#listingLQS-table tbody');
                $tbody.empty();

                if (isLoading) {
                    $tbody.append('<tr><td colspan="10" class="text-center">Loading data...</td></tr>');
                    return;
                }

                if (filteredData.length === 0) {
                    $tbody.append('<tr><td colspan="10" class="text-center">No matching records found</td></tr>');
                    return;
                }
                ...
                filteredData.forEach(item => {
                    ...
                    $row.append($('<td>').html(`
                        <div style="display: flex; align-items: center; gap: 6px;">
                            <input type="text"
                                ...
                                data-sku="${item['(Child) sku']}"
                                style="width: 100px;" />
                            ${cLink ? `<a href="${cLink}" target="_blank" style="font-size: 14px;">🔗</a>` : ''}
                        </div>
                    `));
                    // const cLink = item.raw_data?.link || '';
                    // $row.append($('<td>').html(`
                    ...
                    // ISSUE
                    const issue = item.raw_data?.issue || '';
                    $row.append($('<td>').html(`
                        <input type="text"
                            class="form-control form-control-sm issue-input"
                            value="${issue}"
                            data-sku="${item['(Child) sku']}"
                            style="width: 160px;" />
                    `));

                    // Get the full action text from raw_data
                    const actionText = item.raw_data && item.raw_data['action'] ? String(item.raw_data[
                        'action']) : '';
                    const words = actionText.split(/\s+/);
                    const shortText = words.length > 3 ? words.slice(0, 3).join(' ') + ' ...' : actionText;

                    // Use a custom hover effect instead of Bootstrap tooltip
                    $row.append($('<td>').html(`
                        <span class="action-value-text action-hover"
                            style="margin-left:8px; color:#333; cursor:pointer; position:relative;">
                            ${$('<div>').text(shortText).html()}
                            ${actionText && words.length > 3 ? `
                                            <span class="action-full-text" style="
                                                display:none;
                                                position:absolute;
                                                left:0;
                                                top:100%;
                                            ...
                                            ">${$('<div>').text(actionText).html()}</span>
                                        ` : ''}
                        </span>
                        <span class="action-plus-icon" style="cursor:pointer; color:#28a745; font-size:1.3em; margin-left:6px;" title="Add Action">
            ...
            $(document).off('mouseenter mouseleave', '.action-hover').on('mouseenter',
                '.action-hover',
                function() {
                    $(this).find('.action-full-text').stop(true, true).fadeIn(120);
                }).on('mouseleave', '.action-hover', function() {
                $(this).find('.action-full-text').stop(true, true).fadeOut(120);
            });

            $(document).on('click', '.action-plus-icon', function(e) {
                e.stopPropagation();
                ...
                // Use the correct action value (from .action or .ACTION for backward compatibility)
                const actionValue = rowData && rowData.raw_data && (rowData.raw_data.action || rowData
                        .raw_data.ACTION) ?
                    (rowData.raw_data.action || rowData.raw_data.ACTION) : '';

                // Build modal HTML
                const modalId = 'action-input-modal';
                let $modal = $('#' + modalId);
                if ($modal.length) $modal.remove(); // Remove if already exists
                ...
                $modal = $(`
                    <div class="modal fade" id="${modalId}" tabindex="-1" aria-labelledby="${modalId}-label" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="${modalId}-label">Add Action for SKU: <span style="color:#007bff">${rowData ? rowData['(Child) sku'] : ''}</span></h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <input type="text" class="form-control" id="action-input-field" placeholder="Enter action..." value="${actionValue}">
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                    <button type="button" class="btn btn-primary" id="save-action-btn">Save</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                $modal.off('click', '#save-action-btn').on('click', '#save-action-btn', function() {
                    ...
                    const $row = $(`#listingLQS-table tbody tr`).filter(function () {
                        return $(this).find('.lqs-input').data('sku') === sku;
                    });

                    const lqsValue = $row.find('.lqs-input').val()?.trim() || '';
                    const lqscValue = $row.find('.lqsc-input').val()?.trim() || '';
                    
                    const linkValue = $row.find('.clink-input').val()?.trim() || '';
                    const issueValue = $row.find('.issue-input').val()?.trim() || '';
                    
                    console.log('LQS:', lqsValue, 'LQSC:', lqscValue, 'Link:', linkValue, 'Issue:', issueValue);


                    // Save to backend via AJAX
                    $.ajax({
                        url: '/listing-lqs/save-action',
                        type: 'POST',
                        data: {
                            sku: sku,
                            action: newActionValue,
                            listing_quality_score: lqsValue,
                            listing_quality_score_c: lqscValue,
                            link: linkValue,
                            issue: issueValue, 
                            _token: $('meta[name="csrf-token"]').attr('content')
                        },
                        success: function(response) {
                            showNotification('success', 'Action saved!');
                            if (rowData) {
                                rowData.action = newActionValue;
                                rowData.listing_quality_score = lqsValue;
                                rowData.listing_quality_score_c = lqscValue;
                                rowData.link = linkValue;
                                rowData.issue = issueValue;

                                listingLqsDataCache.updateField(rowData['SL No.'],
                                    'action', newActionValue);
                                listingLqsDataCache.updateField(rowData['SL No.'], 'listing_quality_score', lqsValue);
                                listingLqsDataCache.updateField(rowData['SL No.'], 'listing_quality_score_c', lqscValue);
                                listingLqsDataCache.updateField(rowData['SL No.'], 'link', linkValue);
                                listingLqsDataCache.updateField(rowData['SL No.'], 'issue', issueValue);

                                // Update the action text in the table immediately
                                $row.find('.action-value-text').text(newActionValue);
                                $row.find('.lqs-input').val(lqsValue);
                                $row.find('.lqsc-input').val(lqscValue);
                                $row.find('.clink-input').val(linkValue);
                                $row.find('.issue-input').val(issueValue);
                            }
                            bsModal.hide();
                        },
                        error: function(xhr) {
                            showNotification('danger', 'Failed to save action.');
                        }
                    });
                });
            ...
            window.openModal = function(selectedItem, type) {
                try {
                    ...
                    const cachedData = listingLqsDataCache.get(itemId);
                    const dataToUse = cachedData || itemData;

                    // Store the data in cache if it wasn't already
                    if (!cachedData) {
                        listingLqsDataCache.set(itemId, itemData);
                    }
            ...
            function saveChanges(contentElement, title, slNo, isHyperlink, updatedValue, isCheckbox, rowElement) {
                ...
                listingLqsDataCache.updateField(itemId, title, cacheUpdateValue);
                ...
                if (index !== -1) {
                    ...
                    if (title === 'SPRICE' && filteredData[index].raw_data) {
                        ...
                        listingLqsDataCache.updateField(itemId, 'Spft%', Spft);
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        ...
                        // Revert cache
                        listingLqsDataCache.updateField(itemId, title, originalValue);
            ...
            function initTooltips() {
                $('[data-bs-toggle="tooltip"]').tooltip({
                    trigger: 'hover',
                    placement: 'top',
                    container: 'body', // <--- This is important!
                    boundary: 'window',
                    offset: [0, 5],
            ...
            function calculateTotals() {
                try {
                    ...
                    filteredData.forEach(item => {
                        ...
                        metrics.lqsSum += parseFloat(item.listing_quality_score) || 0;
                        metrics.lqscSum += parseFloat(item.raw_data?.listing_quality_score_c) || 0;
                        metrics.rowCount++;
                    ...
                    const lqsAvg = metrics.rowCount > 0 ? (metrics.lqsSum / metrics.rowCount).toFixed(2) : 0;
                    const lqscAvg = metrics.rowCount > 0 ? (metrics.lqscSum / metrics.rowCount).toFixed(2) : 0;
                    $('#lqs-total').text(lqsAvg);
                    $('#lqsc-total').text(lqscAvg);
