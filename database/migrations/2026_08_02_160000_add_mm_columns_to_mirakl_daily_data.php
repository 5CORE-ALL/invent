...
return new class extends Migration
{
    public function up(): void
    {
        if (Schema::hasTable('mirakl_daily_data')) {
            Schema::table('mirakl_daily_data', function (Blueprint $table) {
                if (! Schema::hasColumn('mirakl_daily_data', 'shopify_order_id')) {
                    $table->string('shopify_order_id')->nullable()->index();
                }
                if (! Schema::hasColumn('mirakl_daily_data', 'pushed_to_shopify_at')) {
                    $table->timestamp('pushed_to_shopify_at')->nullable();
                }
                if (! Schema::hasColumn('mirakl_daily_data', 'import_status')) {
                    $table->string('import_status')->nullable()->index();
                }
                if (! Schema::hasColumn('mirakl_daily_data', 'raw_payload')) {
        ...
        if (Schema::hasTable('mirakl_daily_data')) {
            Schema::table('mirakl_daily_data', function (Blueprint $table) {
                foreach (['shopify_order_id', 'pushed_to_shopify_at', 'import_status', 'raw_payload'] as $col) {
                    if (Schema::hasColumn('mirakl_daily_data', $col)) {
