...
return new class extends Migration
{
    ...
    {
        // Reuse existing ebay_2_metrics as MM link map (item_id = product_id).
        if (! Schema::hasTable('ebay2_order_metrics')) {
            Schema::create('ebay2_order_metrics', function (Blueprint $table) {
                $table->id();
                ...
                $table->timestamps();
                $table->unique(['order_id', 'sku'], 'ebay2_order_metrics_order_sku_unique');
            });
        }
    }
    ...
    {
        Schema::dropIfExists('ebay2_order_metrics');
    }
};
