...
<?php $__env->startSection('content'); ?>
    <?php echo $__env->make('layouts.shared/page-title', ['page_title' => 'eBay', 'sub_title' => 'eBay Analysis'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                    ...
                    <div id="percent-edit-div" class="d-flex align-items-center">
                        <div class="input-group" style="width: 150px;">
                            <input type="number" id="updateAllSkusPercent" class="form-control" min="0"
                                max="100" value="<?php echo e($ebayPercentage); ?>" step="0.01" title="Percent" disabled />
...
<?php $__env->startSection('script'); ?>
    ...
    <script>
    const listingStatusData = <?php echo json_encode($listingStatus, 15, 512) ?>;
    </script>
    <script>
        ...
        $(document).ready(function() {
            ...
            function renderTable() {
                ...
                filteredData.forEach(item => {
                    ...
                    const safeSku = escapeHtml(skuValue);

                    // Fetch listing status from server data using SKU
                    let statusRaw = listingStatusData[skuValue]?.value || "{}"; // fallback to empty JSON
                    ...
                    // Extract buyer/seller links
                    let buyerLink = status.buyer_link?.trim() || "";
                    let sellerLink = status.seller_link?.trim() || "";
                        ...
                        $skuCell.html(`
                            <div class="sku-tooltip-container" style="position: relative; cursor: pointer;">
                                ...
                                ">
                                    ...
                                    <div>
                                        ${sellerLink ? `<a href="${sellerLink}" target="_blank">Seller Link</a>` : '<span style="color:#aaa">No Seller Link</span>'}
                                    </div>
                                </div>
                            </div>
                        `);
            ...
            <i class="fas fa-tag text-warning price-view-trigger" 
                ...
                data-item='${JSON.stringify(item).replace(/'/g, "&apos;")}'></i>
                    ...
                    if (!isNaN(lmpValue) && lmpValue > 0) {
                        ...
                        $wrapper.append(`<span class="dil-percent-value">$${lmpValue.toFixed(2)}</span>`);

                        const lmpEntries = Array.isArray(item.lmp_entries) ? item.lmp_entries : [];
                        const lmpLink = item.lmp_link || null;
                        if (lmpEntries.length > 0 || lmpLink) {
                            const $eyeIcon = $('<i>')
                                .addClass('fas fa-eye text-info lmp-details-trigger')
                                .attr({
                                    title: 'View LMP details',
                                    'data-bs-toggle': 'tooltip',
                                    'data-bs-placement': 'top'
                                })
                                .css('cursor', 'pointer');
                            $eyeIcon.data('lmpDetails', lmpEntries);
                            $eyeIcon.data('sku', item['(Child) sku']);
                            if (lmpLink) {
                                $eyeIcon.data('lmpLink', lmpLink);
                            }
                            $wrapper.append($eyeIcon);
                        }

                        $lmpCell.append($wrapper);
                    }
                    $row.append($lmpCell);
...
<button type="button" id="savePricingBtn" class="btn btn-primary">Save</button>
    ...
    `);
        ...
        $(document).on('click', '.lmp-details-trigger', function() {
            const details = $(this).data('lmpDetails') || [];
            const sku = $(this).data('sku') || '';
            const fallbackLink = $(this).data('lmpLink') || null;
            const $modal = $('#lmpDetailsModal');
            const $body = $('#lmp-details-body');

            $('#lmp-details-modal-title').text(`LMP Details — ${sku}`);
    ...
    <div class="modal fade" id="lmpDetailsModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-md modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="lmp-details-modal-title">LMP Details</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div id="lmp-details-body"></div>
                </div>
            </div>
        </div>
    </div>
...
<?php echo $__env->make('layouts.vertical', ['title' => 'eBay', 'mode' => $mode ?? '', 'demo' => $demo ?? ''], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\5core_inventory\resources\views/market-places/ebay_pricing_increase_decrease.blade.php ENDPATH**/ ?>