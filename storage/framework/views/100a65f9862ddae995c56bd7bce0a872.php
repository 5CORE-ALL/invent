...
<?php $__env->startSection('script'); ?>
    <script>
        window.userPermissions = <?php echo json_encode($permissions ?? [], 15, 512) ?>;
        const productPermissions = window.userPermissions['cp_masters'] || [];
        const emailColumnMap = <?php echo json_encode($emailColumnMap ?? [], 15, 512) ?>;
        // Add this to the top of your DOMContentLoaded event handler
        const currentUserEmail = <?php echo json_encode(auth()->user()->email ?? '', 15, 512) ?>;
        console.log('Current user email:', currentUserEmail);

        document.addEventListener('DOMContentLoaded', function() {
            // Emails and columns setup
            const emails = <?php echo json_encode($emails ?? [], 15, 512) ?>;
...
<?php echo $__env->make('layouts.vertical', ['title' => 'CP Master', 'mode' => $mode ?? '', 'demo' => $demo ?? ''], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/5core_inv/resources/views/productmaster.blade.php ENDPATH**/ ?>